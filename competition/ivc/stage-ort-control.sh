...
for artifact in "${artifact_sources[@]}" "$ssh_identity"; do
    if [[ ! -r "$artifact" ]]; then
        echo "Required ORT control staging input is not readable: $artifact" >&2
        exit 1
    fi
done
for command_name in cargo cut grep mktemp rsync sha256sum ssh; do
    if ! command -v "$command_name" >/dev/null 2>&1; then
        echo "Required ORT control staging command not found: $command_name" >&2
        exit 1
    fi
done

mkdir -p "$lease_dir"
lease_log=$(mktemp "$lease_dir/ort-control-connect.XXXXXX.log")
manifest=$(mktemp "$lease_dir/ort-control-stage.XXXXXX.sha256")
lease_pid=
cleanup() {
...
while ! grep -q '^Allocated board session:' "$lease_log"; do
    if ! kill -0 "$lease_pid" 2>/dev/null; then
        echo "Board lease ended before ORT control staging allocation" >&2
...
rsync -a -e "$rsync_shell" \
    "$manifest" "$ssh_target:$guest_dir/.ort-control-stage.sha256.new"
...
mv -f -- \
    "$guest_dir/.ort-control-stage.sha256.new" \
    "$guest_dir/.ort-control-stage.sha256"
sync
(
    cd "$guest_dir"
    sha256sum -c .ort-control-stage.sha256
)
findmnt -n -o SOURCE,FSTYPE,OPTIONS /
echo BOARD_ORT_CONTROL_STAGE_HASHES_VERIFIED
REMOTE

echo "BOARD_ORT_CONTROL_STAGE_PASS destination=$guest_dir artifacts=${#artifact_names[@]}"
