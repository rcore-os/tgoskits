...
artifact_sources=(
    ...
    "$output_dir/starry-rknpu-rootfs-smoke.img"
)
artifact_names=(
    starryos-rknpu.bin
...
for artifact in "${artifact_sources[@]}" "$ssh_identity"; do
    if [[ ! -r "$artifact" ]]; then
        echo "Required RKNPU staging input is not readable: $artifact" >&2
        exit 1
    fi
done
for command_name in cargo cut grep rsync sha256sum ssh; do
    if ! command -v "$command_name" >/dev/null 2>&1; then
        echo "Required RKNPU staging command not found: $command_name" >&2
        exit 1
    fi
done

mkdir -p "$lease_dir"
lease_log=$(mktemp "$lease_dir/rknpu-connect.XXXXXX.log")
manifest=$(mktemp "$lease_dir/rknpu-stage.XXXXXX.sha256")
lease_pid=
cleanup() {
...
while ! grep -q '^Allocated board session:' "$lease_log"; do
    if ! kill -0 "$lease_pid" 2>/dev/null; then
        echo "Board lease ended before RKNPU staging allocation" >&2
...
rsync -a -e "$rsync_shell" \
    "$manifest" "$ssh_target:$guest_dir/.rknpu-stage.sha256.new"
...
mv -f -- "$guest_dir/.rknpu-stage.sha256.new" "$guest_dir/.rknpu-stage.sha256"
rm -f -- /home/orangepi/rknpu-result.img /home/orangepi/rknpu-result.img.new
sync
(
    cd "$guest_dir"
    sha256sum -c .rknpu-stage.sha256
)
findmnt -n -o SOURCE,FSTYPE,OPTIONS /
echo BOARD_RKNN_STAGE_HASHES_VERIFIED
REMOTE

echo "BOARD_RKNN_STAGE_PASS destination=$guest_dir artifacts=${#artifact_names[@]}"
