...
| restart recovery | `6adf49e09` clean-source smoke 已通过；所有更早失败均按原结论保留为诊断证据 | 从同一 clean commit 执行并聚合正式 3/3 |
...
1. 从 `6adf49e09` clean worktree 执行唯一一次正式 `fault-restart --repeat 3` campaign；任何单次失败都保留整批原始记录且不计入 3/3。
2. 校验每个 run 的 checksum 与预注册计数，生成 campaign summary，并同步完整正式证据到 Windows/WSL2 证据目录。
3. restart formal gate 通过后，冻结 StarryOS manual/neural 的 5 对 AB/BA 顺序并启动正式 full 矩阵。
