...
- **`someboot::smp::StaticCell`**: BSP 在 secondary CPU 启动前一次初始化、运行期不可变共享的启动阶段原语
...
```rust
// platforms/my-platforms/src/lib.rs
use somehal::KernelOp;
...
4. 仅由 BSP 在 secondary CPU 启动前一次构造、之后不可变共享的状态使用 `someboot::smp::StaticCell`；并发初始化或运行期可变状态使用所属子系统明确的 once、同步与所有权模型
