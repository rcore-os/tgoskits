...
pub unsafe fn init_from_ostool_boot_info(boot_info_ptr: usize) -> bool {
    if boot_info_ptr == 0 {
        return false;
    }

    let info = unsafe { &*(boot_info_ptr as *const OstoolBootInfo) };
    if info.magic != OSTOOL_BOOT_INFO_MAGIC || info.version != OSTOOL_BOOT_INFO_VERSION {
        return false;
    }

    let mut regions = Vec::new();
    let count = core::cmp::min(info.region_count as usize, OSTOOL_BOOT_INFO_MAX_RAM_REGIONS);
    for region in &info.regions[..count] {
        if region.size == 0 {
            continue;
        }
        regions
            .push((region.start as usize, region.size as usize))
            .unwrap();
    }
