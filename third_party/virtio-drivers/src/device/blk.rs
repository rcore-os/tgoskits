...
impl<H: Hal, T: Transport> VirtIOBlk<H, T> {
    ...
    ///
    /// When the buffer spans multiple sectors, the entire I/O is submitted as a single
    /// virtio-blk request with the full buffer as a scatter-gather descriptor chain,
    /// then one notification is sent, and we wait for completion. This amortizes the
    /// per-request notify and completion overhead across all sectors.
    ...
    ///
    /// When the buffer spans multiple sectors, the entire I/O is submitted as a single
    /// virtio-blk request, amortizing per-request overhead.
