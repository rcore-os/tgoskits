...
当前 core 已完成 Linux HTree hash 与 checked lookup，但尚未实现 HTree
split/insert/delete，因此 classic directory mutation 仍会清除 `EXT4_INDEX_FL`。在进入
...
internal node 使用 0/8 offset，并按动态 block size、metadata checksum dx tail、24-bit logical
block mask 校验 limit/count、排序和引用范围。lookup 从 root 取得 version/depth，检查 index/leaf
...
保持 clean。本检查点没有实现 collision continuation、casefold/fscrypt name preparation、
split/insert/delete 或 `LARGEDIR` mutation，以上继续登记为红项，不能据此宣称完整 HTree parity。
