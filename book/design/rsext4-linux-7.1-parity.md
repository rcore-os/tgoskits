...
| `legacy-indirect-13-blocks` | non-extent path is unsupported | Linux-compatible mapping | mapping rewrite | 进行中：checked read 与 allocate-before-publish write 已覆盖 direct/single/double/triple、hole、整块 pointer validity、system zone、cycle、data+metadata `i_blocks` 与运行时失败反向 rollback；跨 direct/single 的 Linux image 已通过 umount、e2fsck、remount/read、再次 e2fsck。full ownership preflight 现在不受 `i_size` 裁剪，完整收集 data 与 child-first metadata，拒绝跨树重复物理块和隐藏损坏；legacy direct-only final free 已改用该结果，修复 size 外 pointer 泄漏。recursive indirect truncate/free、punch 与 crash-atomic journal transaction 仍为红项；indirect free 继续在任何 inode/link mutation 前完成只读 preflight 后返回 typed unsupported |
