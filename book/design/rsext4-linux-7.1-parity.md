...
`cargo test -p ax-fs-ng --features ext4` 为 84+3 全绿，`cargo xtask clippy --package ax-fs-ng`
为 6/6 全绿。该修改只删除 readonly fast-return，冻结 host harness 使用 writable mount，因而没有
把无关的 sequential A/B 伪装成 readonly lifecycle 性能证据。
...
当前 feature negotiation 为只读与可写维护不同的最小 incompat mask：只读额外接受 MMP，可写仍在
任何 superblock、journal 或 namespace mutation 前返回同一 typed feature error。observer、journal
replay reload、初始 mount 与 remount 共用该 mode-aware 判定，没有第二份 capability 状态。这里没有
伪造 MMP magic/checksum 或后台刷新；`fs/ext4/mmp.c:1-404` 的 protection block I/O、随机 sequence、
超时复查、周期刷新和 clean shutdown 仍整体属于可写 MMP lifecycle 红项，未来需要通过 OS 无关的
clock、entropy 与调度能力表达。

Linux differential 用 `mkfs.ext4 -O mmp` 生成固定 64 MiB 镜像。测例验证初始 RW mount 精确拒绝、
RO/no-replay mount 可读取根 inode、RO→RW remount 再次精确拒绝且 options 不变；卸载后将整张镜像与
mount 前 snapshot 逐字节比较，最后要求 `e2fsck -fn` clean。Linux 的
`ext4_setup_system_zone()` 本身也不把 `s_mmp_block` 加入 block-validity tree，因此 portable core 不
擅自扩大 metadata zone。该路径只改变 mount-time negotiation，不进入冻结 host benchmark 的 timed
read/write/sync/unmount workload，故不声明无因果关系的 A/B 性能变化。
