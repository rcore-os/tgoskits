...
| `typed-namespace-create` | create/mkdir 接收 absolute UTF-8 path，core 自动创建父目录并在创建后由 adapter 二次修改 mode | `parent inode + FileName + FilePermissions + MutationContext`，path/permission policy 留在 VFS | namespace boundary | 进行中：owned API 已提供 raw-byte regular-file/directory create，并在首次 metadata publish 时应用 uid/gid/umask；ax-fs-ng 尚未迁移，symlink/special inode 与 project/quota 语义仍待 typed API |
...
| `typed-unlink-open-lifecycle` | core final unlink 立即释放 inode/data block，可能破坏仍由 VFS 引用的 open inode；adapter 另写一套 zero-link 逻辑 | `parent inode + FileName -> UnlinkOutcome`；最后 dentry 消失后 inode 保持 allocated/readable，VFS 最后引用释放后显式 reap | namespace/lifecycle boundary | 绿（运行时生命周期）：确定性红测证明旧实现 final unlink 立即清 inode bitmap；typed raw-name unlink 现在返回剩余 nlink，zero-link inode 持续按 inode number 可读写，显式 reap 才释放。ax-fs-ng 消费同一 outcome，并以唯一、失败可重试的 reap claim 串行化 unlink fast path 与 `Inode::drop`；adapter 仍待迁移到私有 `Ext4<D, S>`，完整 crash atomicity 继续由 JBD2 transaction 阶段负责 |
