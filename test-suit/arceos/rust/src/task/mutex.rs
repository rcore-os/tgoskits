...
fn unlock_wakes_a_waiting_task() {
    ...
    let holder = thread::spawn(move || {
        ...
        api::ax_wait_queue_wait(release_holder_signal.as_ref(), None);
        *guard = 1;
    });

    api::ax_wait_queue_wait(holder_ready.as_ref(), None);
    ...
    api::ax_wait_queue_wait(waiter_started.as_ref(), None);
