...
#define NOSEARCH_DIR DIR_PATH "/nosearch"
#define NOSEARCH_FILE NOSEARCH_DIR "/readable"
...
static void openat2_resolve_checks_mode(void)
{
    ...
    CHECK_ERR(try_openat2(dirfd, "secret", O_PATH | O_TRUNC), EINVAL, "openat2 rejects O_TRUNC with O_PATH");
    CHECK_RET(secret_size(), CONTENT_LEN, "refused openat2 left the file intact");
}

/* Every directory on the walk needs search before the final mode bits are consulted. */
static void search_comes_before_the_mode(void)
{
    become_user();
    CHECK_ERR(try_open(NOSEARCH_FILE, O_RDONLY), EACCES, "a readable file under a directory without search is refused");
    int dirfd = open(DIR_PATH, O_RDONLY | O_DIRECTORY);
    CHECK(dirfd >= 0, "open the test directory");
    CHECK_ERR(try_openat2(dirfd, "nosearch/readable", O_RDONLY), EACCES,
              "openat2 with resolve flags checks search on the way");
    close(dirfd);
}
...
int main(void)
{
    ...
    mkdir(NOSEARCH_DIR, 0777);
    CHECK_RET(chmod(NOSEARCH_DIR, 0777), 0, "prepare the directory without search");
    fd = open(NOSEARCH_FILE, O_CREAT | O_WRONLY | O_TRUNC, 0644);
    CHECK(fd >= 0, "create a file readable by everyone");
    if (fd >= 0)
        close(fd);
    CHECK_RET(chmod(NOSEARCH_FILE, 0644), 0, "the file is mode 0644");
    CHECK_RET(chmod(NOSEARCH_DIR, 0776), 0, "other may read and write the directory but not search it");
    ...
    in_child("search comes before the mode", search_comes_before_the_mode);
