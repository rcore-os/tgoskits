/*
 * perf_hw_lost.c -- in-band PERF_RECORD_LOST records.
 *
 * When a sampling ring fills, the kernel drops samples. Besides the read-only
 * PERF_FORMAT_LOST total, Linux writes an in-band PERF_RECORD_LOST (type 2)
 * record into the ring once it has room again, so `perf report`/`perf script`
 * show "LOST n events!" on the timeline. This test opens a sampling event with a
 * TINY ring, overflows it, drains (advances data_tail) between bursts to make
 * room, and asserts a PERF_RECORD_LOST record with a non-zero lost count appears.
 *
 * SUCCESS ==
 *     fd >= 0 AND mmap ok AND at least one PERF_RECORD_LOST record is emitted
 *       with lost > 0 (the ring provably overflowed and the drop was reported
 *       in-band after a drain).
 * On success exactly one line `STARRY_PERF_LOST_OK` is printed.
 *
...
/* A deliberately tiny ring (one data page) so it overflows quickly. */
#define PERF_MMAP_PAGE_SIZE 4096u
#define PERF_MMAP_DATA_PAGES 1u
...
/* Syscall-heavy burst so the QEMU-TCG cycle counter overflows repeatedly. */
...
int main(void) {
    ...
    if (fd < 0) {
        if (errno == ENOSYS) {
            printf("STARRY_PERF_LOST skip: perf_event_open ENOSYS\n");
            printf("STARRY_PERF_LOST_OK\n");
            return 0;
        }
        printf("perf-hw-lost FAILED: perf_event_open errno=%d\n", errno);
        return 1;
    }
    int efd = (int)fd;
    ...
    /* Bursts fill the tiny ring (dropping samples); draining between bursts frees
     * room so the kernel can emit the pending PERF_RECORD_LOST on the next burst. */
    for (int chunk = 0; chunk < 16 && data_size != 0; chunk++) {
        burst();
            ...
            if (hdr.type == PERF_RECORD_SAMPLE) {
                ...
                if ((uint64_t)sizeof(hdr) + 16 <= hdr.size) {
                    ring_copy(data_base, data_size,
                              (rel + sizeof(hdr) + 8) % data_size, &lost, 8);
                }
                lost_records++;
                lost_total += lost;
            }
        ...
        tail = head;
        meta->data_tail = tail; /* drain: free the ring for the next burst */
        __sync_synchronize();
    }
    ...
    printf("STARRY_PERF_LOST samples=%llu lost_records=%llu lost_total=%llu\n",
           (unsigned long long)sample_records, (unsigned long long)lost_records,
           (unsigned long long)lost_total);

    int rc = 0;
    if (lost_records == 0) {
        rc = 1;
        printf("perf-hw-lost FAILED: no PERF_RECORD_LOST record emitted (ring "
               "did not overflow, or in-band lost reporting missing)\n");
    } else if (lost_total == 0) {
        rc = 1;
        printf("perf-hw-lost FAILED: PERF_RECORD_LOST carried lost=0\n");
    }
