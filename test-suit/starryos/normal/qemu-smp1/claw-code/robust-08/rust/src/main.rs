...
fn main() {
    ...
    println!("=== Robust-08: word list generation and analysis ===");
    let output = Command::new("sh").arg("-c").arg(format!("cd /tmp/work && {env} timeout 900 /tmp/claw --allowedTools bash,write prompt 'create a text file with 20 random words, then use bash to sort them alphabetically, count characters per word, and save the full analysis to wordstats.txt' 2>&1")).output().unwrap();
    println!("{}", String::from_utf8_lossy(&output.stdout));

    if std::path::Path::new("/tmp/work/wordstats.txt").exists() {
        let s = fs::read_to_string("/tmp/work/wordstats.txt").unwrap_or_default();
        println!("wordstats.txt: {} lines", s.lines().count());
