...
fn main() {
    ...
    println!("=== Robust-06: find and sort directories ===");
    let output = Command::new("sh").arg("-c").arg(format!("cd /tmp/work && {env} timeout 600 /tmp/claw --allowedTools bash,write prompt 'use bash to find all directories in /, sort them alphabetically, and write the first 10 to topdirs.txt' 2>&1")).output().unwrap();
    println!("{}", String::from_utf8_lossy(&output.stdout));

    if std::path::Path::new("/tmp/work/topdirs.txt").exists() {
        let s = fs::read_to_string("/tmp/work/topdirs.txt").unwrap_or_default();
        println!("topdirs.txt: {} lines", s.lines().count());
