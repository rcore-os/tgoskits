...
fn main() {
    ...
    println!("=== Robust-03: print numbers 1-100 with squares ===");
    let output = Command::new("sh").arg("-c").arg(format!("cd /tmp/work && {env} timeout 600 /tmp/claw --allowedTools bash,write prompt 'write a shell script that prints numbers 1 to 100 with their squares, run it, and save the output to squares.txt' 2>&1")).output().unwrap();
    println!("{}", String::from_utf8_lossy(&output.stdout));

    if std::path::Path::new("/tmp/work/squares.txt").exists() {
        let s = fs::read_to_string("/tmp/work/squares.txt").unwrap_or_default();
        let last = s.lines().last().unwrap_or("");
        println!("squares.txt last line: {last}");
        if last.contains("10000") { println!("ALL_TESTS_DONE"); } else { println!("ALL_TESTS_FAILED"); std::process::exit(1); }
