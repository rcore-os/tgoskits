...
fn main() {
    ...
    if std::path::Path::new("/tmp/work/stats.md").exists() {
        let s = fs::read_to_string("/tmp/work/stats.md").unwrap_or_default();
        println!("stats.md: {} lines", s.lines().count());
        let lower = s.to_lowercase();
        if lower.contains("min") || lower.contains("max") || lower.contains("average") || lower.contains("median") { println!("ALL_TESTS_DONE"); } else { println!("ALL_TESTS_FAILED"); std::process::exit(1); }
