...
#define CHECK_TRUE(call, errno_ok, msg) do {                            \
    errno = 0;                                                          \
    long _r = (long)(call);                                             \
    if (_r >= 0) {                                                      \
        printf("  PASS | %s:%d | %s (ret=%ld)\n",                      \
        ...
        int _eok = (errno_ok);                                          \
        if (_eok) {                                                     \
            printf("  PASS | %s:%d | %s (ret=%ld, errno=%d acceptable)\n", \
                   __FILE__, __LINE__, msg, _r, errno);                 \
            __pass++;                                                   \
        } else {                                                        \
            printf("  FAIL | %s:%d | %s | ret=%ld errno=%d (%s)\n",    \
                   __FILE__, __LINE__, msg, _r, errno, strerror(errno));\
            __fail++;                                                   \
        }                                                               \
    }                                                                   \
    fflush(stdout);                                                     \
