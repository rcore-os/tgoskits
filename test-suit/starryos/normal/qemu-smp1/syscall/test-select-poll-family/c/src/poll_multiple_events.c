...
int run_poll_multiple_events(void) {
    ...
    CHECK_RET(syscall(SYS_poll, &pfd, 1, 10), 1, "write end POLLIN|POLLOUT returns 1");
    ...
    pfds[1].revents = 0;
    CHECK_RET(syscall(SYS_poll, pfds, 2, 10), 1, "read POLLIN + write POLLOUT returns 1");
    ...
    pfd.revents = 0;
    CHECK_RET(syscall(SYS_poll, &pfd, 1, 10), 1, "read end with data POLLIN|POLLOUT returns 1");
