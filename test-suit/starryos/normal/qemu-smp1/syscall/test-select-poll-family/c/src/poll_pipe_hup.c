...
int run_poll_pipe_hup(void) {
    ...
    CHECK_RET(syscall(SYS_poll, &pfd, 1, 100), 1, "read end POLLIN after close write returns 1");
    ...
    pfd.events = POLLIN;
    pfd.revents = 0;
    CHECK_RET(syscall(SYS_poll, &pfd, 1, 100), 1, "read end with data + closed write returns 1");
    ...
    pfd.revents = 0;
    CHECK_RET(syscall(SYS_poll, &pfd, 1, 100), 1, "read end after drain + closed write returns 1");
