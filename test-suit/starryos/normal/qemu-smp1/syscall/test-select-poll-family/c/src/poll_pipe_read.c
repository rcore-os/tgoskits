...
int run_poll_pipe_read(void) {
    ...
    CHECK_RET(syscall(SYS_poll, &pfd, 1, 10), 0, "empty pipe POLLIN timeout returns 0");

    write_exact(fds[1], "A", 1);
    pfd.revents = 0;
    CHECK_RET(syscall(SYS_poll, &pfd, 1, 10), 1, "pipe with 1 byte returns 1");
    CHECK(pfd.revents & POLLIN, "revents has POLLIN");

    char buf[16];
    read_exact(fds[0], buf, 1);
    pfd.revents = 0;
    CHECK_RET(syscall(SYS_poll, &pfd, 1, 10), 0, "pipe after read returns 0 (timeout)");
    ...
    read_exact(fds[0], buf, 1);
    pfd.revents = 0;
    CHECK_RET(syscall(SYS_poll, &pfd, 1, 10), 1, "pipe with 255 remaining bytes returns 1");
