...
int run_pselect_no_sigmask(void) {
    MODULE_START("pselect_no_sigmask");
    ...
    int ret = (int)syscall(SYS_pselect6, sig_pipe[0] + 1, &rfds, NULL, NULL, &ts, NULL);

    CHECK(ret == -1 || ret == 1, "pselect returns -1 (EINTR) or 1 (sig_pipe readable)");
    if (ret == -1) {
    ...
    MODULE_SUMMARY("pselect_no_sigmask");
    MODULE_RETURN();
}
