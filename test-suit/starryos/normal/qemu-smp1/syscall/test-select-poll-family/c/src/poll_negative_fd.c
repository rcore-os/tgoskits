...
int run_poll_negative_fd(void) {
    ...
    CHECK_RET(syscall(SYS_poll, &pfd, 1, 10), 0, "poll fd=-1 returns 0");
    ...
    pfds[1].revents = 0;
    CHECK_RET(syscall(SYS_poll, pfds, 2, 10), 1, "mixed valid+invalid returns 1");
    ...
    pfd.revents = 0;
    CHECK_RET(syscall(SYS_poll, &pfd, 1, 10), 0, "fd=-1 POLLIN|POLLOUT returns 0");
