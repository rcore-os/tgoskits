...
int run_select_bad_nfds(void) {
    ...
    CHECK_ERRNO(syscall(SYS_select, -1, &rfds, NULL, NULL, NULL), EINVAL, "select nfds=-1 returns EINVAL");
    ...
    tv.tv_sec = -1;
    tv.tv_usec = 0;
    CHECK_ERRNO(syscall(SYS_select, 1, &rfds, NULL, NULL, &tv), EINVAL, "select tv_sec=-1 returns EINVAL");
