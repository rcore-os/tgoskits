...
static void heavy_burst(int count) {
    for (volatile int i = 0; i < count; i++) {
        ...
        struct stat st; stat("/", &st); stat("/dev", &st);
...
typedef struct { int tid; uint64_t count; int ok; int cycles; } stress_thr_t;
static void *stress_thread(void *arg) {
    stress_thr_t *s = (stress_thr_t *)arg;
    s->ok = 0;
    int fd = open("/dev/kcov", O_RDWR);
    if (fd < 0) return NULL;
    if (ioctl(fd, KCOV_INIT_TRACE, 64)) { close(fd); return NULL; }
    size_t sz = 64 * sizeof(uint64_t);
    uint64_t *buf = mmap(NULL, sz, PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0);
    ...
    for (int c = 0; c < s->cycles; c++) {
        ...
        for (volatile int i = 0; i < 200; i++) { gettid(); getpid(); }
        ioctl(fd, KCOV_DISABLE, 0);
    }
    s->count = buf[0]; s->tid = (int)gettid();
    munmap(buf, sz); close(fd);
    s->ok = 1;
    return NULL;
}
static void stress_many_threads(void) {
    #define N 8
    pthread_t t[N]; stress_thr_t r[N];
    for (int i = 0; i < N; i++) { r[i].cycles = 50;
        pthread_create(&t[i], NULL, stress_thread, &r[i]); }
    for (int i = 0; i < N; i++) pthread_join(t[i], NULL);
    int all_ok = 1;
    for (int i = 0; i < N; i++) { if (!r[i].ok) all_ok = 0;
...
static void stress_buffer_replace(void) {
    ...
    for (int i = 0; i < 100; i++) {
        ...
        CHECK_PTR(buf, 1, "mmap");
        CHECK_RET(ioctl(fd, KCOV_ENABLE, KCOV_TRACE_PC), 0, "ENABLE");
        for (volatile int j = 0; j < 100; j++) getpid();
        CHECK_RET(ioctl(fd, KCOV_DISABLE, 0), 0, "DISABLE");
        munmap(buf, sz);
    }
    close(fd);
...
static void stress_mode_toggle(void) {
    ...
    for (int i = 0; i < 200; i++) {
        ...
        for (volatile int j = 0; j < 50; j++) getpid();
        CHECK_RET(ioctl(fd, KCOV_DISABLE, 0), 0, "DISABLE");
    }
...
int main(void) {
    ...
    int probe = open_kcov();
    if (probe < 0) { printf("SKIP: /dev/kcov not available\n"); TEST_DONE(); }
    close(probe);
