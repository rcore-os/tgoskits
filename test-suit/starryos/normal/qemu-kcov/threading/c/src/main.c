...
typedef struct { int tid; uint64_t n; int ok; } thr_t;
...
static void *thread_fn(void *arg) {
    ...
    if (fd < 0) return NULL;
    if (ioctl(fd, KCOV_INIT_TRACE, 128)) { close(fd); return NULL; }
    ...
    for (volatile int i = 0; i < 300; i++) { gettid(); getpid(); }
    ioctl(fd, KCOV_DISABLE, 0);
    s->n = buf[0]; s->tid = (int)gettid();
