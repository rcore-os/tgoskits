...
run_case() {
    ...
    echo "running ${case_name}; full log: ${log_file}"
    echo "this may take 1-2 minutes when caches are cold..."

    set +e
    cargo xtask starry test qemu --test-group k230-qemu --arch riscv64 -c "${case_name}" >"${log_file}" 2>&1
    local status=$?
    set -e
...
if [[ "${WITH_REPLAY}" == "1" ]]; then
    ...
    if [[ ! -f "${CAPTURE}" ]]; then
        ...
        set +e
        K230_DEMO_USE_DOCKER=0 bash "${SCRIPT_DIR}/kpu-smoke/c/tools/demo-yolov8n-full-sequence-replay.sh" >"${REPLAY_LOG}" 2>&1
        replay_status=$?
        set -e
