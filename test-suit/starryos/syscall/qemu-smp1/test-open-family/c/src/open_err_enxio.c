 ...
 * test-suit/starryos/bugfix/qemu-smp1/bugfix-smp1/bug-open-unix-socket-no-enxio/c/src/main.c */

/* enxio_fifo_wronly_no_reader(): 原为 "FIFO O_WRONLY|O_NONBLOCK 无读者 → ENXIO" 测试用例;
 * 因 starry 与 Linux 行为不一致(starry 不检查), 已移到 bugfix 复现:
 * test-suit/starryos/bugfix/qemu-smp1/bugfix-smp1/bug-open-fifo-wronly-no-reader-no-enxio/c/src/main.c */
