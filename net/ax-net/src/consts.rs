...
//! smoltcp packet buffers, listen queues, and pending ARP packets. They are
//! intentionally centralized so embedded targets can
...
//! buffers are large enough for common POSIX workloads. Hardware queue and DMA
//! ring capacities come from `NetPollGroupParts` rather than protocol-layer
//! constants. If a value is raised, consider the total cost across all sockets,
//! not only the cost of one queue.
