...
static GLOBAL_COMMIT: CommitAccounting = CommitAccounting::new();
...
///
/// Starry uses Linux always-overcommit mode and maintains this counter for
/// `Committed_AS` reporting and checked address-space ownership.
...
impl CommitAccounting {
    ...
    fn admit(&self, bytes: u64) -> Result<CommitCharge<'_>, AdmissionError> {
        self.committed
            .try_update(Ordering::AcqRel, Ordering::Acquire, |current| {
                current.checked_add(bytes)
            })
            .map_err(|_| AdmissionError::Overflow)?;
        Ok(CommitCharge {
            accounting: self,
            bytes,
        })
...
/// Ownership token for a pending committed-memory charge.
