...
pub struct PageTable64<M: PagingMetaData, PTE: GenericPTE, H: PageFrameProvider + Default> {
    root_paddr: PhysAddr,
    root_frames: usize,
...
impl<M: PagingMetaData, PTE: GenericPTE, H: PageFrameProvider + Default> PageTable64<M, PTE, H> {
...
impl<M: PagingMetaData, PTE: GenericPTE, H: PageFrameProvider + Default> PageTable64<M, PTE, H> {
    fn alloc_table() -> PagingResult<PhysAddr> {
        if let Some(paddr) = H::default().alloc_frame() {
            let ptr = H::default().phys_to_virt(paddr).as_mut_ptr();
    ...
    fn alloc_root_table(root_frames: usize, root_align: usize) -> PagingResult<PhysAddr> {
        ...
        if let Some(paddr) = H::default().alloc_frames(root_frames, root_align) {
            let ptr = H::default().phys_to_virt(paddr).as_mut_ptr();
    ...
    fn table_of<'a>(&self, paddr: PhysAddr) -> &'a [PTE] {
        let ptr = H::default().phys_to_virt(paddr).as_ptr() as _;
        unsafe { core::slice::from_raw_parts(ptr, ENTRY_COUNT) }
    }

    fn table_of_mut<'a>(&mut self, paddr: PhysAddr) -> &'a mut [PTE] {
        let ptr = H::default().phys_to_virt(paddr).as_mut_ptr() as _;
    ...
    fn dealloc_tree(&self, table_paddr: PhysAddr, level: usize) {
        ...
        H::default().dealloc_frame(table_paddr);
    }
}
    ...
    for PageTable64<M, PTE, H>
{
    fn drop(&mut self) {
        ...
        if self.root_allocated_with_frames {
            H::default().dealloc_frames(self.root_paddr(), self.root_frames);
        } else {
            H::default().dealloc_frame(self.root_paddr());
        }
    }
}

/// A cursor created by [`PageTable64::cursor`] to modify the page table.
pub struct PageTable64Cursor<'a, M: PagingMetaData, PTE: GenericPTE, H: PageFrameProvider + Default>
{
