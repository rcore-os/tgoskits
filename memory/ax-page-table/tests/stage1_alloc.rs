...
fn architecture_alias_accepts_runtime_tlb_invalidator() {
    use ax_page_table::stage1::x86_64::{X64PageTable, X64PagingMetaData};
...
-> PagingResult<()> {
    use ax_page_table::stage1::PageTable32;
    ALLOCATED.with_borrow_mut(|it| {
        it.clear();
    });
...
fn test_dealloc_arm32() -> PagingResult<()> {
    run_test_for_32bit::<
        ax_page_table::stage1::arm::A32PagingMetaData,
        ax_page_table::entry::arm::A32PTE,
    >()?;
    Ok(())
}
...
fn test_dealloc_x86() -> PagingResult<()> {
    run_test_for::<HostTestMeta, ax_page_table::entry::x86_64::X64PTE>()?;
    Ok(())
}
...
fn test_dealloc_riscv() -> PagingResult<()> {
    run_test_for::<
        ax_page_table::stage1::riscv::Sv39MetaData<VirtAddr>,
        ax_page_table::entry::riscv::Rv64PTE,
    >()?;
    run_test_for::<
        ax_page_table::stage1::riscv::Sv48MetaData<VirtAddr>,
        ax_page_table::entry::riscv::Rv64PTE,
    >()?;
    Ok(())
}
...
fn test_dealloc_aarch64() -> PagingResult<()> {
    run_test_for::<
        ...
        ax_page_table::entry::aarch64::A64PTE,
    >()?;
    Ok(())
}
...
fn test_dealloc_loongarch64() -> PagingResult<()> {
    run_test_for::<
        ax_page_table::stage1::loongarch64::LA64MetaData,
        ax_page_table::entry::loongarch64::LA64PTE,
    >()?;
    Ok(())
}
