...
- `board-power-control`: project-local skill at `.claude/skills/board-power-control/SKILL.md`
- Use `board-power-control` when an OrangePi-5-Plus needs smart-plug power status, power-on, power-off, or a cold power cycle, especially when serial or SSH recovery cannot reach the RK3588 board.
...
- Before writing, modifying, or reviewing code, fully read (完整阅读) `book/guideline/code-quality.md` and treat it as the mandatory baseline coding standard. Fully read additional domain guidelines only when the changed behavior is in their scope. In particular, fully read `book/guideline/starry/syscall.md` whenever a change or claim affects user-visible StarryOS syscall/Linux ABI semantics, including indirect changes in task, VFS, namespace, signal, socket, credential, memory-management, or other helper code. This trigger is semantic rather than path-based; when it is clearly inapplicable, record the concrete reason in review work. If the conversation context is compacted, resumed from a summary, or you cannot confidently recall an applicable guideline, re-read that complete guideline before continuing.
