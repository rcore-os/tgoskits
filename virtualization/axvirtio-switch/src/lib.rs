//! Pure layer-2 virtual switch core shared by every AxVisor virtio-net port.
//!
//! This crate owns *only* the forwarding decision: it maps a guest MAC to a
//! port, classifies an Ethernet destination (known unicast / broadcast /
//! multicast / unknown unicast), and hands a frame to the target port's
//! ingress sink. It deliberately knows nothing about AxVM, IRQs, DMA buffers,
//! the host virtio-net queue or QEMU — the concrete [`SwitchPort`] sink is
//! injected by the consuming hypervisor glue at registration time, so the core
//! can be unit-tested with fake sinks (design §10 stage 1, §11.1).
//!
//! Concurrency contract (design §7): the port table is one small
//! [`SwitchRegistry`] mutex holding both indices, so registration and lookup
//! never observe a half-updated table. While holding that lock the switch only
//! snapshots `Arc<dyn SwitchPort>` references and makes a forwarding decision;
//! it never copies a frame, calls a sink, or wakes a worker under the lock.
