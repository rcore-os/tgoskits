...
- 两个互相独立的单生产者、单消费者方向；
- 通过 Release/Acquire 发布的固定大小 opaque slot ring；
...
ring 不解释 slot；Message frame 编解码也不依赖 hypercall 或具体 runtime。

# Message V1

每个 256 字节 slot 以手工 little-endian 编码的 24 字节 header 开始：
...
fn send_step(
    ...
    *consumed += progress.consumed();
    // published_slots() 非零时，客户机 glue 可以批量 notify 一次。
    Ok(progress.is_complete())
}
```

ring 满时，`try_write` 返回零进度并保留发送状态，调用方可以等待后重试。因此一条
消息可以同时大于单个 slot 和全部 32 个在途 slots。
...
一个 slot fragment 不会被拆开消费。如果第一个可用 fragment 放不进 output，API
返回 `BufferTooSmall`，且不推进 ring head。应用因资源策略拒绝消息时，可用
`try_discard` 释放其 slots。需要应用级原子可见性的调用方，应自行暂存流式输出，
直到观察到 `LAST` 再提交。
...
6. 发布 slots 或释放 ring capacity 后通知对端；无进度时等待或轮询。

unsafe attach 契约负责阻止重复 producer/consumer 在 `UnsafeCell` slot bytes 上发生竞争。
...
- `IVC_SLOT_SIZE` 为 256 字节，`IVC_SLOT_FRAGMENT_CAPACITY` 为 232 字节，
  `IVC_RING_CAPACITY` 为 32；每个 slot 按 256 字节对齐。
- `IvcRegion` 总大小为 17152 字节，两个 ring 的偏移为 256 和 8704；每个
  `IvcRing` 大小为 8448 字节，slots 从 ring 内偏移 256 开始。
- Message V1 不支持消息交错、重传、乱序重排，也不进行分配。
- API 预留 `PeerReset`；当前 HVC backend 没有 queue generation，暂不会产生该错误。
- 通信双方必须使用相同的 region 布局和 slot 参数，仅 region 版本相同不足以保证兼容。
