...
// Layout checks use normalized PT_LOAD p_paddr values; p_offset is only a file offset.
const BOOT_INFO_BASE: usize = 0x8000_0000;
const BOOT_INFO_SIZE: usize = 0x10_0000;
const COMMAND_LINE_OFFSET: usize = 0x1_000;
const SYSTEM_TABLE_OFFSET: usize = 0x1_0000;
const CONFIG_TABLE_OFFSET: usize = 0x2_0000;
const BOOT_MEMMAP_OFFSET: usize = 0x3_0000;
...
pub(super) const fn boot_args() -> [usize; 3] {
    [
        1,
        BOOT_INFO_BASE + COMMAND_LINE_OFFSET,
        BOOT_INFO_BASE + SYSTEM_TABLE_OFFSET,
    ]
}
...
pub(super) fn load_boot_info(loader: &ImageLoaderCore<'_>) -> AxVmResult {
    ...
    boot_info[COMMAND_LINE_OFFSET..COMMAND_LINE_OFFSET + cmdline.len()]
    ...
    write_u32_at(
        &mut boot_info,
        SYSTEM_TABLE_OFFSET + 8,
        EFI_SYSTEM_TABLE_REVISION,
    );
    write_u32_at(
        &mut boot_info,
        SYSTEM_TABLE_OFFSET + 12,
        EFI_SYSTEM_TABLE_SIZE,
    );

    write_u64_at(&mut boot_info, SYSTEM_TABLE_OFFSET + 104, 2);
    write_u64_at(
        &mut boot_info,
        SYSTEM_TABLE_OFFSET + 112,
        (BOOT_INFO_BASE + CONFIG_TABLE_OFFSET) as u64,
    );
    ...
    write_config_table(
        ...
        (BOOT_INFO_BASE + BOOT_MEMMAP_OFFSET) as u64,
    );
    ...
    let map_end = BOOT_MEMMAP_OFFSET
        .checked_add(40)
        .and_then(|start| start.checked_add(map_size))
        .ok_or_else(|| ax_err_type!(InvalidData, "LoongArch EFI memory map overflows"))?;
    if map_end > BOOT_INFO_SIZE {
        return Err(ax_err_type!(
            InvalidData,
            "LoongArch EFI memory map exceeds the boot-info area"
        ));
    }
    write_u64_at(&mut boot_info, BOOT_MEMMAP_OFFSET, map_size as u64);
    write_u64_at(
        &mut boot_info,
        BOOT_MEMMAP_OFFSET + 8,
        EFI_MEMORY_DESCRIPTOR_SIZE as u64,
    );
    write_u32_at(&mut boot_info, BOOT_MEMMAP_OFFSET + 16, 1);
    write_u64_at(&mut boot_info, BOOT_MEMMAP_OFFSET + 32, map_size as u64);
    for (index, region) in regions.into_iter().enumerate() {
        let descriptor = BOOT_MEMMAP_OFFSET + 40 + index * EFI_MEMORY_DESCRIPTOR_SIZE;
...
fn write_config_table(image: &mut [u8], index: usize, guid: [u8; 16], table: u64) {
    let offset = CONFIG_TABLE_OFFSET + index * 24;
...
const _: () = {
    assert!(COMMAND_LINE_OFFSET + COMMAND_LINE_SIZE <= SYSTEM_TABLE_OFFSET);
    assert!(BOOT_MEMMAP_OFFSET < BOOT_INFO_SIZE);
...
mod tests {
    ...
    const LINUX_QEMU_FILE_OFFSET: usize = 0x1_0000;
    ...
    fn direct_kernel(
        ...
        image[24..32].copy_from_slice(&physical_address.to_le_bytes());
        ...
        image[program_header + 16..program_header + 24]
            .copy_from_slice(&physical_address.to_le_bytes());
    ...
    #[test]
    fn released_linux_qemu_layout_uses_physical_load_address() {
        let kernel = direct_kernel(0x9000_0000_0020_0000, 0x2c32_9f0, LINUX_QEMU_FILE_OFFSET);
        validate_image_layout(
            &kernel,
            Some((GuestPhysAddr::from(0x0800_0000), 0x0380_0000)),
            0x1_0000,
        )
        .expect("the released linux-qemu layout should be accepted");
        assert_eq!(
            boot_args(),
            [
                1,
    ...
    fn direct_boot_layout_rejects_overlapping_images() {
        let valid_kernel = direct_kernel(0x9000_0000_0020_0000, 0x10_0000, LINUX_QEMU_FILE_OFFSET);
        validate_image_layout(
            &valid_kernel,
        ...
        let boot_info_kernel = direct_kernel(
            LOONGARCH_DMW_PREFIX | (BOOT_INFO_BASE + 0x8_000) as u64,
            0x10_0000,
            ELF_HEADER_SIZE + ELF_PROGRAM_HEADER_SIZE,
        );
        assert!(
            validate_image_layout(&boot_info_kernel, None, 0x1_0000)
                .unwrap_err()
                .to_string()
