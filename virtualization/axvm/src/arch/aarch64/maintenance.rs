...
use super::maintenance_state::{self, MaintenancePublication};
...
static HANDLER: Once<Result<IrqHandle, IrqError>> = Once::new();

pub(super) fn register_handler() {
    let result = HANDLER.call_once(|| {
        let irq = ax_hal::irq::gic_maintenance_irq()?;
    ...
    if let Err(error) = result {
        warn!("GIC maintenance IRQ handler is unavailable: {error:?}");
    }
}
