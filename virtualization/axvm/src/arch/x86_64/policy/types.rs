...
use crate::arch::x86_64::policy::decode::X86ByteRegister;
...
impl From<ax_cpu::virtualization::VirtualizationError> for X86VcpuError {
    fn from(error: ax_cpu::virtualization::VirtualizationError) -> Self {
        use ax_cpu::virtualization::VirtualizationError as Cpu;
        match error {
            Cpu::Unavailable
            | Cpu::UnsupportedPaging
            | Cpu::UnsupportedTimer
            | Cpu::UnsupportedMsr
            | Cpu::FeatureControlUnavailable => Self::Unsupported,
            Cpu::InvalidExtendedState
            | Cpu::InvalidRoot
            | Cpu::InvalidVector
            | Cpu::InvalidControlMemory
            | Cpu::InvalidPortRange => Self::InvalidInput,
            Cpu::AlreadyEnabled => Self::ResourceBusy,
            Cpu::NotEnabled | Cpu::InstructionFailed => Self::BadState,
        }
    }
}

impl From<ax_cpu::virtualization::VmxControlError> for X86VcpuError {
    fn from(error: ax_cpu::virtualization::VmxControlError) -> Self {
        use ax_cpu::virtualization::VmxControlError;
        match error {
            VmxControlError::ConflictingMasks => Self::InvalidInput,
            VmxControlError::UnsupportedSet(_) | VmxControlError::UnsupportedClear(_) => {
                Self::Unsupported
            }
            VmxControlError::Cpu(error) => error.into(),
        }
    }
}

impl From<ax_cpu::virtualization::RunError> for X86VcpuError {
    fn from(error: ax_cpu::virtualization::RunError) -> Self {
        match error {
            ax_cpu::virtualization::RunError::Control(error) => error.into(),
            ax_cpu::virtualization::RunError::VmxEntry(_) => Self::BadState,
        }
    }
}
