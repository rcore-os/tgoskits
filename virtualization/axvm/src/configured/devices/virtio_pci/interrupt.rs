...
impl<D: VirtioDeviceCore> VirtioPciFunction<D> {
    fn execute_polled_transition(&self, transition: InterruptTransition) -> DeviceResult {
        // DMA pollers and PCI binding leases are owned by the same sealed
        // DeviceRuntime. Therefore a poll callback cannot race route teardown,
        // unlike an endpoint callback admitted through the PCI root.
