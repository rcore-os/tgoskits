...
use axdevice::{
    DeviceFactoryRegistry, FwCfgPayloadFactory, VirtioBlockFactory, VirtioNetFactory,
...
impl AxVM {
    ...
    pub fn prepare_with_factories(
        ...
        register_vm_owned_device_factories(self, factories)?;
...
pub(crate) fn default_device_factories(vm: &AxVM) -> AxVmResult<DeviceFactoryRegistry> {
    ...
    register_vm_owned_device_factories(vm, &mut factories)?;
    Ok(factories)
}

fn register_vm_owned_device_factories(
    vm: &AxVM,
    factories: &mut DeviceFactoryRegistry,
) -> AxVmResult {
    crate::machine::register_machine_device_factories(vm, factories)?;
    if factories.get(EmulatedDeviceType::VirtioBlk).is_none() {
        let backings = vm.with_config(|config| config.virtio_block_backings().to_vec());
        factories.register(Arc::new(VirtioBlockFactory::new(&backings)))?;
    }
    if factories.get(EmulatedDeviceType::VirtioNet).is_none() {
