...
pub(crate) fn push_existing_vm(vm: AxVMRef) -> AxVmResult {
    ...
    if let Err(error) = registry.insert(vm.clone(), cpu_partition_input) {
...
impl AxvmRuntime {
    ...
    /// Reset a VM while delegating each stop-wait iteration to the caller.
...
pub fn register_vm(vm: AxVMRef) -> AxVmResult {
