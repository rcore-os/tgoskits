...
mod aarch64 {
    ...
    impl ArmVcpuHostIf for ArmVcpuHostIfImpl {
        fn hardware_inject_virtual_interrupt(_vector: u8) {
            todo!("move AArch64 virtual interrupt injection into AxVM")
        }

        fn fetch_irq() -> usize {
            todo!("move AArch64 IRQ fetch into AxVM")
        }
    ...
    impl ArmVgicHostIf for ArmVgicHostIfImpl {
        ...
        fn current_vcpu_id() -> usize {
            todo!("move current vCPU tracking into AxVM")
        }
        ...
        fn read_vgicd_iidr() -> u32 {
            todo!("move host GICD IIDR access into AxVM")
        }

        fn read_vgicd_typer() -> u32 {
            todo!("move host GICD TYPER access into AxVM")
        }

        fn get_host_gicd_base() -> PhysAddr {
            todo!("move host GICD base discovery into AxVM")
        }

        fn get_host_gicr_base() -> PhysAddr {
            todo!("move host GICR base discovery into AxVM")
        }
