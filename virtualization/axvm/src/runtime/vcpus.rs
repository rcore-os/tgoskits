...
fn vcpu_run() {
    ...
    loop {
        ...
        let run_result = {
            // The AArch64 deferred IRQ token still names the pCPU that
            // acknowledged it. Keep the task on that pCPU until the complete
            // bound-run transaction has routed or retained the token.
