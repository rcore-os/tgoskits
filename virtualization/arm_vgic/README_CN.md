...
`ICC_DIR_EL1` 并路由到 `GicV3VcpuBinding::deactivate`。溢出的 HW-backed 项仍通过
受检物理 backend 完成 deactivation，不会退化成软件中断。平台适配层在构造控制器
前必须确认硬件支持 TDIR，或者提供完整的 common CPU Interface sysreg trap 实现。
