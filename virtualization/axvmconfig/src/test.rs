...
use crate::{
    AddressSpacePolicy, AxVmConfigError, EmulatedDeviceType, GuestConfig, GuestDevices, GuestType,
    PhysicalDeviceRef, VMBootProtocol, VMInterruptMode, VMKernelConfig, VmMemMappingType,
};
...
#[test]
fn parses_hidden_legacy_board_device_fields() {
...
excluded_devices = []
emu_devices = [
  ["virtio-blk", 0x0a000000, 0x1000, 48, 0xe1, [0]],
]
"#,
    )
    .unwrap();

    assert_eq!(config.base.legacy_vm_type, Some(1));
    assert_eq!(
        config.devices.legacy_interrupt_mode,
        Some(VMInterruptMode::Emulated)
    );
    assert_eq!(config.devices.aarch64_virtual_timer_irq, Some(27));
    assert_eq!(config.devices.legacy_emulated.len(), 1);
    assert_eq!(
        config.devices.legacy_emulated[0].emu_type,
