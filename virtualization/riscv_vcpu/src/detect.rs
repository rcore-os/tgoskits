...
impl LocalIrqGuard {
    fn new() -> Self {
        ...
        // SAFETY: clearing SIE excludes interrupt-driven migration for this CSR
        // transaction; Drop restores the entry SIE state on the same hart.
...
impl Drop for LocalIrqGuard {
    fn drop(&mut self) {
        if self.restore_sie {
            // SAFETY: this is the inverse of LocalIrqGuard::new and executes on the
            // same hart because SIE remained clear.
