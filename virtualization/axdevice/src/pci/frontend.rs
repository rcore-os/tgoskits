...
use crate::{DeviceLifecycle, DeviceManagerResult, PciError, PciResult};
...
impl PciEcamConfigFrontend {
    /// Creates an ECAM frontend over a bus-zero-based, graph-resolved window.
    pub fn try_new(base: u64, size: u64, binding: Arc<PciRootBinding>) -> PciResult<Self> {
        if !base.is_multiple_of(ECAM_BUS_SIZE) {
            return Err(PciError::InvalidHostAperture {
                detail: "ECAM base is not aligned to one bus window",
            });
        }
        if size == 0 || size > ECAM_MAX_SIZE || !size.is_multiple_of(ECAM_BUS_SIZE) {
            return Err(PciError::InvalidHostAperture {
                detail: "ECAM size must contain between 1 and 256 complete bus windows",
            });
        }
        base.checked_add(size)
            .ok_or(PciError::InvalidHostAperture {
                detail: "ECAM address range overflows u64",
            })?;
        Ok(Self {
            base,
            size,
...
mod tests {
    ...
    fn frontend() -> PciEcamConfigFrontend {
        ...
        PciEcamConfigFrontend::try_new(ECAM_BASE, ECAM_SIZE, binding).unwrap()
    }
    ...
    #[test]
    fn ecam_rejects_windows_that_cannot_be_decoded_without_aliasing() {
        let binding = frontend().binding;
        for (base, size) in [
            (ECAM_BASE + 1, ECAM_SIZE),
            (ECAM_BASE, 0),
            (ECAM_BASE, ECAM_BUS_SIZE + 1),
            (ECAM_BASE, ECAM_MAX_SIZE + ECAM_BUS_SIZE),
            (u64::MAX - ECAM_BUS_SIZE + 1, ECAM_BUS_SIZE),
        ] {
            assert!(matches!(
                PciEcamConfigFrontend::try_new(base, size, binding.clone()),
