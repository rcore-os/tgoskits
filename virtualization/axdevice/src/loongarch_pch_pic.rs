...
pub struct PchPicOutputEvent {
    /// PCH-PIC input whose state changed.
    pub source: usize,
    /// EIOINTC vector selected by the guest-programmed route.
    pub vector: usize,
    /// Whether the routed output became asserted.
    pub asserted: bool,
}
...
impl LoongArchPchPic {
    ...
    pub fn set_irq_level(&self, irq: usize, level: bool) -> Option<usize> {
        ...
        let event = update_irq(&mut state, mask, level);
        let routed = event.map(|event| event.vector);
    ...
    pub fn pending_vector(&self, irq: usize) -> Option<usize> {
        ...
        update_irq(&mut state, 1u64 << irq, true).map(|event| event.vector)
    }
...
fn update_irq(state: &mut PchPicState, mask: u64, level: bool) -> Option<PchPicOutputEvent> {
...
fn update_int_mask(state: &mut PchPicState, val: u32, high: bool) {
    ...
    if newly_unmasked != 0 {
        ...
        if let Some(event) = update_irq(state, mask, true) {
            push_output_event(state, event);
        }
    }
    ...
    if newly_masked != 0 {
        ...
        if let Some(event) = update_irq(state, mask, false) {
            push_output_event(state, event);
        }
    }
}
