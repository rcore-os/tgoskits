...
use axdevice::{
    AccessWidth, DeviceGraphBuilder, DeviceManagerError, DeviceManagerResult, DeviceModel,
    DeviceNodeId, DeviceNodeSpec, GuestPhysAddr, MemoryBlockBackend, ResourcePools,
    ResourceRequest, ResourceSlot, VirtioBlock, VirtioBlockModel,
};
use axdevice_base::{ControllerInputId, DeviceResult, InterruptControllerId};
...
#[test]
fn block_model_uses_graph_planned_mmio_and_irq_resources() {
    ...
    graph
        .add(DeviceNodeSpec::virtual_device(node_id.clone(), model))
        .unwrap();
