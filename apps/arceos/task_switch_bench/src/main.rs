...
fn init_cycle_counter() -> Result<(), &'static str> {
    use ax_cpu::pmu::{CounterId, EventConfig, Pmu};

    if ax_runtime::task::sched::cpu_topology_len().map_err(|_| "read-cpu-topology-failed")? != 1 {
        return Err("cycle-benchmark-requires-one-cpu");
    }
    let _irq = ax_runtime::task::sync::IrqSaveGuard::new();
    // SAFETY: this dedicated single-vCPU guest has no other PMU consumer;
    // the IRQ guard excludes local reentry and migration during setup.
    let mut pmu = unsafe { Pmu::current() }.map_err(|_| "aarch64-pmuv3-unavailable")?;
    // SAFETY: the benchmark owns all counters; no perf events are installed.
    unsafe { pmu.reset() };
    pmu.configure(
        CounterId::CYCLE,
        EventConfig {
    ...
    .map_err(|_| "configure-cycle-counter-failed")?;
    pmu.enable(CounterId::CYCLE)
        .map_err(|_| "enable-cycle-counter-failed")?;
    pmu.start();
    let before = read_cycles();
    for _ in 0..100_000 {
        core::hint::black_box(());
    }
    if read_cycles() == before {
...
fn read_cycles() -> u64 {
    let cycles;
    // SAFETY: setup enables the dedicated counter before sampling. Both
    // benchmark threads run on the sole guest CPU and only read this register.
    // Keep measurement boundaries ordered without holding an IRQ guard over yield.
    unsafe {
        core::arch::asm!(
            "isb",
            "mrs {}, PMCCNTR_EL0",
...
fn spawn_bench_thread(
    ...
    let policy = bench_policy()?;
    ax_runtime::task::thread::ThreadBuilder::new(name.into())
        .policy(policy)
        .affinity(single_cpu_affinity()?)
        .spawn(entry)
