...
#[rustfmt::skip]
const fn file_perm_to_rwx_mode(mode: u32) -> [u8; 9] {
    let mut perm = [b'-'; 9];
    macro_rules! set {
...
#[cfg(unix)]
fn file_perm_to_rwx(permissions: &fs::Permissions) -> [u8; 9] {
    file_perm_to_rwx_mode(permissions.mode())
}
...
fn do_ls(args: &str) {
    ...
    fn show_entry_info(path: &str, entry: &str) -> io::Result<()> {
        ...
        let rwx = file_perm_to_rwx(&metadata.permissions());
