...
在 StarryOS 上运行 Weston (DRM backend + pixman 渲染器) 作为 Wayland
...
| 组件 | 状态 | 备注 |
|---|---|---|
| Weston DRM + pixman | ✅ 正常 | `--renderer=pixman` 路径，214x120 分辨率 |
| doomgeneric + SDL Wayland | ✅ 正常 | SDL_RENDER_DRIVER=opengles2，214x120 分辨率 |
...
1. 启动 Weston（`kiosk-shell`，DRM 后端，pixman 渲染器）
2. 等待 Wayland socket 就绪（最多 60 秒）
3. 启动 doomgeneric（SDL_VIDEODRIVER=wayland，SDL_RENDER_DRIVER=opengles2）
4. 验证进程存活 35 秒
5. 输出 `DOOMGENERIC_TEST_PASSED` 或 `DOOMGENERIC_TEST_FAILED`

## 构建说明

doomgeneric 从源代码构建，源代码位于 `apps/starry/doom/doomgeneric-master.zip`。

### 构建流程

1. 解压 `doomgeneric-master.zip` 到临时目录
2. 修改 `Makefile.sdl` 使用 `gcc` 而不是 `clang`
3. 使用 `Makefile.sdl` 编译 doomgeneric（需要 SDL2 开发库）
4. 复制编译好的二进制文件到 rootfs overlay

### 依赖

构建 doomgeneric 需要主机环境：
- SDL2 开发库 (`libsdl2-dev`, `libsdl2-mixer-dev`)
- 编译器 (`gcc`)
- `make` 工具
- `unzip` 工具
...
- 运行 `prebuild.sh` 构建 rootfs overlay（安装 Alpine 包、复制运行时库、复制 Freedoom WAD）
- 启动 QEMU（virtio-gpu-pci，VNC :0，4 核 2G，214x120 分辨率）
- 等待测试结果（QEMU 进程超时 60 秒）
...
```bash
bash apps/starry/doom/prebuild.sh
```
...
|---|---|---|
| 分辨率 | 284x160 | 214x120 |
| Weston 渲染器 | GL (llvmpipe) | pixman (软件) |
| 测试内容 | 视频播放 | 游戏运行 |
| 输入支持 | 无 | 鼠标 + 键盘（Weston/libinput） |
