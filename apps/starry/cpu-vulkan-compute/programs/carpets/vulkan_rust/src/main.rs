...
unsafe fn run(k: &mut Counter) -> i32 {
    ...
    // libvulkan is linked at build time (ash "linked" feature), matching the C/C++ cells: StarryOS
    // runs this static musl binary with no dynamic loader, so the default dlopen path (Entry::load)
    // is unavailable. Entry::linked() binds the provisioned libvulkan directly and is infallible.
