...
def enum_names(gen, field):
    # pyVulkan pins each enumerate call's backing buffer to the GENERATOR's lifetime (via a
    # WeakKeyDictionary). Materialising with list() drops the generator and frees the buffer, leaving
    # its StrWrap elements dangling - a use-after-free that only faults where the freed pages are
    # actually unmapped (e.g. StarryOS; a glibc host often still reads the stale bytes). Keep the
    # generator alive in _KEEP; StrWrap.__getattr__ already returns a real Python str (via _cstr),
    # and the read is now backed by live memory, so a single pass captures the names safely.
    _KEEP.append(gen)
    return [getattr(e, field) for e in gen]
...
def _spv(name):
    # on-target the compiled SPIR-V lives next to this script (prebuild stages it into <bin>/shaders);
    # in the source tree it lives under the sibling vulkan_c cell. Prefer the on-target layout.
    p = os.path.join(HERE, "shaders", name)
    return p if os.path.exists(p) else os.path.join(HERE, "..", "vulkan_c", "shaders", name)
SPV = _spv("vadd.spv")
MUL_SPV = _spv("mul.spv")
...
layer_names = enum_names(vk.vkEnumerateInstanceLayerProperties(), "layerName")
ok(isinstance(layer_names, list), "vkEnumerateInstanceLayerProperties")
ext_names = enum_names(vk.vkEnumerateInstanceExtensionProperties(None), "extensionName")
ok(len(ext_names) >= 1, "vkEnumerateInstanceExtensionProperties")
...
dev_ext_names = enum_names(vk.vkEnumerateDeviceExtensionProperties(pd, None), "extensionName")
ok(len(dev_ext_names) >= 1, "vkEnumerateDeviceExtensionProperties >=1")
