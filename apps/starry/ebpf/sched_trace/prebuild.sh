#!/usr/bin/env bash
# Build the `sched_trace` eBPF demo (aya loader + embedded bytecode) as a static musl
...
case "$arch" in
    ...
    *) echo "sched_trace prebuild: unsupported arch '$arch'" >&2; exit 1 ;;
esac
...
install_loongarch_loader_link() {
    ...
    if [[ -z "$rootfs" || ! -f "$rootfs" ]]; then
        echo "sched_trace prebuild: STARRY_ROOTFS is required for loongarch64 loader symlink" >&2
        exit 1
    fi

    echo "sched_trace prebuild: installing loongarch64 musl loader compatibility symlink"
...
echo "sched_trace prebuild: building eBPF demo for $musl_target (CC=$cc_bin)"
(
    cd "$app_dir"
...
bin="$app_dir/target/$musl_target/release/sched_trace"
[[ -x "$bin" ]] || { echo "sched_trace prebuild: build did not produce $bin" >&2; exit 1; }

install -Dm0755 "$bin" "$overlay_dir/usr/bin/sched_trace"
echo "sched_trace prebuild: installed $(basename "$bin") -> /usr/bin/sched_trace"
