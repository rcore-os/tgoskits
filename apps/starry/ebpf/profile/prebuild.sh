#!/usr/bin/env bash
# Build the `profile` eBPF demo (aya loader + embedded bytecode) as a static musl
...
case "$arch" in
    ...
    *) echo "profile prebuild: unsupported arch '$arch'" >&2; exit 1 ;;
esac
...
install_loongarch_loader_link() {
    ...
    if [[ -z "$rootfs" || ! -f "$rootfs" ]]; then
        echo "profile prebuild: STARRY_ROOTFS is required for loongarch64 loader symlink" >&2
        exit 1
    fi

    echo "profile prebuild: installing loongarch64 musl loader compatibility symlink"
...
echo "profile prebuild: building eBPF demo for $musl_target (CC=$cc_bin)"
(
    cd "$app_dir"
...
bin="$app_dir/target/$musl_target/release/profile"
[[ -x "$bin" ]] || { echo "profile prebuild: build did not produce $bin" >&2; exit 1; }

install -Dm0755 "$bin" "$overlay_dir/usr/bin/profile"
echo "profile prebuild: installed $(basename "$bin") -> /usr/bin/profile"
