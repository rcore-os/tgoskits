#!/usr/bin/env bash
# Build netmon (aya loader + embedded eBPF bytecode) as a static musl binary
...
case "$arch" in
    ...
    *) echo "netmon prebuild: unsupported arch '$arch'" >&2; exit 1 ;;
esac
...
install_loongarch_loader_link() {
    ...
    [[ -n "$rootfs" && -f "$rootfs" ]] || {
        echo "netmon prebuild: STARRY_ROOTFS required for loongarch64 symlink" >&2; exit 1
    }
...
echo "netmon prebuild: building for $musl_target (CC=$cc_bin)"
(
    cd "$app_dir"
...
bin="$app_dir/target/$musl_target/release/netmon"
[[ -x "$bin" ]] || { echo "netmon prebuild: build did not produce $bin" >&2; exit 1; }

install -Dm0755 "$bin" "$overlay_dir/usr/bin/netmon"
echo "netmon prebuild: installed -> /usr/bin/netmon"
