...
BASE=/tmp/nginx-phase41
CONF="$BASE/conf/sendfile-off.conf"
WWW="$BASE/www"
OUT="$BASE/out"
LOGDIR="$BASE/logs"
TIMEOUT_CMD=

log() { printf 'NGINX_PHASE41_LOG: %s\n' "$*"; }
...
master_process off;
worker_processes 1;
error_log /tmp/nginx-phase41/logs/error.log debug;
pid /tmp/nginx-phase41/nginx.pid;
events { worker_connections 64; }
http {
    include /etc/nginx/mime.types;
    access_log /tmp/nginx-phase41/logs/access.log;
    sendfile off;
    server {
        listen 127.0.0.1:8080;
        root /tmp/nginx-phase41/www;
    }
}
EOF
}
...
test_sendfile_off_large_stability() {
    i=1
    while [ "$i" -le 5 ]; do
...
test_sendfile_off_large_once || fail "sendfile off large file"
test_sendfile_off_large_stability || fail "sendfile off stability"
cleanup_nginx
printf 'NGINX_PHASE41_TEST_PASSED\n'
