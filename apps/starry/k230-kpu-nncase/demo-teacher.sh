...
if [[ "${WITH_REPLAY}" == "1" ]]; then
    ...
    if [[ ! -f "${CAPTURE}" ]]; then
        ...
        set +e
        if command -v stdbuf >/dev/null 2>&1; then
            K230_DEMO_USE_DOCKER=0 stdbuf -oL -eL bash "${K230_QEMU_CASE_DIR}/kpu-smoke/c/tools/demo-yolov8n-full-sequence-replay.sh" 2>&1 | tee "${REPLAY_LOG}"
        else
            K230_DEMO_USE_DOCKER=0 bash "${K230_QEMU_CASE_DIR}/kpu-smoke/c/tools/demo-yolov8n-full-sequence-replay.sh" 2>&1 | tee "${REPLAY_LOG}"
        fi
