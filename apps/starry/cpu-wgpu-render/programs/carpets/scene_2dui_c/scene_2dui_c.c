/* scene_2dui_c - 2D UI compositing RENDER-scene carpet driven through the gfx-rs wgpu-native v22.1.0.2
 * C API (webgpu.h / wgpu.h) on Mesa software adapters (lavapipe Vulkan / llvmpipe GL), no
 * GPU/window/surface. C port of the scene_2dui Rust cell: an offscreen 64x64 Rgba8Unorm texture is
 * rendered through real render pipelines (the SAME pixel-space y-flipped WGSL shaders as the Rust cell),
 * copied to a MAP_READ buffer (256-byte bytesPerRow padding) and read back; every scene primitive has an
 * INDEPENDENT closed-form software reference computed in C (not derived from the GPU output) and asserted
 * per pixel: filled axis-aligned rectangles, an analytic rounded-rect, a nine-patch-style scaled border
 * frame, an 8x8 bitmap-font glyph blit, a scissor-clipped fill, and MULTI-LAYER Porter-Duff over
 * compositing of 3 stacked semi-transparent layers. Closes with a negative control.
 *
 ...
 * only the wgpu-native C binding syntax differs. Uses the v22 callback (callback,userdata) async model
 * driven synchronously with wgpuInstanceProcessEvents / wgpuDevicePoll, like wgpu_render_c. Prints
 * "SCENE_2DUI_C OK <n>" only when FAIL==0 && TOTAL==EXPECTED==PASS. EXPECTED=28 pins the Rust cell. */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <math.h>
...
static int q8(float f) { return clampi((int)lroundf(f * 255.0f), 0, 255); }
...
static int frame(double cr, double cg, double cb, double ca, const DrawOp *ops, int nops, Fb *out) {
    ...
    att.clearValue = (WGPUColor){cr, cg, cb, ca};
    ...
    for (int i = 0; i < nops; i++) {
        ...
        if (op->bind) wgpuRenderPassEncoderSetBindGroup(pass, 0, op->bind, 0, NULL);
...
static void rect_verts(V2 out[6], float x0, float y0, float x1, float y1) {
    out[0] = (V2){{x0, y0}}; out[1] = (V2){{x1, y0}}; out[2] = (V2){{x0, y1}};
    out[3] = (V2){{x0, y1}}; out[4] = (V2){{x1, y0}}; out[5] = (V2){{x1, y1}};
}
static WGPUBuffer rect_vbo(float x0, float y0, float x1, float y1) {
    V2 v[6]; rect_verts(v, x0, y0, x1, y1);
    return make_buf(v, sizeof(v), WGPUBufferUsage_Vertex);
}
/* solid uniform: rgba + vp (8 floats) */
static WGPUBuffer solid_ubo(float r, float g, float b, float a) {
    float d[8] = {r, g, b, a, (float)W, (float)H, 0.0f, 0.0f};
    return make_buf(d, sizeof(d), WGPUBufferUsage_Uniform);
}
...
int main(void) {
    ...
    if (!inst) { printf("SCENE_2DUI_C FAIL\n"); return 1; }
    ...
    ok(1, "offscreen Rgba8Unorm target + readback buffer ready");
    ...
    WGPURenderPipeline pipe_solid = mk_solid(m_solid, solid_pll, &vbl_pos2, NULL);
    WGPUBlendState blend_over = {0};
    ...
    /* ---- Scene A: filled rectangles ---- */
    {
        ...
        DrawOp ops[2] = {
            {pipe_solid, ba, vr1, 6, 0, 0, 0, 0, 0},
            {pipe_solid, bb, vr2, 6, 0, 0, 0, 0, 0},
        };
        mapped = frame(0, 0, 0, 1, ops, 2, &fb);
        int bad = 0;
        for (int y = 0; y < (int)H; y++)
            for (int x = 0; x < (int)W; x++) {
                ...
                if (!peq(&fb, x, y, er, eg, eb, 255, 1)) bad++;
            }
        ok(mapped && bad == 0, "filled rectangles: every pixel matches closed-form rect coverage");
        ok(peq(&fb, 10, 10, 255, 0, 0, 255, 1), "rect A interior red");
        ok(peq(&fb, 44, 40, 0, 255, 0, 255, 1), "rect B interior green");
        ok(peq(&fb, 30, 30, 0, 0, 0, 255, 1), "gap between rects is background");
    ...
    {
        float rrd[16] = {12, 12, 52, 52, 1, 1, 0, 1, 8, 0, 0, 0, (float)W, (float)H, 0, 0};
        WGPUBuffer rr_ubo = make_buf(rrd, sizeof(rrd), WGPUBufferUsage_Uniform);
        WGPUBindGroup rr_bg = bind_ubo(rr_ubo, 64);
        WGPURenderPipeline pipe_rr = mk_solid(m_rr, solid_pll, &vbl_pos2, NULL);
        WGPUBuffer fq = rect_vbo(0, 0, (float)W, (float)H);
        DrawOp op = {pipe_rr, rr_bg, fq, 6, 0, 0, 0, 0, 0};
        mapped = frame(0, 0, 0, 1, &op, 1, &fb);
        int bad = 0, lit = 0;
        for (int y = 0; y < (int)H; y++)
            for (int x = 0; x < (int)W; x++) {
                ...
                if (cov) {
                    ...
                    if (corner) {
                        ...
                        if (sqrtf(dx * dx + dy * dy) > r) cov = 0;
                    }
                }
                if (cov) lit++;
                int er = cov ? 255 : 0, eg = cov ? 255 : 0;
                if (!peq(&fb, x, y, er, eg, 0, 255, 1)) bad++;
            }
        ok(mapped && bad == 0, "rounded-rect: every pixel matches analytic corner-arc coverage");
        ok(lit > 0, "rounded-rect: some pixels covered");
        ok(peq(&fb, 32, 32, 255, 255, 0, 255, 1), "rounded-rect center lit");
        ok(peq(&fb, 12, 12, 0, 0, 0, 255, 1), "rounded-rect clipped corner (12,12) is background");
        ok(peq(&fb, 32, 13, 255, 255, 0, 255, 1), "rounded-rect straight top edge lit");
    ...
    /* ---- Scene C: nine-patch-style scaled border frame ---- */
    {
        ...
        DrawOp ops[2] = {
            {pipe_solid, bblue, vbox, 6, 0, 0, 0, 0, 0},
            {pipe_solid, bdark, vinner, 6, 0, 0, 0, 0, 0},
        };
        mapped = frame(0, 0, 0, 1, ops, 2, &fb);
        int bad = 0;
        for (int y = 0; y < (int)H; y++)
        ...
        ok(peq(&fb, 5, 32, 0, 0, 255, 255, 1), "nine-patch left border blue");
        ok(peq(&fb, 32, 5, 0, 0, 255, 255, 1), "nine-patch top border blue");
        ok(peq(&fb, 32, 32, q8(0.1f), q8(0.1f), q8(0.1f), 255, 1), "nine-patch hollow interior");
    ...
    {
        const uint8_t GLYPH_H[8] = {0x00, 0x42, 0x42, 0x7E, 0x42, 0x42, 0x42, 0x00};
        uint8_t rgba[8 * 8 * 4];
        for (int r = 0; r < 8; r++)
            for (int c = 0; c < 8; c++) {
        ...
        dst.texture = gtex; dst.mipLevel = 0; dst.origin = (WGPUOrigin3D){0, 0, 0};
        ...
        WGPUTextureView gview = wgpuTextureCreateView(gtex, NULL);
        ...
        WGPUBindGroupLayoutDescriptor tld = {0};
        ...
        WGPURenderPipeline pipe_tex = mk_solid(m_tex, tex_pll, &vbl_pos2uv, NULL);

        V2UV gq[6] = {
            {{20, 20}, {0, 0}}, {{28, 20}, {1, 0}}, {{20, 28}, {0, 1}},
            {{20, 28}, {0, 1}}, {{28, 20}, {1, 0}}, {{28, 28}, {1, 1}},
        };
        WGPUBuffer gvbo = make_buf(gq, sizeof(gq), WGPUBufferUsage_Vertex);
        DrawOp op = {pipe_tex, tex_bg, gvbo, 6, 0, 0, 0, 0, 0};
        mapped = frame(0, 0, 0, 1, &op, 1, &fb);
        int bad = 0;
        for (int dy = 0; dy < 8; dy++)
            for (int dx = 0; dx < 8; dx++) {
                ...
                int v = lit ? 255 : 0;
                if (!peq(&fb, sx, sy, v, v, v, 255, 1)) bad++;
            }
        ok(mapped && bad == 0, "glyph blit: all 64 texels match hardcoded 8x8 'H' bitmap");
        ok(peq(&fb, 21, 23, 255, 255, 255, 255, 1), "glyph crossbar lit (col1,row3)");
        ok(peq(&fb, 23, 20, 0, 0, 0, 255, 1), "glyph row0 blank");
        ok(peq(&fb, 24, 21, 0, 0, 0, 255, 1), "glyph row1 middle blank (0x42)");
    ...
    /* ---- Scene E: scissor-clipped fill ---- */
    {
        ...
        WGPUBuffer sq = rect_vbo(0, 0, (float)W, (float)H);
        DrawOp op = {pipe_solid, bmag, sq, 6, 1, 16, 16, 20, 20};
        mapped = frame(0, 0, 0, 1, &op, 1, &fb);
        int bad = 0;
        for (int y = 0; y < (int)H; y++)
            for (int x = 0; x < (int)W; x++) {
                ...
                if (!peq(&fb, x, y, er, 0, eb, 255, 1)) bad++;
            }
        ok(mapped && bad == 0, "scissor-clipped fill: magenta only within [16,36)^2");
        ok(peq(&fb, 20, 20, 255, 0, 255, 255, 1), "scissor inside magenta");
        ok(peq(&fb, 40, 40, 0, 0, 0, 255, 1), "scissor outside background");
    ...
    /* ---- Scene F: MULTI-LAYER Porter-Duff over compositing ---- */
    {
        ...
        WGPUBuffer ubos[3]; WGPUBindGroup bgs[3]; WGPUBuffer vbos[3];
        DrawOp ops[3];
        for (int i = 0; i < 3; i++) {
            ...
            ops[i] = (DrawOp){pipe_blend, bgs[i], vbos[i], 6, 0, 0, 0, 0, 0};
        }
        mapped = frame(bg[0], bg[1], bg[2], bg[3], ops, 3, &fb);
        int bad = 0;
        for (int y = 0; y < (int)H; y++)
            for (int x = 0; x < (int)W; x++) {
                ...
                if (!peq(&fb, x, y, q8(c[0]), q8(c[1]), q8(c[2]), q8(c[3]), 2)) bad++;
            }
        ok(mapped && bad == 0,
           "multi-layer over: every pixel matches Porter-Duff over accumulation (incl partial-overlap regions)");
        {
            ...
            ok(peq(&fb, 32, 32, q8(c[0]), q8(c[1]), q8(c[2]), q8(c[3]), 2),
        ...
        }
        {
            ...
            ok(peq(&fb, 10, 32, q8(er), q8(eg), q8(eb), q8(ea), 2),
    ...
    /* ---- Negative control ---- */
    {
        ...
        DrawOp op = {pipe_solid, ba, vr1, 6, 0, 0, 0, 0, 0};
        frame(0, 0, 0, 1, &op, 1, &fb);
        ok(!peq(&fb, 10, 10, 0, 255, 0, 255, 4), "negative control: red rect pixel is NOT green");
        ok(!peq(&fb, 30, 30, 255, 0, 0, 255, 4), "negative control: background is NOT red");
