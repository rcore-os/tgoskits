...
case "$arch" in
    ...
    *) echo "error: unsupported gcc app arch: $arch" >&2; exit 1 ;;
esac
...
install_toolchain() {
    ...
    echo "GCC_PREBUILD installing gcc toolchain into staging rootfs"
...
print(f"GCC_PREBUILD staged {changed} changed file(s)")
PY
}
