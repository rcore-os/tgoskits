...
- `build-<target>.toml` ×4。
- `host-carpets/{yosys-cli-carpet.sh,yosys-sta-carpet.sh}` —— yosys 命令树 + yosys-sta PPA 流程的宿主地毯（host-validated 辅助，不参与 on-target 门控，见下文）。
...
`host-carpets/` 下两份脚本覆盖 yosys 综合器与 yosys-sta ASIC 流程的命令/选项面，是 **host-validated 辅助**：
yosys 及其下游 STA/功耗工具是宿主独占的 EDA 工具，没有在 StarryOS 上直跑的运行时（on-target 门控跑的是
yosys 综合出的门级网表的仿真，即上文 yosys 腿）。这两份脚本不参与 on-target `TEST PASSED` 门控，随 PR 附入供审阅。
