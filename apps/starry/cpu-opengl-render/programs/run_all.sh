#!/bin/sh
# On-target runner: set up the software desktop-OpenGL RENDER runtime and run the render carpet cells.
...
echo "cpu-opengl-render: detected CPU count = $ncpu; llvmpipe pinned single-threaded (LP_NUM_THREADS=1)"
...
marker_for() {
    case "$1" in
        opengl_render_cpp)  echo "OPENGL_RENDER_CPP_FULL_API OK" ;;
        opengl_render_rust) echo "OPENGL_RENDER_RUST_FULL_API OK" ;;
        opengl_render_py)   echo "OPENGL_RENDER_PY_FULL_API OK" ;;
...
run() {
    ...
    [ -x "$prog" ] || { echo "cpu-opengl-render: $name in manifest but binary absent at runtime"; total=$((total + 1)); fail=$((fail + 1)); return 0; }
    total=$((total + 1))
...
cd "$BIN" || exit 1
# Capability manifest: prebuild lists the render cells it provisioned on this arch. opengl_render_cpp
# (surfaceless-EGL C++, every arch), the four real-scenario C++ cells (scene_2dui / scene_3dmodel /
# scene_anim / scene_codec, every arch) and opengl_render_rust (glow, dynamic musl, every arch) are the
# guaranteed native cells; opengl_render_py (moderngl) appends where provisioned. Each cell renders
# clear/solid/gradient/checker/viewport/scissor/depth/blend (and, for the scene cells, UI compositing /
# a projected 3D model / a keyframed animation / a streaming-codec block pipeline) into an FBO
# and checks pixels back against a closed-form reference, plus a negative control.
MANIFEST="$BIN/expected_cells"
[ -f "$MANIFEST" ] || { echo "cpu-opengl-render: expected_cells manifest missing - prebuild provisioned no carpet"; echo "TEST FAILED"; exit 1; }
...
echo "cpu-opengl-render: $pass/$total render carpets OK on $(uname -m) (expected $EXPECTED: $(tr '\n' ' ' < "$MANIFEST"))"
if [ "$EXPECTED" -ge 1 ] && [ "$fail" -eq 0 ] && [ "$total" -eq "$EXPECTED" ] && [ "$pass" -eq "$EXPECTED" ]; then
    echo "TEST PASSED"; exit 0
else
    echo "cpu-opengl-render: GATE FAILED - need all $EXPECTED manifest carpets to pass (>=1 floor); got fail=$fail total=$total pass=$pass"
