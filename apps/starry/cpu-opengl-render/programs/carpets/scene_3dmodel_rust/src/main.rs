// scene_3dmodel_rust - 3D indexed-mesh RENDER-scene carpet on EGL-surfaceless / desktop GL 4.5 core /
// llvmpipe, driven by glow + khronos-egl (same context bring-up + off-screen FBO as opengl_render_rust).
...
fn run() -> i32 {
    ...
    unsafe {
        ...
        // VERSION + RENDERER strings are the loader working (mirrors the C++ cell's glr_load() plus its
        // glUniformMatrix4fv-via-eglGetProcAddress asserts, both of which glow resolves implicitly here).
        ok(!gl.get_parameter_string(glow::VERSION).is_empty(), "load GL render entry points (GL VERSION string)");
        ok(!gl.get_parameter_string(glow::RENDERER).is_empty(), "load matrix-uniform path (GL RENDERER string)");
        ...
        let prog = {
            ...
            gl.shader_source(f, "#version 450 core\nin vec3 vc;\nlayout(location=0) out vec4 o;\nvoid main(){ o=vec4(vc,1.0); }\n");
