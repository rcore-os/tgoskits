#!/usr/bin/env python3
# scene_3dmodel_py.py - 3D indexed-mesh RENDER-scene carpet via PyOpenGL on a surfaceless EGL desktop-GL
...
# desktop-GL entry points are reached through the current context; the non-empty GL VERSION + RENDERER
# strings are the loader working (mirrors the C++ cell's glr_load() plus its glUniformMatrix4fv-via-
# eglGetProcAddress asserts, both of which PyOpenGL resolves implicitly here).
...
rs = bytes(ren) if isinstance(ren, (bytes, bytearray)) else (ctypes.cast(ren, ctypes.c_char_p).value or b"")
ok(len(rs) > 0, "load matrix-uniform path (GL RENDERER string)")
