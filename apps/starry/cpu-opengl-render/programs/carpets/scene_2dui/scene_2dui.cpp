// scene_2dui.cpp - 2D UI compositing RENDER-scene carpet on EGL-surfaceless / GL 4.5 core / llvmpipe.
// Same context bring-up + off-screen FBO + assertion harness as opengl_render_cpp_full_api.cpp: a
// surfaceless EGL desktop-GL 4.5 core context (entry points via eglGetProcAddress through
// gl_render_loader.h), an off-screen RGBA8 color texture + DEPTH24 renderbuffer FBO, and glReadPixels
// closed-form per-pixel asserts. Orthographic pixel-space projection (gl_FragCoord.xy is the pixel
// center). Every scene primitive has an INDEPENDENT closed-form software reference computed in C++ (not
// derived from the GL output) and asserted per pixel: filled axis-aligned rectangles, an analytic
// rounded-rect (inside/corner-arc/outside coverage), a nine-patch-style scaled border frame (fixed
// corners, stretched edges), an 8x8 bitmap-font glyph blit (assert every lit/unlit texel), a
...
int main(){
  ...
  int EXPECTED=38, TOTAL=PASS+FAIL;
