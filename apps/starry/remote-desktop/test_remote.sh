...
# Reaching the desktop over VNC is the point of this app, so the gate goes
# through the VNC server itself: the RFB handshake and the banner pixels in a
# full framebuffer update.
python3 /usr/bin/rfb-check.py || fail "the desktop could not be verified over VNC"
