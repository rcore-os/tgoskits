...
# Assembles an Alpine rootfs overlay for a headless remote desktop on StarryOS:
# Xvfb (a pure software X framebuffer, no /dev/dri) drives feh, and x11vnc
# exports that framebuffer over VNC. QEMU forwards a host port to the guest's
# x11vnc, so the GUI is viewed remotely with no GPU device present at all.
# Reuses the Alpine-apk-over-qemu-user staging model from the web-browser app;
# only the package set (Xvfb + x11vnc instead of Weston/NetSurf) differs.
...
resize_rootfs() {
    ...
    echo "[remote-desktop prebuild] enlarging rootfs from ${current_mib}M to ${target_mib}M (+${extra}M)..."
...
REPO
    ...
    if [[ ! -f "$zlib_apk" ]]; then
        echo "[remote-desktop prebuild] downloading zlib apk..."
    ...
    if [[ -f "$zlib_apk" ]] && [[ -s "$zlib_apk" ]]; then
        ...
        echo "[remote-desktop prebuild] extracted zlib from zlib-1.3.2-r0.apk"
    fi

    echo "[remote-desktop prebuild] installing Xvfb + x11vnc via qemu-user apk..."
    ...
    LD_LIBRARY_PATH="$staging_root/lib:$staging_root/usr/lib" \
        "$qemu_runner" -L "$staging_root" \
            ...
            --no-progress \
            --no-scripts \
            add xvfb x11vnc tigervnc feh \
                xsetroot twm xterm font-misc-misc \
                ttf-dejavu fontconfig busybox-extras netcat-openbsd python3
}

populate_overlay() {
    echo "[remote-desktop prebuild] copying usr/ tree from staging to overlay..."
    ...
    if [[ -d "$staging_root/lib" ]]; then
        echo "[remote-desktop prebuild] copying lib/ tree from staging to overlay..."
    ...
    for sub in etc/fonts etc/X11; do
...
FC
    ...
    # Generate a font-free banner PNG so the remote view shows clear content
    # even if X fonts are unavailable; feh renders it into the Xvfb framebuffer.
    python3 "$app_dir/make_banner.py" "$overlay_dir/usr/share/remote-desktop/banner.png"

    install -Dm0755 "$app_dir/test_remote.sh" "$overlay_dir/usr/bin/test-remote.sh"
    install -Dm0755 "$app_dir/rfb_check.py" "$overlay_dir/usr/bin/rfb-check.py"
}
