...
frame. `REMOTE_DESKTOP_TEST_PASSED` is printed only after `rfb_check.py`, a
minimal VNC client run inside the guest, completes the RFB handshake with Xvnc
and finds the banner corners in a full framebuffer update.
