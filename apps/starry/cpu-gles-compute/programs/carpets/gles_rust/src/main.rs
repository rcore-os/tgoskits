...
fn run() -> i32 {
    ...
    ok(
        vendor.is_ok() && vendor.map(|s| !s.to_bytes().is_empty()).unwrap_or(false),
    ...
    // The chosen config must actually advertise the ES3 renderable bit we requested.
    {
        ...
        ok(
            rt.map(|v| (v & khronos_egl::OPENGL_ES3_BIT) != 0)
                .unwrap_or(false),
    ...
    unsafe {
        ...
        ok(
            64 <= wgs,
            "local_size_x(64) <= MAX_COMPUTE_WORK_GROUP_SIZE[0]",
        );
        ok(
            gl.get_error() == glow::NO_ERROR,
            "no GL error after limit queries",
        );

        // --- shader compile: NEGATIVE (bad shader) first ---
        let bad = gl
            .create_shader(glow::COMPUTE_SHADER)
            .expect("create bad shader");
        gl.shader_source(bad, CS_BAD);
        gl.compile_shader(bad);
        ok(
            !gl.get_shader_compile_status(bad),
            "bad shader COMPILE_STATUS == false",
        );
        ok(
            !gl.get_shader_info_log(bad).is_empty(),
            "bad shader info log non-empty",
        );
        ...
        ok(
            !gl.get_program_link_status(empty_prog),
            "empty program LINK_STATUS == false",
        );
        ok(
            !gl.get_program_info_log(empty_prog).is_empty(),
            "empty program info log non-empty",
        );
        ...
        ok(
            gl.get_error() == glow::NO_ERROR,
            "no GL error after shader/program build",
        );
        ...
        ok(
            buf0 != buf1 && buf1 != buf2 && buf0 != buf2,
            "glGenBuffers distinct names",
        );
        ...
        ok(
            gl.get_buffer_parameter_i32(glow::SHADER_STORAGE_BUFFER, glow::BUFFER_SIZE)
                == bytes as i32,
        ...
        // GLES 3.1: index >= MAX_SHADER_STORAGE_BUFFER_BINDINGS generates GL_INVALID_VALUE.
        {
            ...
            gl.bind_buffer_base(
                glow::SHADER_STORAGE_BUFFER,
                max_bind as u32 + 4096,
                Some(buf0),
            );
            let e = gl.get_error();
            ok(
                e == glow::INVALID_VALUE,
                "bind_buffer_base out-of-range -> INVALID_VALUE",
            );
            // restore binding 0
            gl.bind_buffer_base(glow::SHADER_STORAGE_BUFFER, 0, Some(buf0));
        }
        ...
        // --- ERROR INJECTION: invalid enum to bind_buffer target ---
        {
            ...
            ok(
                e == glow::INVALID_ENUM,
                "bind_buffer bad target -> INVALID_ENUM",
            );
            gl.bind_buffer(glow::SHADER_STORAGE_BUFFER, Some(buf2));
        }
        ...
        ok(
            gl.get_error() == glow::NO_ERROR,
            "glDispatchCompute vadd no error",
        );
        ...
        ok(m.is_some(), "glMapBufferRange READ");
        {
            let g = m
                .as_ref()
                .map(|v| (0..N).all(|i| feq(v[i], a[i] + b[i])))
                .unwrap_or(false);
        ...
        // SAME element-wise checker now rejects it.
        {
            ...
            poisoned[N / 2] += 1.0;
            let still_ok = (0..N).all(|i| feq(poisoned[i], a[i] + b[i]));
        ...
        gl.memory_barrier(glow::SHADER_STORAGE_BARRIER_BIT);
        {
            ...
            let g = m
                .as_ref()
                .map(|v| (0..N).all(|i| feq(v[i], 3.0 * a[i] + b[i])))
                .unwrap_or(false);
        ...
        // --- fence sync: fence_sync + client_wait_sync, assert command completion ---
        {
            ...
            if let Ok(f) = fence {
                ...
                ok(
                    r == glow::ALREADY_SIGNALED || r == glow::CONDITION_SATISFIED,
        ...
        // --- indirect dispatch: same saxpy(alpha=3) driven by a DISPATCH_INDIRECT_BUFFER ---
        {
            ...
            ok(
                gl.get_error() == glow::NO_ERROR,
                "glDispatchComputeIndirect no error",
            );
            gl.memory_barrier(glow::SHADER_STORAGE_BARRIER_BIT);
            gl.bind_buffer(glow::SHADER_STORAGE_BUFFER, Some(buf2));
            let m = map_read(&gl, N);
        ...
        gl.bind_buffer(glow::SHADER_STORAGE_BUFFER, Some(buf2));
        {
            ...
            let g = m
                .as_ref()
                .map(|v| (0..N).all(|i| feq(v[i], 2.0 + b[i])))
                .unwrap_or(false);
        ...
        // --- BOUNDARY: large, non-divisible size (>=1,000,000) verified element-wise vs closed form ---
        {
            ...
            let ngroups = BIG.div_ceil(64) as u32;
            ...
            if let Some(v) = m.as_ref() {
                ...
                ok(
                    !still_ok,
                    "neg-control large: corrupted tail element flagged",
                );
            } else {
        ...
        let idxz =
            get_program_resource_index(prog_raw, glow::SHADER_STORAGE_BLOCK, name_z.as_ptr());
        ok(
            idxz == GL_INVALID_INDEX,
            "glGetProgramResourceIndex(bad) == INVALID_INDEX",
        );
        ...
        ok(
            gl.get_error() == glow::NO_ERROR,
            "glGetError == GL_NO_ERROR (final happy path)",
        );
...
//
// # Safety
// The caller must uphold the OpenGL mapping protocol for this call:
// - a current GL context owning `gl` must be active on this thread;
// - the `SHADER_STORAGE_BUFFER` target must be bound to the buffer to read, and that buffer must
//   hold at least `n` `f32`s (`n * size_of::<f32>()` bytes) of initialized, readable storage;
// - the buffer must not already be mapped.
// The returned data is copied out before this function returns, so the mapped pointer never
// escapes; the buffer is left mapped and the caller must `glUnmapBuffer` it afterwards.
unsafe fn map_read(gl: &glow::Context, n: usize) -> Option<Vec<f32>> {
    ...
    // SAFETY: map_buffer_range returned a non-null pointer to `bytes` readable bytes for the
    // currently bound SSBO (caller contract above), so `n` consecutive f32s are valid to read.
