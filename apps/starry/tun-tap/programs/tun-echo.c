...
// StarryOS. Scoped to the IFF_TUN | IFF_NO_PI packet path; the ioctl ABI matrix
// is covered by test-suit/starryos/qemu/system/bugfix-bug-tun-tap-abi.
//
...
//      checksum is valid
//
// A second phase sends a UDP datagram to another host on the tun0 subnet and
// reads it back off the fd.
//
// Every check prints PASS/FAIL with a label; the process exit status carries
// the verdict to run-tun-tap.sh.
