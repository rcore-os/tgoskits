...
int main(void) {
    ...
    /* -------- a timestamp with an extra colon field is malformed, not re-timed -------- */
    static const char *VTT_EXTRA_FIELD = "WEBVTT\n\n01:02:03:04.005 --> 01:02:05.000\nX\n";
    track tx;
    rc = parse_vtt(VTT_EXTRA_FIELD, &tx);
    gate_check(&g, rc != 0, "timestamp with an extra colon field rejected");
    if (rc == 0) track_free(&tx);
