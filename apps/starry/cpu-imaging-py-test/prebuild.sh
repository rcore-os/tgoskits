...
provision_pip_wheels() {
    ...
    # Host pip otherwise checks the target wheels against the host interpreter and rejects them.
    "$pip" install --no-index --find-links "$WHEELS" --target "$sitepkgs" --upgrade --no-deps \
        --only-binary=:all: --python-version "$pymm" --implementation cp --abi "cp${pymm//./}" \
        --platform "$pip_plat" --platform any "$WHEELS"/*.whl 2>"$WHEELS/install.log" \
