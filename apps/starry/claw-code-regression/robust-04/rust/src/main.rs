...
fn main() {
    ...
    println!("=== Robust-04: date and /etc listing report ===");
    let output = Command::new("sh").arg("-c").arg(format!("cd /tmp/work && {env} timeout 600 /tmp/claw --allowedTools bash,write prompt 'create a markdown report with the current date and a listing of files in /etc, save to report.md' 2>&1")).output().unwrap();
    println!("{}", String::from_utf8_lossy(&output.stdout));

    if std::path::Path::new("/tmp/work/report.md").exists() {
        let s = fs::read_to_string("/tmp/work/report.md").unwrap_or_default();
        println!("report.md: {} lines", s.lines().count());
        if s.to_lowercase().contains("date") || s.contains("20") { println!("ALL_TESTS_DONE"); } else { println!("ALL_TESTS_FAILED"); std::process::exit(1); }
