...
fn main() {
    ...
    println!("=== Robust-10: system reconnaissance report ===");
    let output = Command::new("sh").arg("-c").arg(format!("cd /tmp/work && {env} timeout 900 /tmp/claw --allowedTools bash,write prompt 'create a comprehensive system reconnaissance report: check uname -a, list mount points, show memory info from /proc/meminfo, list all running processes, and compile everything into a sysreport.md file' 2>&1")).output().unwrap();
    println!("{}", String::from_utf8_lossy(&output.stdout));

    if std::path::Path::new("/tmp/work/sysreport.md").exists() {
        let s = fs::read_to_string("/tmp/work/sysreport.md").unwrap_or_default();
        println!("sysreport.md: {} lines", s.lines().count());
        let lower = s.to_lowercase();
        if lower.contains("uname") || lower.contains("linux") || lower.contains("starry") { println!("ALL_TESTS_DONE"); } else { println!("ALL_TESTS_FAILED"); std::process::exit(1); }
