    ...
    LD_LIBRARY_PATH="$staging_root/lib:$staging_root/usr/lib" \
        "$qemu_runner" -L "$staging_root" \
            ...
            add weston weston-backend-drm weston-shell-desktop \
                ...
                weston-clients netpbm

    # apk --no-scripts skipped the triggers that build GTK's runtime caches, so
    # build them with the host tools; the caches are architecture-independent.
    # GTK picks its icon decoder through the mime database, and an empty
    # GdkPixbuf loaders.cache would register no loaders, not even the built-in
    # PNG one, so the cache is removed rather than generated.
    rm -f "$staging_root/usr/lib/gdk-pixbuf-2.0/2.10.0/loaders.cache"
    if command -v update-mime-database >/dev/null 2>&1; then
        echo "[browser prebuild] building shared-mime-info mime.cache (host)..."
        update-mime-database "$staging_root/usr/share/mime" >/dev/null 2>&1 \
            || { echo "[browser prebuild] update-mime-database failed" >&2; exit 1; }
    fi
    [[ -s "$staging_root/usr/share/mime/mime.cache" ]] \
        || { echo "[browser prebuild] mime.cache is missing; install shared-mime-info on the host" >&2; exit 1; }
    if command -v gtk-update-icon-cache >/dev/null 2>&1; then
        for theme in Adwaita hicolor; do
            [[ -d "$staging_root/usr/share/icons/$theme" ]] || continue
            gtk-update-icon-cache -f -t "$staging_root/usr/share/icons/$theme" >/dev/null 2>&1 || true
        done
    fi
}
...
FC
    ...
    install -Dm0644 "$app_dir/render-probe.css" "$overlay_dir/usr/share/web-browser/render-probe.css"
    # Serves test.html over loopback. A merged-/usr image ships it in usr/bin,
    # which the tree copy above already carried; older images keep it in bin/,
    # which that copy skips.
    if [[ -f "$staging_root/bin/busybox-extras" ]]; then
        install -Dm0755 "$staging_root/bin/busybox-extras" "$overlay_dir/usr/bin/busybox-extras"
    fi
    [[ -x "$overlay_dir/usr/bin/busybox-extras" ]] \
        || { echo "[browser prebuild] busybox-extras is missing from the staged tree" >&2; exit 1; }
