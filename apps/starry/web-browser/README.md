...
| `test.html` | Self-contained local page (HTML/CSS/tables); its `#36b2a0` block is what the test finds in a screen capture |
| `render-probe.css` | Stylesheet linked only from `test.html`; its request in the httpd log shows NetSurf parsed the page |
| `test_browser.sh` | Guest-side test: start Weston, launch NetSurf on the page, and require the page request, the stylesheet request and the page colour in a capture of the output |
...
1. `prebuild.sh` (host): stage an Alpine rootfs and `apk add` Weston + NetSurf +
   its GTK3/cairo/pango/gdk-pixbuf dependencies and fonts, plus
   `weston-screenshooter` and netpbm for the render check, then copy the tree
...
2. `test_browser.sh` (guest): start Weston with the DRM backend and pixman
   ...
   window, the httpd log shows it requested `render-probe.css` (only a browser
   parsing `test.html` asks for it), and a capture of the composited output taken
   with `weston-screenshooter` holds most of the page's 240x120 `#36b2a0` block.
   Weston runs with `--debug` because that is what authorizes the capture. Set
   `BROWSER_URL` to load another page instead; such a run is held only to the
   process check.
