...
static void test_cache_present(enum arch a) {
    ...
    for (int i = 0;; i++) {
         ...
         *
         * Only cpu0 is online, so EVERY leaf collapses to the single member
         * cpu0: shared_cpu_map == "1" (hex bit 0) and shared_cpu_list == "0",
         * whatever its sharing scope. The kernel builds these per arch:
         * aarch64 / loongarch64 use the level-only cache_leaves_are_shared()
         * rule (L1 private, L2+ shared by all online CPUs), while x86 honours
         * CPUID.4 num_threads_sharing: a leaf shared beyond its owner is the
         * package domain (every online CPU) under this single-socket topology,
         * and only an L1 / num_threads_sharing==1 leaf is private. Every one of
         * those scopes renders to bit 0 at one online CPU. The scope difference
         * (an aarch64 L2/L3 listing "0-3"
         * while L1 stays "0") is only observable with >1 online CPU; that
         * multi-core case is covered by the -smp 4 kernel system-suite test
         * test-sysfs-cpu-topology. Here we assert the smp1 collapse for every
         * leaf.
         */
...
static void test_node_meminfo(void) {
    ...
    /*
     * node0/distance (drivers/base/node.c:node_read_distance, node_dev_attrs[]
     * installs it unconditionally). For the single UMA node here it is the
     * self-distance LOCAL_DISTANCE == 10 (include/linux/topology.h).
     */
    char dist[128] = "";
    snprintf(path, sizeof path, "%s/distance", NODE0);
    check(path_exists(path), "node0/distance present");
    if (read_file(path, dist, sizeof dist) == 0) {
        printf("  node0/distance = %s\n", dist);
        long d = parse_ulong(dist);
        check(d >= 10, "node0/distance parses to >= LOCAL_DISTANCE(10)");
        check(d == 10, "node0/distance == 10 (single UMA node self-distance)");
    } else {
        check(0, "node0/distance readable");
    }
}

/* ---- cpu0/topology (drivers/base/topology.c) ---- */

/* Read cpu0/topology/<file> into out; returns 0 on success. */
static int read_topology(const char *file, char *out, size_t cap) {
    char path[256];
    snprintf(path, sizeof path, "%s/topology/%s", CPU0, file);
    return read_file(path, out, cap);
}

static void test_topology(enum arch a) {
    char path[256], val[128];
    printf("\n== cpu0/topology ==\n");

    /*
     * Unconditional attributes: drivers/base/topology.c installs core_siblings
     * (and its _list) with no #ifdef in bin_attrs[], backed by core_cpumask -
     * the package/socket domain, the same mask package_cpus renders.
     */
    struct {
        const char *file;
    } files[] = {
    ...
    if (read_topology("core_id", val, sizeof val) == 0) {
        check(parse_ulong(val) >= 0, "topology/core_id parses as >=0 int");
    } else {
        check(0, "topology/core_id readable");
    }
    if (read_topology("physical_package_id", val, sizeof val) == 0) {
    ...
    if (read_topology("core_cpus_list", val, sizeof val) == 0) {
        check(strcmp(val, "0") == 0, "topology/core_cpus_list == \"0\" (single core)");
    } else {
        check(0, "topology/core_cpus_list readable");
    }
    if (read_topology("thread_siblings_list", val, sizeof val) == 0) {
    ...
    /*
     * core_siblings is Linux's core_cpumask == the package domain, so it must
     * equal package_cpus byte-for-byte (both are the %*pb hex mask), and its
     * _list sibling must equal package_cpus_list.
     */
    char csib[128] = "", pkg[128] = "";
    if (read_topology("core_siblings", csib, sizeof csib) == 0 &&
        read_topology("package_cpus", pkg, sizeof pkg) == 0) {
        printf("  core_siblings=%s package_cpus=%s\n", csib, pkg);
        check(strcmp(csib, pkg) == 0, "topology/core_siblings == package_cpus (same core_cpumask)");
    } else {
        check(0, "topology/core_siblings + package_cpus readable");
    }
    char csibl[128] = "", pkgl[128] = "";
    if (read_topology("core_siblings_list", csibl, sizeof csibl) == 0 &&
        read_topology("package_cpus_list", pkgl, sizeof pkgl) == 0) {
        check(strcmp(csibl, pkgl) == 0, "topology/core_siblings_list == package_cpus_list");
    } else {
        check(0, "topology/core_siblings_list + package_cpus_list readable");
    }

    /*
     * cluster_* gate behind Linux's TOPOLOGY_CLUSTER_SYSFS: present on
     * x86_64/aarch64/riscv64, absent on loongarch64. Where present, at -smp 1
     * cluster_cpus_list collapses to "0" (this CPU's own cluster).
     */
    int cluster_expected = (a == ARCH_X86_64 || a == ARCH_AARCH64 || a == ARCH_RISCV64);
    snprintf(path, sizeof path, "%s/topology/cluster_cpus", CPU0);
    int cluster_present = path_exists(path);
    printf("  topology/cluster_cpus present = %d (expected %d for this arch)\n", cluster_present,
           cluster_expected);
    check(cluster_present == cluster_expected,
          "topology/cluster_cpus presence matches TOPOLOGY_CLUSTER_SYSFS arch set");
    if (cluster_expected && cluster_present) {
        if (read_topology("cluster_cpus_list", val, sizeof val) == 0) {
            printf("  topology/cluster_cpus_list = %s\n", val);
            check(strcmp(val, "0") == 0, "topology/cluster_cpus_list == \"0\" (single CPU cluster)");
        } else {
            check(0, "topology/cluster_cpus_list readable");
        }
        if (read_topology("cluster_id", val, sizeof val) == 0) {
            check(parse_ulong(val) >= 0, "topology/cluster_id parses as >=0 int");
        } else {
            check(0, "topology/cluster_id readable");
        }
    }

    /*
     * die_* gate behind TOPOLOGY_DIE_SYSFS: only x86_64 among the four targets
     * defines topology_die_id, so die_id/die_cpus[_list] must be x86-only and
     * absent everywhere else (they were fabricated on aa/rv/la before this fix).
     */
    int die_expected = (a == ARCH_X86_64);
    snprintf(path, sizeof path, "%s/topology/die_cpus", CPU0);
    int die_present = path_exists(path);
    printf("  topology/die_cpus present = %d (expected %d for this arch)\n", die_present,
           die_expected);
    check(die_present == die_expected,
          "topology/die_cpus presence matches TOPOLOGY_DIE_SYSFS (x86_64 only)");
}
