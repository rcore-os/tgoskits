// scene_3dmodel.cpp - 3D indexed-mesh RENDER-scene carpet on EGL-surfaceless / GLES 3.1 / llvmpipe.
// Same context bring-up + off-screen FBO (RGBA8 + DEPTH24) + glReadPixels harness as
// gles_render_cpp_full_api.cpp. Renders an indexed cube mesh with a hand-computed Model-View-
// Projection matrix (perspective), depth-buffered occlusion (GL_LESS), and Gouraud shading
// (per-vertex color interpolated by GL across each triangle). The assertion is an INDEPENDENT
// software reference rasterizer written in C++: verts are transformed by the SAME MVP -> clip ->
// NDC (perspective divide) -> viewport pixels; for each pixel we compute barycentric coordinates
// against every projected triangle, do a perspective-correct interpolated depth test in a private
// z-buffer, and interpolate the vertex colors, then compare the reference framebuffer to the GL
// readback per pixel (with a small tolerance for edge-sample and rounding differences, counting the
// fraction of matching pixels rather than requiring bit-exactness on antialias-free edges). GL uses
// NDC z in [-1,1] (GL convention). Closes with a negative control. Prints "SCENE_3DMODEL OK <n>"
...
int main(){
  ...
  int EXPECTED=18, TOTAL=PASS+FAIL;
