// scene_anim.cpp - keyframe-animation RENDER-scene carpet on EGL-surfaceless / GLES 3.1 / llvmpipe.
// Same context bring-up + off-screen FBO (RGBA8 + DEPTH24) + glReadPixels harness as
// gles_render_cpp_full_api.cpp. Renders N=4 keyframes of a transformed unit quad into the FBO. Each
// frame the quad's model transform is a rotation about the FBO center composed with a translation and
// uniform scale, all interpolated by the frame parameter t in {0, 0.25, 0.5, 0.75}. The transform is
// applied in the vertex shader from a hand-built pixel-space model matrix (rotation matrix from
// angle(t), scale, translate) and an ortho pixel->NDC map. For every frame the four rotated/scaled/
// translated quad CORNERS are computed INDEPENDENTLY in C++ (closed-form: R(theta)*S*local + T), and
// the readback is asserted at those exact corner pixels (color present) plus a point just outside the
// quad (background), so the assertion pins the closed-form transformed geometry, not a visual gestalt.
// A cubic ease function eased(t)=3t^2-2t^3 drives the scale, and its value is asserted at each t. NDC
// z is unused (2D). Closes with a negative control. Prints "SCENE_ANIM OK <n>" only when FAIL==0 &&
// TOTAL==EXPECTED==PASS. Software rasterizer (llvmpipe), deterministic.
#include <EGL/egl.h>
...
int main(){
  ...
  int EXPECTED=45, TOTAL=PASS+FAIL;
