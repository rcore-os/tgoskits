#!/usr/bin/env python3
# scene_codec_py.py - streaming/codec-math RENDER-scene carpet via PyOpenGL on a surfaceless EGL ES 3.1
# context over Mesa llvmpipe (OpenGL.EGL + OpenGL.GLES3, same bring-up as gles_render_py_full_api.py).
# Mirrors scene_codec.cpp behaviour-identically, each path asserted against an INDEPENDENT numpy
# closed-form reference: (1) BT.601 full-range YUV->RGB in a three-plane fragment shader; (2) chroma
# 4:2:0 -> 4:4:4 NEAREST upsample; (3) bilinear 2x downscale (GL_LINEAR vs a 2x2 box average); (4)
# CPU-path round-trips - an 8-sample DCT-II forward/IDCT reconstruction identity, plus an RLE
# encode/decode identity. Closes with a negative control. Prints "SCENE_CODEC_PY OK <n>" only when
# FAIL==0 && TOTAL==EXPECTED==PASS. Software rasterizer (llvmpipe), deterministic.
...
gl.glTexParameteri(gl.GL_TEXTURE_2D, gl.GL_TEXTURE_WRAP_S, gl.GL_CLAMP_TO_EDGE)
gl.glTexParameteri(gl.GL_TEXTURE_2D, gl.GL_TEXTURE_WRAP_T, gl.GL_CLAMP_TO_EDGE)
prog = mkprog(VS_UV, "#version 310 es\nprecision highp float;\nin vec2 uv;\nlayout(location=0) out vec4 o;\nuniform sampler2D s;\nvoid main(){ o=texture(s,uv); }\n")
