...
sysbench 仅手动执行，不接入 CI。`apps/.ignore` 将它排除在应用批量发现和 `app qemu --all` 之外；显式指定 `-t benchmark/sysbench` 仍可运行下面的 QEMU 与板卡命令。工具回归也按需手动执行。
...
```bash
cargo xtask starry app board -t benchmark/sysbench -b OrangePi-5-Plus
```
...
```bash
cargo xtask starry app board -t benchmark/sysbench --linux-stage
```
...
```bash
cargo xtask starry app qemu -t benchmark/sysbench --arch aarch64
cargo xtask starry app qemu -t benchmark/sysbench --arch aarch64 --qemu-config qemu-aarch64-matrix.toml
```
