...
| 完整开发示例 | [docs/docs/contributing/demo.md](docs/docs/contributing/demo.md) | 从零新增一个组件或者修改组件的完整开发流程示例 |
| 组件开发说明 | [docs/docs/development/components.md](docs/docs/development/components.md) | 从 `components/` 或 `os/arceos/modules/` 开始 |
| 开发 ArceOS | [docs/docs/development/arceos.md](docs/docs/development/arceos.md) | `cargo xtask arceos qemu --package ax-helloworld --arch aarch64` |
| 开发 StarryOS | [docs/docs/development/starryos.md](docs/docs/development/starryos.md) | `cargo xtask starry qemu --arch aarch64` |
| 开发 Axvisor | [docs/docs/development/axvisor.md](docs/docs/development/axvisor.md) | `cargo xtask axvisor qemu --arch aarch64` |
| 理解命令构建和测试矩阵 | [docs/docs/design/build/flow.md](docs/docs/design/build/flow.md) | `cargo xtask test` |
| 理解仓库如何组织众多独立组件 | [docs/docs/contributing/repo.md](docs/docs/contributing/repo.md) | `python3 scripts/repo/repo.py list` |
...
与组件仓库之间如需同步，请由维护者显式执行 `scripts/repo/repo.py pull/push`。详见 [docs/docs/contributing/repo.md](docs/docs/contributing/repo.md)。
...
Axvisor 不能只靠 `build/qemu` 两条命令直接跑起来，因为运行前仍需要 Guest 镜像、VM 配置和 rootfs。推荐先用 `os/axvisor/scripts/setup_qemu.sh` 自动准备这些运行时资源，再执行 `cargo xtask axvisor qemu --arch <arch>`。完整说明见 [docs/docs/build/qemu.md](docs/docs/build/qemu.md) 和 [docs/docs/development/axvisor.md](docs/docs/development/axvisor.md)。
...
![VS Code 调试目标选择与断点命中效果](docs/docs/debug/images/debug_target.png)

首次使用前请确保已安装 [CodeLLDB](https://marketplace.visualstudio.com/items?itemName=vadimcn.vscode-lldb) 扩展、执行 `rustup target add aarch64-unknown-none-softfloat`、以及宿主机 `PATH` 中有 `qemu-system-aarch64`。完整说明见 [调试设计文档](docs/docs/debug/overview.md)。
