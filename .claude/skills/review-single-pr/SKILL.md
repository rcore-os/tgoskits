...
description: 审查本 tgoskits 仓库中一个指定的 GitHub 拉取请求。适用于用户指定 PR 编号或网址，要求审查、复审、先分析当前精确提交的 CI job、step 和日志、暂缓仍有相关 CI 运行的 PR、跳过 CI 证据缺失或不可证实的 PR、对照 Linux/POSIX/RFC/VirtIO 语义、检查重复实现或相关开放 PR、建立并关闭 PR 专属审查清单、验证测试位置与 CI 执行链路、对直接修改 apps/** 的可运行应用执行必要的真实 app run、修复安全的合并冲突、提交面向初学者的中文行内评论、批准或请求修改，以及审查后依据 .github/MAINTAINERS.md 推荐并分配审查人。
---
...
没有完整阅读本技能和所有适用规范时，不得提交 `APPROVE`、`REQUEST_CHANGES`、不提交审查的总结或任何面向 PR 的评论。唯一例外是“当前提交 CI 前置门禁”返回 `CI_DEFERRED` 或 `CI_SKIPPED` 时，只向用户报告当前 SHA、相关检查和状态或证据缺口；该门禁报告不是审查结论，也不得写入 PR。规则重叠时采用更严格者；跳过要求时记录具体理由和证据。
...
在线 PR 在详细判断、完整加载领域规范、创建清单、建立工作树或运行本地命令前，先按“当前提交 CI 前置门禁”读取最少的当前提交、变更路径和检查状态。门禁返回 `CI_DEFERRED` 或 `CI_SKIPPED` 时，不创建 PR 专属清单，也不继续处理该 PR。
...
每个清单项写明具体受影响范围和预期证据，覆盖：当前提交信息、既有审查讨论、CI 覆盖台账、工作树、必要时的冲突、每个受影响模块及审查视角、代码质量基线、功能开发规范适用性、领域语义、测试的位置/构建/发现/选择/CI 执行、重复与重叠分析、直接修改 `apps/**` 时的应用运行证据、阻塞问题与评论、当前提交刷新、审查提交、审查人分配和清理。每个受影响应用及每个新增或迁移测试都建立独立证据项目；普通验证项目以当前提交的检查名、任务、命令、架构或配置、日志和后置条件关闭。禁止使用“审查代码”“运行测试”之类泛化项目。
...
只审查指定的一个 PR，先分析当前精确提交的 CI job、step 和日志，再在隔离工作树中完成静态代码分析；普通审查不运行本地格式化、构建、clippy、测试、QEMU 测试、元数据、打包或发布演练。只有 PR 直接新增、修改或重命名进入 `apps/**` 的可运行应用，且当前提交 CI 未执行同一应用和目标时，才按“CI 与应用验证”运行真实 app。审查同时判断它是否重复基准分支已有功能、与其他开放 PR 重叠、冲突或已被取代。没有阻塞问题时提交 `APPROVE`；存在正确性、规范、重复、测试或 CI 问题时，以中文行内评论提交 `REQUEST_CHANGES`。审查完成后，仅在仍需领域跟进时依据 `.github/MAINTAINERS.md` 分配合适的人类审查人。
...
优先使用 GitHub 连接器获取结构化数据，本地 `git` 用于获取、工作树和静态差异分析；只有连接器无法满足当前分支发现、GraphQL 讨论、Actions 日志或带锚点提交等需求时才使用 `gh`。除直接修改 `apps/**` 的应用运行例外外，不用本地命令填补 CI 证据缺口。
...
本节只适用于存在真实在线 PR 的正常模式；离线基准模式保持原约定。先解析当前精确 `headRefOid` 或 `head.sha`，再用最少的变更路径和 PR 声明判断哪些检查与本 PR 相关。任何详细代码审查、领域规范加载、任务清单、工作树、应用运行或 PR 写操作都必须等待本门禁完成。
...
若任一相关检查或任务为 `queued`、`pending`、`waiting` 或 `in_progress`，将结果标为 `CI_DEFERRED` 并立即停止。不得等待或轮询到结束，不得完整审查、创建工作树、运行应用、创建 PR 专属清单、解决讨论、提交评论或审查、创建或更新议题、修复冲突或更改审查人；只向用户报告 PR、当前 SHA、未结束检查名和状态。无关发布任务或按变更范围明确不适用的任务不触发暂缓，但必须记录不相关理由。

没有相关运行中检查时，建立 CI 覆盖台账。每项台账记录受影响行为或声明、检查与任务名、当前 SHA、结论、实际命令、架构或配置、用例或二进制、关键日志和可观察后置条件。只有 `success` 且工作流定义与日志表明当前精确提交实际执行了同一命令、架构或配置和目标行为，成功标记真实出现且失败能传播到 job 结论时，才接受为覆盖；检查名或宽泛汇总绿灯、旧 SHA、路径或矩阵跳过、架构或 feature 不同、命令不等价、未达到成功标记，或无法证明新增用例被发现和执行，都不算覆盖。摘要不足时必须检查 job、step 和完整相关日志，不能只看 check rollup。

使用四种稳定结果：`CI_DEFERRED`、`CI_SKIPPED`、`APPROVE`、`REQUEST_CHANGES`。相关 CI 失败时进入后续日志归因和详细审查。对非应用运行项，只要相关 CI 缺失、取消、陈旧、被跳过、覆盖可疑或日志不能证明真实执行，就标为 `CI_SKIPPED` 并立即停止：不创建工作树或 PR 专属清单，不做详细代码审查，不提交 GitHub review，只向用户报告 PR、当前 SHA、检查或任务、证据缺口和跳过原因。只有直接新增、修改或重命名进入 `apps/**` 可运行应用目录的应用执行缺口可以进入后续真实 app run；其他相关 CI 缺口仍会使整个 PR 成为 `CI_SKIPPED`。CI 证据和应用运行都不能替代代码、架构、ABI、生命周期、安全性、文档和测试可信度审查。
...
取消、缺失、陈旧、跳过或不可证实的相关检查已在前置门禁返回 `CI_SKIPPED`，不进入本节。对门禁允许继续的每个失败检查，提交审查前检查 job、step、完整相关日志和基准分支状态，并分类为“与 PR 相关”“与 PR 无关”或“无法确定”：
...
- 无法确定：合理检查 job、step、完整相关日志和基准分支状态后仍无法判断归因时，标为 `CI_SKIPPED`；不提交 GitHub review，只向用户报告当前 SHA、失败任务、已检查证据和无法确定的原因。
...
仅在用户明确要求，或审查没有其他阻塞问题、本应 `APPROVE` 且当前 `mergeStateStatus=DIRTY`、`maintainerCanModify=true` 时修复。修复并推送、对新提交重新执行 CI 门禁和日志分析前不得批准。
...
推送后刷新 PR，并对新 `headRefOid` 重新执行“当前提交 CI 前置门禁”。相关 CI 仍在运行时返回 `CI_DEFERRED`；相关 CI 缺失、跳过、陈旧或不可证实时返回 `CI_SKIPPED`。不得在新 SHA 上运行本地验证来替代 CI；只有新提交直接改变 `apps/**` 可运行应用且缺少同应用、同目标的 CI 执行时，才应用 app run 例外。冲突消失后，`BLOCKED`/`UNSTABLE` 仍可能由 CI 或审查状态导致，不能仅据此判定冲突修复失败。只做冲突演练时不得推送或提交审查；记录 PR、批准状态、冲突文件、语义解法、差异卫生结果和未修改 GitHub 的事实，然后清理工作树。
...
错误修复必须有确定性回归或复现：未修复实现必然失败，修复后同一测试通过；除非有具体证据说明环境不可能做到，否则缺少失败/通过证明即阻塞。普通的当前提交成功 CI 只证明修复态 green；未修复态 red 只能由测试逻辑必然失败的推导、作者提供的 red/green 记录或明确执行未修复变体的专门 CI 提供，不运行本地基准验证。原始系统调用修复优先直接覆盖 `syscall(SYS_...)`，避免 libc 封装掩盖返回值或错误码。
...
每个直接新增、修改或重命名进入 `apps/**` 可运行应用目录的 StarryOS/ArceOS 应用，都建立独立证据项，列明文档化环境准备、架构、运行命令和可观察后置条件。当前精确提交的成功 CI 若证明实际执行了同一准备、应用、命令、目标和后置条件，则关闭为 CI 已覆盖；否则按“CI 与应用验证”运行真实 app。纯删除不触发应用运行。`apps/**` 之外的应用支持声明、运行器、rootfs、打包、日志解析或配置改动只分析 CI 和源码，不触发本地应用运行；相关 CI 存在缺口时返回 `CI_SKIPPED`。文档本身无法让用户准备环境、需要未记录的临时绕过或声明范围与证据不一致时，仍提交 `REQUEST_CHANGES`。
...
普通审查只使用当前精确提交的远端 CI 和只读源码分析。允许读取 Git 历史、差异、清单、工作流定义、CI job、step 和日志，也允许用 `rg` 等只读搜索定位证据；禁止运行本地格式化检查、构建、clippy、测试、QEMU 测试、模拟器、元数据解析、依赖树、打包、发布演练或 PR 声明的其他验证命令。CI 缺口不能由这些本地命令填补。

对每项相关 CI 核对当前 SHA、job 和 step 结论、实际命令、架构或配置、用例或二进制、成功标记、可观察后置条件及失败传播。成功汇总不够；日志必须证明目标行为真实运行，新测试被发现和选择，客户机或工具输出达到通过条件，且内部失败不会被 shell、超时封装或正则分类吞掉。失败、取消、缺失、陈旧、跳过或不可证实的处理遵循“当前提交 CI 前置门禁”和“审查讨论与终态 CI 分类”。

依赖元数据变更仍须静态扫描所有 `[patch]`、普通依赖来源、相对路径、版本和发布边界，并从当前提交 CI 取得元数据解析、依赖树及适用的完整工作区发布演练证据。相关 CI 没有执行或日志不能证明预期软件包、版本、来源和发布结果时返回 `CI_SKIPPED`，不在本地运行 `cargo metadata`、`cargo tree` 或 `cargo publish`。

只有直接新增、修改或重命名进入 `apps/**` 可运行应用目录的内容允许本地运行。以 PR 文件状态识别范围：纯删除不触发；不属于可运行应用目录的 `apps/**` 文件不触发。在详细代码审查前，对每个受影响的可运行应用执行以下流程：

1. 从仓库文档和应用目录确定环境准备、应用名、架构或板卡、真实运行命令、配置和可观察后置条件；不使用未记录的机器知识补全命令。
2. 当前精确提交 CI 已成功执行同一应用、目标、准备、命令和后置条件时，接受 CI 证据，不再本地运行。
3. CI 仅缺少该应用执行时，运行文档规定的真实入口，例如 `cargo xtask starry app qemu ...`、`cargo xtask starry app board ...`、`cargo xtask arceos qemu ...`、`cargo xtask arceos board ...` 或应用自带且文档指定的运行封装。不能用 build、测试套件、语法检查、解析、`app list` 或 `--help` 代替。
...
6. QEMU 应用因外部环境、权限、凭据、服务或宿主能力而不能运行时返回 `CI_SKIPPED`，报告命令、环境缺口和未取得的后置条件；可归因于 PR 的真实运行失败进入 `REQUEST_CHANGES`。
7. 只能通过实体板卡运行且当前没有可用板卡时，记录未运行的 `app board` 命令、缺少的板卡和风险，继续静态审查；该限制本身不阻塞 `APPROVE`。已有当前 SHA 板卡 CI 时仍按同应用、同目标标准记录证据。

`apps/**` 之外的应用运行器、rootfs、QEMU/board 配置、准备、打包、符号化或日志解析改动不触发本地 app run；只能依靠当前提交 CI 和静态分析，CI 缺口返回 `CI_SKIPPED`。应用支持同时包含系统调用或内核错误修复时，对应测试套件执行必须由当前提交 CI 单独证明，应用运行不能替代。

分组 QEMU 及其他新增或迁移测试必须核对位置、发现、构建或安装、子用例选择、功能门控、正则分类和失败传播，并从当前提交 CI 日志证明具体用例、子用例或二进制真实执行。宽泛汇总、确定性发现推导或本地测试都不能替代执行日志；证据缺失时返回 `CI_SKIPPED`。测试位置错误、静默跳过、覆盖层级不对或错误修复缺少必然失败的回归逻辑仍是代码审查阻塞问题。

远端 CI 是普通审查的必需运行证据；没有相关检查不等于通过，而是 `CI_SKIPPED`。CI 不能替代静态分析、语义审查、覆盖真实性检查，也不能自动提供错误修复的未修复态 red 证据。应用本地运行只能填补对应应用执行项，不能填补其他相关 CI 缺口。

## 阻塞问题

前置门禁产生的 `CI_DEFERRED` 或 `CI_SKIPPED` 在详细审查前停止；失败归因无法确定或必要 QEMU app run 受外部环境阻止时，也以 `CI_SKIPPED` 停止后续处理。以上状态都不进入阻塞判断，也不提交 GitHub review。只有流程仍可继续时，才判断以下阻塞问题；除非有明确证据表明不阻塞，否则：
...
- 当前提交 CI 中针对性测试、格式化、静态检查或其他相关任务失败；
- 当前提交的 StarryOS/ArceOS 应用在 CI 或允许的真实本地 app run 中按文档失败，或失败未传播到 `xtask`；
- 应用或 QEMU 声明只验证发现、TOML 解析、旧提交或他人结果；
- 直接新增、修改或重命名进入 `apps/**` 的可运行 QEMU 应用缺少当前精确提交的同目标成功 CI 或允许的真实 app run 证据，或文档缺环境、命令、参数、就绪条件；
- 真实 app run 失败可归因于 PR，或需要未记录的临时绕过才能通过；仅板端 app 因无可用板卡而未运行不属于该阻塞项；
- 新行为、语义或错误修复缺测试，或测试错位、未发现、未构建或安装、未选择、未直接覆盖 ABI；
- PR 通过布局、路径过滤、功能门控、子用例、安装规则或仅手工执行的位置削弱或跳过既有覆盖；
- PR 声明的 CI 验证与实际命令、目标、输出或通过条件不匹配；
...
- 合并冲突未解决，修复复活过时的基准 API，或推送后的新提交未重新通过 CI 前置门禁和适用的 app run 例外；
- 应用流程与测试套件的语义覆盖层级错误；
- 仅测试的伪修复未实现真实行为；
...
- 实现逻辑、项目语义、当前 SHA 的 CI 命令、job、step、日志与结果；
- 测试要求、位置、构建、发现、选择和执行证据；
- 每个直接变更应用的当前提交、准备文档来源、架构、运行命令、可观察后置条件，以及精确 CI 或允许的真实 app run 证据；板端未运行时列出缺少的板卡和剩余风险；
- 审查清单审计、无测试时复核的声明、四态审查结果、CI 状态、无关失败证据和跟踪议题；
...
提交前用同一个任务清单工具逐项审计。任何必需项仍为 `pending`、不可验证或无证据时不得 `APPROVE`。PR 导致的测试缺失或真实应用运行失败进入 `REQUEST_CHANGES`；外部 QEMU 环境阻止必要 app run 时返回 `CI_SKIPPED`，板端无可用板卡时记录风险并允许继续。`CI_DEFERRED` 或 `CI_SKIPPED` 都不得提交 GitHub review。
...
当前提交变化时先对新 SHA 重新执行 CI 前置门禁；相关 CI 仍在运行时返回 `CI_DEFERRED`，相关证据缺失或不可证实时返回 `CI_SKIPPED`。门禁通过后再重新获取、更新工作树并在新的右侧变更行复核每个问题；除新提交直接改变 `apps/**` 可运行应用且缺少同应用、同目标 CI 证据外，不运行本地验证。
