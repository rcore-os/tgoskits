...
- On LS2K1000, repeated `failed to lock LS2K1000 LIOINTC when claiming LIOINTC IRQ` messages immediately after block hctx activation identify a hard-IRQ/controller-lock inversion, not a harmless spurious interrupt. LIOINTC claim/complete must use probe-published MMIO/domain state and an atomic enable snapshot instead of looking up or locking the `rdrive` controller; otherwise a level interrupt can continuously re-enter before the interrupted task releases its device guard.
