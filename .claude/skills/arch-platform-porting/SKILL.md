...
- **Forwarded guest IRQ affinity**: resolve every guest-forwarded physical IRQ with `ax_hal::irq::resolve_external_irq` to a typed host `IrqId`, then route it with `ax_hal::irq::set_affinity`. AArch64 Hybrid routes use full GIC INTIDs; RISC-V passthrough routes use PLIC-local `HwIrq`s. Keep `PlatformIrqInjectorIf` limited to registering the injection callback.
- **AxVM IRQ route sources**: AArch64 `Hybrid` derives its typed physical SPI routes exclusively from canonical `passthrough_devices` selections in the host FDT, before any UEFI guest-DTB early return; the final guest DTB cannot add Hybrid ownership. Resolve each stored full GIC INTID to a host `IrqId`, then prepare ownership and affinity synchronously inside the serialized VM startup transaction before `Running` and before vCPU task creation. Require the primary Hybrid vCPU to resolve to exactly one effective pCPU; if preparation or lifecycle commit fails, release only that runtime generation. Use an exact VINTID/HW/PINTID/state match for GIC list registers. The architectural virtual timer PPI 27 is not an SPI route: forward it only for the currently running Hybrid vCPU through a hardware-mapped `VINTID=27` / `PINTID=27` LR, and never place it in the global SPI ownership table. Keep AArch64 `Passthrough` on its legacy full-backend IRQ list, and reject RISC-V `Hybrid` until it has a complete forwarding backend.
