...
- **Guest CPU FDT filtering**: when a guest FDT retains only a subset of firmware CPU nodes, prune `/cpus/cpu-map` leaves that reference removed CPUs and remove empty topology containers. Rewrite architecture-specific per-CPU controller arrays such as RISC-V PLIC `interrupts-extended` at the same boundary. Parse provider-specific tuple widths against the original complete firmware tree, then retain only tuples whose providers remain in the filtered guest tree; reject missing providers, invalid cell counts, and truncated tuples instead of leaving dangling phandles or guessing defaults.
...
1. Discover enabled CPUs from firmware data and keep firmware IDs separate from logical CPU IDs. Map the currently executing boot CPU to logical CPU 0 even when firmware CPU nodes are unordered, while preserving the relative order of the remaining CPUs.
   Read the boot CPU ID only while the architecture's early boot record is still valid. After
   per-CPU metadata is published, runtime hardware-ID/index/count queries must use that frozen
   metadata and must not revisit firmware discovery, early boot registers, or a guessed CPU 0.
2. Bound-check CPU indices and avoid assuming hart/apic/mpidr/cpuid values are dense.
   For FDT-derived VM configs, `phys_cpu_ids` select `/cpus/cpu@<unit-address>` nodes; the
   selected node's `reg` value is the firmware/hardware CPU ID. Convert that hardware ID through
   the published runtime CPU metadata before constructing scheduler affinity masks. Do not pass
   the unit-address directly to the hardware-ID lookup, and do not treat FDT CPU-node enumeration
   order as a runtime CPU index.
