...
- **Runtime IPI identity**: dynamic platforms expose the runtime IPI IRQ through `somehal::irq::ipi_irq()`, `axplat-dyn`, and `ax_hal::irq::ipi_irq()`. Do not route runtime IPI registration through `ax-config`; on RISC-V the IRQ number is the flagged supervisor software interrupt cause, not bare PLIC source `1`.
