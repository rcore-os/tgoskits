...
- On Apple Silicon QEMU/HVF AArch64 guests, validate timer and GIC behavior separately before changing kernel code: EL1 physical timer use may hang before shell, while switching to CNTV without the HVF GIC workaround can trip QEMU's `hvf_handle_exception` assertion. Prefer feature-gated `cntv-timer` and `qemu-hvf-gic` plumbing over broad Starry kernel, driver-interface, or MMIO rewrites.
