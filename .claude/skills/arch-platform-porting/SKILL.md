...
- **Axvisor virtual-device planning**: keep VM initialization order architecture-owned. Each architecture creates its own deterministic `VmResourcePlan`, registers its interrupt-controller bundle before ordinary IRQ/MSI devices, builds devices from named claims, and seals `DeviceRuntime` only after every claim is retained by a bundle. Do not restore a shared cross-architecture device sequence or a second interrupt fabric. AArch64 creates one immutable plan before final DTB serialization; the same `ArmVgicConfig` and resolved resources must drive VGIC construction, all GICR/ITS nodes, and device endpoints. Host-derived GICD, GICC/GICR, and ITS windows must be disjoint and non-overflowing. RISC-V retains PLIC hart/context ordering, x86 retains LAPIC/IOAPIC/PIT/APIC-access ordering, and LoongArch retains IOCSR/EXTIOI/PCH-PIC cascading. Keep this contract synchronized with `book/design/arm-vgic-interrupt-topology.md`.
