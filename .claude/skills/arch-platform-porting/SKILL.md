...
- **Page-table ownership**: `page-table-generic` owns only architecture-neutral walking, mapping, frame lifetime, flags, and errors. Runtime stage-1 PTE formats, active-architecture metadata, virtual-address canonicalization, page-fault access flags, and TLB invalidation belong to `ax-cpu`; boot-table formats remain in `someboot`, and stage-2/EPT/NPT formats remain in their virtualization owner. `ax-hal` may provide the runtime `FrameAllocator` adapter and page-table aliases, but must not select architectures or define PTE bits. Do not reintroduce multi-architecture selectors under `memory/`.
