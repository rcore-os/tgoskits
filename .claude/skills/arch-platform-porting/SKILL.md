...
- **AArch64 guest timer wait and migration**: WFI schedules the earliest deliverable CNTV/CNTP deadline. `Aarch64TimerBinding::wait_generation`, not `ArmTimerContext`, owns callback invalidation. AxVM must reuse the ArceOS physical clockevent and `ktimers/%u` owner instead of creating an `axvm-timer-*` queue or worker. After a callback wins the generation check, it must re-evaluate the captured timer snapshot against the current physical counter and publish the resulting PPI levels into the VGIC before waking the vCPU; waking first can let the vCPU observe no pending interrupt and sleep again indefinitely. A stale callback must neither publish nor wake. Cancellation uses a `VmTimerHandle` containing its owner CPU; remote cancellation executes against that owner base and a local cancellation rearms its comparator. Before vCPU migration, complete any CPU-local host timer activation on the old pCPU; reset/stop/drop call the binding invalidation transition, clear virtual lines, and advance the wait generation so stale events cannot become valid again. Keep VGIC queries and timer rearming outside the scheduler wait-queue critical section. Capture the VM runtime notification generation before those checks, recheck pending state after arming, and let the wait-queue predicate compare only lifecycle state and that atomic generation so a notification between the checks and blocking cannot be lost.
