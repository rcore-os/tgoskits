...
- **AArch64 AxVM passthrough ownership**: keep guest GICD/GICR accesses trapped and filter every mixed register by VM ownership; never map whole host GIC registers into the guest. Discover the host IPI and timer through typed host capabilities, discover GIC maintenance and guest EL1 timer PPIs from host/guest FDT, and keep these core INTIDs out of board VM TOML. Context-switch VM-owned SGI/PPI state at vCPU load/save while preserving host-owned pending state, and keep host IRQs masked from the start of the physical Redistributor load until its host state has been restored after VM exit. Reserve a device SPI during VM construction, but snapshot and hand off its physical GIC state only on the first vCPU activation after the host driver has released the device; on failure or Drop, disable it and restore the saved group, priority, trigger, route, pending, active, and enable state. Keep guest MPIDR affinity separate from the logical host `CpuId`: before creating direct-interrupt vCPUs, resolve each configured MPIDR through the host CPU topology into one available one-hot pCPU mask, store that normalized mask in the VM configuration, and make both the vCPU task scheduler and GIC binding consume it. Reject missing, multi-CPU, or unavailable routes instead of letting the task scheduler fall back to a different CPU. Derive the guest SPI count from the physical `GICD_TYPER`, and do not expose GITS MMIO without an isolated physical ITS command capability.
