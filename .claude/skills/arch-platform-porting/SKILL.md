...
- **AArch64 AxVM passthrough ownership**: keep guest GICD/GICR accesses trapped and filter every mixed Distributor register by VM ownership; never map whole host GIC registers into the guest. Discover the host IPI and timer through typed host capabilities, discover GIC maintenance and guest EL1 timer PPIs from host/guest FDT, and keep these core INTIDs out of board VM TOML. Keep guest SGI/PPI and Redistributor state entirely VM-local, save the full ICH CPU interface on vCPU exit, and never overwrite host GICR private-interrupt state. Claim each assigned device SPI during VM construction. Before registering its host handler or changing physical affinity, snapshot the physical enable, pending, active, priority, trigger, route, and group state, then mask and clear the SPI for the ownership handoff. On failure or Drop, remove the host handler first, then restore the complete snapshot. Keep guest MPIDR affinity separate from the logical host `CpuId`: before creating hardware-forwarded physical IRQ routes, resolve each configured MPIDR through the host CPU topology into one available one-hot pCPU mask, store that normalized mask in the VM configuration, and make both the vCPU task scheduler and GIC binding consume it. Reject missing, multi-CPU, or unavailable routes instead of letting the task scheduler fall back to a different CPU. Derive the guest SPI count from the physical `GICD_TYPER`. Never expose or map host GITS MMIO without an isolated physical ITS capability; a VM-local software ITS may use a trapped guest aperture independently.
- **AArch64 secure-firmware boundary**: treat host SCMI shared memory, SMC transports, and host CPU DVFS/idle controls as host-owned firmware resources, not ordinary whole-machine passthrough MMIO. FDT `shmem` phandles are required ownership dependencies; if no VM-specific mediated firmware capability exists, remove the transport and dependent host-control properties from guest FDT. `arm_vcpu` implements PSCI and bounded SMCCC v1.1 architecture discovery locally, reserves architecture-owned call ranges for both SMC and HVC conduits, and returns SMCCC `NOT_SUPPORTED` for every unregistered owner. Never forward arbitrary guest SMC arguments to ATF or identity-map unallocated low host RAM.
- **AxVM machine planning contract**: convert strict guest policy plus one immutable host FDT/ACPI capability snapshot into `VmMachinePlan` before mutating VM state. Virtual machines must not query or map host I/O; Passthrough machines may identity-map only the planned non-RAM I/O aperture and must punch holes for host-exclusive/reserved, denied, guest RAM/boot blobs, and virtual device/controller windows. Treat FDT nodes with `status = "disabled"` as inactive resource aliases: they neither claim ownership nor contribute I/O apertures or mapping holes. An active assigned node may authorize an overlapping alias range, while an inactive-only range remains unmapped. Load guest images before claiming physical devices, revalidate the snapshot generation, acquire every device in one transaction, and preserve host IRQ/DMA/route state in RAII leases so every later failure rolls back. Build in RAM, vCPU, controller/binding, device/topology, bus/mapping, firmware, boot-state, commit order. Firmware descriptions must be generated from the same resolved resources registered on the device bus; do not copy host AML, expose an unfiltered host FDT, or mutate `AxVMConfig` from a firmware parser.
- **AxVM device/IRQ endpoint contract**: architecture profiles own controller layout, mandatory timer/IPI/reset resources, and deterministic virtual MMIO/PIO/IRQ pools. Virtual-device models first declare named requirements and then build only from resolved slots. Devices may hold controller-created `IrqLine`/`MsiEndpoint` objects but must not see vCPUs, controller IDs, guest vectors/INTIDs, or host IRQ IDs. Controller capabilities connect device inputs to vCPU ports or parent controllers and validate cascades before the VM starts. Keep raw vector injection inside an unmigrated architecture adapter only; do not reintroduce shared `inject_irq`, `IrqResolver`, or topology-wide line mutation. `InterruptTopology` is independent of physical-source policy. A single VM-local controller may combine software-backed virtual-device endpoints with ownership-checked, HW-backed physical endpoints; select backing per planned endpoint, never per whole VM. `interrupts_passthrough` normalizes to `PhysicalInterruptPolicy` and applies only to assigned physical sources. Hardware forwarding still exits to EL2 and uses a HW-backed LR; true CPU/device bypass belongs to a separate static-partition machine mode.
...
- **Runtime console ownership**: once Starry or another OS runtime binds the firmware-selected UART to an interrupt-driven tty/serial driver, call `ax_hal::console::claim_runtime_output()` and stop the low-level boot/platform console path from writing the same UART registers directly. For a reversible physical-device handoff such as an AxVM direct-passthrough console, hold `ax_hal::console::BootConsoleOutputLease` for the complete host-device lease lifetime instead; do not use the permanent runtime handoff. The hardware console must have one register owner; otherwise kernel log output and guest/tty output can interleave at the UART register level and corrupt test markers or user input/output.
...
- **Axvisor fixed guest RAM**: a startup VM configuration that uses fixed `host`, `shared`, or `reserved` memory backing must publish those host physical ranges through the retained `ax_reserved_phys_ram` linker section. `axplat-dyn` consumes the section before `axruntime` initializes the host allocator. Reject fixed backing during runtime VM creation; a future late-claim path needs an explicit assignable-memory capability and ownership transaction rather than trusting generic host `RESERVED` regions, which also include live kernel and per-CPU storage.
- **AxVM identity-allocated guest RAM**: treat `guest_base = 0` as a configuration placeholder, not a fixed `[0, size)` guest range. The machine plan records the dynamic identity-placement requirement without punching a low I/O hole or publishing an early host reservation; after the host allocator selects RAM, AxVM maps it with GPA=HPA, relocates direct-boot images from the prepared primary region, and rebuilds FDT memory nodes from the runtime ranges.
