...
- On Apple Silicon QEMU/HVF AArch64 guests, validate timer and GIC behavior separately before changing kernel code. Keep the normal AArch64 EL1 default timer path intact unless the platform explicitly selects otherwise; for macOS/HVF, pass generic boot arguments such as `someboot.aarch64_timer=virtual someboot.aarch64_gicd_spi=off` so someboot uses CNTV/CNTVCT, somehal selects the FDT virtual timer PPI, and GICv3 avoids QEMU/HVF-unsupported SPI distributor array accesses. Do not add app-private timer or GIC Cargo features for this case, and prefer platform/FDT/EL semantics over broad Starry kernel, driver-interface, or MMIO rewrites.
