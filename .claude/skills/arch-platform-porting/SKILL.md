...
- **x86 QEMU IRQ contract**: the dynamic x86 path targets modern QEMU `q35` with ACPI/MADT, Local APIC or x2APIC, IOAPIC, and PCI INTx routing. Do not add 8259/PIC fallback, i440fx-specific IRQ assumptions, non-ACPI IRQ probing, raw GSI enable bypasses, or vector arithmetic outside the IOAPIC controller. `AcpiGsiRoute` is firmware metadata and must not contain a CPU vector; early devices such as COM1 report their GSI, not `vector_base + gsi`. `X86IoApicIntc` independently allocates a legal external vector, validates the full `u32` GSI and controller-local input, writes a masked redirection entry, and only then Release-publishes the vector reverse map and fixed-capacity GSI endpoint. Reservation failure must roll back without publishing a route or vector. Hard-IRQ mask/unmask may perform bounded atomic endpoint lookup and take the shared IOREGSEL/IOWIN IRQ-safe MMIO lock, but it must not allocate, scan devices, enter rdrive, or take the controller control-plane lock. LAPIC/x2APIC owns timer, IPI, EOI, and spurious handling. x2APIC paths must preserve full `u32` APIC IDs for CPU-local operations, while xAPIC and IOAPIC physical destinations must reject APIC IDs that cannot be encoded without truncation.
- **LoongArch QEMU IRQ contract**: the dynamic LoongArch path targets QEMU `virt`/LS7A-style firmware routing through CPU-local timer/IPI lines, EIOINTC, and PCH-PIC. `somehal::begin_irq(raw)` receives the CPU interrupt line from `ESTAT.IS`, not an ACPI GSI or PCI vector; only the timer line, IPI line, and EIOINTC cascade line may enter runtime dispatch. EIOINTC owns claim/complete of external vectors, while PCH-PIC owns PCH input state, ACPI trigger/polarity configuration, mask state, and route memory through its `rdif_intc::Intc` lock. Probe must validate that the PCH input count is `1..=64`, that `base + count` fits the 8-bit EIO vector space, and that the MMIO window covers the register block; reject a second PCH instance before reset or registration until the fast path supports controller identity. While every child is masked, publish route lookup and the immutable MMIO completion capability together as one write-before-release object retained until shutdown; hard IRQ must not reacquire the rdrive/controller lock. Match Linux hierarchical irqchip ordering: immediately after claim and before the handler action, acknowledge a latched edge child with PCH `CLEAR` followed by a LoongArch device-write `dbar`; complete its EIOINTC parent only after the action. Do not W1C a level child whose device still owns the asserted level. Derive PCH-PIC hardware vectors only from the controller's local input plus its validated local base; `AcpiGsiRoute` has no CPU-vector field and unknown CPU-local interrupt lines must not be dispatched as PCH-PIC IRQs.
...
- **Bounded serial IRQ continuation**: `rdif-serial` hard IRQ service has fixed RX/TX/pass budgets. OS glue must translate `SerialIrqOutcome::budget_exhausted` into a coalesced task-context `SerialSoftWork::RESERVICE` request and continue in bounded batches until the driver reports no remaining work. Bound each service-thread activation as well as each hard IRQ; after the task batch limit, preserve pending bits and explicitly yield before re-entering the wait/drain loop. Do not silently discard the bit, spin without a budget in hard IRQ or task context, or use a shell pipeline/subprocess stress case as evidence for raw console RX/TX behavior.
