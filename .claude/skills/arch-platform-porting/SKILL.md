...
- **Runtime IPI identity**: dynamic platforms expose the runtime IPI IRQ as a typed `IrqId` through `somehal::irq::ipi_irq()`, `axplat-dyn`, and `ax_hal::irq::ipi_irq()`. Do not route dynamic runtime IPI registration through `ax-config`; on RISC-V the IRQ is the flagged supervisor software interrupt cause in the CPU-local domain, not bare PLIC source `1`.
  ...
  transport is busy.
- **Runtime clockevent startup**: early per-CPU platform initialization only prepares the one-shot timer in a masked, non-firing state. The runtime clockevent owner must publish a finite deadline and keep the complete device transition under local-IRQ exclusion. Entering the active state makes the interrupt source observable first, serializes any posted controller update, and only then installs the comparator; reprogramming an already-active source changes only the comparator. Stopping a source masks it before disarming or clearing the comparator, and re-online repeats the active-state transition. `SystimerArch::systimer_cancel_oneshot` is mandatory on architectures with independent timer hardware; x86 implements the same capability in somehal because its timer belongs to the local APIC. x86 clears `IA32_TSC_DEADLINE` or the legacy LAPIC initial count, AArch64 moves the selected EL compare value to `u64::MAX`, RISC-V installs SBI timer `u64::MAX`, and LoongArch disables TCFG before installing its largest valid one-shot interval and clearing pending TI. Ordinary IRQ disable remains mask-only. On x86, only the xAPIC TSC-deadline transition requires an `mfence` between the posted LVT MMIO update and the subsequent deadline MSR write; legacy initial-count and x2APIC paths must not pay that fence cost. Timer interval conversion must clamp expired deadlines to a non-zero architectural minimum and must not overflow while aligning or forming an absolute compare value.
...
- **RISC-V guest CSR ownership**: `RiscvVcpu::setup()` constructs guest reset state without reading or writing live host `sstatus`/`hstatus`; the assembly guest-entry boundary is the only owner that swaps saved host and guest status. The physical `sie` source mask remains owned by the host IRQ runtime across every guest run. Like Linux KVM, vCPU entry may clear the global `sstatus.SIE` bit and must restore its entry state after exit, but it must never set or clear host `sie.SSIE`, `sie.STIE`, or `sie.SEIE`; guest virtual enables and pending state belong to `vsie`, `hie`, and `hvip`. A pending scheduler generation and `sip.SSIP` with `sie.SSIE == 0` explains an idle hart stuck in WFI and must be repaired at this ownership boundary, not by retrying the scheduler doorbell. If the guest target exposes F/D, initialize HS `sstatus.FS` from that guest ISA contract while keeping the architectural guest FS state in `vsstatus`. A host `InstructionFault` immediately after returning from guest WFI points to a leaked host/guest status boundary; a later guest floating-point illegal instruction points to an inconsistent guest ISA/reset state. Do not repair either failure by copying current host CSR bits into the guest. Keep this contract synchronized with `docs/design/axvm-riscv-sbi-ipi.md`.
...
- **AArch64 VGIC lifecycle**: start the deferred vCPU-kick worker before binding physical SPIs and publishing hard-IRQ routes. Every worker start/stop result is part of the lifecycle transaction: activation failures must restore `Inactive` and combine cleanup failures with the primary error, while deactivation drops quiesced routes before stopping the worker, publishes `Inactive` after that irreversible teardown, and still returns a worker-stop error. Never publish `Active` without a running kick worker or restore `Active` after routes have been dropped.
- **AArch64 guest timer wait and migration**: WFI schedules the earliest deliverable CNTV/CNTP deadline as an explicit hard restartable timer in ax-task's single per-CPU clockevent owner; do not create an `axvm-timer-*` queue or worker. `Aarch64TimerWaitState` owns a checked monotonic generation and a `ThreadWakeHandle` obtained once in vCPU task context. The hard callback may only re-evaluate its immutable timer snapshot, perform bounded atomics, rearm the same stable registration after an early edge, and borrow the prebound handle to wake the vCPU when due. It must not allocate/free, clone/drop owning references, log, look up VM/runtime state, acquire VGIC or ordinary locks, or publish a PPI level; completion payload reclamation belongs to `ktimers/%u`. The vCPU entry path recomputes timer outputs and publishes their levels before VGIC load, matching Linux `kvm_bg_timer_expire()` and `kvm_timer_vcpu_load()` ordering. A stale callback must neither publish nor wake. Cancellation uses a `KernelTimerHandle` containing its logical-base owner CPU; remote cancellation must not rewrite another CPU's physical comparator, and any outstanding edge is reconciled only by the single `LocalClockEvent::Firing` transaction. Before vCPU migration, complete any CPU-local host timer activation on the old pCPU; reset/stop/drop invalidate the armed generation and clear virtual lines. Keep VGIC queries and timer rearming outside the scheduler wait-queue critical section. Capture the VM runtime notification generation before those checks, recheck pending state after arming, and let the wait-queue predicate compare only lifecycle state and that atomic generation so a notification between the checks and blocking cannot be lost.
...
- **LoongArch page-table allocation window**: DMW `PAGE_OFFSET` is a RAM direct map and bypasses the page walker, so it must never be the base of the kernel address space used by `vmap`, generic `iomap`, or coherent-DMA aliases. Match Linux `vm_map_base = 0 - (1 << cpu_vabits)`: derive the page-table-backed XKVRANGE base from the raw CPUCFG VABITS sign-bit index, keep every DMW address outside that range, and retain the final relocated kernel mapping in the same high page-table space. The `loongArch64` crate's `get_valen()` returns that raw field plus one, so convert its address length back to the sign-bit index before shifting. A mapping inserted under a DMW virtual address is not an alias; hardware ignores its PTE and accesses the DMW-selected physical address instead.
- **LoongArch targeted TLB invalidation**: one TLB entry covers an even/odd base-page pair. Align targeted invalidations to that pair and use `INVTLB_ADDR_GTRUE_OR_ASID` with the live ASID, matching Linux's split kernel/user invalidation semantics in one architecture boundary. `INVTLB_ADDR_GFALSE_AND_ASID` cannot invalidate global kernel mappings and can leave the odd half of a previously refilled pair stale after installing its PTE.
...
- On x86_64, install one Linux-compatible kernel CR0 state on the BSP and every AP before kernel runtime code: set `PE/MP/ET/NE/WP/AM/PG` and clear `EM/TS/CD/NW`. An INIT/SIPI AP does not inherit the BSP's `WP`, and read-modify-write from its reset state can preserve disabled caches. The BSP page-table path and AP trampoline must share one `CR0_STATE` constant; the earliest per-CPU trap initialization reads back and rejects a broken entry contract instead of repairing it late. Without WP, a ring-0 user copy silently writes through a read-only COW PTE onto the shared frame, so guard with `bugfix-ap-cow-write-protect`: pin the COW `read()` write to every allowed CPU, verify the actual CPU, and require the parent page to remain unchanged. A plain `mprotect(PROT_READ)`/`EFAULT` probe validates user-memory permissions but does not by itself prove CR0.WP, because the software access check may reject the write before any supervisor store.
