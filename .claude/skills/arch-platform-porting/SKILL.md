...
- **Forwarded guest IRQ affinity**: resolve every guest-forwarded physical IRQ with `ax_hal::irq::resolve_external_irq` to a typed host `IrqId`, then route it with `ax_hal::irq::set_affinity`. AArch64 Hybrid routes use full GIC INTIDs; RISC-V Hybrid and passthrough routes use PLIC-local `HwIrq`s. Keep `PlatformIrqInjectorIf` limited to registering a typed `IrqId` injection callback; do not discard the interrupt-controller domain.
- **AxVM Hybrid IRQ ownership**: represent controller-local routes with the common `PhysicalIrqRoute` and `ControllerIrqRegistry` primitives. Bind each registry to exactly one typed IRQ domain, reserve all routes for the VM runtime generation, set affinity, and publish the claim only after setup succeeds. The hard-IRQ path must validate the current VM ID and exact runtime generation before injecting, and teardown must release only that generation so a late exit cannot revoke a restarted VM's routes. AArch64 `Hybrid` derives its physical SPI routes exclusively from canonical `passthrough_devices` selections in the host FDT, before any UEFI guest-DTB early return; the final guest DTB cannot add Hybrid ownership. Require the primary Hybrid vCPU to resolve to exactly one effective pCPU. Use an exact VINTID/HW/PINTID/state match for GIC list registers. The architectural virtual timer PPI 27 is not an SPI route: forward it only for the currently running Hybrid vCPU through a hardware-mapped `VINTID=27` / `PINTID=27` LR, and never place it in the global SPI ownership table. RISC-V `Hybrid` uses the same generation-scoped ownership contract for configured PLIC sources, while AArch64 `Passthrough` keeps its legacy full-backend IRQ list.
- **Stopped-VM boot restore**: a stopped AxVM restart must join every old vCPU task, clear only allocator-owned guest RAM, replay the completed ordered boot-write snapshot, and only then rebuild vCPUs, devices, and architecture runtime state. Record a boot write only after the complete destination operation succeeds; retain fill operations for firmware flash padding without materializing a full-sized byte copy. A restore failure must leave the VM stopped and must not commit IRQ ownership or spawn a vCPU task.
- **Default-VM startup with the management shell**: Axvisor must start every successfully initialized default VM before entering the management shell. Use the per-VM non-blocking runtime start path so guest vCPU tasks and the shell run concurrently; do not call the blocking VMM-wide wait path, and do not leave all default VMs in `Ready` merely to make the shell available.
- **AArch64 TLS across guest entry**: `ArmVcpu` host runtime context must save both `SP_EL0` and `TPIDR_EL0` before guest entry. On VM exit, capture the guest `TPIDR_EL0` into exit scratch before restoring the host value, then feed that captured value into the guest system-register snapshot; reading `TPIDR_EL0` later from host Rust corrupts guest TLS. Restore the host value before returning to host Rust, otherwise guest TLS state can make host panic/error handling recursively fault and hide the original guest exit.
- **Stopping a running vCPU**: changing lifecycle state and waking a host wait queue cannot stop a vCPU that is executing guest WFI. After publishing the stop state, wake waiters and send the existing host runtime IPI to every pCPU running one of the VM's vCPU tasks so each vCPU exits guest context and observes `Stopping`.
- **Starting a remote vCPU task**: AxVM pins vCPU tasks to remote pCPUs, including after a stopped VM rebuilds its runtime. Keep `ax-std/wake-ipi`, enqueue the task, then send the selected pCPU a bare host wake IPI. Its handler must return and complete the GIC EOI before the idle loop schedules the vCPU; enabling the full `ax-std/ipi` callback path can switch into guest context while the host SGI is still active and block lower-priority physical guest IRQ delivery.
