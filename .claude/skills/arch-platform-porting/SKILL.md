...
- **Boot entropy**: capture trusted boot entropy in `someboot` before UEFI `ExitBootServices`. Prefer the UEFI RNG protocol, then an exact 32-byte FDT `/chosen/rng-seed`; expose the captured seed through `somehal`, `axplat-dyn`, and `ax-hal`. Consumers that require per-boot uniqueness must report unsupported or fail explicitly when neither source exists, never substitute monotonic time, counters, addresses, or other replayable state.
