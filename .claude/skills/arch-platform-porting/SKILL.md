...
- **x86 physical legacy console exception**: a physical PC/AT board whose COM1
  input is proven to assert master-8259 IRQ4 but not IOAPIC GSI4 may explicitly
  select `AX_X86_LEGACY_PIC_CONSOLE=1`. This is a build-time platform contract,
  not runtime fallback: QEMU and ordinary ACPI systems remain IOAPIC-only. The
  selected path allocates a distinct `X86LegacyPic` IRQ domain, remaps both PICs
  while masked, unmasks only IRQ4, configures ExtINT only on the BSP LINT0,
  keeps every secondary LINT0 masked, and completes the PIC before the
  local-APIC EOI. It must not represent PIC IRQ4 as IOAPIC GSI4. The raw UART
  source remains masked until the framework action and controller transport are
  enabled. Missing evidence, unsupported firmware, or setup failure must disable
  task input rather than poll or guess another IRQ. Keep this contract
  synchronized with `docs/design/axvisor-x86-legacy-console-pic.md`.
...
- **RISC-V guest Sstc ownership**: when `henvcfg.STCE` is enabled, VS owns `vstimecmp` and
  VSTIP while the host scheduler retains the HS comparator and supervisor-timer trap. Guest timer
  programming must not call the host SBI `set_timer`, and an HS supervisor-timer exit must return
  to the host IRQ path rather than being converted into a guest timer interrupt. Preserve unrelated
  `henvcfg` bits when enabling STCE and restore the previous value when per-CPU virtualization is
  disabled.
- **Passthrough FDT exclusions**: a physical device listed under `devices.disabled` must be absent
  from both the guest address-space mapping and generated DTB, including its descendants. This is
   especially important when an empty passthrough list expands the machine's default root selector:
   reserving a PCI ECAM range while leaving its host bridge visible makes the guest probe an
   inaccessible device. Keep machine-owned virtual interrupt-controller nodes available even when
   their physical ranges are excluded.
- **Runtime console selection**: Dynamic platforms expose the firmware-selected hardware console through `somehal::console_device_id()` and `ax_hal::console::device_id()`. The value is `Result<rdrive::DeviceId, ConsoleDeviceIdError>` derived from bootargs `console=`, ACPI SPCR, or FDT `stdout-path`; static platforms return `Err(NotSpecified)`. Linux-style `ttyS<N>` and `ttyAMA<N>` select the Nth ordinary FDT serial node, while Rockchip `ttyFIQ<N>` is a distinct `ConsoleSpec::RockchipFiq(N)` and must resolve against the Nth enabled `rockchip,fiq-debugger` node rather than aliasing an ordinary UART index. Numeric `tty<N>`, bare `tty`, and `ttynull` are virtual selections and must not bind a hardware device. On ACPI systems that deliberately skip AML namespace loading, an SPCR-selected supported UART must still seed one runtime serial descriptor from the SPCR register model, address space/range, GSI, clock, and baud rate; deduplicate it by register base when namespace probing already registered the UART. An SPCR entry without a populated namespace/resource device must receive one cached synthetic `DeviceId` shared by console selection and runtime serial creation; never allocate a fresh ID in each consumer. If firmware omits the SPCR interrupt-type flags but leaves a nonzero raw GSI or legacy IRQ field, use that field only to form the host runtime route and resolve legacy IRQs through MADT source overrides; do not reinterpret it as a valid guest snapshot IRQ or replace missing SPCR fields with a hard-coded board UART, IRQ-vector arithmetic, or polling input. OS code such as Starry should match `Ok(id)` against probed serial devices, use `ttyS0` as the Linux-style hardware-console fallback only for `Err(NotSpecified)`, and leave `/dev/console` unbound (`ENODEV`) for non-hardware console selections, unmatched selected hardware devices, or when no serial console TTY exists. Keep the console-spec parser and FDT node-to-`DeviceId` mapping together in `somehal`; do not reparse FDT or bootargs in the tty layer.
