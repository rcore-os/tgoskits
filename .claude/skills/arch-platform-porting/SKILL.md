...
- **AArch64 GIC maintenance IRQ**: copy the unique maintenance interrupt specifier from the GICv3 FDT node during probe, then translate and configure it through the exact registered `Intc` and its dynamically allocated domain. Validate that translation preserves that domain and resolves a PPI INTID in the architectural range 16 through 31; accepting an SGI merely because it is below 32 is invalid. Publish the write-once capability through `somehal`, `axplat-dyn`, `ax-plat`, and `ax-hal` as a typed status that distinguishes uninitialized, available `IrqId`, unavailable firmware data, and the concrete discovery `IrqError`; never encode absence as `IrqError::Unsupported`, and publish discovery/lock `Busy` as `Error(Busy)` rather than confusing it with duplicate-publication `Busy`. A missing or malformed optional maintenance capability must not roll back an otherwise successful host GIC probe. AxVM registers one host-lifetime per-CPU handler only after virtualization is enabled on all usable CPUs; retain the handler and expose a read-only status that distinguishes uninitialized, registered, unavailable, and the original registration error without retrying. The handler records only `(VMId, VCpuId, bind generation)` under local IRQ exclusion, while the normal platform IRQ transaction remains the sole owner of acknowledge/EOI. AArch64 `ArmHostOps::fetch_pending_host_irq()` must return transaction ownership with the vector after acknowledge, dispatch, and `ActiveIrq` drop/EOI; deferred work consumes that ownership and only checks timers for `FetchHandled`. Fold ICH state before unbind. Do not hard-code PPI 25, select an arbitrary controller with `get_one()`, or dispatch the same host IRQ again from the deferred exit path.
