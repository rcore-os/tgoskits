...
- **Axvisor configured virtio-blk contract**: generic virtio-blk models are owned and explicitly registered by AxVM, while backing-file creation, formatting, sizing, and writeback remain a separate image-policy concern. Configured instances request MMIO and wired IRQ resources with `ResourceRequest::Auto`; the architecture's resource pools resolve them, and the same `ResolvedDeviceGraph` supplies those final resources to runtime construction and typed FDT/ACPI contributions. Do not restore architecture-local virtio-blk addresses, IRQs, firmware nodes, or file-image policy.
- **LoongArch guest PCI ECAM contract**: LoongArch UEFI PCI0/MCFG is backed by a graph-owned `PciEcamDevice` with one fixed resource. Runtime and ACPI consume the same resolved ECAM range, and a host-derived range mismatch must fail guest firmware construction during VM boot preparation. Never identity-map host ECAM or allocate 128 MiB through `MapAlloc`; absent BDF reads return width-matched all ones. UEFI selects strict ACPI plus an auxiliary available-FDT artifact, while direct-FDT guests retain strict FDT resolution and do not register the ACPI-only ECAM model.
