...
- On x86_64, every CPU must run with CR0.WP set from its first long-mode instruction. The AP real-mode trampoline includes `X86_CR0_WP` in its CR0 activation mask exactly like Linux's `CR0_STATE`, and `paging::enable_write_protect()` is the single enforcement implementation called from both the BSP page-feature setup and every CPU's `init_local()`. Without WP, a ring-0 user copy on that CPU silently writes through a read-only COW PTE onto the shared frame, so usercopy COW failures look scheduling-dependent. Guard with the `bugfix-ap-cow-write-protect` system case: it pins to each allowed CPU and requires `read()` into an `mprotect(PROT_READ)` page to return `EFAULT`.
