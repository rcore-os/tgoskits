...
- **LoongArch LS2K LIOINTC contract**: controller probe must initialize the hardware and publish shutdown-lifetime IRQ-side state (domain, register/ISR mappings, parent lines, and an atomic enabled mask) before enabling the parent cascade. Hard-IRQ claim/complete must use only that pre-published state; it must not call `rdrive::get_list`, take a task-owned `rdrive` device lock, allocate, or block. Serialize task-side W1 enable/disable MMIO with an IRQ-safe bounded lock, publish enable after the hardware write, and hide a disabled input from claim before disabling it in hardware.
