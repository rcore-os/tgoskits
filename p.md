...
| ax-page-table-multiarch 与 page-table-generic 是能力不同的两套引擎，ax-page-table-entry 单独提供页表项；另有多套 AddrSpace 包装       | ax-page-table 统一公共契约，内部保留面向 Stage-1 与可变层级 Stage-2/boot 的两套执行引擎；ax-memory-set 只提供直接虚拟内存区域操作                                  | 删除重复 crate 和公共契约，同时避免为统一 walker 或通用回滚协议增加热路径开销   |
...
ArceOS、StarryOS 与 Axvisor 分别拥有内核虚拟内存、Linux 进程虚拟内存和客户机地址空间策略，三者并列消费公共页表与虚拟内存区域机制；只有 StarryOS 承担 Linux 系统调用级事务语义。
...
| ax-page-table | 已实施 | 旧三个 crate 已删除；stage1/stage2/boot feature 分离；无消费者的 AccessFlags/MemConfig/PageTableOp 已删除；stage 依赖按 feature 裁剪 | 各目标板地址转换后备缓冲区 shootdown 延迟与镜像裁剪数据 |
