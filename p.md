...
| ax-memory-set | 部分实施 | 使用 `BTreeMap` 和直接 backend 操作；通用五阶段事务、逐页 `SavedMapping` 和操作级临时 Vec 已删除；metadata-only API 保留给专用所有权流程；ArceOS/Axvisor 分配型 unmap 恢复逐页对称释放 | `MappingBackend` typed error、整区 unmap 的 `unwrap`、protect 失败传播；Starry `MREMAP_FIXED` 覆盖失败测试；跨区直接操作失败语义测试 |
