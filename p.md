...
| allocator 用途统计在所有构建的 alloc/free 热路径更新全局原子计数                                                                    | 只保留单一 UsageKind 计数表，并通过 `stats` feature 仅为 StarryOS 等诊断消费者启用                                                                             | 嵌入式默认构建不触碰共享统计 cache line，StarryOS 仍可生成 procfs/sysinfo      |
...
| ax-page-table-multiarch 与 page-table-generic 是能力不同的两套引擎，ax-page-table-entry 单独提供页表项；统一后的 ax-page-table 又把主机、启动和客户机页表暴露为一个公共组件 | 主机第一阶段页表及架构页表项归 `axcpu::paging`，客户机第二阶段页表归 `axvm`，启动页表归 `someboot`；保留原 `page-table-generic` 名称但只留下无架构选择的 walker 和 frame provider 契约 | 删除重复实现并收窄原 generic 组件，避免新的大一统页表 facade；内存策略层不再处理目标架构，启动和虚拟化也不依赖主机页表实现 |
| Linux 虚拟内存策略与 StarryOS kernel、虚拟文件系统、task、signal 耦合，overcommit_memory=0 与实际总是接受的行为不一致                             | 独立 `no_std` 的 `starry-mm` 承载不依赖具体操作系统的 Linux 虚拟内存策略、记账和能力；具体虚拟文件系统与页表后端留在 kernel adapter；只实现当前实际使用的 Always overcommit | 避免再造通用 Linux mm 框架，同时使可复用策略和 Linux 可见行为一致             |
...
| ------- | ------------ | ---------------- | ---------------------- | -------------------- | -------------- | ---------------------- | ------------------- |
| TGOSKits 当前整改方案 | 固件内存图由 kernutil/someboot 规范化；运行期只保留 Normal、Dma32 两种请求约束 | `ax-alloc` 统一入口；同一 Buddy 按 count/align/zone 处理连续页，Dma32 只限制可达地址；启动期 bump 在交接后冻结 | 固定 size-class per-CPU Slab；只有存在实际消费者时增加中断请求/no-fail 固定池，不建立通用 pool manager | 普通分配立即返回 `NoMemory`，不阻塞、不隐式回收；中断请求/实时 路径禁止 GlobalAlloc、Slab 扩容和 Buddy | `axcpu::paging` 处理主机页表，`axvm` 处理客户机第二阶段页表，`someboot` 处理启动页表；`page-table-generic` 只复用无架构 walker，`ax-memory-set` 只提供区域操作 | allocator 不回收；仅 `starry-mm` 在允许上下文做一次有界 clean-page 回收；使用 Always overcommit；无 swap/compaction/内存不足终止器 | `dma-api::DeviceDma` 持有 mask/domain/cache 约束和 move-only 资源获取即初始化 owner；Dma32 仅表示物理可达范围，不替代 map/unmap、cache 或输入输出内存管理单元语义 |
...
最终按资源、通用算法、架构适配和策略分层：

```text
Boot ownership
someboot + kernutil::memory
├── early Ram bump ────────────────> someboot::paging
└── frozen Free ranges ────────────> ax-alloc
                                      └── buddy-slab-allocator

Architecture-neutral mechanisms
ax-memory-addr
├── page-table-generic
│   ├── axcpu::paging ── ax-hal provider ──> ax-mm / Starry kernel mm
│   ├── axvm Stage-2 ── NestedPageTableOps ─> axaddrspace
│   └── someboot::paging（仅启动期）
└── ax-memory-set
    ├── ax-mm
    ├── Starry kernel mm + starry-mm
    └── axaddrspace

Device capability boundaries
dma-api / mmio-api ── OS adapter ──> ax-alloc / ax-mm
```

这些是职责边界，不要求每个内部机制都成为 crate。
...
- `page-table-generic` 只复用与指令集无关的 walker、表页所有权和错误；它没有 `target_arch` 分支，也不提供 stage1/stage2/boot facade。
- 主机第一阶段页表归 `axcpu::paging`，具体页框和跨核失效由 `ax-hal` 注入；客户机第二阶段页表归 `axvm`，启动页表归 `someboot`。
- PageTable Core 和 AddressSpace Core 是并列公共机制；操作系统策略组合能力，不要求 ax-memory-set 直接依赖具体页表实现。
...
| 页表通用算法     | page-table-generic      | 无架构选择的 walker、表页所有权、`PageFrameProvider`、`PageTableEntry`、公共错误                         | 保留一个小型共享核心，避免 someboot 与 axvm 复制 walker       |
| 主机页表         | axcpu::paging        | 主机第一阶段页表项、页表层级、cursor、权限修改、本地地址转换后备缓冲区失效和页故障权限语义              | 由 axcpu 持有；不依赖 ax-alloc、ax-hal 或运行时处理器间中断   |
| 启动页表         | someboot::paging     | 启动页表项、启动映射和 Ram no-free provider                                                             | 由 someboot 持有                                               |
| 客户机页表       | axvm                 | 第二阶段页表项、可变层级映射和客户机地址转换失效                                                        | 由 axvm 持有；通过 NestedPageTableOps 接入 axaddrspace        |
| 地址空间公共核心 | ax-memory-set        | 基于 `BTreeMap` 的虚拟内存区域查找、重叠检查、split/shrink/grow 和直接 backend 操作                              | 保留并精简；Linux 系统调用级事务由 StarryOS 策略层处理        |
...
公共内存核心只有 `ax-memory-addr`、`ax-alloc` 和 `ax-memory-set`。`page-table-generic` 是架构无关的底层算法组件；`ax-mm`、`starry-mm` 和 `axaddrspace` 是并列策略消费者，不再增加公共包装层。
...
| someboot::mem        | kernutil::memory、ax-memory-addr、page-table-generic；启动页表项和映射适配由 someboot 自身持有                     |
...
| page-table-generic      | ax-memory-addr；其余仅为固定容量容器和错误支持；不得包含 `target_arch` 分支                                      |
| axcpu::paging        | ax-memory-addr、page-table-generic；不依赖 ax-alloc、ax-hal、ax-runtime 或 ax-ipi                                   |
| ax-memory-set        | ax-memory-addr；可选 ax-errno 仅用于 adapter 错误转换                                                            |
| ax-mm                | ax-memory-addr、ax-alloc、ax-hal 提供的主机页表、ax-memory-set                                                  |
| starry-mm            | ax-memory-addr、ax-errno；文件、页来源和具体页表 backend 通过 capability 注入                                  |
| axaddrspace          | ax-memory-addr、axvm-types、ax-memory-set；页分配和页表通过 NestedPageTableOps 注入                             |
| axvm                 | ax-memory-addr、page-table-generic、axaddrspace；具体第二阶段页表项和失效由 axvm 架构模块持有                      |
...
- page-table-generic 与 axcpu 不依赖 ax-alloc；`PageFrameProvider` 由 someboot、ax-hal 或 axvm adapter 实现。
...
- `PageFrameProvider`、task stack 和外部 split alloc/free trait 所需的低层 raw page pair；只允许 adapter 使用，调用方必须保存页数和 `UsageKind` 并成对释放。`MemoryZone` 是分配时地址约束，不是独立释放来源。
- 内核 Rust `GlobalAlloc`。
- per-CPU Slab 的 CPU 启动初始化。
- feature-gated 的单一 `AllocatorStats`；仅从一张 `UsageKind` 计数表生成只读快照。
...
启用 `stats` feature 时，AllocatorStats 使用一张按 `UsageKind` 索引的原子计数表；一次 alloc/free 只更新一个 bucket，`usage`、总量和 procfs/sysinfo 输出均从同一快照派生。嵌入式默认构建关闭该 feature，不在 per-CPU Slab 命中路径更新任何全局统计原子。
...
| `dealloc_pages`                               | 保留具体释放接口；传回原页数和 UsageKind，资源获取即初始化页对象只保存地址、页数和用途；释放不需要 MemoryZone      |
...
页表按执行上下文归属。`page-table-generic` 不是统一页表 facade，只提供 someboot 与 axvm 真实共用的无架构 walker 和页框能力契约：

```text
page-table-generic
├── PageFrameProvider / PageTableEntry / TableMeta
├── PageTable / Frame / walker
├── map / unmap / translate / drop
└── PagingError / PteConfig

axcpu::paging
├── GenericPTE / MappingFlags
├── x86_64 / RISC-V / AArch64 / LoongArch 页表项
├── 32/64 位主机页表、cursor、protect、copy
└── 主机页表本地失效指令与元数据

someboot::paging                 axvm::npt + axvm::arch
├── 启动页表项                    ├── 第二阶段页表项
├── 启动映射与 MAIR               ├── EPT/NPT/Stage-2 配置
└── Ram no-free provider          └── 客户机地址转换失效
```

| 能力                     | 边界                                                                        |
| ------------------------ | --------------------------------------------------------------------------- |
| 页大小和地址             | ax-memory-addr                                                              |
| 通用表页分配能力         | page-table-generic::PageFrameProvider；不依赖 ax-alloc                         |
| 主机第一阶段页表         | axcpu::paging；ax-hal 注入运行时页框和跨核失效                              |
| 客户机第二阶段页表       | axvm；通过 NestedPageTableOps 向 axaddrspace 暴露                            |
| 启动页表                 | someboot；使用 Ram no-free provider                                         |
| 本地地址转换后备缓冲区失效 | axcpu；跨核处理器间中断协调由 ax-hal，客户机失效由 axvm                     |
| AArch64 memory attribute | axcpu 与 someboot 各自执行所属页表上下文的 MAIR/AttrIndx 契约               |
| 输入输出内存管理单元页表 | 由实际输入输出内存管理单元 driver/domain adapter 管理，不进入 CPU 页表组件   |

- walker 不进行堆分配。
- 页表热路径使用泛型静态分发。
- 遍历次数只与固定页表层级和请求页数相关。
- 地址转换后备缓冲区待刷新地址使用固定容量数组，溢出时执行全量 flush。
- someboot 使用现有 `someboot::mem::ram::Ram` no-free provider，并保留 `Ram` 原名。
- axcpu、someboot 和 axvm 不互相依赖，也不通过布尔参数混合主机、Guest 与 boot 语义。
- page-table-generic 不知道 frame 来自 someboot Ram、ax-hal 的 Normal Buddy 还是 axvm 的 HostPagingHandler。
...
- `axcpu::paging` 在主机页表上下文内统一页表项 AttrIndx 与 MAIR_EL1/MAIR_EL2。
- `someboot::arch::aarch64::paging` 在启动上下文内统一启动页表项 AttrIndx 与启动 MAIR。
- 两个上下文都使用 Device-nGnRE、Normal write-back 和 Normal non-cacheable 三个硬件槽位，但不通过依赖主机运行时组件实现共享。
...
page-table-generic 只统一 PteConfig、PagingError、PageFrameProvider、PageTableEntry 和 TableMeta；主机 TlbInvalidator、MappingFlags 和架构页表项归 axcpu。两套旧引擎的行为测试分别迁入 page-table-generic 和 axcpu，迁移期间不允许静默降级。

### 2.8 地址空间设计

#### 2.8.1 ax-memory-set

ax-memory-set 使用按起始地址索引的 `BTreeMap<Addr, MemoryArea>`。find/overlap 为 O(log n)，虚拟内存区域插入和删除不需要移动后续元素，适合 StarryOS 多虚拟内存区域场景，也避免在没有基准数据时引入排序 Vec 的全量迁移。ax-memory-set 只负责：
...
- map/unmap/protect；
- clear/drop 规则；
- `map_metadata`、`unmap_metadata` 和 `replace_area_metadata` 等已经由专用所有权流程完成页表操作后的元数据发布。

公共层不提供 prepare/abort/commit/rollback/finalize 五阶段事务，不为每个页表项分配 `SavedMapping`，也不要求 ArceOS 和 Axvisor 为 Linux 系统调用语义承担通用 undo 日志。各 backend 的单次 map 操作必须自行清理已经安装的前缀；分配型 backend 已在失败分支释放本次取得的页帧。

`replace()` 采用直接 unmap 后 map 的低层语义。要求覆盖操作全成或回滚的 StarryOS 系统调用必须在 `AddrSpace` 层完成范围、资源、commit accounting 和页表移动预检，并维护其专用恢复信息。不得重新把跨虚拟内存区域事务下沉到 ax-memory-set。

`map_metadata` 只允许已经安装页表项并取得对应 owner 的专用流程发布虚拟内存区域。Starry 写时复制 fork 使用该入口；发布失败时由写时复制 backend 撤销 child 页表项、引用和常驻内存集大小。

#### 2.8.2 操作系统策略

ArceOS、StarryOS 与 Axvisor 分别拥有内核虚拟内存、Linux 进程虚拟内存和客户机地址空间策略；主机页表经 ax-hal 使用 axcpu，客户机页表经 axvm 使用，虚拟内存区域机制由 ax-memory-set 提供。只有 StarryOS 承担 Linux 系统调用级事务语义。
...
| 模式 | 行为 | Linux 可见值 |
| ---- | ---- | ----------- |
| Always | 维护全局 committed bytes，但不按 CommitLimit 拒绝；仍检查地址范围、RLIMIT_AS、算术溢出和元数据资源 | `overcommit_memory=1` |
...
- Always 模式的原子 charge 用于准确报告 Committed_AS 和校验地址空间所有权，不因 CommitLimit 拒绝请求。
...
- `DeviceDma` 保留既有 mask、alignment、boundary 和 `DmaDomainId` 字段；当前生产调用方统一使用 `legacy_global()`，本次不扩展新的 domain validation 或输入输出内存管理单元接口。
- 当前平台使用直接/旁路地址转换。真实输入输出内存管理单元控制器、设备 attach/detach 和输入输出页表仅在出现生产消费者后单独设计，不新增空 trait、空 feature 或 facade。
...
每类资源必须能够指出唯一所有者、释放入口和并发保护方式；不可复制令牌、地址空间外层独占和锁顺序共同阻止重复释放与并发修改。
...
| page-table-generic      | 保留并收窄           | 沿用原组件名称，只保留无架构 walker、表页所有权、provider trait 和公共错误；不提供 stage facade                      |
| axcpu::paging           | 接收主机页表         | 接收原 multiarch 的架构页表项、cursor、protect、copy 和地址转换后备缓冲区 batching                                   |
| axvm                    | 接收第二阶段页表     | 接收可变层级第二阶段页表适配、客户机页表项和失效                                                                      |
| someboot                | 接收启动页表         | 持有启动页表项、启动映射和 Ram no-free provider                                                                       |
| ax-page-table           | 删除                 | 完成上述归属迁移后删除 package、feature、re-export 和非历史引用                                                       |
| ax-page-table-entry     | 合并后删除           | 主机页表项迁入 axcpu::paging::entry                                                                                    |
| ax-page-table-multiarch | 合并后删除           | 主机页表能力迁入 axcpu::paging                                                                                         |
| ax-memory-set           | 保留并精简           | 统一虚拟内存区域查找和直接 backend 操作；删除通用事务日志，专用恢复留在策略层                                                  |
...
| 输入输出内存管理单元 page table        | 不新建公共内存 crate | 实际控制器支持加入`drivers/iommu/<controller>` 私有模块和 DmaDomain adapter，不进入 axcpu 主机页表                  |
...
| ax-hal、somehal、axklib | 保留为 adapter       | 分别组合 axcpu/someboot 的页表能力、dma-api 和 mmio-api，不形成新的内存管理层                                        |
...
- backend unmap 失败返回错误而不 panic；
- 单个分配型 backend map 中途失败时清理本次已安装前缀和页帧；
- 跨多个虚拟内存区域的 unmap/protect 故障注入记录直接操作的部分提交边界，不误报为全成或回滚；
- protect 成功后虚拟内存区域/页表项 flags 一致，backend 失败不能被报告为成功；
...
#### 3.2.3 阶段 1B：收敛 MappingBackend

本阶段删除所有消费者共同承担的五阶段事务和逐页撤销快照，使公共地址空间路径保持直接、可审计和无额外事务堆分配。

1. `MappingBackend` 只保留 map、unmap、protect、split 和必要的 shrink hook。
2. 删除 `MappingOperation`、`MapPrecondition`、`MappingPlan`、`CommitState`、`SavedMapping` 及 prepare/abort/commit/rollback/finalize。
3. `MemorySet` 恢复按起始地址索引的 `BTreeMap`，不为单区操作构造多个临时 Vec。
4. ArceOS 和 Axvisor backend 直接调用各自页表；分配型 map 中途失败时只清理本次已安装前缀。
5. StarryOS 写时复制 clone、mremap 和 commit accounting 在 `AddrSpace` 层保留专用 preflight/rollback，不复制到公共层。
6. 保留 metadata-only API 供已经取得页表项和 owner 的专用流程发布状态。
7. 用大范围线性映射回归确认不再按 4 KiB 页保存快照，并验证 map 中途失败不泄漏本次分配。
...
7. 保持既有 `legacy_global()` domain 语义；没有真实输入输出内存管理单元消费者时不新增 domain validation、identity 更名或空壳输入输出内存管理单元 crate。
...
8. 将现有 UsageKind 计数收敛为单一表并置于 `stats` feature；StarryOS 启用，嵌入式默认关闭。
...
#### 3.2.6 阶段 4：按执行上下文归属页表

本阶段先以行为对照测试固定旧实现语义，再把架构代码放回实际所有者，只复用确实相同的无架构 walker。

1. 为原 multiarch 和 generic 建立 map/unmap/query、huge page、drop、错误和页框释放行为基线。
2. 将 `PageSize` 移入 ax-memory-addr，删除 starry-mm 和 axaddrspace 的重复类型或页表 crate 依赖。
3. 收窄 page-table-generic，只保留 PageFrameProvider、PageTableEntry、TableMeta、PteConfig、PagingError 及无架构 walker；禁止 `target_arch` 和 stage feature。
4. 将 MappingFlags、GenericPTE、架构主机页表项、32/64 位主机页表、cursor、protect、copy 和失效 batching 迁入 `axcpu::paging`。
5. ax-hal 实现运行时 PageFrameProvider 和跨核地址转换后备缓冲区失效，并导出当前架构主机 PageTable/PageTableCursor；axcpu 不依赖 allocator、处理器间中断或 OS。
6. 将第二阶段页表项、EPT/NPT/Stage-2 配置和客户机失效保留在 axvm；axaddrspace 生产代码只通过 NestedPageTableOps 使用。
7. 将启动页表项、启动映射、MAIR 配置和 Ram no-free provider 保留在 someboot；somehal 只转发启动错误类型。
8. 实现主机失效策略：AArch64 hardware broadcast、其他多核架构同步 remote 处理器间中断、单核 local；禁止多核静默退化。
9. 分别运行 page-table-generic、axcpu、someboot、ax-hal、axvm 和 axaddrspace 原有测试及四架构构建。
10. 删除 ax-page-table、ax-page-table-entry、ax-page-table-multiarch 及其 feature、re-export、依赖和非历史引用。
...
3. 实现 Always overcommit 和准确 Committed_AS 统计；报告 overcommit_memory=1，执行原子记账但不按 CommitLimit 拒绝。
4. kernel 实现 VmFile/PageSource、page-cache 和 memfd adapter，不允许 starry-mm 回引 kernel 类型。
5. 运行 fork/写时复制、file/shared mapping、mremap、memfd、exec/exit accounting test-suit。
...
- `components/axcpu/src/paging`；
- `virtualization/axvm/src/npt.rs` 及对应架构第二阶段页表模块；
...
当前工作区已合入 `origin/dev@dbc942796507`。下表仅记录已在当前工作区实现并验证的状态。

| 范围 | 状态 | 已完成验证 | 尚需验收 |
| ---- | ---- | ---------- | -------- |
| Runtime allocator | 已实施 | `ax-alloc` 已迁入 `memory/`；Buddy 使用 `SpinNoIrq`；旧 allocator crate、backend feature 和 reclaim callback 已删除；详细 UsageKind 统计仅由 `stats` feature 启用 | 目标板 P99/max、锁等待、碎片和镜像差值 |
...
| ax-memory-set | 部分实施 | 使用 `BTreeMap` 和直接 backend 操作；通用五阶段事务、逐页 `SavedMapping` 和操作级临时 Vec 已删除；metadata-only API 保留给专用所有权流程；unmap/protect/clear 后端失败会显式返回，并保持所有已成功操作区域的页表与元数据一致 | `MappingBackend` typed error；Starry `MREMAP_FIXED` 覆盖失败测试；真实 backend 的跨区直接操作失败注入 |
| 页表归属 | 已实施 | ax-page-table 已删除；无架构 walker 位于 page-table-generic；主机第一阶段页表位于 axcpu::paging；启动页表位于 someboot；第二阶段页表位于 axvm；starry-mm 不再依赖页表 crate | 各目标板地址转换后备缓冲区 shootdown 延迟与镜像裁剪数据 |
| starry-mm | 已实施 | 写时复制、常驻内存集大小/虚拟内存大小、Always overcommit 的 committed-memory accounting、单次有界回收和 mremap 页表项移动的 host 测试通过；fault 页表项/accounting 失败回滚通过 kernel axtest；RISC-V `mremap` 57/57、procfs/commit 20/20 通过 | 目标镜像 QEMU/板级压力回归 |
...
未完成项不得通过兼容层、静默 fallback 或放宽测试关闭。`MREMAP_FIXED` 晚期故障注入完成前，不宣称所有跨虚拟内存区域操作全成或回滚；目标板数据生成前，阶段 7 的 per-CPU page cache 保持不实现。
...
- page-table-generic 已通过原 generic 行为测试且不包含目标架构分支；axcpu::paging 已通过原 multiarch 行为测试；旧统一 facade、旧 multiarch 引擎和独立 entry crate 已删除。
- ArceOS/StarryOS 只通过 ax-hal 使用 axcpu 主机页表；Axvisor 的第二阶段页表只位于 axvm；启动页表只位于 someboot。
...
- 输入输出内存管理单元 page table 只存在于实际控制器 driver/domain adapter，不进入 axcpu 主机页表。
...
- 单个 backend map 中途失败时清理本次新建映射和页帧；StarryOS 需要原子语义的 clone/mremap 路径由 `AddrSpace` 专用恢复逻辑验证。普通跨虚拟内存区域 unmap/protect 暂不宣称 all-or-nothing。
- 写时复制保留上限检测；跨页 clone、父子页表项、引用计数和 accounting 任一步失败均正确回滚。
- Starry 报告 overcommit_memory=1，不执行未被产品配置使用的 strict admission。
- RLIMIT_AS 默认 unlimited 且实际执行；CommitLimit 根据可管理内存容量报告，Committed_AS 随映射所有权变化。
...
- 启用 `stats` 时一次 alloc/free 只更新一个 `UsageKind` bucket；关闭时热路径不包含统计原子。
...
- 启用 `stats` 时 UsageKind 视图由同一 AllocatorStats 数据派生；关闭时分配热路径不包含统计原子更新；
...
- allocation-backed map 在第一个、中间和最后一个页表项失败时均清理本次已映射前缀；StarryOS 专用 clone/mremap 回滚覆盖各故障点；
- huge page 和地址溢出边界；
...
- axcpu 主机页表与 someboot 启动页表分别保证自身 MAIR layout 和 AttrIndx 一致；
- page-table-generic 与 axcpu::paging 对原 generic/multiarch 场景的结果和错误与迁移前基线一致；
...
- committed-memory charge 的预留、归还和失败回滚；
- Committed_AS 随映射变化，CommitLimit 与可管理内存容量一致；
...
- 既有 `legacy_global()` 直接/旁路 DMA 正常工作；未出现真实输入输出内存管理单元消费者前不扩展 domain 接口；
