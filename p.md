...
| starry-mm | 已实施 | 写时复制、常驻内存集大小/虚拟内存大小、commit accounting、Always/Strict、单次有界回收和 mremap charge transaction 的 host 测试通过；fault 页表项/accounting 失败回滚通过 kernel axtest；RISC-V `mremap` 57/57、procfs/commit 20/20 通过 | strict 配置的目标镜像 QEMU/板级压力回归 |
