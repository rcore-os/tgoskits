...
| 地址空间 | ax-memory-set、ax-mm、StarryOS mm/aspace、axaddrspace | `ax-mm::AddrSpace`、Starry `mm::aspace::AddrSpace` 和 `axaddrspace::AddrSpace` 都公开 `map/unmap/protect`，其 backend 都再次实现 `MappingBackend` | VMA split/shrink、错误转换和页表更新规则分散在公共机制与三个策略层 |
...
| allocator 反向调用上层回收 | `os/arceos/modules/axalloc/src/lib.rs:22-42` 保存 `PageReclaimFn`；`os/StarryOS/kernel/src/entry.rs:34` 将 `ax_fs_ng::vfs::page_cache_reclaim` 注册进去 | allocator 依赖运行时注册顺序，并可在未知锁和上下文中进入 VFS/page-cache 逻辑 |
| OOM 内部执行多次策略重试 | `os/arceos/modules/axalloc/src/buddy_slab.rs:207-226` 分配失败后最多 4 次调用 `try_page_reclaim(num_pages.max(16))` 并重新获取 allocator 锁 | 单次页分配的最大执行时间包含文件页扫描和四次重试，无法由 allocator 算法本身界定 |
...
| 上下文约束没有进入接口 | `alloc_pages(num_pages, alignment, UsageKind)` 和 `GlobalAlloc::alloc(Layout)` 都没有 context/capability；后者失败直接进入 `handle_alloc_error` | IRQ、bottom-half 与普通线程看到相同入口，类型和运行时检查都不能阻止实时路径进入扩容或 OOM 处理 |

### 1.3 页表和地址空间重复

| 问题 | 整改前代码示例 | 直接影响 |
| ---- | -------------- | -------- |
| PTE 与页表执行器被拆成不对称边界 | `memory/page_table_entry/src/lib.rs:41` 定义 `GenericPTE`；真正的 `PagingError`、`PagingHandler` 和 cursor 位于 `memory/page_table_multiarch/src/lib.rs`、`bits32.rs`、`bits64.rs` | entry crate 不能独立完成映射，使用者仍必须了解另一 crate 的内部契约 |
| generic 重新定义相同领域概念 | `memory/page-table-generic/src/lib.rs:64-91` 定义 `PageTableEntry/PageTableOp`，`src/def.rs:129` 再定义 `PagingError`，`src/table.rs:87-219` 再实现 map/unmap/walk | 同一个架构 PTE、错误和 frame provider 需要不同 adapter，行为对照只能依靠人工维护 |
| 三个地址空间层重复包装 | `os/arceos/modules/axmm/src/aspace.rs:17`、`os/StarryOS/kernel/src/mm/aspace/mod.rs:45`、`virtualization/axaddrspace/src/address_space/mod.rs:29` 各有 `AddrSpace`；分别再次提供 map/unmap/protect | 公共 VMA 机制的修复需要逐个迁移三个 wrapper，策略差异和机械转发混在一起 |
| overwrite map 不是事务 | `MemorySet::map` 在 `memory/memory_set/src/set.rs:161-169` 先执行 `self.unmap(...)`，随后才调用 `area.map_area(...)` | 新 backend 映射失败时，原重叠 VMA/PTE 已经删除，调用方得到错误但状态无法恢复 |
| unmap 可产生部分提交或 panic | `memory/memory_set/src/set.rs:194-202` 在 `BTreeMap::retain` 中逐个 `unmap_area(...).unwrap()` 并立即删除；之后 `:204-230` 才处理边界 VMA | 任一后续 backend 失败时，前面的区域已释放；整区失败还会由 `unwrap` 直接 panic |
| protect 可产生 PTE/VMA 不一致 | `memory/memory_set/src/set.rs:353-402` 边遍历边 split、protect 和修改 flags，没有 undo；`src/area.rs:121-123` 甚至丢弃 backend `protect` 的 bool 返回并固定返回 `Ok(())` | 前半段成功、后半段失败时无法回滚；backend 失败还可能被报告为成功 |
...
| 问题 | 整改前代码示例 | 直接影响 |
| ---- | -------------- | -------- |
| Linux VM 策略直接依赖 kernel 实现 | `os/StarryOS/kernel/src/mm/aspace/backend/cow.rs:8-20` 同时导入 `ax_fs_ng::FileBackend`、`ax_runtime::hal`、`ax_sync::Mutex` 和 kernel `AddrSpace/MemoryAccounting` | COW 规则无法在不构建 Starry kernel、VFS 和 HAL adapter 的情况下独立测试或复用 |
| COW refcount 上限处理破坏状态 | `os/StarryOS/kernel/src/mm/aspace/backend/cow.rs:23-25` 使用 `u8`；`clone_map` 在 `:581-587` 先执行 `frame.count += 1`，加到 `u8::MAX` 后才返回错误 | 失败返回时当前页计数已经改变；若错误发生在多页 clone 中间，之前页面也没有统一回滚 |
| fork 提交顺序没有回滚 | `os/StarryOS/kernel/src/mm/aspace/backend/cow.rs:588-594` 依次修改父 PTE、映射子 PTE、复制 accounting；任一步之后的操作失败都直接 `?` 返回 | 父 PTE 可能已只读、子 PTE 可能已存在、refcount 可能已增加，但 fork 对外报告失败 |
...
| 回收入口位于 allocator 而不是 Starry 策略层 | `os/StarryOS/kernel/src/entry.rs:34` 注册 VFS page-cache callback；随后任何 ax-alloc 页分配失败都可能触发它 | fault、普通内核页和 DMA32 页申请共享同一隐式回收入口，调用方不能限制只回收一次或禁止当前上下文回收 |
...
| legacy DMA 缺少设备/domain 约束 | `DMAInfo` 只有 `cpu_addr` 和 `bus_addr`；`ax-dma::phys_to_bus` 固定 identity；另一侧 `KlibDma::domain_id()` 固定 `legacy_global()` | DMA mask、设备 domain、direction 和 mapping owner 不随 legacy token 传递，不能表达不同设备或 IOMMU 域 |
...
| 问题 | 整改前代码示例 | 直接影响 |
| ---- | -------------- | -------- |
| 页分配最坏路径无固定上界 | `os/arceos/modules/axalloc/src/buddy_slab.rs:207-226` 的一次失败最多触发 4 次 VFS 回收和 4 次重新分配 | Buddy 操作本身即使有界，外层调用时间仍取决于 page cache 数量、锁竞争和回收结果 |
| API 不区分执行上下文 | `GlobalAlloc::alloc(Layout)`、`alloc_pages(..., UsageKind)` 及 Slab 扩容入口都没有 IRQ/RT 状态或受限 capability | 同一 `Box/Vec/Arc` 调用在普通线程和实时区间走相同全局入口，审计只能依赖调用约定 |
| 固定对象仍由可增长容器持有 | `components/timer_list/src/lib.rs:32-33,71-72` 用 `BinaryHeap::push` 保存定时事件，`:113-121` 为回调分配 `Box`；`components/irq-framework/src/registry.rs:27-35,51-59` 用可增长 `Vec` 和 `Box<Action>` 注册 IRQ | timer/IRQ 对象没有静态容量和耗尽行为；要求 no-fail 或硬实时 profile 时不能证明注册路径不扩容 |
...
| TLSF、Buddy-Slab、stub 和 backend feature 并存                                                                                      | 物理页固定使用 Buddy，仅保留 Normal/Dma32 两个 zone；小对象使用 per-CPU Slab，IRQ/no-fail 对象按实际消费者使用固定池                                             | 删除选择、兼容分支、空 profile 和多余页分类，减小代码体积，分配延迟可测且有界 |
...
| IRQ、bottom-half、RT critical section 可触达通用分配路径                                                                            | 先审计具体目标；已纳入 hard-RT profile 的 IRQ/RT 路径只使用固定池或预留资源，禁止 GlobalAlloc、Slab 扩容、Buddy、回收、文件 IO 和阻塞                         | 形成可验收的实时路径约束，不把尚未审计的默认构建误报为 hard-RT                 |
| task、timer、wait、IRQ 和驱动对象的生命周期不同，不能统一套用一种池                                                                 | 仅有固定 task 上限的 hard-RT 目标使用静态表；timer/wait 优先嵌入 owner；只为实际 IRQ/no-fail 或实测热点实例化本地固定池                                       | 保持确定性且不增加无用 pool、静态内存和公共管理层                             |
| allocator 失败后可隐式回调 page cache 并多次重试                                                                                    | ax-alloc 失败立即返回 NoMemory；仅 starry-mm 在允许上下文显式回收并最多重试一次                                                                                | 底层不依赖 VFS，最坏执行时间和回收上下文可验证                                |
...
| ax-page-table-multiarch 与 page-table-generic 是能力不同的两套引擎，ax-page-table-entry 单独提供 PTE；另有多套 AddrSpace 包装       | ax-page-table 统一公共契约，内部拆分 common/stage1/stage2/boot；stage2 由 feature 裁剪；ax-memory-set 统一 VMA 事务                                            | Stage-1 热路径不携带 Stage-2 代码，同时统一地址、PTE、错误和 frame 契约       |
| Linux VM 策略与 StarryOS kernel、VFS、task、signal 耦合，overcommit_memory=0 与实际总是接受的行为不一致                             | 独立 no_std`starry-mm` 承载不依赖具体 OS 的 Linux VM 策略、记账和 capability；具体 VFS/页表 backend 留在 kernel adapter；默认 Always overcommit，Strict 可选 | 避免再造通用 Linux mm 框架，同时使可复用策略和 Linux 可见行为一致             |
| 旧 crate、旧 feature、re-export、类型别名和转发模块长期兼容                                                                         | 迁移完成即删除旧源码、依赖、feature、shim 和非历史引用                                                                                                         | 依赖图单一，避免双实现和后续维护成本                                          |
| 缺少明确复杂度边界                                                                                                                  | 不引入 NUMA、复杂 zone、页迁移、compaction、多代 LRU、匿名页 swap、通用 OOM killer、Maple Tree 或 RCU VMA                                                      | 保持嵌入式实现规模和内存占用，避免引入 Linux 级复杂度                         |
...
| OS/方案 | 物理内存组织 | 页与连续内存分配 | 内核堆、小对象与固定池 | 分配上下文与实时约束 | 页表与地址空间 | 内存压力与 overcommit | DMA、内存属性与隔离 |
| ------- | ------------ | ---------------- | ---------------------- | -------------------- | -------------- | ---------------------- | ------------------- |
| TGOSKits 当前整改方案 | 固件内存图由 kernutil/someboot 规范化；运行期只保留 Normal、Dma32 两个物理 zone | `ax-alloc` 统一入口；每个 zone 使用 Buddy，连续页由带 count/align/zone 的 `PageRequest` 请求；启动期 bump 在交接后冻结 | 固定 size-class per-CPU Slab；只有存在实际消费者时增加 IRQ/no-fail 固定池，不建立通用 pool manager | 普通分配立即返回 `NoMemory`，不阻塞、不隐式回收；IRQ/RT 路径禁止 GlobalAlloc、Slab 扩容和 Buddy | `ax-page-table` 统一 boot/stage1/stage2 契约；`ax-memory-set` 负责事务；`ax-mm`、`starry-mm`、`axaddrspace` 分别承载 ArceOS、Linux、Guest 策略 | allocator 不回收；仅 `starry-mm` 在允许上下文做一次有界 clean-page 回收；默认 Always overcommit、Strict 可裁剪；无 swap/compaction/OOM killer | `dma-api::DeviceDma` 持有 mask/domain/cache 约束和 move-only RAII owner；Dma32 仅表示物理可达范围，不替代 map/unmap、cache 或 IOMMU 语义 |
| [Zephyr](https://docs.zephyrproject.org/latest/kernel/memory_management/index.html) | 单 heap 或 [Shared Multi Heap](https://docs.zephyrproject.org/latest/kernel/memory_management/shared_multi_heap.html)；后者基于 `sys_multi_heap` 按内存属性选择独立 heap | 以 `sys_heap/k_heap` 的可变长块分配为主；没有统一的 Linux 式 zone Buddy 页层 | [`k_mem_slab`](https://docs.zephyrproject.org/latest/kernel/memory_management/slabs.html) 使用固定块、固定容量；`sys_heap` 处理一般小对象 | `sys_heap` 用编译期循环上限约束搜索；`k_heap` 增加同步和 timeout，ISR 路径不能等待 | 提供 MPU/MMU、用户态内存域和按配置启用的虚拟内存能力；不提供 Linux 进程 VMA、COW、file mapping 体系 | heap 分配失败或按 timeout 等待；低层 heap 不执行 page-cache reclaim、swap 或 OOM 策略 | multi-heap 可区分 DMA/非缓存区；设备 DMA 映射、cache 和生命周期仍属于设备/BSP 边界 |
| [FreeRTOS](https://www.freertos.org/Documentation/02-Kernel/02-Kernel-features/09-Memory-management/01-Memory-management) | 应用选择一个 heap 实现；`heap_5` 可把多个不连续 RAM region 组成系统 heap | 无独立物理页 allocator；`heap_1` 只分配，`heap_4/5` 使用可合并空闲块的可变长分配，`heap_2` 为 legacy | task、queue、semaphore 等支持应用提供静态内存；不要求通用 Slab | 动态分配不执行内核回收或阻塞等待；确定性路径可关闭动态对象分配 | 内核不提供通用 MMU 页表、进程 VMA、COW 或文件映射 | 分配失败返回空指针并可调用 failure hook；无 reclaim、swap、overcommit 和 OOM killer | DMA 地址、cache、一致性和专用内存由 port、BSP 或驱动处理，heap 不拥有设备 DMA 生命周期 |
| [RT-Thread](https://www.rt-thread.io/document/site/programming-manual/memory/memory/) | small memory、Slab 或 memheap 按构建选择；memheap 可管理多个独立或不连续内存区 | RTOS 核心不建立统一 zone/page 层；连续内存能力取决于选定 heap 和 BSP | Slab、memory pool 或静态对象承担固定大小分配；不同时保留所有通用 heap backend | ISR 中禁止可能导致挂起的动态分配/释放；普通分配失败直接返回 | RTOS 核心不提供 Linux 级进程 VMA、COW 和 page-cache VM 策略 | 底层 allocator 不回调文件系统，不执行 swap、compaction 或通用 OOM 恢复 | 特殊内存区由 memheap/BSP 划分，DMA mask、cache 和 owner 由设备层维护 |
| [ThreadX](https://github.com/eclipse-threadx/rtos-docs-asciidoc/blob/main/rtos-docs/threadx/modules/ROOT/pages/chapter3.adoc) | 应用可为不同物理区域分别创建多个 byte pool 或 block pool | [byte pool](https://github.com/eclipse-threadx/rtos-docs-asciidoc/blob/main/rtos-docs/threadx/modules/ROOT/pages/chapter3.adoc#memory-byte-pools) 使用 first-fit 可变长块；没有独立物理页 Buddy | [block pool](https://github.com/eclipse-threadx/rtos-docs-asciidoc/blob/main/rtos-docs/threadx/modules/ROOT/pages/chapter3.adoc#memory-block-pools) 使用固定大小块和空闲链表，避免外部碎片 | block/byte pool 可让线程按 timeout 等待；byte pool 不允许从 ISR 调用，block pool 路径更确定 | 内核不提供 Linux 进程 VMA、COW、匿名页或文件页策略 | 资源不足时立即失败或等待 timeout；没有内核级 reclaim、swap、overcommit 或 OOM killer | 可用独立 pool 放置高速或设备专用内存；DMA 地址和 cache 所有权仍由应用/驱动维护 |
| [Linux](https://docs.kernel.org/mm/physical_memory.html) | NUMA node + DMA/DMA32/Normal 等 zone；启动期 memblock，运行期按 zone 管理 page frame | zone Buddy 提供高阶连续页，per-CPU pageset 缓存 order-0 页；高阶失败可进入 compaction/reclaim | SLUB/kmalloc 负责小对象，vmalloc 负责虚拟连续区，mempool 为关键对象保留最低容量 | GFP flags 表达可阻塞、可回收、NOIO/NOFS 和 atomic 上下文；PREEMPT_RT 仍不是最小硬实时 allocator | 完整 VMA、页表、COW、anonymous/file/shared mapping、THP、NUMA 和并发 VM 机制 | direct/background reclaim、writeback、swap、compaction、overcommit 和 OOM killer 组合运行 | 完整 DMA API、IOMMU domain、swiotlb/bounce、cache maintenance 和设备 mask 模型 |
| [Rust-Shyper](https://atomgit.com/iSureSystem/rust-shyper) | 平台 RAM 静态配置；全局 heap 初始使用 4 MiB 区域，首次耗尽后扩展到其余 RAM | 连续 [`PageFrame`](https://atomgit.com/iSureSystem/rust-shyper/blob/80b69c1b2d3bf2f65a5164433e7066add0311643/src/mm/page_frame.rs) 从全局 heap 零化分配，没有独立 zone/page allocator | [`LockedHeapWithRescue`](https://atomgit.com/iSureSystem/rust-shyper/blob/80b69c1b2d3bf2f65a5164433e7066add0311643/src/mm/heap.rs) Buddy 同时承担全局堆和 frame backing；无独立 Slab/per-CPU 固定池 | 没有 IRQ/RT 分配 capability；扩展后的 heap 再次耗尽时 panic | 架构目录直接实现 Guest Stage-2 页表；`PageFrame` RAII 持有页表目录和中间页，不提供 Linux 进程 VM | 无通用 reclaim、swap、overcommit 或 OOM 恢复，主要依赖静态 VM 资源配置 | 面向 Guest RAM、SMMU 和设备直通；隔离目标集中在 hypervisor/guest，而非 Linux dma-buf 生命周期 |
| [Asterinas](https://github.com/asterinas/asterinas) | OSTD 以 frame capability 和 metadata 表达物理页所有权；frame allocator 使用 Buddy 与 per-CPU pool/cache | [frame allocator](https://github.com/asterinas/asterinas/tree/d971fee615cd0952b3e1bdf31064183bd287bea0/osdk/deps/frame-allocator) 提供页和连续 frame，CPU-local cache 降低共享 Buddy 锁竞争 | [heap allocator](https://github.com/asterinas/asterinas/tree/d971fee615cd0952b3e1bdf31064183bd287bea0/osdk/deps/heap-allocator) 使用固定 size-class Slab 和 CPU-local object cache | CPU-local cache 优化并发延迟；设计目标是安全通用 OS，不承诺 RTOS 式禁止分配路径 | OSTD 提供 `VmSpace`、安全页表边界和 range cursor；kernel 在其上实现 VMAR、VMO、page cache、fault 和 Linux ABI | Linux 兼容策略位于 kernel service，OSTD frame allocator 不反向调用具体文件回收策略 | OSTD 将 IO/DMA、页表和 frame 的 `unsafe` 收敛到小型可信边界，以 capability/owner 表达资源生命周期 |

TGOSKits 行描述整改目标，不代表所有 hard-RT 约束已经由当前默认构建实现；当前已落地项与后续工作以第 3 节实施方案和第 4 节验收条件为准。
...
5. OS Policy
   ...
   ArceOS VM      Linux process VM   Guest Stage-2 VM
```
...
- `ax-hal`、`axklib`、VFS、task、signal 是 adapter，不是内存管理核心；`starry-vm` 只是用户指针访问 shim，不是 StarryOS kernel mm 或 Linux 进程 VM。
- PageTable Core 和 AddressSpace Core 是并列公共机制；OS Policy 组合两者，不要求 ax-memory-set 直接依赖具体页表实现。
...
| Guest 类型       | axvm-types           | GuestPhysAddr、VM 配置和虚拟化领域类型                                                                  | 保留，不承载分配逻辑                                         |
| 启动内存         | kernutil::memory     | 内存描述、MemoryDescriptor 专属区间规则、reserved/free 计算                                             | 保留并吸收 ranges-ext 的最小必需逻辑                         |
| 启动分配         | someboot::mem::ram   | 调度器和 IRQ 启用前的现有 bump、Ram frame provider 及启动内存交接                                       | 保留现有实现，补 checked arithmetic 和冻结状态               |
...
| 地址空间公共核心 | ax-memory-set        | VMA 查找、重叠检查、split/shrink/grow 和元数据/PTE 事务                                                 | 保留并修正                                                   |
| ArceOS 策略      | ax-mm                | kernel direct map、iomap、基础 ArceOS 地址空间和平台 glue                                               | 保留                                                         |
| Linux VM 策略    | starry-mm            | COW 引用规则、共享页 owner、RSS/VSS、overcommit/commit、fault outcome、有界回收和文件/页来源 capability | 新建；保持薄策略层，strict commit 可选                       |
| Guest VM 策略    | axaddrspace          | GuestPhysAddr、Stage-2 flags、NPT/VMID 和 Guest 地址空间                                                | 保留                                                         |
...
| 平台适配         | axklib、ax-hal       | DMA、MMIO、cache、TLB 和地址转换 glue                                                                   | 保留为 adapter                                               |
...
| starry-mm            | ax-memory-addr、ax-page-table 的 PageSize、ax-errno、scope-local、ax-kspin；文件和页来源通过 capability 注入   |
...
- OS、VM、驱动和 adapter 不得直接依赖 buddy-slab-allocator。
- ax-alloc 不依赖页表、地址空间、DMA、VFS、StarryOS 或 AxVM。
...
- starry-mm 不依赖 StarryOS kernel、syscall dispatch、具体 VFS、task 或 signal 实现。
...
- VMA、PTE、fault、COW、commit accounting。
- DMA map/unmap、IOMMU、cache maintenance 或 MMIO 映射。
- VFS/page-cache reclaim callback、阻塞等待或内部重试。
...
当前没有能够证明需要保证前向进展的 reserve 消费者，因此不预置 EmergencyReserve、capability、feature 或静态页栈。若后续出现明确消费者，必须先给出容量、耗尽行为和所有权测试，再作为独立变更引入，不能成为普通 `alloc_pages` 的 OOM fallback。
...
| `dealloc_pages`                               | 保留具体释放接口；必须恢复原 MemoryZone 和 UsageKind，RAII 页对象保存请求元数据                         |
...
- 仅在 BSP、单核、调度器和 IRQ 启用前使用。
- BSP 在冻结前通过现有 someboot SMP prealloc 为全部配置 CPU 分配 PerCpuMeta、secondary stack 和 per-CPU data，并写入启动内存描述。
...
- 固定池只用于 IRQ/no-fail 路径或基准确认的热点，不按对象名称批量创建通用 pool。
...
- 已审计并启用固定池的 IRQ 路径只访问 irq-safe 固定池；池耗尽立即返回错误，不回退到 GlobalAlloc、Slab 扩容或 Buddy。
...
| embedded-hard-rt | RT 路径只用固定池；非 RT 控制面使用 Buddy + Slab              | 不启用                 |
| starry           | Buddy + per-CPU Slab                                             | starry-mm 显式有界回收 |
| hypervisor       | Buddy + per-CPU Slab；Guest RAM 预留或显式分配                | 默认不启用             |

这些是 OS/BSP 的配置组合，不是 ax-alloc feature 名称；当前没有经过消费者审计的 embedded-hard-rt 构建项时，不创建同名空 feature。后续引入具体组合时，未启用的能力和静态状态不得进入链接结果。
...
- ax-alloc 不注册 `PageReclaimFn`，不回调 VFS 或 page cache。
- 不在 IRQ、RT critical section 或持不可睡眠锁时回收。
- 不执行匿名页 swap、同步写回、全系统 VMA 扫描、无上界重试或通用 OOM victim 评分。
...
├── entry      architecture PTE + memory attribute index
├── stage1     4K、cursor、protect、copy、TLB batching     [feature = stage1]
...
| TLB invalidation         | TlbInvalidator，由架构 adapter 选择 local、hardware broadcast 或 remote IPI |
| AArch64 memory attribute | entry::aarch64 的唯一 MemAttrLayout；平台写 MAIR，PTE 只引用 index          |
| IOMMU page table         | 不属于 ax-page-table；由 IOMMU driver/domain adapter 管理                   |

- walker 不进行堆分配。
- 页表热路径使用泛型静态分发。
- 遍历次数只与固定页表层级和请求页数相关。
- TLB 待刷新地址使用固定容量数组，溢出时执行全量 flush。
...
- stage1、stage2 和 boot 关闭后，对应执行代码、PTE 类型和静态状态不得进入链接结果。
- ax-page-table 不知道 frame 来自 someboot Ram 还是 Normal Buddy。
- 不支持的架构、stage 或页大小在编译期不可见或返回 typed `Unsupported`，不得 panic。

SMP TLB 规则：

- AArch64 优先使用 Inner Shareable hardware broadcast，不再叠加逐 CPU IPI flush。
- 没有硬件 broadcast 的架构由 ax-hal 提供同步 remote IPI shootdown；单核只执行 local flush。
- primary IPI ready 之前只执行本地 flush；primary 发布 ready 后显式启用 remote shootdown。secondary CPU 必须在发布 ready 前安装 kernel root 并执行 full local flush，不能向未就绪 CPU 发送同步 IPI。
- SMP 构建必须在编译或初始化时确认存在 broadcast 或 IPI 实现，禁止静默退化为仅本 CPU flush。
- hard RT critical section 不执行需要跨 CPU 同步的页表修改；普通线程路径允许有界 batching 和同步完成。
...
- Stage-1/boot PTE adapter 只编码 layout 中的 index，不自行定义另一组常量。

统一后的能力必须同时覆盖：

- ax-page-table-multiarch 的 ax-memory-addr 地址类型、4K Stage-1 路径、cursor、protect、copy 和 TLB batching；
- page-table-generic 的可变层级、可变页大小、Stage-2 和独立 PageTableEntry/PteConfig 能力；
- ax-page-table-entry 的 MappingFlags、GenericPTE 和各架构 PTE 位编码。
...
ax-memory-set 使用按起始地址排序的紧凑 `Vec<MemoryArea>`：find/overlap 使用二分查找，map/unmap/protect 在 commit 前通过 `try_reserve` 预留最终 VMA 容量，提交后只移动已有元素。该实现避免不可预留的树节点分配和额外 VMA allocator，同时保持 page-fault 查找为 O(log n)；VMA 修改为 O(n)，后续只有基准证明其成为瓶颈时才替换索引。ax-memory-set 只负责：
...
- clear/drop 规则；
- backend typed error；
- VMA 元数据和 backend/PTE 的事务顺序。

事务规则：

- map、unmap、protect 先完成整段范围检查，只克隆相交 VMA 并准备 remove/insert 元数据差量，再完成 backend validation 和所有可失败资源预留；此阶段不得修改 live PTE/VMA，也不得克隆整个 `MemorySet`。
- map plan 使用 `MapPrecondition::Vacant` 或 `Replacing` 明确区分全新映射与同事务覆盖；重叠替换的 new map prepare 可以观察并保存即将由前序 unmap 删除的旧 PTE，普通 map 发现旧 PTE 必须返回 `AlreadyExists`。
- commit 执行已经 prepare 的 backend/PTE 操作；全部 backend 成功后发布已预留容量的 VMA 元数据，再 finalize 不再需要的旧资源。
- 跨多个 VMA 的操作必须 all-or-nothing；不能出现前几个 VMA 已删除 PTE、后续失败而元数据仍描述原映射的状态。
- backend 应将资源申请和可预检失败放入 prepare。当前页表操作仍可能返回错误，因此 commit 返回 typed result，并由该 backend 在返回错误前恢复当前操作；MemorySet 逆序回滚此前已经提交的操作。
- map 成功后插入 VMA；unmap 成功后删除或拆分全部目标 VMA；protect 成功后统一提交 PTE、实际 flags 和 reported flags。
- `MappingBackend` prepare 返回 typed `MappingResult<Plan>`，至少区分参数错误、冲突、内存不足和状态损坏；具体页表/VFS 错误由 OS adapter 转换，不再使用 bool 丢失失败类别。
- `map_metadata` 只允许专用事务已经安装 PTE 并取得对应 owner 后发布 VMA。Starry COW fork 使用该入口，发布失败时先由 COW backend 撤销当前 child PTE、引用和 RSS；普通 map 禁止绕过 backend 事务。
...
| 组件        | 保留职责                                                                                                      | 禁止职责                                                                    |
| ----------- | ------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------- |
| ax-mm       | kernel direct map、iomap、linear/allocated mapping、内核 root 和 TLB glue                                     | Linux COW/file VM、Guest Stage-2                                            |
| starry-mm   | OS 无关的 Linux VM policy、COW 引用、共享页 owner、RSS/VSS、commit、fault outcome、回收编排和 capability 数据 | 具体 PTE cursor、VFS cache/listener、memfd、task/signal 和 syscall dispatch |
| axaddrspace | GuestPhysAddr、Stage-2 flags、NPT/VMID、Guest region                                                          | Linux VMA 和 host kernel iomap                                              |
| starry-vm   | 用户指针检查和 copy                                                                                           | VMA/PTE 所有权                                                              |
...
StarryOS kernel 保留 `AddrSpace`、具体 PageTableCursor backend、VFS/page-cache/memfd listener adapter、syscall ABI、task/process 挂接和 signal/errno 转换。不得在 kernel 重复实现已进入 starry-mm 的记账、COW 计数、overcommit 或 fault/reclaim policy。
...
- RLIMIT_AS 始终实现，默认值为 unlimited；mmap、brk、mremap 和 fork 扩大 VSS 前检查。
- AddrSpace 在现有锁内只维护私有可写匿名/COW 和私有可写文件映射的 committed bytes；只读私有匿名映射不计费。共享匿名内存由 allocator-owned `SharedPages` 在创建时持有一次全局 RAII charge，最后一个 `Arc` 释放时归还，fork 和重复 VMA 不重复计费。
- 全局 Committed_AS 使用单一原子增减，在 VMA 事务或共享匿名 owner 创建/销毁时更新，不进入 page fault 热路径；普通文件、linear/device 和 imported `SharedPages` 不计费。
- Always 模式只用于准确报告 Committed_AS，不因 commit limit 拒绝请求。
- Strict 模式复用同一计数，在扩大私有可写匿名/COW、私有可写文件承诺或创建 shared-anonymous owner 前执行原子 reserve；失败不修改 VMA 或泄漏已分配页。
- unmap、exec 和进程退出归还计数；fork/COW、mremap 和失败路径必须事务回滚。
- `/proc/meminfo` 的 CommitLimit/Committed_AS 和 `/proc/sys/vm/overcommit_memory` 必须反映实际配置，不保留固定占位值。
- 当前 COW 的 u8 计数上限检查保留为回归条件，但实现改为锁内 u32 或合适的原子计数。
- clone 前先检查全部目标页和计数容量，再提交引用计数、父 PTE、子 PTE 和 accounting；任一步失败必须按逆序回滚本次已提交页面。
- COW clone 已安装 child PTE 后只发布 VMA metadata，不重复执行普通 map；未发布 child 的失败清理必须同时覆盖当前 VMA 和此前已发布 VMA。
...
- identity 平台使用 identity adapter；IOMMU 平台实现 domain-specific map/unmap。
- IOMMU controller 和 I/O page table 归属 `drivers/iommu/<controller>` 及其 domain adapter，不进入 CPU ax-page-table::stage2。
- IOMMU driver 可以复用 ax-memory-addr、PageFrameProvider 和无 OS 语义的低层 helper，但独立定义 IOPTE 格式、IOVA、domain 生命周期、IOTLB invalidation 和 device attach/detach。
- dma-api 只持有 `DmaDomain` capability 并调用 map/unmap；axklib/OS glue 为 IOMMU 页表注入 Normal 页，用途记为 UsageKind::PageTable。
- 当前 IOMMU-bypass 平台继续使用 identity adapter；未实现的 IOMMU controller 返回 typed Unsupported，不伪装成 identity 映射。
- 保留现有不可复制且实现 Drop 的 coherent、contiguous、streaming 和 CpuDmaBuffer RAII wrapper，驱动只持有这些 owner。
...
- DmaAllocHandle 和 DmaMapHandle 是 backend token，不是新的 RAII wrapper；删除其 Copy/Clone，dealloc/unmap 按值消费 token，查询方法只借用。
...
- descriptor/ring 在 probe 或启动阶段预分配；IRQ completion 只归还预分配 descriptor。
- bounce buffer 的分配和 copy-in 在 submit 前完成，copy-out 和释放在非 IRQ 路径完成。
...
- axklib/ax-mm adapter 建立 CPU VA 到设备物理区的 device mapping。
- MMIO 物理区不进入 Buddy，不获得 RAM 所有权，unmap 只释放 VA/PTE。
...
| Starry 匿名/COW 页  | Starry kernel backend 持有具体 PTE/页，starry-mm 提供引用规则和 PageSource owner | 引用归零后由注入的 PageSource 释放                                                                                                     |
| Guest RAM           | axaddrspace/AxVM                                                                 | VM teardown 后通过 ax-alloc 释放                                                                                                       |
| DMA buffer          | dma-api RAII owner/domain；StarryOS dma-buf 由`Arc` 保留该 owner               | 自有 buffer 不可复制；导入方只借用 address/length 并持有 operation-lifetime retainer；底层 token move-only；map/unmap、alloc/free 成对 |
| MMIO 区域           | 平台设备资源                                                                     | 只管理 VA/PTE，不释放设备物理区                                                                                                        |

锁顺序固定为：

1. AddrSpace/VMA lock；
2. page-table cursor；
...
- reclaim、VFS、signal、task wakeup 和驱动 callback 不得在 allocator、reserve 或页表内部锁中执行。
...
| IRQ                  | irq-safe 固定池、预分配 ring    | GlobalAlloc、Slab 扩容、Buddy、回收 |
| 未线程化 bottom-half | 固定池                          | 阻塞和通用分配                      |
| RT critical section  | 固定池、预留资源                | 回收、文件 IO、通用堆扩容           |
| 普通内核线程         | Buddy、Slab 扩容                | 持不可睡眠锁回收                    |
| Starry 用户缺页      | starry-mm；允许时在外层有界回收 | IRQ 锁内 fault handling             |
...
| ax-page-table-entry     | 合并后删除           | PTE 迁入 ax-page-table::entry                                                                                         |
| ax-page-table-multiarch | 合并后删除           | cursor、protect、copy 和 TLB batching 迁入 ax-page-table                                                              |
| page-table-generic      | 合并后删除           | 可变层级、页大小和 Stage-2 能力迁入 ax-page-table                                                                     |
| ax-memory-set           | 保留并修正           | 统一 VMA 操作和事务，使用 typed MappingResult                                                                         |
| ax-mm                   | 保留并收窄           | 只保留 ArceOS kernel VM 和平台 glue                                                                                   |
| starry-mm               | 新建                 | 接收 OS 无关的 Linux VM policy、accounting、owner 和 capability；默认 Always overcommit，strict commit 可选           |
| StarryOS kernel/src/mm  | 拆分并删除重复       | 保留具体 VFS/PageTableCursor backend 和 ABI glue，删除已迁移 policy、重复状态和无意义 re-export                       |
| axaddrspace             | 保留并收窄           | 只保留 Guest Stage-2 策略                                                                                             |
| starry-vm               | 保留并收窄           | 只保留用户指针访问边界                                                                                                |
| dma-api                 | 保留并统一           | 成为唯一驱动 DMA API                                                                                                  |
| axklib::KlibDma         | 保留为 adapter       | 将 dma-api 接到 ax-alloc 和平台 cache/IOMMU 操作                                                                      |
| IOMMU page table        | 不新建公共内存 crate | 实际控制器支持加入`drivers/iommu/<controller>` 私有模块和 DmaDomain adapter，不进入 CPU ax-page-table               |
...
- 现有 DMA RAII wrapper 的 Drop 只释放一次；
- COW 已有 u8 上限检测、跨页 clone、页表/accounting 失败和并发 fault 的回滚；
- backend unmap 失败后 VMA/PTE 不丢失；
- 跨多个 VMA 的 unmap/protect 在中间 backend 失败时完全不提交；
- protect 后 VMA/PTE flags 一致；
...
2. 收紧 DmaPod。
3. 保留现有 DMA RAII wrapper；删除 DmaAllocHandle/DmaMapHandle 的 Copy/Clone，使 dealloc/unmap 消费底层 token。
4. 将 COW 计数改为锁内 u32 或合适原子类型，在修改前 checked increment。
5. 为跨页 clone 建立 preflight/commit/rollback，覆盖引用计数、父子 PTE 和 accounting。

#### 阶段 1B：修复 MappingBackend 事务
...
3. MemorySet 在任何修改前为整段范围生成 MappingPlan，并预留 VMA split 所需元数据。
4. 重写 unmap，禁止在 retain closure 中 unwrap；跨多个 VMA 的 PTE 和元数据一次性提交。
5. 重写 protect，使 backend/PTE、actual flags 和 reported flags 一次性提交。
6. commit 失败的 backend 必须携带 undo 数据，在返回错误前恢复当前操作；MemorySet 回滚此前操作。
7. 使用逐 VMA 故障注入验证 map/unmap/protect 要么全成功，要么 PTE/VMA 完全保持原状。
...
7. 不增加没有生产消费者的 ReserveAccess、EmergencyReserve 或通用 RT guard，不新增按用途拆分的页 class。
...
10. BSP 冻结 bump 前确认全部 CPU 的 meta、stack、per-CPU data 已由 someboot SMP prealloc 分配；AP 只初始化本地 Slab。
...
3. 将 ax-page-table-entry 的 MappingFlags、GenericPTE 和架构 PTE 编码迁入 entry 模块。
4. 建立 feature-gated stage1 模块，迁移 multiarch 的 cursor、protect、copy、4K 路径和 TLB batching。
5. 建立 feature-gated stage2 API 视图，迁移 generic 的可变层级、可变页大小、walker 和 Guest PTE。
6. 建立 feature-gated boot API 视图，使用 someboot Ram no-free provider；stage2/boot 可共享 private flexible 引擎。
7. 统一 AArch64 MemAttrLayout，迁移 axcpu/someboot/somehal 的 MAIR 写入和 PTE index。
8. 实现 TlbInvalidator 策略：AArch64 hardware broadcast、其他 SMP 架构同步 remote IPI、单核 local；禁止 SMP 静默退化。
...
3. 迁移不依赖具体 PageTableCursor/VFS/task/signal 的 range policy、共享页 owner 和纯状态操作。
4. 保持 StarryOS kernel adapter 调用旧实现与新实现的行为一致，运行基础 mmap/munmap/mprotect/brk test-suit。

#### 阶段 5B：迁移 policy 和 accounting

1. 迁移 COW 引用计数、SharedPages、RSS/VSS、commit accounting 和统计快照；具体 anonymous/linear/COW/file/shared PageTableCursor backend 留在 kernel adapter。
2. 由 starry-mm 提供 mremap/clone/fork 的纯策略与事务数据，kernel adapter 执行具体 PTE/VFS 操作；实现 RLIMIT_AS，默认 unlimited。
...
6. 分别运行默认 Always 和 strict 配置的 fork/COW、file/shared mapping、mremap、memfd、exec/exit accounting test-suit。

#### 阶段 5C：迁移 fault、回收和 kernel 接线

1. starry-mm 提供 fault/reclaim 决策和 FaultOutcome；kernel backend 执行具体 PTE fault，并负责 signal/errno 转换。
...
5. 删除 kernel/src/mm 中重复的 accounting、COW 引用、commit、fault/reclaim policy 和无意义 re-export；保留具体 backend adapter。
6. 收窄 ax-mm 和 axaddrspace，删除与 ax-memory-set 重复的机械包装。

#### 阶段 6：静态池和系统配置

1. 保持 embedded-default、starry 和 hypervisor 为文档化的 OS/BSP 组合；只有目标板给出 RT critical section、容量和耗尽行为后才增加具体 embedded-hard-rt 构建配置，删除 ax-alloc 内无行为的空 profile feature。
2. 先审计 IRQ/no-fail 调用点和基准；只为实际使用者实例化 IrqEventPool、DmaDescriptorPool 或驱动专属 IoRequestPool。
...
该 cache 只能缓存 order-0，必须支持 drain，单核配置必须禁用，不引入 migratetype 或完整 PCP。
...
当前工作区基于 `origin/dev@1b3f97d20a7e`。
...
| DMA | 已实施 | move-only token 与非法 `DmaPod` compile-fail；RAII 单次释放；固定 pool 耗尽立即 `NoMemory`；最后一个 `Arc` retainer 释放 backing；AArch64 `qemu-rga/system` 23/23 通过 | 真实非一致性 DMA 平台的 cache/bounce 指标 |
| ax-memory-set | 已实施 | 排序 VMA Vec 在 commit 前 fallible reserve；map/unmap/protect 的 prepare/commit/rollback；metadata split 失败保持原 VMA；不可 split backend 返回 typed error；clear 失败保留元数据 | Starry `MREMAP_FIXED` 覆盖已有目标后发生晚期页表分配失败的专用故障注入 |
| ax-page-table | 已实施 | 旧三个 crate 已删除；stage1/stage2/boot feature 分离；全 feature 页表测试通过 | 各目标板 TLB shootdown 延迟与镜像裁剪数据 |
| starry-mm | 已实施 | COW、RSS/VSS、commit accounting、Always/Strict、单次有界回收和 mremap charge transaction 的 host 测试通过；fault PTE/accounting 失败回滚通过 kernel axtest；RISC-V `mremap` 57/57、procfs/commit 20/20 通过 | strict 配置的目标镜像 QEMU/板级压力回归 |
| someboot | 已实施 | checked bump、Active/Frozen、固件多段内存规范化和 BSP CPU 区域预分配；host 测试通过 | 各 BSP 启动内存图和 AP 启动实测 |
| 性能与 hard-RT | 未验收 | 已保留固定容量 pool、无 allocator 隐式回收和 profile 裁剪边界 | 按 4.3 在目标板采样；未满足测量触发条件前不增加 per-CPU page cache |
...
- ax-memory-set 是唯一 VMA 公共机制，ax-mm、starry-mm 和 axaddrspace 只保留各自策略。
- starry-mm 独立承载 OS 无关的 Linux VM policy；StarryOS kernel 只保留具体 VFS/页表 backend 和 ABI adapter，不保留重复 policy 或无意义转发模块。
...
- IOMMU page table 只存在于实际控制器 driver/domain adapter，不进入 CPU ax-page-table。
- 生产依赖图无旧 crate、兼容 facade、双 backend 或循环依赖。
- StarryOS 复杂 VM 代码不进入最小 ArceOS/Axvisor 构建。
...
- 现有 DMA RAII wrapper 保持不可复制且只释放一次；DmaAllocHandle/DmaMapHandle 不再实现 Copy/Clone，dealloc/unmap 消费 token。
...
- ax-alloc 分配失败立即返回 NoMemory；IRQ/RT 路径不触发通用分配或回收。
- Buddy、Slab、Starry、Guest 和 DMA 对同一物理页不存在重复所有权。
- 只有 Normal/Dma32 两个 MemoryZone。
- map/unmap/protect 跨一个或多个 VMA 均为 all-or-nothing；prepare 失败不修改状态，commit 后 VMA/PTE 一致。
- COW 保留上限检测；跨页 clone、父子 PTE、引用计数和 accounting 任一步失败均正确回滚。
...
- local、hardware-broadcast 和 remote-IPI TLB flush 次数及耗时；
- reclaim 次数、回收页数和耗时；
- DMA bounce 次数和复制字节数。
...
- hard-RT 配置中已识别 RT critical section 的通用堆分配计数为 0。
...
- heap integrity；
- BSP 冻结前已为全部配置 CPU 预分配 meta、stack 和 per-CPU data；AP 启动不调用 bump；
- someboot ram allocator 在 Frozen 后拒绝分配，交接前后内存描述无重叠；
- MemoryZone 和 UsageKind 视图由同一 AllocatorStats 数据派生且结果正确；
- 仅对已经审计并实际启用固定池的 IRQ/RT 消费者验证耗尽立即失败且不回退到通用 allocator；没有生产消费者时不创建空池或通用 guard。

#### 页表和地址空间

- map/unmap/protect 后 VMA/PTE 一致；
- VMA 中间 split 和两端 shrink；
- 第一个、中间和最后一个 backend prepare/commit 故障注入均满足全成或全回滚；
- huge page 和地址溢出边界；
- Drop 只释放 owned table frame；
- boot no-free provider；
- Stage-1/Stage-2 flags 隔离；
- AArch64 hardware-broadcast、其他架构 remote-IPI 和单核 local TLB 路径分别验证；SMP 缺少有效 shootdown 实现时构建或初始化失败；
- axcpu、someboot、somehal 写入相同 MAIR layout，所有 AArch64 PTE AttrIndx 均来自该 layout；
- ax-page-table 对 multiarch/generic 共同场景的结果和错误与迁移前基线一致；
- 可变层级/页大小/Stage-2 与 cursor/protect/copy/TLB batching 均保留；
- MappingBackend 错误完整传播。

#### StarryOS

- fork/COW 引用上限、失败回滚和并发 fault；
...
- identity/IOMMU-bypass 正常工作，未实现 IOMMU domain 明确返回 Unsupported；
