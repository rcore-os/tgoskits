...
PROFILE = ROOT / "broad.profdata"
UNPROFILED = {"starryos", "core", "alloc", "compiler_builtins", "profiler_builtins"}
PGO_FLAGS = {
...
def main():
    ...
    if profiled and mode == "use":
        expected = (ROOT / "profile.sha256").read_text().split()[0]
    ...
    if target is not None:
        with (ROOT / "wrapper.jsonl").open("a") as output:
