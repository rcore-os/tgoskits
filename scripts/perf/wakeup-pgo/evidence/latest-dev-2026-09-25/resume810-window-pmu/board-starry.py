#!/usr/bin/env python3
"""Collect matched A/F forward-window PMU diagnostics, never full20 evidence."""
...
def main():
    ...
    while True:
        try:
            session = json.loads(request("/api/v1/sessions", "POST", {
                ...
                "board_id": BOARD,
                "required_tags": [],
                "client_name": "issue2308-resume810-window-starry",
            }))
            break
    ...
    status = {
        ...
        "limitations": ["PMU read syscalls perturb cache state and add end-read cost", "diagnostic benchmark is not frozen full20"],
        "rounds": [],
    ...
    try:
        ...
        files = [
            ("A.bin", IMAGES["A"][0]),
            ("F.bin", IMAGES["F"][0]),
            ("resume810.sh", ROOT / "guest.sh"),
            ("bench-window-pmu", BENCH),
        ]
        ...
        for index, tag in enumerate(ROUNDS):
            ...
            send(f"curl --connect-timeout 10 --max-time 20 -fsS "
                 f"{GUEST_API}/share/sessions/{sid}/resume810.sh -o /tmp/resume810.sh "
                 f"&& sh /tmp/resume810.sh {sid} {tag}")
            terminal = wait_for(f"RESUME810_STARRY_DONE {tag} [01]".encode(), 700)
            rc = int(re.search(f"RESUME810_STARRY_DONE {tag} ([01])".encode(), terminal).group(1))
            record = {"tag": tag, "kind": kind, "guest_exit": rc, "logs": {}}
            names = ["sha256"] + [
                f"{event}-{policy}-{round}.log"
                for event in ("instructions", "cycles", "l1i_refill", "l1d_refill")
                for round in (1, 2)
                for policy in ("other", "fifo")
            ]
            for name in names:
                content = request(f"/share/sessions/{sid}/resume810-{tag}-{name}")
        ...
        print("RESUME810_STARRY_COLLECTED", len(status["rounds"]), flush=True)
    except Exception as error:
