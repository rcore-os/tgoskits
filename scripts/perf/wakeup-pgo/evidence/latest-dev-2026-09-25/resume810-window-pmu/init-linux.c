...
{
    printf("RESUME810_CASE_START event=%s policy=%s round=%d\n",
           event, policy, round);
    pid_t child = fork();
    if (child < 0) {
        perror("fork");
    ...
    if (child == 0) {
        char *const argv[] = {
            "/bench-window-pmu", "--policy", (char *)policy,
    ...
    printf("RESUME810_CASE_DONE event=%s policy=%s round=%d exit=%d\n",
...
int main(void)
{
    static const char *const events[] = {
        "instructions", "cycles", "l1i_refill", "l1d_refill",
    };
    static const char *const policies[] = {"other", "fifo"};
    setvbuf(stdout, NULL, _IONBF, 0);
    puts("RESUME810_LINUX_START");
    int failures = 0;
    ...
    printf("RESUME810_LINUX_DONE failures=%d\n", failures);
    for (;;) {
        pause();
    }
}
