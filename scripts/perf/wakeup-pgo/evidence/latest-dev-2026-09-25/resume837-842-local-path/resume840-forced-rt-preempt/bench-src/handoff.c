...
static _Atomic int forced_rt_receiver_priority_delta;
...
static int run_receiver(struct handoff_state *state)
{
    ...
    if (error == 0) {
        error = bench_apply_policy(state->policy,
                                   state->fifo_priority +
                                       atomic_load_explicit(
                                           &forced_rt_receiver_priority_delta,
                         ...
                         struct latency_result *result)
{
    ...
    int priority_delta = same_cpu && policy == BENCH_POLICY_FIFO;
    atomic_store_explicit(&forced_rt_receiver_priority_delta, priority_delta,
                          memory_order_release);
    if (priority_delta) {
        fprintf(stderr,
                "DIAGNOSTIC_FORCED_RT_PREEMPT sender_priority=%d receiver_priority=%d\n",
                state->fifo_priority,
                state->fifo_priority + priority_delta);
    }

    pthread_t receiver;
    int error = pthread_create(&receiver, NULL, thread_receiver, state);
    if (error != 0) {
        atomic_store_explicit(&forced_rt_receiver_priority_delta, 0,
                              memory_order_release);
        errno = error;
