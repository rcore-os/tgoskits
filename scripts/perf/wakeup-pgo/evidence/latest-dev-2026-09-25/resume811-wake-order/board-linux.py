#!/usr/bin/env python3
"""Boot frozen Linux RT with the diagnostic wake-order benchmark."""
...
def main():
    ...
    assert sha(ROOT / "bench-order") == BENCH_SHA
    ...
    while True:
        try:
            session = json.loads(request("/api/v1/sessions", "POST", {
                ...
                "board_id": BOARD,
                "required_tags": [],
                "client_name": "issue2308-resume811-order-linux",
            }))
            break
    ...
    try:
        ...
        boot = wait_for(rb"RESUME811_LINUX_DONE failures=\d+", 360)
        ...
        status["failures"] = int(re.search(rb"RESUME811_LINUX_DONE failures=(\d+)", boot).group(1))
        status["boot_sha256"] = sha(ROOT / "linux-boot.log")
        status["state"] = "collected"
        print("RESUME811_LINUX_COLLECTED", status["failures"], flush=True)
    except Exception as error:
