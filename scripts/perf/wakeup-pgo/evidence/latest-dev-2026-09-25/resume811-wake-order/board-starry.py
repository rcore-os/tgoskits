#!/usr/bin/env python3
"""Collect matched A/F wake-order diagnostics, never full20 evidence."""
...
BENCH_SHA = "3c72c5ead9c0aad2e0bd271f244807f16cad206d125141199fa52deab86e6189"
IMAGES = {
    "A": (
...
def main():
    ...
    while True:
        try:
            session = json.loads(request("/api/v1/sessions", "POST", {
                ...
                "board_id": BOARD,
                "required_tags": [],
                "client_name": "issue2308-resume811-order-starry",
            }))
            break
    ...
    status = {
        ...
        "limitations": ["sender marker follows futex_wake syscall but preemption between syscall return and marker remains possible", "diagnostic benchmark is not frozen full20"],
        "rounds": [],
    ...
    try:
        ...
        for index, tag in enumerate(ROUNDS):
            ...
            send(f"curl --connect-timeout 10 --max-time 20 -fsS "
                 f"{GUEST_API}/share/sessions/{sid}/resume811.sh -o /tmp/resume811.sh "
                 f"&& sh /tmp/resume811.sh {sid} {tag}")
            terminal = wait_for(f"RESUME811_STARRY_DONE {tag} [01]".encode(), 700)
            rc = int(re.search(f"RESUME811_STARRY_DONE {tag} ([01])".encode(), terminal).group(1))
            ...
            for name in names:
                content = request(f"/share/sessions/{sid}/resume811-{tag}-{name}")
        ...
        print("RESUME811_STARRY_COLLECTED", len(status["rounds"]), flush=True)
    except Exception as error:
