...
struct handoff_state {
    ...
    _Atomic size_t receiver_before_user_marker;
...
static int run_receiver(struct handoff_state *state)
{
    ...
    size_t receiver_before_user_marker = 0;
    ...
    atomic_store_explicit(&state->not_parked_samples, not_parked_samples,
                          memory_order_release);
    atomic_store_explicit(&state->receiver_before_user_marker,
                          receiver_before_user_marker, memory_order_release);
    return 0;
}
                         ...
                         struct latency_result *result)
{
    ...
    if (same_cpu) {
        size_t samples = atomic_load_explicit(&state->valid_samples,
                                              memory_order_acquire);
        size_t before = atomic_load_explicit(
            &state->receiver_before_user_marker, memory_order_acquire);
        printf("WAKEUP_ORDER {\"policy\":\"%s\",\"samples\":%zu,"
               "\"receiver_before_user_marker\":%zu,"
               "\"receiver_after_user_marker\":%zu}\n",
               bench_policy_name(policy), samples, before, samples - before);
    }
