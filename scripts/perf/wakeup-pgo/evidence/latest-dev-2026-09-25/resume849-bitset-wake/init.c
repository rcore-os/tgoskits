...
{
    printf("RESUME849_LINUX_START round=%d\n", round);
    pid_t child = fork();
    if (child < 0) {
        perror("fork");
        return 1;
    ...
    printf("RESUME849_LINUX_DONE round=%d exit=%d\n", round, result);
    return result;
}
...
int main(void)
{
    ...
    puts("RESUME849_LINUX_INIT_START");
    int failures = run_case(1) != 0;
    failures += run_case(2) != 0;
    failures += run_case(3) != 0;
    printf("RESUME849_LINUX_INIT_DONE failures=%d\n", failures);
    for (;;) {
        pause();
    }
}
