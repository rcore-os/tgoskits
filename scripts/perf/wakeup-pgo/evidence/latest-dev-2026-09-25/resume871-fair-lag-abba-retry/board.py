#!/usr/bin/env python3
"""Run source-matched ordinary-release A-B-B-A full20 boots."""
...
BUILD_ARTIFACTS = EXPERIMENTS / "resume870-fair-lag-fastpath"
CONTROL_WORKTREE = Path("/tmp/tgoskits-issue2308-resume870-A")
BUILD = BUILD_ARTIFACTS / "build.toml"
IMAGES = {
    "A": ("resume870-A.bin", BUILD_ARTIFACTS / "A.bin"),
    "B": ("resume870-B.bin", BUILD_ARTIFACTS / "B.bin"),
}
...
def main():
    assert not (ROOT / "session.json").exists(), "experiment already started"
    ...
    assert subprocess.check_output(
        ["git", "rev-parse", "HEAD"], cwd=CONTROL_WORKTREE, text=True).strip() == source_head
    assert not subprocess.check_output(["git", "status", "--porcelain"], cwd=CONTROL_WORKTREE)
    patch = subprocess.check_output(["git", "diff", "--binary"], cwd=WORKTREE)
    patch_sha = hashlib.sha256(patch).hexdigest()
    assert patch_sha == "5f796013c5d75bbc7354dced7f4142235591f9b45ffcd5722aeef367973d9f9a"
    ...
    while True:
        try:
            session = json.loads(request("/api/v1/sessions", "POST", {
                ...
                "required_tags": [], "client_name": "issue2308-resume871-fair-lag-abba-retry",
            }))
            break
    ...
    try:
        ...
        for name, path in list(IMAGES.values()) + [("resume756.sh", EXPERIMENTS / "resume788-pgo-repeat" / "guest.sh"), ("bench", BENCH)]:
        ...
        for index, tag in enumerate(SEQUENCE):
            ...
            if failure is None:
                ...
                raise RuntimeError(f"invalid full20 boot {tag}: {failure}")
        print("RESUME871_COMPLETE", len(rounds), flush=True)
    finally:
        stop.set()
