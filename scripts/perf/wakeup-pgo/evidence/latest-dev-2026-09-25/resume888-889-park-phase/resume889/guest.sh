...
mkdir -p "$out"
put() {
  curl -fsS -X PUT -H "X-File-Path: resume889-$1" --data-binary "@$out/$1" "$base/api/v1/sessions/$sid/files" >/dev/null
}
download=1
...
echo "RESUME889_BOARD_DONE $overall"
[ "$overall" -eq 0 ]
