...
mkdir -p "$out"
put() {
  curl -fsS -X PUT -H "X-File-Path: resume836-$1" --data-binary "@$out/$1" "$base/api/v1/sessions/$sid/files" >/dev/null
}
download=1
...
overall=0
for policy in fifo other; do
  for round in 1 2; do
    name="$policy-thread_futex_cross_cpu-$round"
    cat /sys/kernel/debug/scheduler_metrics > "$out/$name-before"
    status=0
    "$out/bench" --policy "$policy" --case thread_futex_cross_cpu > "$out/$name.log" 2>&1 || status=$?
