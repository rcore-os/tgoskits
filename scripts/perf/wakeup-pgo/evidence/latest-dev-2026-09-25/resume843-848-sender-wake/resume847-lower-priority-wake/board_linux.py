#!/usr/bin/env python3
"""Run the same parked-wake binary on the frozen Linux RT image."""
...
INITRAMFS_SHA = "3f68116401fa975984e0328a0e6a3216384fd4303a029e2221a4abf3182369dc"
...
def main():
    ...
    session = json.loads(helper.request("/api/v1/sessions", "POST", {
        ...
        "required_tags": [], "client_name": "issue2308-resume847-linux-rt",
    }))
    sid = session["session_id"]
    status = {"experiment": "resume847", "diagnostic_only": True,
    ...
    try:
        ...
        boot = wait_for(rb"RESUME847_LINUX_INIT_DONE failures=\d+", 420)
        (RUN / "boot.log").write_bytes(boot)
        assert b"Linux version" in boot and b"PREEMPT_RT" in boot
        failures = int(re.search(rb"RESUME847_LINUX_INIT_DONE failures=(\d+)", boot).group(1))
        assert failures == 0
        rows = [json.loads(line.split(" ", 1)[1])
                for line in boot.decode(errors="replace").splitlines()
                if line.startswith("RESUME847_RESULT ")]
        assert len(rows) == 4 and all(row["samples"] == 20000 for row in rows)
        assert [row["case"] for row in rows] == [
            "empty", "parked_lower_priority", "empty", "parked_lower_priority"]
