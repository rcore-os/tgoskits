...
mkdir -p "$out"
put() {
  curl -fsS -X PUT -H "X-File-Path: resume904-$1" --data-binary "@$out/$1" "$base/api/v1/sessions/$sid/files" >/dev/null
}
...
echo "RESUME904_BOARD_DONE $overall"
[ "$overall" -eq 0 ]
