...
put() {
  curl -fsS -X PUT -H "X-File-Path: resume864-$1" \
...
test -f /sys/kernel/debug/futex_wake_window

overall=0
for sequence in other-1 fifo-1 other-2 fifo-2 other-3; do
  policy="${sequence%-*}"
  name="$sequence-thread_futex_same_cpu"
  cat /sys/kernel/debug/scheduler_metrics > "$out/$name-sched-before"
  cat /sys/kernel/debug/futex_wake_window > "$out/$name-futex-before"
  status=0
  "$out/bench" --policy "$policy" --case thread_futex_same_cpu \
    > "$out/$name.log" 2>&1 || status=$?
  cat /sys/kernel/debug/scheduler_metrics > "$out/$name-sched-after"
  cat /sys/kernel/debug/futex_wake_window > "$out/$name-futex-after"
  echo "DIAGNOSTIC_EXIT $status" >> "$out/$name.log"
  put "$name.log"
  put "$name-sched-before"
  put "$name-sched-after"
  put "$name-futex-before"
  put "$name-futex-after"
  if [ "$status" -ne 0 ]; then overall=1; fi
done

echo "RESUME864_DONE $overall"
[ "$overall" -eq 0 ]
