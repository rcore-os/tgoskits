...
恢复源码后在另一 target 缓存目录重建的 A 镜像 SHA 不同；两份镜像
均为 17465344 字节，只有 `.kallsyms` 二进制区间 `0x842000..0x1042000`
存在差异，区间前后逐字节一致。不能把重建镜像误认为板测过的 A，
也不能把 target 目录误认为源码目录；整镜像逐字节复现仍未证明。
