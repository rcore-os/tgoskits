#!/usr/bin/env python3
"""Compare the same-source ordinary same-mm switch candidate on Plus-1."""
...
IMAGES = {
    "A": ("resume898-A.bin", BUILD_ARTIFACTS / "ordinary-control.bin"),
    "B": ("resume898-B.bin", BUILD_ARTIFACTS / "ordinary-candidate.bin"),
}
...
def main():
    ...
    assert CONTROL_WORKTREE == WORKTREE
    assert not subprocess.check_output(["git", "diff", "--binary"], cwd=WORKTREE)
    patch_sha = sha(BUILD_ARTIFACTS / "source.patch")
    assert patch_sha == "d8c8b95ed5a82fb082054cc3522496f61f443c722328239d28b995896b17bb94"
    subprocess.run(["git", "apply", "--check", str(BUILD_ARTIFACTS / "source.patch")],
    ...
    while True:
        try:
            session = json.loads(request("/api/v1/sessions", "POST", {
                ...
                "required_tags": [], "client_name": "issue2308-resume898-same-mm-full20",
            }))
            break
    ...
    try:
        ...
        print("RESUME898_FULL20_COMPLETE", len(rounds), flush=True)
    finally:
        stop.set()
