#!/usr/bin/env python3
"""Recalculate the AArch64 same-mm screening from archived raw full20 logs."""
...
PATCH_SHA = "d8c8b95ed5a82fb082054cc3522496f61f443c722328239d28b995896b17bb94"
...
def main():
    ...
    assert result["image_sha256"] == {
        ...
        "B": "08bdacb5f6515c169ad59687f7c7945f93b557e78370839aee237bab8bb39497",
    }
    ...
    assert (baseline[worst]["p50_ns"], candidate[worst]["p50_ns"]) == (8458, 26250)
    control_p50 = median([valid[tag][worst]["p50_ns"] for tag in ("A1", "A2")])
    assert control_p50 == 27854
    assert round(100 * (candidate[worst]["p50_ns"] / control_p50 - 1), 3) == -5.759
    ...
    assert len(risks) == 6
    assert round(risks[("fifo", "absolute_timer_same_cpu"), "p999_ns"], 3) == 19.766
    assert round(risks[("fifo", "futex_wait_mismatch"), "p99_ns"], 3) == 16.686
    print("valid=A1,B2,A2 invalid=B1; B2_90pct=10/20; worst_rt_over_B=32.221%")
    print("OTHER_same_cpu_A_median=27854 B2=26250 exploratory_delta=-5.759%")
    print("single_B_screening_regressions_gte_3pct=6; formal_two_boot_B_gate=unproven")
