...
put() {
  curl -fsS -X PUT -H "X-File-Path: resume862-$1" \
...
test -f /sys/kernel/debug/futex_wake_probe
...
for sequence in other-1 fifo-1 other-2 fifo-2 other-3; do
  ...
  cat /sys/kernel/debug/futex_wake_probe > "$out/$name-futex-before"
  status=0
  ...
  cat /sys/kernel/debug/futex_wake_probe > "$out/$name-futex-after"
