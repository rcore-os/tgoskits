#!/usr/bin/env python3
"""Boot frozen Linux RT with stratified forward-window PMU diagnostics."""
...
def main():
    ...
    assert sha(ROOT / "bench-stratified-pmu") == BENCH_SHA
    ...
    while True:
        try:
            session = json.loads(request("/api/v1/sessions", "POST", {
                ...
                "board_id": BOARD,
                "required_tags": [],
                "client_name": "issue2308-resume813-stratified-linux",
            }))
            break
    ...
    try:
        ...
        boot = wait_for(rb"RESUME813_LINUX_DONE failures=\d+", 360)
        ...
        status["failures"] = int(re.search(rb"RESUME813_LINUX_DONE failures=(\d+)", boot).group(1))
        status["boot_sha256"] = sha(ROOT / "linux-boot.log")
        status["state"] = "collected"
        print("RESUME813_LINUX_COLLECTED", status["failures"], flush=True)
    except Exception as error:
