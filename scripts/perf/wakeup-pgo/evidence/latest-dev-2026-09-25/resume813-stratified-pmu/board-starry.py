#!/usr/bin/env python3
"""Collect matched A/F stratified PMU diagnostics, never full20 evidence."""
...
BENCH_SHA = "75939fe5a8ccd15476e1f15f967b11c19e5bb680df3da39be141ab391cb73b76"
IMAGES = {
    "A": (
...
BENCH = ROOT / "bench-stratified-pmu"
ROUNDS = ("A1", "F1", "F2", "A2")
...
def save_status(status):
    (ROOT / "starry-status.json").write_text(json.dumps(status, indent=2) + "\n")


def main():
    assert not (ROOT / "starry-status.json").exists(), "diagnostic already started"
    ...
    while True:
        try:
            session = json.loads(request("/api/v1/sessions", "POST", {
                ...
                "board_id": BOARD,
                "required_tags": [],
                "client_name": "issue2308-resume813-stratified-starry",
            }))
            break
    ...
    status = {
        ...
        "limitations": ["PMU read syscalls perturb cache state and add end-read cost", "sender marker does not distinguish in-syscall from return-boundary switch", "diagnostic benchmark is not frozen full20"],
        "rounds": [],
    ...
    try:
        ...
        files = [
            ("A.bin", IMAGES["A"][0]),
            ("F.bin", IMAGES["F"][0]),
            ("resume813.sh", ROOT / "guest.sh"),
            ("bench-stratified-pmu", BENCH),
        ]
        ...
        serial = (ROOT / "starry-serial.log").open("xb", buffering=0)
        buffer = bytearray()
        interrupted = False
        ...
        for index, tag in enumerate(ROUNDS):
            ...
            send(f"curl --connect-timeout 10 --max-time 20 -fsS "
                 f"{GUEST_API}/share/sessions/{sid}/resume813.sh -o /tmp/resume813.sh "
                 f"&& sh /tmp/resume813.sh {sid} {tag}")
            terminal = wait_for(f"RESUME813_STARRY_DONE {tag} [01]".encode(), 700)
            rc = int(re.search(f"RESUME813_STARRY_DONE {tag} ([01])".encode(), terminal).group(1))
            record = {"tag": tag, "kind": kind, "guest_exit": rc, "logs": {}}
            names = ["sha256"] + [
                f"{event}-{policy}-{round}.log"
                for event in ("instructions", "cycles", "l1i_refill")
                for round in (1, 2)
                for policy in ("other", "fifo")
            ]
            for name in names:
                content = request(f"/share/sessions/{sid}/resume813-{tag}-{name}")
        ...
        print("RESUME813_STARRY_COLLECTED", len(status["rounds"]), flush=True)
    except Exception as error:
