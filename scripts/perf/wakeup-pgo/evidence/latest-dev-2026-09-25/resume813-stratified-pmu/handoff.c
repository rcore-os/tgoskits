...
struct handoff_state {
    ...
    _Atomic uint32_t sender_user_marker;
...
static void report_pmu(struct handoff_state *state, size_t count)
{
    ...
    uint64_t *before_marker = read_cost + state->measured_samples;
    size_t before_count = 0;
    for (size_t index = 0; index < count; index++) {
        if (before_marker[index] != 0) {
            swap_u64(&raw[before_count], &raw[index]);
            swap_u64(&read_cost[before_count], &read_cost[index]);
            swap_u64(&state->samples[before_count], &state->samples[index]);
            swap_u64(&before_marker[before_count], &before_marker[index]);
            before_count++;
        }
    }
    for (int group = 0; group < 2; group++) {
        size_t start = group == 0 ? 0 : before_count;
        size_t length = group == 0 ? before_count : count - before_count;
        const char *name = group == 0 ? "before" : "after";
        if (length == 0) {
            printf("WAKEUP_PMU_STRATUM {\"event\":\"%s\","
                   "\"stratum\":\"%s\",\"samples\":0}\n",
                   pmu_event_name, name);
            continue;
        }
        qsort(raw + start, length, sizeof(*raw), compare_u64);
        qsort(read_cost + start, length, sizeof(*read_cost), compare_u64);
        qsort(state->samples + start, length, sizeof(*state->samples),
              compare_u64);
        uint64_t raw_p50 = raw[start + (length - 1) / 2];
        uint64_t read_p50 = read_cost[start + (length - 1) / 2];
        printf("WAKEUP_PMU_STRATUM {\"event\":\"%s\","
               "\"stratum\":\"%s\",\"samples\":%zu,"
               "\"raw_p50\":%llu,\"read_p50\":%llu,"
               "\"adjusted_p50\":%llu,\"p50_ns\":%llu}\n",
               pmu_event_name, name, length,
               (unsigned long long)raw_p50,
               (unsigned long long)read_p50,
               (unsigned long long)(raw_p50 > read_p50 ? raw_p50 - read_p50 : 0),
               (unsigned long long)state->samples[start + (length - 1) / 2]);
    }
            ...
            if (outcome == FUTEX_WAIT_WOKEN) {
                if (state->pmu_fd >= 0) {
                    ...
                    state->samples[2 * state->measured_samples + valid_samples] =
                        pmu_read_cost;
                    state->samples[3 * state->measured_samples + valid_samples] =
                        atomic_load_explicit(&state->sender_user_marker,
    ...
    int receiver_cpu, int private_futex, size_t *mapping_size)
{
    ...
    if (config->handoff_samples >
        (SIZE_MAX - sizeof(struct handoff_state)) / (4 * sizeof(uint64_t))) {
        errno = EOVERFLOW;
        return NULL;
    }
    *mapping_size = sizeof(struct handoff_state) +
                    4 * config->handoff_samples * sizeof(uint64_t);
    ...
    *state = (struct handoff_state) {
    ...
    bench_prefault_samples(state->samples, 4 * config->handoff_samples);
    return state;
}
                      ...
                      const struct bench_config *config)
{
    ...
    for (uint32_t sequence = 1; sequence <= total_samples; sequence++) {
        ...
        atomic_store_explicit(&state->sender_user_marker, sequence,
                              memory_order_release);
        if (wait_for_sequence(state, &state->done, sequence,
                              state->private_futex) != 0) {
            return abort_sender(state);
        }
    }
