...
INITRAMFS_SHA = "a8b7b0ee2020e401c6d91009a473b55cd0b7f2d0ed1fd9d6a830fbf6fce522a6"
...
def main():
    ...
    session = json.loads(helper.request("/api/v1/sessions", "POST", {
        ...
        "required_tags": [], "client_name": "issue2308-resume851-linux-rt",
    }))
    sid = session["session_id"]
    status = {"experiment": "resume851", "diagnostic_only": True,
    ...
    try:
        ...
        boot = wait_for(rb"RESUME851_LINUX_INIT_DONE failures=\d+", 600)
        (RUN / "boot.log").write_bytes(boot)
        assert b"Linux version" in boot and b"PREEMPT_RT" in boot
        failures = int(re.search(rb"RESUME851_LINUX_INIT_DONE failures=(\d+)", boot).group(1))
        assert failures == 0
        ...
        for mode in modes:
            ...
            assert boot.count(
                f"RESUME851_POLICY mode={mode} sender=1 receiver={expected_policy} cpu=0".encode()
            ) == 3
        assert boot.count(b"RESUME851_DONE 0") == 6
        assert b"RESUME851_INVALID" not in boot
