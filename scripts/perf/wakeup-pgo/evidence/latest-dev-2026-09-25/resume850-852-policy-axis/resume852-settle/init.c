...
static int run_case(int round, int other)
{
    ...
    printf("RESUME852_LINUX_START round=%d mode=%s\n", round, mode);
    pid_t child = fork();
    if (child < 0) {
        perror("fork");
        return 1;
    ...
    printf("RESUME852_LINUX_DONE round=%d mode=%s exit=%d\n", round, mode, result);
    return result;
}
...
int main(void)
{
    ...
    puts("RESUME852_LINUX_INIT_START");
    ...
    printf("RESUME852_LINUX_INIT_DONE failures=%d\n", failures);
    for (;;) {
        pause();
    }
}
