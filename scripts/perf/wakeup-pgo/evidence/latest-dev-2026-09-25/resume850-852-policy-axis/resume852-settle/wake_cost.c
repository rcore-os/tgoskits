...
static void report(const char *name, uint64_t *samples)
{
    ...
    printf("RESUME852_RESULT {\"mode\":\"%s\",\"case\":\"%s\",\"samples\":%d,"
...
int main(int argc, char **argv)
{
    ...
    printf("RESUME852_POLICY mode=%s sender=%d receiver=%d cpu=%d\n",
    ...
    for (int sequence = 1; sequence <= TOTAL; sequence++) {
        ...
        struct timespec settle = {.tv_nsec = 500000};
        ...
        state.missed_empty += empty_count != 0;
        state.missed_miss += miss_count != 0;
        state.missed_hit += hit_count != 1;
        if (sequence > WARMUP) {
    ...
    if (error != 0 || result != NULL || state.missed_empty != 0 ||
        state.missed_miss != 0 || state.missed_hit != 0) {
        printf("RESUME852_INVALID join=%d receiver=%d empty=%d miss=%d hit=%d\n",
               error, atomic_load(&state.receiver_error), state.missed_empty,
               state.missed_miss, state.missed_hit);
        return 1;
    }
    report("empty", state.empty_samples);
    report("bitset_miss", state.miss_samples);
    report("bitset_hit", state.hit_samples);
    puts("RESUME852_DONE 0");
    return 0;
}
