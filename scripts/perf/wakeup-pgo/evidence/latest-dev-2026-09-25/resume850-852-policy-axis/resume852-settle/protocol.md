# resume852 receiver-policy wake discriminator
...
This restarts the invalid `resume851` experiment from scratch. That attempt
used 50 us settle and had unexpected wake returns in two OTHER rounds; one
runner also lost per-process logs. None of its rows enter this comparison.
...
sender settles for 500 us after `armed`, outside all timed calls; a
lower-priority receiver cannot
...
The program reports unexpected empty, mismatch and match returns separately
if a process is invalid; all six raw process logs must be fetched before
the runner checks the aggregate exit code.
