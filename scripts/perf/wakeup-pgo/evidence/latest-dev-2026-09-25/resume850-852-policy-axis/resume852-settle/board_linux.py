#!/usr/bin/env python3
"""Compare FIFO and OTHER receiver wake costs on frozen Linux RT."""
...
INITRAMFS_SHA = "b45d2a993ae4600c099662524a128f175c82c3d38934c4dc93ce9c6d5e9ec41a"
...
def main():
    ...
    session = json.loads(helper.request("/api/v1/sessions", "POST", {
        ...
        "required_tags": [], "client_name": "issue2308-resume852-linux-rt",
    }))
    sid = session["session_id"]
    status = {"experiment": "resume852", "diagnostic_only": True,
    ...
    try:
        ...
        boot = wait_for(rb"RESUME852_LINUX_INIT_DONE failures=\d+", 600)
        (RUN / "boot.log").write_bytes(boot)
        assert b"Linux version" in boot and b"PREEMPT_RT" in boot
        failures = int(re.search(rb"RESUME852_LINUX_INIT_DONE failures=(\d+)", boot).group(1))
        assert failures == 0
        ...
        assert len(rows) == 18 and all(row["samples"] == 20000 for row in rows)
        assert [row["case"] for row in rows] == [
            "empty", "bitset_miss", "bitset_hit"] * 6
        assert [row["mode"] for row in rows] == [mode for mode in modes for _ in range(3)]
        for mode in modes:
            expected_policy = 1 if mode == "fifo" else 0
            assert boot.count(
                f"RESUME852_POLICY mode={mode} sender=1 receiver={expected_policy} cpu=0".encode()
            ) == 3
        assert boot.count(b"RESUME852_DONE 0") == 6
        assert b"RESUME852_INVALID" not in boot
