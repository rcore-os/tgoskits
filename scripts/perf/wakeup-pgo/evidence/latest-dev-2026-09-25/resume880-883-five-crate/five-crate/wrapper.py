#!/usr/bin/env python3
"""Instrument the five runtime crates; the root binary stays separate."""
import json
import os
import sys
...
INSTRUMENTED = {"ax_task", "ax_sched", "ax_runtime", "starry_kernel", "ax_sync"}
PGO_FLAGS = {
...
def main():
    ...
    for item in args:
        if item.startswith("-Cprofile-generate=") and Path(item.split("=", 1)[1]) != PROFILE_DIR:
            raise SystemExit(f"resume883: unexpected profile directory: {item}")
