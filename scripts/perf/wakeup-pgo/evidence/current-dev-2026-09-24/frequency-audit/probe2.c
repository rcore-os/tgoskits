...
int main(int argc, char **argv)
{
    ...
    uint64_t previous_enabled_ns = 0;
    uint64_t previous_running_ns = 0;
    for (int rep = 0; rep < 3; rep++) {
        ...
        uint64_t enabled_ns = value.time_enabled_ns - previous_enabled_ns;
        uint64_t running_ns = value.time_running_ns - previous_running_ns;
        previous_enabled_ns = value.time_enabled_ns;
        previous_running_ns = value.time_running_ns;
        if (sched_getcpu() != cpu || value.cycles == 0 ||
            running_ns < 400000000ULL ||
            enabled_ns < 400000000ULL ||
        ...
        printf("FREQ_PROBE cpu=%d rep=%d cycles=%" PRIu64
               ...
               (double)value.cycles * 1000.0 / (double)running_ns,
               (uint64_t)sink);
        fflush(stdout);
    }
    if (close(fd) != 0) {
