...
OUT="${1:-${ROOT}/tmp/task2/icpc-bench-client}"
WORKDIR="$(mktemp -d /tmp/task2-icpc-bench-XXXXXX)"
...
echo "[task2] Building icpc-bench client with ${CC}"
"${CC}" -static -fno-PIE -no-pie -O2 \
  -I "${SRC}" \
  -o "${OUT_ABS}" \
  "${SRC}/icpc-wire.c" \
  "${SRC}/icpc-bench-client.c"
chmod 0755 "${OUT_ABS}"
