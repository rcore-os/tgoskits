...
impl ArceOS {
    ...
    async fn test(&mut self, args: ArgsTest) -> anyhow::Result<()> {
        match args.command {
            TestCommand::Qemu(args) => {
                if args.only_c {
                    self.test_c_qemu(args).await
                } else if args.only_rust {
                    self.test_rust_qemu(args).await
                } else {
                    self.test_rust_qemu(args.clone()).await?;
                    self.test_c_qemu(args).await
                }
            }
    ...
    async fn test_c_qemu(&mut self, args: ArgsTestQemu) -> anyhow::Result<()> {
        ...
        for (index, c_test) in c_tests.iter().enumerate() {
            ...
            // Build and run via make -C os/arceos A=... ARCH=... run
            let result = StdCommand::new("make")
            ...
            match result.with_context(|| format!("c test `{}` failed", c_test.name)) {
...
mod tests {
    ...
    fn arch_from_target_extracts_correct_arch() {
        ...
        assert_eq!(
            arch_from_target("riscv64gc-unknown-none-elf"),
            "riscv64"
        );
        assert_eq!(
