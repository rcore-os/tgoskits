...
impl BuildInfo {
    ...
    pub(crate) fn build_cargo_args(
        ...
        let mut rustflags: Vec<String> = if plat_dyn {
        ...
        if features.iter().any(|f| f == "kcov") {
            rustflags.push("-Cllvm-args=-sanitizer-coverage-level=3".to_string());
            rustflags.push("-Cllvm-args=-sanitizer-coverage-trace-pc".to_string());
            rustflags.push("-Cpasses=sancov-module".to_string());
        }
