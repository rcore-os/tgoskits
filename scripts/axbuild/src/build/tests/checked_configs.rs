use super::{
    config::{
        ...
        declares_removed_platform_feature, declares_removed_runtime_feature,
    },
    *,
};
...
#[test]
fn checked_in_build_configs_do_not_declare_removed_runtime_features() {
    let workspace = crate::context::workspace_root_path().unwrap();
    let mut offenders = Vec::new();

    for path in checked_in_toml_files(checked_in_build_config_roots(&workspace)) {
        let content = fs::read_to_string(&path).unwrap();
        if declares_removed_runtime_feature(&content) {
            offenders.push(
                path.strip_prefix(&workspace)
                    .unwrap_or(&path)
                    .display()
    ...
    assert!(
        offenders.is_empty(),
        "IRQ and multi-task scheduling are mandatory; remove legacy features: {offenders:#?}"
    );
}
