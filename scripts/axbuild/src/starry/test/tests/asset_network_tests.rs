...
#[test]
fn aka_wifi_configs_clean_credentials_on_entry_and_shell_exit() {
    let workspace_root = Path::new(env!("CARGO_MANIFEST_DIR")).join("../..");
    let configs = [
        workspace_root.join("apps/starry/iperf3/board-aka-00-sg2002.toml"),
        workspace_root.join(
            "test-suit/starryos/board-aka-00-sg2002/wifi-iperf-smoke/board-aka-00-sg2002.toml",
        ),
    ];
    ...
    for config_path in configs {
        ...
        let shell = config
            ...
            .unwrap();
        let script = if shell.contains("wifi-session-helper") {
            None
        } else {
            let script_name = shell
                .split("${sessionFile:")
                .nth(1)
                .and_then(|value| value.split('}').next())
                .expect("Wi-Fi shell must reference a session script");
            let script_path = config_path.parent().unwrap().join(script_name);
            Some(fs::read_to_string(script_path).expect("Wi-Fi session script must exist"))
        };
        let cleanup_source = script.as_deref().unwrap_or(shell);
        let cleanup = "wifi_credential_cleanup()";
        let trap = "trap wifi_credential_cleanup EXIT HUP INT TERM";
        assert!(
            cleanup_source.contains(cleanup) && cleanup_source.contains(trap),
            "{} must install a credential cleanup trap",
            config_path.display()
        );
        let helper_index = cleanup_source
            .find("\"$helper\"")
            .or_else(|| cleanup_source.find("wifi-session-helper"))
            .expect("Wi-Fi shell must invoke the common credential helper");
        let cleanup_index = cleanup_source
            .find("wifi_credential_cleanup\n")
            .or_else(|| cleanup_source.find("wifi_credential_cleanup()"))
            .unwrap();
        assert!(
            helper_index < cleanup_index,
            "{} must remove credentials after invoking the helper",
            config_path.display()
        );
    }
}
