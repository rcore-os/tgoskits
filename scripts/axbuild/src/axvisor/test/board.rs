...
mod tests {
    ...
    #[test]
    fn asus_nuc_guest_leaves_primary_cpu_for_management_tasks() {
        let config = axvmconfig::GuestConfig::from_toml(include_str!(
            "../../../../../os/axvisor/configs/vms/asus-nuc15crh/linux-smp1.toml"
        ))
        .unwrap();

        assert_eq!(config.base.phys_cpu_sets.as_deref(), Some(&[0b10][..]));
    }
