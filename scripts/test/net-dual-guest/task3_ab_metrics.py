...
def parse_log(log: str, mode: str) -> Run:
    ...
    for sample in ordered:
        ...
        if control is not None:
            ...
            if int(control["request"]) != sample.request:
                raise ValueError(f"sample {sample.index} CONTROL request mismatch")
        ...
        if status is not None:
            ...
            if int(status["request"]) != sample.request:
                raise ValueError(f"sample {sample.index} STATUS request mismatch")
            if control is None:
