...
static bool send_frame(int sockfd, const struct sockaddr_in *peer, uint8_t *frame,
			       size_t length, const char *operation)
{
	int sent = sendto(sockfd, frame, length, 0, (const struct sockaddr *)peer,
			  sizeof(*peer));
	if (sent == (int)length) {
		return true;
	}
	rt_kprintf("TASK2_SAFE state=Safe event=SendFailure operation=%s sent=%d expected=%u "
		   "errno=%d\n",
		   operation, sent, (unsigned)length, errno);
	return false;
}

static bool same_peer(const struct sockaddr_in *source, const struct sockaddr_in *peer)
{
	return source->sin_family == peer->sin_family && source->sin_port == peer->sin_port &&
...
int main(void)
{
	...
	for (;;) {
		...
		if (poll_result > 0 && (pollfd.revents & POLLIN) != 0) {
			...
			if (received < 0) {
				rt_kprintf("TASK2_REJECTED receive_error errno=%d\n", errno);
			} else if (!same_peer(&source, &peer)) {
				rt_kprintf("TASK2_REJECTED unexpected_source\n");
			} else if (parse_frame(inbound, (size_t)received, &kind, &flags,
				...
				if (!send_frame(sockfd, &source, outbound, error_length, "session-error")) {
					return 1;
				}
				...
				if (kind == KIND_ACK) {
					if (pending_length > 0 && acknowledgement == pending_sequence) {
						...
						rt_kprintf("TASK2_REJECTED invalid_ack ack=%u\n", acknowledgement);
					}
				} else if (kind == KIND_ERROR) {
					state_safe = 1;
					rt_kprintf("TASK2_SAFE state=Safe event=RemoteError code=%u ack=%u\n",
					       error, acknowledgement);
				} else if (kind == KIND_HEARTBEAT) {
					if (!valid_heartbeat(payload, payload_length)) {
						size_t error_length = make_error(outbound, 0,
											ERROR_INVALID_PARAMETER);
						if (!send_frame(sockfd, &source, outbound, error_length,
								"heartbeat-error")) {
							return 1;
						}
						state_safe = 1;
					...
					if (!valid) {
						...
						if (kind == KIND_CONTROL && drop_control_ack_mode != 0) {
							...
							if (!send_frame(sockfd, &source, outbound, ack_length, "ack")) {
								return 1;
							}
						}
						...
						if (action == 1) {
							managed_state = 1;
							/* Bounded values were validated on the wire; retain a
							 * defensive clamp at the plant boundary. */
						...
						uint8_t status_payload[12] = {0};
						status_payload[0] = managed_state;
						...
						pending_sent = now;
						retry_count = 0;
						if (!send_frame(sockfd, &source, pending_frame, pending_length,
								"status")) {
							return 1;
						}
						last_tx = now;
						...
						if (kind == KIND_CONTROL && drop_control_ack_mode == 2) {
							...
							if (!send_frame(sockfd, &source, outbound, ack_length,
									"duplicate-ack")) {
								return 1;
							}
						}
		...
		if (pending_length > 0 && now - pending_sent >= RETRANSMIT_AFTER_MS) {
			if (retry_count >= MAX_RETRIES) {
				...
				retry_count++;
				pending_sent = now;
				if (!send_frame(sockfd, &peer, pending_frame, pending_length,
						"retransmit")) {
					return 1;
				}
#ifndef TASK1_QUIET
		...
		if (now - last_tx >= HEARTBEAT_INTERVAL_MS) {
			...
			if (!send_frame(sockfd, &peer, outbound, heartbeat_length, "heartbeat")) {
				return 1;
			}
			last_tx = now;
		}
