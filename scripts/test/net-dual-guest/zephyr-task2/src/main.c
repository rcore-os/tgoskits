...
static bool send_frame(int socket, const struct sockaddr_in *peer, uint8_t *frame,
			       size_t length, const char *operation)
{
	int sent = zsock_sendto(socket, frame, length, 0, (const struct sockaddr *)peer,
				sizeof(*peer));
	if (sent == (int)length) {
		return true;
	}
	printk("TASK2_SAFE state=Safe event=SendFailure operation=%s sent=%d expected=%u "
	       "errno=%d\n",
...
int main(void)
{
	...
	for (;;) {
		...
		if (poll_result > 0 && (pollfd.revents & ZSOCK_POLLIN) != 0) {
			...
			if (received < 0) {
				printk("TASK2_REJECTED receive_error errno=%d\n", errno);
			} else if (!same_peer(&source, &peer)) {
				printk("TASK2_REJECTED unexpected_source\n");
				...
				if (!send_frame(socket, &source, outbound, error_length, "session-error")) {
					return 1;
				}
				...
				if (kind == KIND_ACK) {
					if (pending_length > 0 && acknowledgement == pending_sequence) {
						...
						printk("TASK2_REJECTED invalid_ack ack=%u\n", acknowledgement);
					}
				} else if (kind == KIND_ERROR) {
					state_safe = 1;
					printk("TASK2_SAFE state=Safe event=RemoteError code=%u ack=%u\n",
					       error, acknowledgement);
				} else if (kind == KIND_HEARTBEAT) {
					if (!valid_heartbeat(payload, payload_length)) {
						size_t error_length = make_error(outbound, 0,
											ERROR_INVALID_PARAMETER);
						if (!send_frame(socket, &source, outbound, error_length,
								"heartbeat-error")) {
							return 1;
						}
						state_safe = 1;
					...
					if (!valid) {
						...
						if (kind == KIND_CONTROL && drop_control_ack_mode != 0) {
							...
							if (!send_frame(socket, &source, outbound, ack_length, "ack")) {
								return 1;
							}
						}
						...
						pending_sent = now;
						retry_count = 0;
						if (!send_frame(socket, &source, pending_frame, pending_length,
								"status")) {
							return 1;
						}
						last_tx = now;
						...
						if (kind == KIND_CONTROL && drop_control_ack_mode == 2) {
							...
							if (!send_frame(socket, &source, outbound, ack_length,
									"duplicate-ack")) {
								return 1;
							}
						}
		...
		if (pending_length > 0 && now - pending_sent >= RETRANSMIT_AFTER_MS) {
			if (retry_count >= MAX_RETRIES) {
				...
				retry_count++;
				pending_sent = now;
				if (!send_frame(socket, &peer, pending_frame, pending_length,
						"retransmit")) {
					return 1;
				}
		...
		if (now - last_tx >= HEARTBEAT_INTERVAL_MS) {
			...
			if (!send_frame(socket, &peer, outbound, heartbeat_length, "heartbeat")) {
				return 1;
			}
			last_tx = now;
		}
