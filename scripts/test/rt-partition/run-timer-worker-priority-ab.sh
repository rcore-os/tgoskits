...
duration_sec="${RT_TIMER_WORKER_AB_DURATION_SEC:-30}"
repeats="${RT_TIMER_WORKER_AB_REPEATS:-1}"
start_cell="${RT_TIMER_WORKER_AB_START_CELL:-baseline}"
output_root="${RT_TIMER_WORKER_AB_OUTPUT_ROOT:-${repo_root}/results/task1/timer-worker-priority/pilot}"
...
linux_kernel="${RT_TIMER_WORKER_AB_LINUX_KERNEL:-${repo_root}/tmp/rt-partition/linux-trace-gcc13/linux-qemu-trace}"
baseline_board="${repo_root}/scripts/test/rt-partition/board-qemu-aarch64-rt.toml"
modified_board="${repo_root}/scripts/test/rt-partition/board-qemu-aarch64-rt-timer-worker-boost.toml"
canonical_archive=""
last_run_dir=""

for value in "$duration_sec" "$repeats"; do
    [[ "$value" =~ ^[0-9]+$ ]] || {
        printf 'error: timer-worker A/B option is not numeric: %s\n' "$value" >&2
        exit 2
    }
done
(( duration_sec > 0 && repeats > 0 )) || {
    printf 'error: timer-worker A/B duration and repeats must be positive\n' >&2
    exit 2
}
[[ "$start_cell" == "baseline" || "$start_cell" == "modified" ]] || {
    printf 'error: RT_TIMER_WORKER_AB_START_CELL must be baseline or modified\n' >&2
    exit 2
}
[[ ! -e "$output_root" ]] || {
    printf 'error: RT_TIMER_WORKER_AB_OUTPUT_ROOT already exists: %s\n' "$output_root" >&2
    exit 2
}
[[ -f "$linux_kernel" ]] || {
...
if baseline != modified:
    raise SystemExit("timer-worker A/B board profiles differ outside features")
if modified_features - baseline_features != {"timer-worker-priority-boost"}:
    raise SystemExit(
        "modified board must add only timer-worker-priority-boost"
    )
...
deduplicate_archive() {
    ...
    for file in linux-qemu rt-linux-initramfs.cpio.gz \
        ...
        cmp -s "$canonical_archive/$file" "$candidate_dir/$file" || {
            printf 'error: timer-worker A/B input changed: %s\n' "$file" >&2
            exit 1
        }
...
run_cell() {
    ...
    printf 'RT_TIMER_WORKER_RUN_START cell=%s run=%s board=%s\n' "$cell" "$run_id" "$board"
    ...
    printf 'RT_TIMER_WORKER_RUN_ACCEPTED cell=%s run=%s output=%s\n' \
...
python3 "$comparison_tool" \
    --baseline-label timer-worker-priority-89 \
    --baseline "${baseline_runs[@]}" \
    --modified-label bounded-timer-worker-priority-91 \
...
experiment=timer-worker-priority-ab
...
wfi_policy=trap
baseline_timer_worker_priority=89
modified_timer_worker_priority=91
modified_event_budget_per_wake=1
...
printf 'RT_TIMER_WORKER_AB_COMPLETE repeats=%s output=%s\n' "$repeats" "$output_root"
