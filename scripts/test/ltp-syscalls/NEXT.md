...
该尝试按评审提议撤回。x86_64 CI 上 `fcntl14` 的第一个变体在 38.957 秒处超时，未进入正式集合；本轮不降低默认 5000 次操作数、不增加 `LTP_TIMEOUT_MUL`、也不降低门槛，[#2341](https://github.com/rcore-os/tgoskits/issues/2341) 保持开放。未来若重新接入，必须先恢复 `fcntl14` 到 `cases-x86_64.txt` 并恢复 `minimum-passes.txt` 中的 `fcntl14 2` 完成门槛。

三个旧 C 测试按本次用户决定清理，不再保留原程序。清理只依赖现有正式 LTP 锁用例 `fcntl17`、`fcntl15`、`fcntl34`、`fcntl36` 提供的路径级部分覆盖，不声称 `fcntl14` 已通过、不声称为正式迁移，也不声称保留原测试；逐项未承接断言完整记录在 `migration.csv`。未承接断言如下：
...
`02-lock-probe-x86_64.log` 历史记录 `fcntl14` 在上游给定的 38 秒期限内未完成，0 TPASS、1 TBROK，wrapper 返回 2；x86_64 CI 上第一个变体在 38.957 秒处超时。为达时限尝试的 Rust 任务栈缓存、子进程页表映射失效策略以及 `F_GETLK` ABI 回写改动已随本 PR 一并撤回，不进入最终 diff，也未见稳定收益。#2341 保持开放，`fcntl14` 不进入正式集合，不降低默认 5000 次操作数、不增加 `LTP_TIMEOUT_MUL`、不降低门槛。`bug-fcntl-len-negative`、`bug-fcntl-posix-lock` 和 `bug-fcntl-whence` 按本次用户决定清理，只由现有正式 LTP 锁用例提供路径级部分覆盖。
...
| 问题 | 议题 | 保留的原程序 |
| --- | --- | --- |
| fcntl14 超时未完成 | [#2341](https://github.com/rcore-os/tgoskits/issues/2341) | 撤回正式集合，议题保持开放；bug-fcntl-len-negative、bug-fcntl-posix-lock、bug-fcntl-whence 按用户决定清理 |
...
这些议题尚未修复，本批迁移不关闭它们；#2341 因 `fcntl14` 超时保持开放。后续新增暂缓项也应记录复现输入、失败证据与对应议题，不以原程序保留代替问题跟踪。

## 4. 验证与兼容性

本 PR 只做测试清理与迁移账本更新，不保留任何为 `fcntl14` 达时限而加入的 Rust 改动：任务栈缓存、未发布子进程页表的映射失效策略和 `F_GETLK` ABI 结构体回写均随本 PR 撤回，最终 diff 不包含内核实现修改。兼容性结论限定到新接入用例实际证明的输入；原程序未被承接的断言不因其他用例同名 syscall 就视为继续覆盖。

### 4.1 验证入口

通过 `cargo xtask starry test qemu --arch <arch> -c qemu/system/ltp-syscalls` 在 x86_64、aarch64、riscv64、loongarch64 串行运行候选。`03-candidates-<arch>.log` 保存固定源码数量契约下的执行结果。逐项提交之后还要运行累计集合和完整 `qemu/system`，核对实际执行程序、重复、已清理程序残留与失败候选误接入。`fcntl14` 已从 `cases-x86_64.txt` 撤回，正式集合不再包含它。
...
全部 295 个原程序已逐项处置：134 个部分替代并清理、69 个无对应项保留、89 个失败跳过（保留原测试）、3 个按明确决定清理（现有正式 LTP 锁用例仅提供路径级部分覆盖）。前三类共 292 行，`commit_subject` 为 `test(starry): migrate bugfix and syscall probes to LTP`；后三个 fcntl 测试的清理提交主题为 `test(starry): drop fcntl bugfix C tests and revert fcntl14`，其未承接断言单独保留在 `migration.csv` 与 2.9 节。其他行保留历史批次或范围外状态，不计入上述数量。混合目录保留 `test-special-fd-write-precedence`，仅删除已通过迁移的两个程序及它们的构建条目。
