...
mod tests {
    ...
    fn test_device_downcast() {
        ...
        let weak = container.get_one::<TestDevice>().unwrap();
        let mut device = weak.lock().unwrap().downcast::<IrqTest>().unwrap();
        assert!(device.is_ok(), "Expected device to be ok");
        assert!(
            weak.try_lock().is_err(),
            "projection retains the outer borrow"
        );
        drop(device);
        assert!(
            weak.try_lock().is_ok(),
            "projection drop releases the borrow"
        );
        assert!(matches!(
            weak.lock().unwrap().downcast::<Empty>(),
            Err(GetDeviceError::TypeNotMatch)
        ));
        assert!(
            weak.try_lock().is_ok(),
            "failed projection releases the borrow"
        );
    }
