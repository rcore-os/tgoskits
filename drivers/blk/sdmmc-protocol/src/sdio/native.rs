...
use super::{
    host::{BusWidth, SdMmcIrqHost},
    init::{MmcSwitchRequest, MmcSwitchRequestState, mmc_switch_deadline_passed},
    nonzero_block_size,
    transport::{DmaSubmitError, ProtocolDataRequest, ProtocolHost},
};
use crate::{
...
pub struct SdMmcCard<H: SdMmcIrqHost + 'static> {
...
pub struct SdMmcDataRequest<'a, H: SdMmcIrqHost + 'static> {
    pub(super) inner: ProtocolDataRequest<'a, H>,
}

impl<H: SdMmcIrqHost + 'static> SdMmcDataRequest<'static, H> {
...
pub struct SdMmcCommandRequest;

/// Submitted `CMD13 SEND_STATUS` transaction.
pub struct SdMmcStatusRequest {
    pub(super) inner: SdMmcCommandRequest,
}

/// Submitted MMC `CMD8 SEND_EXT_CSD` data transaction.
pub struct ExtCsdRequest<'a, H: SdMmcIrqHost + 'static> {
    pub(super) inner: SdMmcDataRequest<'a, H>,
}

/// Submitted SD `CMD6 SWITCH_FUNC` data transaction.
pub struct SwitchFunctionRequest<'a, H: SdMmcIrqHost + 'static> {
    pub(super) inner: SdMmcDataRequest<'a, H>,
}

impl<H: SdMmcIrqHost + 'static> SdMmcCard<H> {
    ...
    pub fn submit_read_blocks_into<'a>(
        ...
        addr: u32,
        buf: &'a mut [u8],
    ) -> Result<SdMmcDataRequest<'a, H>, Error>
    where
        H: 'a,
    {
    ...
    pub fn advance_data_request<'a>(
        &mut self,
        request: &mut SdMmcDataRequest<'a, H>,
        cause: ProgressCause,
    ...
    pub(crate) fn abort_data_request(
        &mut self,
        request: &mut SdMmcDataRequest<'_, H>,
    ...
    pub fn submit_command_request(&mut self, cmd: &Command) -> Result<SdMmcCommandRequest, Error> {
        self.host.submit_command(cmd)?;
        Ok(SdMmcCommandRequest)
    }

    pub fn advance_command_request(
        &mut self,
        _request: &mut SdMmcCommandRequest,
        cause: ProgressCause,
    ) -> Result<CommandResponseProgress, Error> {
        self.host.advance_command_response(cause)
    }

    pub fn submit_status(&mut self) -> Result<SdMmcStatusRequest, Error> {
        let cmd = crate::cmd::cmd13(self.rca);
        let inner = self.submit_command_request(&cmd)?;
        Ok(SdMmcStatusRequest { inner })
    }

    pub fn advance_status_request(
        &mut self,
        request: &mut SdMmcStatusRequest,
        cause: ProgressCause,
    ...
    pub(crate) fn abort_status_request(
        &mut self,
        _request: &mut SdMmcStatusRequest,
    ) -> Result<(), Error> {
