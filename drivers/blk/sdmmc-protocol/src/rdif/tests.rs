...
use crate::{
    ...
    sdio::{
        card::SdioSdmmc,
        host::{ClockSpeed, HostEvent, HostEventKind, SdioHost, SdioIrqHost, SdioIrqSource},
...
fn interrupt_pio_completion_returns_the_exact_owned_buffer() {
    ...
    IQueue::service_events(&mut queue, &queue_event().for_queue(0).unwrap(), &mut sink).unwrap();
...
fn interrupt_pio_error_returns_buffer_only_after_proof_gated_reclaim() {
    ...
    assert_eq!(
        IQueue::service_events(&mut queue, &queue_event().for_queue(0).unwrap(), &mut sink,),
        Err(BlkError::Io)
    );
...
fn runtime_queue_requires_dma_and_is_exclusive() {
    ...
    let controller_cookie = device.control.controller_cookie();
    let queue = Interface::create_queue(&mut device).expect("DMA IRQ queue must exist");
    assert!(Interface::create_queue(&mut device).is_none());

    prove_queue_quiesced_and_close(queue, controller_cookie);

    let replacement =
        Interface::create_queue(&mut device).expect("shutdown queue must release its claim");
    prove_queue_quiesced_and_close(replacement, controller_cookie);
}

fn prove_queue_quiesced_and_close(mut queue: QueueHandle, controller_cookie: usize) {
    queue
        .bind_interrupt_controller(controller_cookie, rdif_block::ControllerEpoch::INITIAL)
        .unwrap();
    let proof = unsafe {
        // SAFETY: MockHost owns no real DMA and this test has accepted no request.
        rdif_block::DmaQuiesced::new(rdif_block::ControllerEpoch::new(2), controller_cookie)
    };
    queue
        .reclaim_after_quiesce(&proof, &mut OneCompletionSink::default())
        .unwrap();
    queue.close().unwrap();
}
...
fn queue_contract_is_interrupt_source_zero_and_serialized() {
    ...
    IQueue::shutdown(&mut queue).unwrap();
}
...
#[test]
fn interface_irq_contract_is_explicit_and_source_is_one_shot() {
    ...
    assert!(Interface::take_irq_source(&mut disabled, 0).is_none());
    ...
    assert!(Interface::take_irq_source(&mut enabled, 1).is_none());
    let source = Interface::take_irq_source(&mut enabled, 0).unwrap();
    assert!(Interface::take_irq_source(&mut enabled, 0).is_none());
    let (mut endpoint, _control) = source.into_parts();
    let rdif_irq::IrqCapture::Captured { event, .. } = endpoint.capture() else {
        panic!("mock completion IRQ must produce one stable event");
    };
...
#[test]
fn ready_device_acquires_runtime_source_only_after_init_source_retires() {
    let mut host = MockHost::default();
    host.irq_source_taken = true;
    let mut device = BlockDevice::from_card_for_test(SdioSdmmc::new(host), dma_config());

    device
        .control
        .raw
        .with_mut(|card| card.host_mut().irq_source_taken = false);

    assert!(
        Interface::take_irq_source(&mut device, 0).is_some(),
        "ready construction must not cache a failed take while the init action is still live"
    );
}
...
fn irq_handler_does_not_enter_shared_card_core() {
    let mut device =
        BlockDevice::from_card_for_test(SdioSdmmc::new(MockHost::default()), dma_config());
    let source = Interface::take_irq_source(&mut device, 0).unwrap();
    let (mut endpoint, _control) = source.into_parts();
    let _guard = device
        .control
        .raw
    ...
    let rdif_irq::IrqCapture::Captured { event, .. } = endpoint.capture() else {
        panic!("IRQ endpoint must capture without entering the task-side core");
    };
...
#[test]
fn queue_irq_fact_is_routed_without_exposing_runtime_request_identity() {
    let control = block_control(dma_config());
    let raw = control.raw.clone();
    let mut queue = BlockQueue::<MockHost>::new(control, 0);
    ...
    let mut sink = OneCompletionSink::default();
    let sibling = Event::from_queue_bits(1 << 1);
    assert!(sibling.for_queue(0).is_none());
...
#[test]
fn shared_core_contention_is_an_owner_violation_not_a_service_retry() {
    let control = block_control(interrupt_pio_config());
    let raw = control.raw.clone();
    let mut queue = BlockQueue::<MockHost>::new(control, 0);
...
#[test]
fn active_queue_must_reclaim_with_proof_before_explicit_shutdown() {
    let control = block_control(dma_config());
    let raw = control.raw.clone();
    let mut queue = BlockQueue::<MockHost>::new(control, 0);
    IQueue::submit_owned(
    ...
    assert_eq!(IQueue::shutdown(&mut queue), Err(BlkError::Busy));
    let proof = dma_quiesced(&queue.control);
    let mut sink = OneCompletionSink::default();
    IQueue::reclaim_after_quiesce(&mut queue, &proof, &mut sink).unwrap();
    assert_eq!(sink.take().id, RequestId::new(19));
    IQueue::shutdown(&mut queue).unwrap();
    drop(queue);
    ...
    raw.with_mut(|card| {
        assert_eq!(
            card.host().aborts,
            1,
            "only proof-gated reclaim may abort an accepted request"
        );
    });
}
...
fn staged_initialization_rejects_progress_before_irq_is_bound_and_enabled() {
    ...
    let source = rdif_block::InitialController::take_irq_source(&mut staged, 0)
...
impl rdif_irq::IrqEndpoint for MockIrqEndpoint {
...
impl rdif_irq::IrqSourceControl for MockIrqControl {
    type Error = crate::sdio::SdioIrqControlError;
...
fn acknowledged_control_irq_does_not_activate_the_block_queue() {
    let source = SdioIrqSource::new(MockIrqEndpoint { control_only: true }, MockIrqControl);
    let source = crate::rdif::irq::into_block_irq_source(source);
    let (mut endpoint, _control) = source.into_parts();

    let rdif_irq::IrqCapture::Captured { event, .. } = endpoint.capture() else {
        panic!("acknowledged control status must remain a captured source event");
    };
...
impl SdioHost2Irq for Host2Mock {
    type Event = MockEvent;
    type IrqEndpoint = MockIrqEndpoint;
    ...
    fn take_irq_source(&mut self) -> Option<SdioIrqSource<Self::IrqEndpoint, Self::IrqControl>> {
        if core::mem::replace(&mut self.irq_source_taken, true) {
            return None;
        }
