...
fn string_value_for_store(source: &Object) -> Result<String, AmlError> {
    source.as_string().map(Cow::into_owned)
}

fn store_through_ref_of(
    target: &WrappedObject,
    source: &WrappedObject,
    token: &ObjectToken,
    integer_width: usize,
) -> Result<(), AmlError> {
    let source = unsafe { source.gain_mut(token) }.clone();
    match unsafe { target.gain_mut(token) } {
        Object::Integer(target) => {
            *target = source.to_integer(integer_width)?;
            Ok(())
        }
        Object::String(target) => {
            *target = string_value_for_store(&source)?;
            Ok(())
        }
        Object::Buffer(target) => {
            *target = source.to_buffer(integer_width)?;
            Ok(())
        }
        target @ Object::Uninitialized => {
            *target = source;
            Ok(())
        }
        _ => Err(AmlError::LibUnimplemented),
    }
}

fn size_of_value(object: WrappedObject) -> Result<usize, AmlError> {
    let object = object.unwrap_reference();
    match *object {
        Object::Buffer(ref buffer) => Ok(buffer.len()),
        Object::String(ref string) => Ok(string.len()),
        Object::Package(ref package) => Ok(package.len()),
...
fn deref_of_reference(object: WrappedObject) -> Option<WrappedObject> {
    matches!(&*object, Object::Reference { .. }).then(|| object.unwrap_reference())
}

fn deref_of_reference_value(
    object: WrappedObject,
    integer_width: usize,
) -> Result<Option<WrappedObject>, AmlError> {
    let Some(object) = deref_of_reference(object) else {
        return Ok(None);
    };
    let Object::BufferField { length, .. } = &*object else {
        return Ok(Some(object));
    };
    let byte_length = length.div_ceil(8);
    if byte_length <= integer_width {
        let mut bytes = [0u8; 8];
        object.read_buffer_field(&mut bytes[..integer_width])?;
        Ok(Some(Object::Integer(u64::from_le_bytes(bytes)).wrap()))
    } else {
        let mut bytes = vec![0u8; byte_length];
        object.read_buffer_field(&mut bytes)?;
        Ok(Some(Object::Buffer(bytes).wrap()))
    }
}

fn index_field_selector(bit_offset: usize) -> Result<usize, AmlError> {
...
fn resolve_local_or_arg(object: WrappedObject, behaviour: ResolveBehaviour) -> WrappedObject {
    match behaviour {
        ResolveBehaviour::SuperName
        | ResolveBehaviour::SuperNameIfExists
        | ResolveBehaviour::Target => Object::Reference {
            kind: ReferenceKind::LocalOrArg,
            inner: object,
        }
        .wrap(),
        _ => object.unwrap_transparent_reference(),
    }
}

fn match_relation(
    candidate: &WrappedObject,
    operator: u8,
    operand: &WrappedObject,
) -> Result<bool, AmlError> {
    if operator == 0 {
        return Ok(true);
    }

    let candidate = candidate.clone().unwrap_reference();
    let operand = operand.clone().unwrap_reference();
    let ordering = match (&*candidate, &*operand) {
        (Object::Integer(candidate), Object::Integer(operand)) => candidate.cmp(operand),
        (Object::String(candidate), Object::String(operand)) => candidate.cmp(operand),
        (Object::Buffer(candidate), Object::Buffer(operand)) => candidate.cmp(operand),
        _ => return Ok(false),
    };

    match operator {
        1 => Ok(ordering == core::cmp::Ordering::Equal),
        2 => Ok(ordering != core::cmp::Ordering::Greater),
        3 => Ok(ordering == core::cmp::Ordering::Less),
        4 => Ok(ordering != core::cmp::Ordering::Less),
        5 => Ok(ordering == core::cmp::Ordering::Greater),
        _ => Err(AmlError::InvalidMatchOperator(operator)),
    }
}

fn match_package_element(
    package: &[WrappedObject],
    start_index: usize,
    operator1: u8,
    operand1: &WrappedObject,
    operator2: u8,
    operand2: &WrappedObject,
) -> Result<u64, AmlError> {
    for (index, candidate) in package.iter().enumerate().skip(start_index) {
        if match_relation(candidate, operator1, operand1)?
            && match_relation(candidate, operator2, operand2)?
        {
...
struct MethodScopeStack<'a> {
    namespace: &'a Spinlock<Namespace>,
    active: Vec<AmlName>,
}

impl<'a> MethodScopeStack<'a> {
    fn new(namespace: &'a Spinlock<Namespace>) -> Self {
        Self {
            namespace,
            active: Vec::new(),
        }
    ...
    fn enter(&mut self, path: AmlName) -> Result<(), AmlError> {
        self.namespace
            .lock()
    ...
    fn leave(&mut self, path: &AmlName) -> Result<(), AmlError> {
        debug_assert_eq!(self.active.last(), Some(path));
        self.namespace.lock().remove_level(path.clone())?;
        self.active.pop();
        Ok(())
    }
}

impl Drop for MethodScopeStack<'_> {
    fn drop(&mut self) {
        while let Some(path) = self.active.pop() {
            let _ = self.namespace.lock().remove_level(path);
        }
    }
}
    ...
    H: Handler,
{
    ...
    pub fn acquire_global_lock(&self, timeout: u16) -> Result<(), AmlError> {
        ...
        // Until SCI-backed Global Lock wakeups are implemented, fail closed rather than spinning
        // forever while firmware owns the lock.
    ...
    fn do_execute_method(&self, mut context: MethodContext) -> Result<WrappedObject, AmlError> {
        ...
        let mut method_scopes = MethodScopeStack::new(&self.namespace);
        if let BlockKind::Method { method_scope } = &context.current_block.kind {
            method_scopes.enter(method_scope.clone())?;
        }
        loop {
                ...
                .pop_if(|op| op.arguments.len() == op.expected_arguments)
            {
                match op.op {
                    ...
                    Opcode::DataRegion => {
                        ...
                        log::warn!(
                            "DefDataRegion encountered in AML! We don't actually support these - \
                             produced region will be incorrect"
                        );
                    ...
                    Opcode::DerefOf => {
                        ...
                        let integer_width = if self.dsdt_revision >= 2 { 8 } else { 4 };
                        let result = if let Some(result) =
                            deref_of_reference_value(object.clone(), integer_width)?
                        {
                            if let Object::FieldUnit(field) = &*result {
                                self.do_field_read(field)?
                            } else {
                                result
                            }
                    ...
                    Opcode::Return => {
                        ...
                        let method_scope = context
                            .method_scope()
                            .expect("Return executed outside an AML method")
                            .clone();
                        method_scopes.leave(&method_scope)?;
            ...
            if context.in_flight.last().map(OpInFlight::resolve_behaviour)
                == Some(ResolveBehaviour::MatchOpcode)
            {
                let operator = context.next()?;
                if operator > 5 {
                    return Err(AmlError::InvalidMatchOperator(operator));
                }
                context.contribute_arg(Argument::ByteData(operator));
                continue;
            }
                    ...
                    match context.current_block.kind {
                        ...
                        BlockKind::Method { method_scope } => {
                            ...
                            if let Some(prev_context) = context_stack.pop() {
                                ...
                                context
                                    .contribute_arg(Argument::Object(Object::Uninitialized.wrap()));
                                continue;
                            } else {
            ...
            match opcode {
                ...
                Opcode::Local(local) => {
                    ...
                    let behaviour = context.last_op()?.resolve_behaviour();
                    context
                        .last_op()?
                        .arguments
                        .push(Argument::Object(resolve_local_or_arg(local, behaviour)));
                }
                Opcode::Arg(arg) => {
                    ...
                    context
                        .last_op()?
                        .arguments
                        .push(Argument::Object(resolve_local_or_arg(arg, behaviour)));
                }
                ...
                | Opcode::ParentPrefixChar => {
                    ...
                    match behaviour {
                        ...
                        ResolveBehaviour::MatchOpcode => unreachable!(),
                    }
                }
    ...
    fn do_match(&self, context: &mut MethodContext, op: OpInFlight) -> Result<(), AmlError> {
        let [
            ...
            Argument::ByteData(operator1),
            Argument::Object(operand1),
            Argument::ByteData(operator2),
        ...
        let package = package.clone().unwrap_reference();
        let Object::Package(package) = &*package else {
            return Err(AmlError::ObjectNotOfExpectedType {
                expected: ObjectType::Package,
                got: package.typ(),
            });
        };
        let start_index = start_index.clone().unwrap_reference().as_integer()? as usize;
        let result = match_package_element(
            package,
            start_index,
            *operator1,
            operand1,
    ...
    fn do_size_of(&self, context: &mut MethodContext, op: OpInFlight) -> Result<(), AmlError> {
        ...
        let result = size_of_value(object.clone())?;
        ...
        match target {
            ...
            Argument::Object(target) => {
                match unsafe { target.gain_mut(&token) } {
                    ...
                    Object::String(target) => {
                        *target = string_value_for_store(&object)?;
                    }
                    ...
                    Object::Reference { kind, inner } => {
                        match kind {
                            ReferenceKind::RefOf => store_through_ref_of(
                                inner,
                                &object,
                                &token,
                            ...
                            ReferenceKind::LocalOrArg => {
                                ...
                                } = &**inner
                                {
                                    ...
                                    // implicit cast
                                    unsafe {
                                        *inner_inner.gain_mut(&token) =
                                            object.gain_mut(&token).clone();
                                    }
                                } else {
                                    // Overwrite the value
                                    unsafe {
    ...
    fn do_field_read(&self, field: &FieldUnit) -> Result<WrappedObject, AmlError> {
        ...
        match field_kind {
            FieldUnitKind::Normal { .. } | FieldUnitKind::Index { .. } => {}
            // FieldUnitKind::Bank { region, bank, bank_value } => {
            FieldUnitKind::Bank { .. } => {
        ...
        for i in 0..native_accesses_needed {
            ...
            let raw = match field_kind {
                FieldUnitKind::Normal { region } => {
                    let Object::OpRegion(region) = &**region else {
                        return Err(AmlError::InvalidOperationOnObject {
                ...
                FieldUnitKind::Index { index, data } => {
                    self.do_index_field_read(index, data, aligned_offset, access_width_bits / 8)?
                }
                FieldUnitKind::Bank { .. } => unreachable!(),
            };
    ...
    fn do_index_field_read(
        ...
        let Object::FieldUnit(index_field) = &**index else {
            return Err(AmlError::InvalidOperationOnObject {
                op: Operation::ReadBufferField,
                typ: index.typ(),
            });
        };
        let Object::FieldUnit(data_field) = &**data else {
            return Err(AmlError::InvalidOperationOnObject {
                op: Operation::ReadBufferField,
                typ: data.typ(),
            });
        };
        if !matches!(index_field.kind, FieldUnitKind::Normal { .. })
            || !matches!(data_field.kind, FieldUnitKind::Normal { .. })
        {
            return Err(AmlError::LibUnimplemented);
        }

        let selector = index_field_selector(bit_offset)?;
        self.do_field_write(index_field, Object::Integer(selector as u64).wrap())?;
        let value = self.do_field_read(data_field)?;
        match &*value {
            Object::Integer(value) => Ok(*value),
            Object::Buffer(bytes) if bytes.len() >= access_bytes => {
                let mut raw = [0u8; 8];
                raw[..access_bytes].copy_from_slice(&bytes[..access_bytes]);
    ...
    fn do_field_write(&self, field: &FieldUnit, value: WrappedObject) -> Result<(), AmlError> {
        ...
        if let FieldUnitKind::Index { index, data } = &field.kind {
            return self.do_index_field_write(field, index, data, value_bytes);
        }
    ...
    fn do_index_field_write(
        ...
        let access_width_bits = field.flags.access_type_bytes()? * 8;
        let access_bytes = access_width_bits / 8;
        let native_accesses_needed =
            (field.bit_length + (field.bit_index % access_width_bits)).div_ceil(access_width_bits);
        let mut written_so_far = 0;
        ...
        for i in 0..native_accesses_needed {
            ...
            let mut bytes =
                if dst_index > 0 || (field.bit_length - written_so_far) < access_width_bits {
                    match field.flags.update_rule() {
                        FieldUpdateRule::Preserve => self
                            .do_index_field_read(index, data, aligned_offset, access_bytes)?
                            .to_le_bytes(),
                        FieldUpdateRule::WriteAsOnes => [0xff; 8],
                        FieldUpdateRule::WriteAsZeros => [0; 8],
                    }
            ...
            object::copy_bits(value_bytes, written_so_far, &mut bytes, dst_index, length);
            self.do_index_field_write_access(
                index,
                data,
                aligned_offset,
                u64::from_le_bytes(bytes),
    ...
    fn do_index_field_write_access(
        &self,
        index: &WrappedObject,
        data: &WrappedObject,
        bit_offset: usize,
        ...
        let selector = index_field_selector(bit_offset)?;
        self.do_field_write(index_field, Object::Integer(selector as u64).wrap())?;
        self.do_field_write(data_field, Object::Integer(value).wrap())
    }
...
enum ResolveBehaviour {
    ...
    AsPackageElements,
    /// Consume a raw `MatchOpcode` byte between the dynamic operands of `Match`.
    MatchOpcode,
    ...
    fn resolve_behaviour(&self) -> ResolveBehaviour {
        if let Some(behaviour) = self.resolve_behaviour.get(self.arguments.len()) {
            *behaviour
            ...
            || (self.op == Opcode::VarPackage && !self.arguments.is_empty())
        {
            ResolveBehaviour::AsPackageElements
        } else {
            panic!(
...
impl MethodContext {
    fn method_scope(&self) -> Option<&AmlName> {
        core::iter::once(&self.current_block)
            .chain(self.block_stack.iter().rev())
            .find_map(|block| match &block.kind {
                BlockKind::Method { method_scope } => Some(method_scope),
                _ => None,
            })
    }
...
mod compatibility_tests {
    ...
    #[test]
    fn deref_of_recognizes_a_reference_to_an_integer() {
        let reference = Object::Reference {
            kind: ReferenceKind::RefOf,
            inner: Object::Integer(42).wrap(),
        }
        .wrap();

        assert_eq!(deref_of_reference(reference).unwrap().as_integer(), Ok(42));
    }

    #[test]
    fn deref_of_materializes_an_indexed_buffer_byte() {
        let buffer = Object::Buffer(vec![0x2a]).wrap();
        let reference = Object::Reference {
            kind: ReferenceKind::RefOf,
            inner: Object::BufferField {
                buffer,
        ...
        assert_eq!(
            deref_of_reference_value(reference, 8)
                .unwrap()
                .unwrap()
                .as_integer(),
            Ok(0x2a)
    ...
    #[test]
    fn index_field_selector_uses_the_byte_offset() {
        assert_eq!(index_field_selector(384), Ok(48));
    }

    #[test]
    fn local_or_arg_resolution_distinguishes_value_and_writable_target() {
        let local = Object::Integer(7).wrap();

        assert_eq!(
            resolve_local_or_arg(local.clone(), ResolveBehaviour::TermArg).as_integer(),
            Ok(7)
        );
        assert!(matches!(
            &*resolve_local_or_arg(local, ResolveBehaviour::Target),
            Object::Reference {
                kind: ReferenceKind::LocalOrArg,
                ..
            }
    ...
    #[test]
    fn store_replaces_a_string_target_with_the_source_string() {
        let source = Object::String(String::from("PNP0C50"));

        assert_eq!(string_value_for_store(&source), Ok(String::from("PNP0C50")));
    }

    #[test]
    fn size_of_follows_a_reference_to_a_package() {
        let package =
            Object::Package(vec![Object::Integer(1).wrap(), Object::Integer(2).wrap()]).wrap();
        let reference = Object::Reference {
            kind: ReferenceKind::RefOf,
            inner: package,
        }
        ...
        assert_eq!(size_of_value(reference), Ok(2));
    }

    #[test]
    fn store_through_ref_of_updates_the_referenced_integer() {
        let target = Object::Integer(1).wrap();
        let source = Object::Integer(42).wrap();
        let token = unsafe { ObjectToken::create_interpreter_token() };

        store_through_ref_of(&target, &source, &token, 8).unwrap();

        assert_eq!(target.as_integer(), Ok(42));
    }

    #[test]
    fn match_finds_the_first_package_element_satisfying_both_relations() {
        let package = [1, 4, 7].map(|value| Object::Integer(value).wrap());

        let result = match_package_element(
            &package,
            0,
            4,
            &Object::Integer(4).wrap(),
            3,
            &Object::Integer(7).wrap(),
        );
    ...
    #[test]
    fn match_returns_ones_when_no_package_element_matches() {
        let package = [1, 4, 7].map(|value| Object::Integer(value).wrap());
