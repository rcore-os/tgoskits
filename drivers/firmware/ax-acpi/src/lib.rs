...
// SAFETY: the owned mapping is movable when its handler can be moved between threads.
unsafe impl<H> Send for AcpiTables<H> where H: Handler + Send {}
// SAFETY: shared table access only exposes immutable mapped bytes, and the handler can be shared.
unsafe impl<H> Sync for AcpiTables<H> where H: Handler + Sync {}
...
pub trait Handler: Clone {
    ...
    fn handle_fatal_error(&self, fatal_type: u8, fatal_code: u32, fatal_arg: u64) {
        panic!(
            "Fatal error while executing AML (encountered DefFatalOp). fatal_type = {}, \
             fatal_code = {}, fatal_arg = {}",
