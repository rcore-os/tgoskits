...
/// Shared, interior-mutable counter store (`Rc`'d so the harness can snapshot
/// without a ref to the device, which is buried inside `Jbd2Dev`). Single
/// threaded, so `Rc` is sufficient and avoids atomic overhead.
///
...
impl ProfilingBlockDevice {
    fn new(size: usize, ctr: Rc<Counters>) -> Self {
        Self {
            data: vec![0; size],
