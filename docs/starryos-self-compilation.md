...
#    免 sudo 路径：--bootstrap（在 QEMU 内从 Alpine base 制备完整工具链，~15-20 min）：
./scripts/self-compile.sh --arch x86_64 --bootstrap
#    或者维护者路径（需 sudo / debootstrap / systemd-nspawn，预热离线缓存）：
...
**CI 不运行的原因**: selfhost rootfs 镜像制备后约 8-12GB（含 rustup nightly 工具链 ~6.9GB、预缓存 crate、系统包）。下载/校验逻辑已在 `self-compile.sh` 中预留（URL + SHA-256），但镜像尚未上传到 tgosimages release（维护者托管待发布）；当前 CI 容器无法下载。免 sudo 的 reviewer 验证路径为 `--bootstrap`（见下文）。

**手动运行**:
```bash
# Blueprint: 将 selfhost rootfs 置于 tmp/axbuild/rootfs/rootfs-x86_64-selfhost.img。
# 免 sudo 路径：./scripts/self-compile.sh --arch x86_64 --bootstrap
# 维护者路径（需 sudo）：sudo ./scripts/prepare-selfhost-rootfs.sh --arch x86_64 --force
# 可下载蓝图已规划但尚未发布（下载 URL + SHA-256 已预留在 self-compile.sh 中）。
