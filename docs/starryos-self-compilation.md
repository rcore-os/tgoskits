...
| x86_64 | ✅ | ✅ (KVM) | 已就绪 | ✅ | ~10 min (KVM) | KVM + SMP=4；guest 构建走 xtask musl-PIE std 流程（动态平台） |
...
**修复**: 将其改为 `0x3_0000_0000`（12GB，与 QEMU `-m 12G` 一致）。x86_64 当前使用 `axplat-dyn` + `somehal::mem::memory_map()` 动态检测，不受此值限制。注：`axconfig_overrides` 机制已在 #1478 中移除（动态平台为唯一路径）。
...
**当前方案**: x86_64 动态平台由 `somehal::mem::memory_map()` 运行时检测，无需 axconfig；riscv64 使用 `os/StarryOS/.axconfig.toml`（当前 `0x2_0000_0000` = 512MB）或 prebuild.sh 的 `gen_axconfig()` 为 x86_64 guest 生成的 minimal config（`0x3_0000_0000` = 12GB）。
