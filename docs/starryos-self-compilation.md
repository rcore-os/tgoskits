...
**CI 不运行的原因**: selfhost rootfs 镜像制备后约 8-12GB（含 rustup nightly 工具链 ~6.9GB、预缓存 crate、系统包）。`self-compile.sh` 中仅保留了下载/校验的**占位符**（注释中的 URL/SHA-256 变量），尚无实际下载代码；镜像也尚未上传到 tgosimages release（维护者托管待发布），当前 CI 容器无法下载。`--bootstrap` 可在免 sudo 下在 QEMU 内完成工具链制备 + 固件下载 + 离线缓存预热，其产出的 rootfs 可直接用于自编译。
