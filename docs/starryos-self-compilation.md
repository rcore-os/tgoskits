...
#    self-compile.sh 在 blueprint 缺失时自动尝试从 tgosimages release 下载预构建镜像
#   （xz 压缩，SHA-256 校验）。若下载不可用，由维护者通过 prepare-selfhost-rootfs.sh
#    制备（该脚本需 sudo / debootstrap / systemd-nspawn）。
