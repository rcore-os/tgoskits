          ...
          └─ StarryOS 内核
              └─ selfhost rootfs (ext4)
                  └─ guest 编译脚本（x86_64: self-compile-inner.sh; riscv64: self-compile.sh）
                      ├─ filter-workspace.sh（架构过滤，riscv64）
...
**修复**: 改为 `phys-memory-size = "0x3_0000_0000"` (12GB，与 QEMU `-m 12G` 一致)。x86_64 使用 `axplat-dyn` + `somehal::mem::memory_map()` 动态检测，无此问题。
...
#    免 sudo 的 provisioning 验证（在 QEMU 内制备工具链后停止；不预热离线缓存，
#    其产出的 rootfs 无法独立完成自编译）：
...
**CI 不运行的原因**: selfhost rootfs 镜像制备后约 8-12GB（含 rustup nightly 工具链 ~6.9GB、预缓存 crate、系统包）。`self-compile.sh` 中仅保留了下载/校验的**占位符**（注释中的 URL/SHA-256 变量），尚无实际下载代码；镜像也尚未上传到 tgosimages release（维护者托管待发布），当前 CI 容器无法下载。`--bootstrap` 可在免 sudo 下验证 QEMU 内 provisioning，但它不预热离线缓存，无法独立完成自编译。
...
# 免 sudo provisioning 验证（不预热缓存，无法独立完成自编译）：./scripts/self-compile.sh --arch x86_64 --bootstrap
