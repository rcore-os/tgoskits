...
```bash
# 1. 准备 rootfs（首次，每架构一次，需要 sudo）
#    debootstrap Debian rootfs，装齐 musl 工具链、kallsyms、firmware、完整源码及依赖闭包。
...
**手动运行**:
```bash
# 准备 rootfs（首次，需要 sudo）
# debootstrap Debian rootfs 并预装 x86_64 self-compile 所需的全部前置：
# musl toolchain、kallsyms tools、firmware、完整源码及离线依赖闭包。
sudo ./scripts/prepare-selfhost-rootfs.sh --arch x86_64 --force
