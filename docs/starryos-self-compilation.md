...
|------|---------|----------|--------|--------|------|------|
| riscv64 | ✅ | ✅ | 已就绪 | ✅ | ~100 min (TCG) | TCG 模拟，SMP=1；种子/guest 均为静态平台 bare-metal，构建流程一致 |
| x86_64 | ✅ | ✅ (KVM) | 已就绪 | ✅ | ~25 min (KVM) | KVM + SMP=4；guest 构建走 xtask musl-PIE std 流程（动态平台）。实测约 24.6 min（463 tg-xtask + 448 kernel crate → 16 MB ELF, 10737 kallsyms 符号） |

> **x86_64 自编译已通过运行时验证（2026-06）**: 完整闭环已验证——guest 驱动 `cargo xtask starry build`（经 `tg-xtask` + musl-PIE std 流程 + `-Zbuild-std` + linker wrapper），编译全部 crate + 链接 + kallsyms + objcopy，产物经 OVMF/UEFI 启动到 StarryOS shell 提示符。详见下方 §x86_64 自编译构建流程不匹配（已修复）。
...
- x86_64 目标触发动态平台路径（`cargo_dynamic_platform_boot_arch` 通过 target triple
  `x86_64-unknown-none` / `x86_64-unknown-linux-musl` 检测，`boot.rs:309-311`）。因此
  **种子构建**经 axbuild 改写为 ArceOS std/PIE 流程：有效目标是
  `scripts/targets/std/pie/x86_64-unknown-linux-musl.json`，`-Zbuild-std`，并通过
  **自定义链接器 wrapper**（`std_build.rs`）将所有 rlib 包进单个 `--start-group/--end-group`
  并强制 `-pie`。（`_head`/`kernel_entry` 由 `someboot.x` 的 `ENTRY`/`ABSOLUTE` 引用，
  无 `#[used]`、无 Rust 调用方。）
...
riscv64 不受影响：其 target triple 不触发动态平台路径
（`cargo_dynamic_platform_boot_arch` 对 riscv64 返回 `None`），种子与 guest 均为
bare-metal `riscv64gc-unknown-none-elf`，**两套流程一致**，故链接通过。
...
**根因**: 早期已淘汰的平台配置路径在 `os/StarryOS/.axconfig.toml` 中硬编码 `phys-memory-size = 0x2000_0000`。

**修复**: 将其改为 `0x2_0000_0000`（8GB；即当前 `os/StarryOS/.axconfig.toml` 的实际值）。注：riscv64 QEMU 用 `-m 12G`，但静态 axconfig 将内核可见内存封顶在 8GB——自编译已够用，如需更多可同步上调此值。x86_64 当前使用 `axplat-dyn` + `somehal::mem::memory_map()` 动态检测，不受此值限制。注：`axconfig_overrides` 机制已在 #1478 中移除（动态平台为唯一路径）。
...
**状态**: 自编译脚本中原先尝试挂载 tmpfs（registry 1500M + workspace 100M）作为 ext4 缓存一致性 workaround，但两个挂载始终失败（用户态 `mount(8)` 经新版 mount API 挂载新 tmpfs 失败——见上文 fsopen 回退问题；内核自身挂载的 tmpfs——`/tmp`、`/dev/shm` 的 `MemoryFs`，见 `os/StarryOS/kernel/src/pseudofs/mod.rs:93-98`——在所有架构上正常可用）。x86_64 的 tmpfs 挂载代码已移除——缓存一致性问题的真正根因已通过 journal coherence 修复解决（见 §8 bug #7）。riscv64 自编译脚本（`apps/starry/selfhost/selfhost-full-kernel/sh/self-compile.sh:14`）仍显式执行用户态挂载 `mount -t tmpfs -o size=12G tmpfs /tmp`（依赖上文 fsopen→传统 `mount(2)` 回退），并以 `/tmp/build/target` 作为 `CARGO_TARGET_DIR`；这层用户态 tmpfs 会覆盖内核自挂载的 MemoryFs。
...
**当前方案**: x86_64 动态平台由 `somehal::mem::memory_map()` 运行时检测，无需 axconfig；riscv64 使用 `os/StarryOS/.axconfig.toml`（当前 `0x2_0000_0000` = 8GB）或 prebuild.sh 的 `gen_axconfig()` 为 x86_64 guest 生成的 minimal config（`0x3_0000_0000` = 12GB）。

## 脚本编排

| 脚本 | 功能 | 运行环境 |
|------|------|---------|
| `scripts/prepare-selfhost-rootfs.sh` | 维护者工具：创建 selfhost Debian rootfs blueprint（debootstrap + systemd-nspawn + 离线依赖闭包）。需 sudo；支持 riscv64/x86_64/aarch64 | Host (maintainer, sudo) |
...
│   └── qemu-x86_64.toml             # QEMU config (16G, smp 4, shell_prefix/shell_init_cmd, 4h timeout=14400s)
...
1. **`phys-memory-size` 硬编码 8GB (riscv64)**: 动态 RAM 检测因启动阶段地址空间不一致无法实现（`os/StarryOS/.axconfig.toml` = `0x2_0000_0000`；riscv64 QEMU `-m 12G` 会被此值封顶在 8GB）。x86_64 使用 axplat-dyn + somehal 动态检测，无需硬编码。
