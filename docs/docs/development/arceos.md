...
components/
└── ax-task/          # OS 无关的任务状态机、SMP 调度器与等待队列
...
└── src/
    ...
    ├── task.rs       # TaskRuntime 接入、TaskSystem/CpuLocal 所有权与 facade
    ├── guard.rs      # IRQ/preempt 嵌套状态及 LockRuntime 接入
    ├── irq.rs        # runtime IRQ 初始化
    └── mp.rs         # 副核启动顺序
```
...
- **`task.rs`**：保存 runtime 创建的 `TaskSystem` 与 per-CPU `CpuLocal`，并向 `ax-std` 转导应用 API。
- **init 函数**：模块暴露职责明确的初始化阶段，由 `axruntime::rust_main()` 和副核入口按启动顺序调用。
...
- 启用 `smp` 会把多核能力传播到 `ax-runtime` 和系统 API；`ax-task` 核心始终按 SMP/IRQ/抢占安全语义编译
...
| 调度器（`ax-task` + `ax-runtime` 接入） | `cargo xtask clippy --package ax-task` | `cargo xtask arceos test qemu --target riscv64gc-unknown-none-elf` |
