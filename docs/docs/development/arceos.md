...
```bash
cargo xtask arceos qemu --package arceos-std-helloworld --arch aarch64 --features mymod
```
...
| 改动类型 | 验证命令 | 扩展验证 |
|----------|---------|---------|
| 基础 crate（`axerrno`, `kspin`, `page_table_multiarch`） | `cargo test -p <crate>` | `cargo xtask arceos qemu --package arceos-std-helloworld --arch riscv64` |
| HAL（`axhal`） | `cargo xtask arceos qemu --package arceos-std-helloworld --arch aarch64` | 多架构验证 |
| 调度器（`axtask`） | `cargo xtask arceos qemu --package arceos-std-helloworld --arch riscv64` | `cargo xtask arceos test qemu --target riscv64gc-unknown-none-elf` |
| 网络（`axnet` / `axnet-ng`） | `cargo xtask arceos qemu --package arceos-std-httpserver --arch aarch64 --net` | 检查 TCP 连接和吞吐 |
| 文件系统（`axfs` / `axfs-ng`） | `cargo xtask arceos qemu --package arceos-std-shell --arch aarch64 --blk` | 检查文件读写 |
| 驱动（`axdriver`） | `cargo xtask arceos qemu --package arceos-std-helloworld --arch aarch64` | 启用对应设备 `--blk` / `--net` |
...
```bash
cargo xtask arceos qemu --package arceos-std-myapp --arch aarch64
```
...
|----------|-------------------|---------|
| 最小运行 | `ax-std` | `--package arceos-std-helloworld` |
| 多任务 | `multitask` | 在应用 Cargo.toml 中启用 |
| 网络 | `net` 或 `net-ng` | `--package arceos-std-httpserver` |
| 文件系统 | `fs` 或 `fs-ng` | `--package arceos-std-shell` |
...
```bash
cargo xtask arceos qemu --package arceos-std-helloworld --arch <arch> --platform <platform-name>
```
...
| 架构 | 编译目标 | QEMU 启动命令 |
|------|---------|-------------|
| aarch64 | `aarch64-unknown-none-softfloat` | `cargo xtask arceos qemu --package arceos-std-helloworld --arch aarch64` |
| riscv64 | `riscv64gc-unknown-none-elf` | `cargo xtask arceos qemu --package arceos-std-helloworld --arch riscv64` |
| x86_64 | `x86_64-unknown-none` | `cargo xtask arceos qemu --package arceos-std-helloworld --arch x86_64` |
| loongarch64 | `loongarch64-unknown-none-softfloat` | `cargo xtask arceos qemu --package arceos-std-helloworld --arch loongarch64` |
...
cargo xtask arceos qemu --package arceos-std-helloworld --arch riscv64

# 改动驱动/设备
cargo xtask arceos qemu --package arceos-std-helloworld --arch aarch64
cargo xtask arceos qemu --package arceos-std-helloworld --arch riscv64
cargo xtask arceos qemu --package arceos-std-helloworld --arch x86_64

# 改动调度器/多核
cargo xtask arceos qemu --package arceos-std-helloworld --arch aarch64  # SMP=4
cargo xtask arceos qemu --package arceos-std-helloworld --arch riscv64  # SMP=4
```
...
```bash
# 网络
cargo xtask arceos qemu --package arceos-std-httpserver --arch aarch64 --net

# 块设备
cargo xtask arceos qemu --package arceos-std-shell --arch aarch64 --blk
```
