...
| `os/arceos/ulib/` | ArceOS 用户库 | `ax-std`、`ax-libc` | ArceOS 示例与用户应用；系统软件可使用 `ax-std` 标准接口 |
...
flowchart TD
    ...
    ArceosModules --> ArceosUserLib
    ArceosUserLib --> ArceosApps
...
| ArceOS 的 feature 或应用接口 | 能力所属模块的 `Cargo.toml`、`os/arceos/ulib/axstd`、`os/arceos/ulib/axlibc` | ArceOS 应用与上层系统 |
...
- 仅属于 ArceOS 的用户库接口：放 `os/arceos/ulib/`；内核能力放对应 `os/arceos/modules/` crate
...
在 ArceOS 中集成组件的典型链路为：在 `components/` 或 `os/arceos/modules/` 实现复用逻辑，在拥有该能力的 crate 中定义 feature；Rust 应用接口接入 `axstd`，C/POSIX 接口接入 `axlibc`，最后通过 `apps/arceos/*` 或 `test-suit/arceos/*` 进行验证。
...
- 新增 feature 开关：修改实际拥有该能力的模块，并在 `axstd` 或 `axlibc` 中按需映射
