...
vCPU 的生命周期由 `virtualization/axvcpu/` 管理：
...
`virtualization/axvcpu/src/percpu.rs` 管理每个物理 CPU 上的虚拟化状态，包括当前运行的 vCPU、虚拟化控制寄存器等。
...
| 改动位置 | 先验证 | 再验证 |
|----------|--------|--------|
| `virtualization/axvm`、`axvcpu`、`axdevice` | `cargo xtask axvisor build` | 准备好 Guest 后 QEMU 测试 |
