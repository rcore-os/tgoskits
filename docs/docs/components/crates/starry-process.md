...
- `src/process.rs`：`Process` 主体，实现父子关系、child subreaper、线程组、zombie 状态、reap 语义、进程组切换入口。
...
- `Process`：核心进程对象，内部持有 `pid`、`is_zombie`、`is_child_subreaper`、`ThreadGroup`、`children`、`parent`、`group`。
...
flowchart TD
    ...
    Zombie --> Reparent["子进程转交最近的 child subreaper 或 init"]
...
- `Process::exit()` 会把孤儿进程交给父链上最近的活跃 child subreaper；若不存在则交给 init 进程。
- `Process::free()` 只允许回收 zombie 进程，并从父进程的 `children` 中移除。
- `Process::exit()` 检测到 init 进程后直接返回，不会把 init 标记为 zombie。
...
- 创建子进程、维护父子层级和孤儿进程收养。
- 维护进程级 child subreaper 标志，并按 Linux `PR_SET_CHILD_SUBREAPER` 语义选择孤儿收养者。
- 维护线程组成员列表与进程退出码。
- 维护进程组和会话层级。
...
4. 修改孤儿收养逻辑时，必须同时覆盖普通 init 收养和 child subreaper 收养路径。
