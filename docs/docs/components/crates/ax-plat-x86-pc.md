...
### 集成测试
- `arceos-std-helloworld-myplat` 和系统级 smoke test：验证 Multiboot 启动、串口、IOAPIC/LAPIC 和 secondary path。
...
graph LR
    ...
    arceos_helloworld_myplat["arceos-std-helloworld-myplat"] --> current
...
## 跨项目定位
### ArceOS
`ax-plat-x86-pc` 不在 ArceOS 目录内部，但被 `arceos-std-helloworld-myplat`、`ax-hal` 等 ArceOS crate 直接依赖，说明它是该系统的共享构件或底层服务。
