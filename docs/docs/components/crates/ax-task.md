...
- 普通 preemption depth 和 `need_resched` 按架构选择所有者：x86_64 使用固定
  `CpuRuntimeAnchor`，AArch64/RISC-V/LoongArch 使用随任务发布的
  `CurrentThreadHeader`；scheduler baton 与 IRQ 状态始终属于 CPU。
