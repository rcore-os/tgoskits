...
主核 scheduler online 后、设备和应用启动前，runtime 会创建永久的
`ax-task-reaper` 服务线程。hard IRQ、timer 和 context-switch tail 只通过 sticky
doorbell 发布工作并直接唤醒这个已有线程；它们不会调用 OS extension、销毁资源或在
当前调度栈上递归执行 task work。

服务线程是 Deadline overrun callback、exit callback、detached thread reap 和 coroutine/
resource reclaim 的唯一批处理消费者，顺序固定为：

```text
Deadline callback → exit callback → thread reap → deferred resource reclaim
```

每一类每轮最多处理 64 项，饱和或竞争到 `ThreadBusy` 时保持 pending 并让出 CPU 后
重试。Deadline callback 通过持久 round-robin slot cursor 选择，低 slot 连续产生
overrun 也不能饿死高 slot 的 callback、exit 和 reap。

context-switch tail 只有在新上下文确认物理离开 previous stack、清除 `on_cpu` 后才发布
退出工作；它本身不运行 callback。资源销毁由外部生命周期 lease 控制：
`ThreadHandle` 和 `ThreadWakeHandle` 各持有 lease，runqueue、switch handoff 及 inbox
内部 `Arc<ThreadCore>` 不冒充外部所有权。migration/policy inbox 另有 inflight delivery
lease；退出与 owner delivery 由原子门串行化，晚到 Exited 消息只释放 payload 并成为
no-op，不会重新入队或重新注册 Deadline bandwidth。已经从 inbox 摘下的完整 batch
由 RAII 持有，因此任一消息报错也会释放其余 raw Arc 和 delivery lease。
...
3. timer 与 scheduler IPI handler 可用后才把 CPU 发布到 online mask；主核随后启动
   task-work 服务线程，再初始化可能创建 worker 或发布 deferred work 的设备和应用。
...
- 并发测试必须覆盖 wake-before/during-park、远端 inbox publication、policy requeue、
  migration/exit inflight lease、错误 batch payload release、IPI epoch、late waker、timer
  cancel/complete/reclaim、Deadline callback fairness 和 PI donor accounting。
