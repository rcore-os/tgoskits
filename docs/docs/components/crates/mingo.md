...
这个依赖面非常说明问题：`mingo` 不依赖 `ax-runtime`、`ax-hal`、`ax-driver`、`axplat-dyn` 等 ArceOS 主线部件，它不是内核体系中的一个可插拔模块，而是一个边缘位置很清晰的独立工具程序。
...
`mingo` 本身不关心 payload 来自哪个 crate，只要求它是一个可以从 `0x8_0000` 起跳的 AArch64 镜像。因此它与动态平台上的 ArceOS 示例程序、甚至其他自制内核的关系都是“加载器与被加载物”的关系，而不是 Rust 级别的依赖关系。

也就是说：

- `mingo` 不链接 `axplat-dyn` 或任何平台 crate。
- 但 ArceOS 在 Raspberry Pi 上构建出的镜像，可以作为 `mingo` 装载的 payload。
...
### 7.2 与 AArch64 payload 的关系

动态平台和下一级内核解决的是“下一级系统在 Raspberry Pi 4 上如何启动和运行”的平台抽象问题；`mingo` 解决的是“下一级系统镜像如何更快地送到板上并开始执行”的装载问题。两者互补，但不相互依赖：

- 动态平台属于下一级 payload 的平台支持。
- `mingo` 属于板端加载器。
