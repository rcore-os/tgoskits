...
### 集成测试
- 用 `arceos-helloworld` 和系统级 smoke test 验证 `_start -> ax_plat::call_main -> __axplat_main` 链条。
...
## 跨项目定位
### ArceOS
`ax-plat-macros` 主要通过 `arceos-affinity`、`arceos-helloworld`、`arceos-httpclient`、`arceos-httpserver`、`arceos-irq` 等（另有 26 项） 等上层 crate 被 ArceOS 间接复用，通常处于更底层的公共依赖层。
