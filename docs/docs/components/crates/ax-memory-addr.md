...
`ax-memory-addr` 是整个仓库内存子系统最底层的语义基石之一。它不做页表，不做映射策略，也不做物理页分配；它只做一件事：用类型化方式表达“地址”和“地址区间”，并提供对齐、算术和分页遍历等最基础但最容易出错的操作。`axplat`、`ax-mm`、`page-table-generic`、`axaddrspace`、虚拟化栈以及 StarryOS 的内存路径都把它当作共同语言层。
...
这条主线在 `ax-mm`、`ax-memory-set`、`page-table-generic` 和 `axaddrspace` 里都能看到。
...
- 与 `page-table-generic`、`ax-cpu` 及虚拟化页表实现集成时，`PhysAddr` 是页表项和页表页操作的基础地址类型
