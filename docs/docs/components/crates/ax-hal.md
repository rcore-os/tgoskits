...
- `CpuRuntimeAnchor`：固定在 CPU 区域前缀中的运行时状态，持有当前线程发布槽、
  CPU 体系结构状态以及 x86_64 的 per-CPU preemption 快路径字。
- `CurrentThreadHeader`：由 `cpu-local` 统一定义的当前线程发布头；调度切换通过
  CPU 区域前缀发布，不再维护第二份 per-CPU 任务指针；load/store 架构的
  preemption 状态随该 header 一起选择。
