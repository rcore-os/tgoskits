...
## 跨项目定位
### ArceOS
`ax-arm-pl031` 主要通过 `arceos-affinity`、`arceos-std-helloworld`、`arceos-std-helloworld-myplat`、`arceos-std-httpclient`、`arceos-std-httpserver`、`arceos-irq` 等（另有 26 项） 等上层 crate 被 ArceOS 间接复用，通常处于更底层的公共依赖层。
