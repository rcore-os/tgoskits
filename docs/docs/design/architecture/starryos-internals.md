...
StarryOS 的 Linux 兼容行为**明确定义在 syscall 分发与各子模块实现中**。`syscall/` 目录下按功能域组织为 11 个模块，覆盖以下主要类别：
...
| syscall | `kernel/src/syscall/*` | 按 Linux syscall 语义分发系统调用（11 个功能域，覆盖进程、文件、内存、信号、网络、IPC 等） | `task`、`mm`、`file`、`ipc`、`net` |
