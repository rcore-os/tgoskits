...
ArceOS 内核若需要一个按需填页的 16 KiB虚拟区，由 `ax-mm::AddrSpace` 创建 `MemoryArea<Backend>`，`Backend::Alloc { populate: false }` 在 VMA建立阶段只创建 empty mapping，后续 fault 才逐页申请 `Normal × VirtMem`。
...
flowchart LR
    ...
    PT --> Empty["empty PTE entries"]
    Fault["kernel/user fault"] --> Backend["ax-mm Alloc backend"]
    Backend --> Alloc["ax-alloc Normal pages"]
    Backend --> PT
```

如果调用方使用 `populate: true`，backend prepare/commit 在事务中准备并安装全部物理页；中间失败必须释放已经取得的 frame并保持 VMA/PTE 不变。`Backend::Linear` 则只映射外部 PA，unmap 不释放其 backing。

| 层 | 16 KiB alloc mapping 的责任 |
| --- | --- |
| `ax-mm` | 选择虚拟范围、backend 和 flags |
| `ax-memory-set` | VMA split与跨区间事务 |
| Stage-1 | 写 PTE、分配下级 table frame、TLB invalidation |
| `ax-alloc` | 提供实际 data frame和 table frame |

ArceOS 不经过 `starry-mm`，也不需要 Linux commit/RSS。将 RSS 或 overcommit 放进 `ax-mm` 会使嵌入式消费者承担无关策略。

### 8.2 Starry 匿名映射

Starry 的 16 KiB `MAP_PRIVATE | MAP_ANONYMOUS | PROT_READ | PROT_WRITE` 先通过 syscall ABI验证，再执行 RLIMIT_AS和 commit delta，最后由 `MemorySet<Backend>` 发布 Cow VMA。未使用 `MAP_POPULATE` 时，建立 mapping不立即消耗四个 resident frame。
...
later write fault
  ...
  -> map PTE + record RSS Anon
```

mapping 成功时 VSS 和 private commit增加 16 KiB，RSS仍可能为 0；每个首次写 fault 再使 RSS Anon增加一页。syscall 返回值和 errno由 Starry kernel处理，`starry-mm` 只返回 typed policy/fault结果。

### 8.3 Axvisor 客户机内存

Axvisor 的 16 KiB allocation-backed Guest RAM 使用 Guest physical range作为 `axaddrspace` 的地址键，再由 `NestedPageTableOps` 将 GPA映射到 Host PA。它不建立 Host进程 VMA，也不维护 Linux RSS。

```mermaid
flowchart LR
    VM["VM configuration"] --> AS["axaddrspace AddrSpace"]
    ...
    MS --> NPT["ax-page-table stage2"]
    NPT --> Vcpu["GPA visible to vCPU"]
```

lazy Guest RAM 在 nested fault时逐页申请，eager RAM在 map阶段准备。VM teardown 的硬顺序是停止 vCPU、停止或隔离设备 DMA、清除 Guest mappings、销毁 NPT，最后释放 Host frames。

| Teardown 顺序 | 原因 |
| --- | --- |
| 1. stop vCPU | 防止继续执行 Guest load/store |
| 2. quiesce device DMA | 防止 passthrough/emulated queue继续访问 Guest RAM |
| 3. unmap Guest RAM | 移除 GPA → HPA translation |
| 4. destroy NPT | 释放页表 frame |
| 5. release remaining owners | Host frame回到 allocator |

把第 5 步提前会使仍运行的 vCPU或设备访问已复用页面。`ax-page-table` 只负责第 3、4 步的翻译结构，无法替代 VM lifecycle同步。
...
这条路径跨越多层是因为每层分别拥有 device constraint、RAII token、platform cache policy 和物理页；不能为了减少调用层数把 mask/domain/cache状态塞进通用 allocator。相反，不含新不变量的转发 facade应删除。
