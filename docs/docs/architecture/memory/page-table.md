...
`memory/ax-page-table` 将架构 PTE、Host Stage-1、Guest Stage-2 和 early boot 页表放在同一 crate 内，通过独立 feature 裁剪。它只实现页表结构与地址翻译机制，物理 frame 来源由 `PageFrameProvider` 注入，VMA 和系统策略由上层持有。
...
| `entry` | 始终 | `GenericPTE`、`MappingFlags`、架构 PTE、AArch64 `MemAttrLayout` | Stage-1 和平台 PTE adapter |
...
公共类型将地址有效性、frame ownership、PTE 配置和 TLB 失效能力分开。上层只实现自己拥有的部分。
...
| 类型 | 表达内容 | 不表达的内容 |
| --- | --- | --- |
| `AccessFlags` | read/write/execute/user 等通用访问权限 | VMA pathname、COW owner |
...
上层在 OS 边界把 `PagingError` 转为 `AxError` 或领域错误。页表实现不直接返回 Linux errno，也不记录 VMA metadata。

## 3. 主机页表

Stage-1 面向 Host CPU 地址空间，按 pointer width 选择 `PageTable32` 或 `PageTable64`。架构 metadata 决定层数、物理/虚拟地址宽度、地址类型和 TLB invalidator。

### 3.1 架构元数据

`PagingMetaData` 将固定架构事实集中在类型参数中。当前公共 `PageSize` 支持 4 KiB、1 MiB、2 MiB 和 1 GiB，具体架构只会使用硬件允许的组合。

| 架构 metadata | 层数/地址形态 | TLB scope |
| --- | --- | --- |
| `A64PagingMetaData` | 4 层、48-bit VA/PA | `HardwareBroadcast` |
| `Sv39MetaData` | 3 层、39-bit VA | `Local` |
| `Sv48MetaData` | 4 层、48-bit VA | `Local` |
...
PTE 修改与 TLB shootdown 是同一个正确性协议的两部分。页表核心描述 invalidator scope，系统层负责保证所有可能运行该地址空间的 CPU 都观察到失效。

### 4.1 失效范围

`TlbInvalidator<A>` 提供 `const SCOPE`、`invalidate(Option<A>)` 和批量 `invalidate_list(&[A])`。`Some(vaddr)` 表示单地址，`None` 表示全部；scope 明确硬件操作能覆盖的 CPU 范围。默认批量实现逐地址执行，运行时 adapter 可覆盖为一次 remote IPI 批处理。

| `TlbScope` | 含义 | 当前示例 |
| --- | --- | --- |
| `Local` | 只失效当前 CPU | RISC-V `sfence.vma`、x86、LoongArch 本地操作 |
| `HardwareBroadcast` | 架构指令广播到 shareable domain | AArch64 `tlbi ...is` |
| `RemoteIpi` | invalidator 自身完成 remote IPI | 接口支持，当前 Stage-1 架构实现未使用 |

AArch64 `A64TlbInvalidator` 使用 inner-shareable TLBI，并在汇编序列中执行 DSB/ISB。其他 local-only 架构需要系统 IPI 配合。

### 4.2 SMP 启动检查

`stage1::smp_invalidation_available<M>(remote_ipi)` 判断 metadata scope 或系统 IPI 是否足够。`ax-mm::init_memory_management()` 调用 `ax_hal::paging::validate_smp_invalidation()`；多 CPU 且既无硬件 broadcast 又未启用 `ipi` 时会 assert 失败。

| 运行配置 | 是否允许多 CPU Stage-1 |
| --- | --- |
| AArch64 hardware broadcast | 允许，即使页表层不依赖软件 IPI |
| local-only 架构 + `ax-hal/ipi` | 允许，由上层 shootdown 协议覆盖 remote CPU |
| local-only 架构、无 IPI、CPU 数大于 1 | 初始化失败 |
...
非 AArch64 SMP 在 BSP 初始化 runtime page table 时，IPI callback 尚未发布可用。`ax-hal` 因此把 remote shootdown 分为两个明确阶段：

| 阶段 | 行为 | 安全依据 |
| --- | --- | --- |
| primary IPI ready 之前 | 只刷新 boot CPU 本地 TLB | secondary CPU 尚未运行 runtime address space |
...
shootdown 读取 enable 状态使用 Acquire。已 ready CPU 的 IPI 错误是不可恢复的一致性故障；尚未 ready 的 CPU 返回 `CpuOffline` 时可以跳过，因为其 ready 发布前必定执行全量本地失效。AArch64 始终使用 inner-shareable hardware broadcast，不进入该软件开关。

## 5. AArch64 内存属性

AArch64 PTE 的 `AttrIndx` 必须与 MAIR_EL1/EL2 slot 完全一致。`entry::aarch64::MemAttrLayout` 是统一常量来源，避免 boot、Host CPU 和 PTE encoder 各自定义索引。
...
`components/axcpu/src/aarch64/init.rs`、`someboot` EL1/EL2 初始化和 boot PTE adapter 都引用同一 layout。修改 slot 时必须一起验证寄存器值、Stage-1 PTE 和 boot PTE。
...
| `platforms/someboot/src/arch/aarch64/paging/pte.rs` | boot PTE index encode/decode |
| `memory/ax-page-table/src/entry/arch/aarch64.rs` | Stage-1 `A64PTE` encode/decode |

DMA cache maintenance 不能仅靠把 PTE 改为 uncached 代替。coherent/streaming ownership 和同步时序属于 `dma-api` 与平台 cache adapter。
...
`MapConfig` 提供 VA、PA、size、PTE template、`allow_huge` 和 `flush`。递归 mapper 只有在 level、剩余大小及 VA/PA 对齐都满足时才创建 block mapping。
...
| `stage2` export | 由 VM 生命周期决定 | Guest IPA/GPA → Host PA |
...
| `axcpu` | `stage1` + arch feature | CPU init、PTE/MAIR types |

ArceOS/Starry production tree 不应出现 `stage2`，Axvisor 也不应为普通 Guest mapping 链接 `boot`。

### 7.2 源码检查点

以下文件覆盖统一页表的关键不变量。架构改动应配合 entry round-trip、map/query/unmap 和 TLB scope 测试。

| 源码 | 审计重点 |
| --- | --- |
| `memory/ax-page-table/src/common.rs` | provider、error、PTE neutral config、TLB scope |
...
| `os/arceos/modules/axhal/src/paging.rs` | runtime provider 与 SMP capability check |
...
页表行为应从地址位划分、页表页来源和失效范围三个维度同时分析。只看到 `map()` 成功并不能证明 provider ownership 或 SMP TLB 一致性正确。
...
若 L4、L3 已存在而 L2 指向空 entry，映射 4 KiB 页需要为 L1 table 申请一个 frame，再写最终 leaf PTE。任何中间 frame allocation 返回 `None` 都转换为 `PagingError::NoMemory`，已经临时建立但未链接的 frame 必须释放。
...
flowchart LR
    ...
    L3 --> L2["L2[0x091]"]
    L2 --> L1["L1[0x145]"]
    L1 --> Frame["target PA + flags"]
```

如果目标使用 2 MiB mapping，leaf 停在 L2，VA、PA 和 size 都必须 2 MiB 对齐；query 返回该 block 的 base PA，再加 `PageSize::align_offset(vaddr)` 得到最终物理地址。已有 huge entry 下不能静默创建更低一级 table，否则会破坏原映射。
...
| AxVM Host provider | Host page API | 返回 Host allocator | VM/NPT owner |

因此 boot table 的 Drop 不能用于判断物理页已回收到 Buddy；相反，runtime provider 的 owned `PageTable` 若未对称释放每个子表 frame 就是泄漏。

### 8.3 映射与失效顺序

假设 cursor 连续 protect 20 个 4 KiB 页。每次修改将 VA 加入固定容量 `ArrayVec`，cursor flush 时逐地址调用 invalidator；若修改 40 页，第 33 个地址使状态升级为 `Full`，后续不再记录单地址。
...
在 RISC-V、x86_64 或 LoongArch64 等 local-only 实现中，shared kernel mapping 的 unmap 还需要上层 IPI 等待远端 CPU 完成失效。AArch64 inner-shareable TLBI 由硬件覆盖 shareable domain，但仍必须保留架构要求的 DSB/ISB 顺序。

### 8.4 客户机映射实例

假设 Guest GPA `0x4000_0000..0x4020_0000` 映射到 Host PA `0x9000_0000..0x9020_0000`。当 `allow_huge=true` 且 geometry 允许 2 MiB block 时，flexible mapper可以生成一个 block entry；任一端不对齐时必须下降到 base-page entries。

| GPA | HPA | 大小 | 选择 |
| --- | --- | ---: | --- |
| `0x4000_0000` | `0x9000_0000` | 2 MiB | 可使用 2 MiB block |
| `0x4000_1000` | `0x9000_0000` | 2 MiB | GPA 未对齐，降级或返回 alignment error |
| `0x4000_0000` | `0x9000_1000` | 2 MiB | HPA 未对齐，降级或返回 alignment error |
| `0x4000_0000` | `0x9000_0000` | 12 KiB | 使用三个 4 KiB leaf |

Stage-2 entry 只描述 Guest translation，不拥有 Guest RAM policy。allocation-backed Guest RAM 由 `axaddrspace` backend 保存并在 transaction finalize/teardown 释放；linear Guest mapping 删除 entry 时不能释放调用方传入的 Host PA。
