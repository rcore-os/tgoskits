...
地址空间事务需要同时观察 VMA Vec、PTE 和 backend owner。下面用两个 VMA 的跨区间 unmap 展示 `plan_unmap()`、`execute()` 和 metadata 发布的精确输入输出。
...
若两个 backend commit 都成功，metadata 发布后 live Vec 只剩 `[0x1000, 0x3000)` 和 `[0x9000, 0xa000)`。旧 frame 或临时 COW hold 在 Vec 发布后由两个 `finalize()` 释放。

### 9.2 中间失败回滚

延续 9.1，假设 A 的 commit 已经清除两个 PTE并返回 `CommitStateA`，B 的 commit 在修改第一个 PTE时失败。B backend 必须先恢复自己的部分修改再返回错误；`MemorySet::execute()` 随后逆序 rollback 已完整提交的 A。
...
sequenceDiagram
    ...
    participant Meta as live VMA Vec
    ...
    A-->>Set: CommitState A
    Set->>B: commit(plan B)
    B->>B: restore partial PTE change
    B-->>Set: error
...
成功结果从两个 VMA 变为两个 VMA，本例不需要扩大 live Vec；如果 protect 一个 VMA 中间范围，单个 VMA会 split 为三个，final length 增加两个。`MetadataPlan::reserve()` 在任何 backend prepare 之前计算最终长度并调用 `try_reserve()`。
...
假设初始只有 `[0x1000, 0x7000)` RW，对 `[0x3000, 0x5000)` 执行只读 protect，计划是 remove 1、insert 3，final length 从 1 变为 3。若 `try_reserve(2)` 失败，PTE、actual flags、reported flags 和原 VMA 均保持不变。

| 成功后的 VMA | actual flags | reported flags |
| --- | --- | --- |
| `[0x1000, 0x3000)` | RW | RW |
...
`map(area, unmap_overlap=true)` 和 `replace()` 会把旧 unmap operations 放在新 map operation 之前，并把新 map 标记为 `MapPrecondition::Replacing`。这允许 prepare 阶段看到尚未删除的旧 PTE，同时要求 commit 顺序先撤旧、后建新。
