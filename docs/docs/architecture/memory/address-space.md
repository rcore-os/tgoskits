...
`BackendTransaction` 按 `NestedPageTableOps::query()` 返回的实际 `PageSize` 保存旧映射。一个 128 MiB、按 2 MiB block 建立的 Linear 区域只保存 64 个 `SavedMapping`，清理和回滚时使用原 block 大小，不展开成 32,768 个 4 KiB 快照。未映射页只用于验证范围完整性，不占用 `previous` 向量。若操作边界切入一个现有 block 中部，prepare 返回 `MappingState`，不得把整个 block 静默删除；需要局部修改时必须先由页表层提供可回滚的 block 拆分能力。
