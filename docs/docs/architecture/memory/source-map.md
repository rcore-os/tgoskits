...
每类事实只由一篇文档完整维护，其他文档仅在需要解释本地行为时给出摘要和链接。该约束避免组件表、架构矩阵、锁规则和测试命令随修改产生多个不一致副本。

| 文档 | 唯一维护的内容 | 其他文档允许引用的内容 |
| --- | --- | --- |
| `overview.md` | 全局分层、资源流和设计取舍 | 一段结论或链接，不复制总体图 |
| `source-map.md` | crate/目录索引、依赖方向、修改位置 | 机制文档可列本模块的关键文件 |
| `architecture-support.md` | 架构地址、寄存器、页表项和失效能力 | 机制文档只保留影响本机制的差异 |
...
| `testing.md` | 测试矩阵、命令、性能指标和验收条件 | 设计文档只链接对应测试章节 |

新增内容应先确定唯一归属，再决定其他文档是否需要交叉引用。若一段内容同时适用于多个机制，应放入上层专题，而不是复制到每个消费者文档。
