...
| 12 GiB Linear Map prepare | plan 内存与页数无关，不保存 3,145,728 个空映射快照 |
| 大范围 Map 中间失败 | backend 删除已安装前缀，metadata 与 owner 均回到操作前状态 |
| 大范围 Unmap/Protect reserve | 容量不足在首个页表项修改前失败，原映射完整保留 |
...
| 地址空间事务 | prepare 临时字节数、保存的旧映射数、失败回滚页数、最大事务范围 |
