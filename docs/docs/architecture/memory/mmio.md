...
MMIO 的核心不变量是“寄存器窗口不是普通内存”。驱动只持有映射对象并执行易失性读写，操作系统适配层决定物理地址如何进入内核地址空间，页表层负责使用设备内存属性建立页表项。
...
初始化必须发生在第一次 `ioremap()` 或 `ioremap_raw()` 之前，并且初始化期间不能有其他 CPU 同时读取全局槽位。当前未初始化访问会触发不可恢复错误，因此启动顺序和驱动测试必须显式安装后端，不能依赖默认的空实现。
...
`boot-memory.md` 中的 `MemoryRegionFlags::DEVICE` 和 `Reserved` 区间负责阻止设备窗口进入运行时页分配器；`page-table.md` 中的架构属性布局负责把设备映射编码为正确的页表项。
...
MMIO 验证需要覆盖输入范围、页对齐、设备属性、易失性访问和对象生命周期。只验证“读到了某个数值”不足以证明映射属性和边界正确。

### 8.1 必测不变量

单元测试可使用记录型 `MmioOp` 验证映射与解除映射次数，平台测试则需要检查实际页表属性和设备寄存器行为。

| 测试 | 证明内容 |
| --- | --- |
| 零长度和地址末端溢出 | 无效范围在建立映射前返回明确错误 |
| 未初始化后端 | 启动或测试接线缺失能够被确定性发现 |
| 未对齐物理区间 | 页对齐映射覆盖完整范围并恢复页内偏移 |
| 析构一次 | 一个 `Mmio` 所有者只触发一次 `iounmap()` |
| 克隆 `MmioRaw` | 克隆不被误认为独立映射所有者 |
| 四字节末端访问 | `offset + 4` 不得超过映射长度 |
| 设备页属性 | 页表项使用设备内存属性，而不是普通缓存属性 |
| 驱动初始化失败 | 已获得的 `Mmio` 随错误路径正确析构 |

硬件验证应同时覆盖寄存器只读、写一清零和需要屏障的访问顺序。测试代码不得用普通切片写入替代易失性访问，否则可能掩盖发布构建中的编译器优化问题。

### 8.2 源码检查点

下面的文件构成从公共能力到内核页表和驱动消费的完整路径。修改任一层时，需要同步检查相邻层的所有权与错误转换。
...
MMIO 接口中的不安全构造、跨线程访问和裸指针转换都必须保留相邻的安全契约。若未来实现真实 `iounmap()`，还必须增加虚拟区间回收、重复映射引用和并发驱动销毁测试。
