...
这里两次获取 Buddy 锁是有意缩短临界区；`pool.add_slab()` 不在全局 Buddy lock 内执行。首次 class 扩容仍可能失败，所以 IRQ/RT 路径不能把“per-CPU Slab”误解为无条件、无界延迟的固定池。
...
假设返回 PA `0xffff_c000`，16 KiB 范围正好结束于 `0x1_0000_0000`，仍满足 32-bit mask；若起点为 `0xffff_d000`，末端越界，lowmem path 必须继续扫描或返回 `NoMemory`。`GlobalPage` 保存原 request 与 usage，Drop 不需要调用方重新传递 `_dma32` 一类布尔值。
...
如果调用方需要把页面交给另一个长期 owner，必须由那个 owner保存完整释放元数据或接管 `GlobalPage`；只拷贝地址后让 `GlobalPage` 提前 Drop 会形成悬空 PTE 或 DMA 地址。
