...
`scope_local!` 定义的 `RSS_ACCOUNTING` 用于把当前正在执行 fault/populate 的 owning `MemoryAccounting` 暴露给通用 backend bridge，使 backend 在不持有 AddrSpace 引用的前提下也能调用 `record_charge()`。该机制依赖 AddrSpace lock 的外部不变量，不是跨线程全局 registry。
...
`SharedPages::new()` 的关键不变量是“commit 先于物理页申请”，因此 allocation 失败时 commit token 通过 `Drop` 自动归还；具体页面分配失败时已经取得的页逆序释放，保证不会泄漏。下面是 `memory/starry-mm/src/pages.rs` 的核心逻辑。
