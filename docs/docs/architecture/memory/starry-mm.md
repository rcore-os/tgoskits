...
| Kernel 路径 | 当前职责 |
| --- | --- |
| `mm/aspace/mod.rs` | `AddrSpace` lock、`MemorySet`、page table、commit/RSS owner、fault 与 clone orchestration |
| `mm/aspace/backend/` | Linear、Cow、Shared、File backend 和 PTE transaction |
| `syscall/mm/` | mmap/munmap/mprotect/mremap/madvise/msync/mlock/brk/mincore ABI |
| `task/resources.rs` | `RLIMIT_AS`、`RLIMIT_DATA`、`RLIMIT_STACK` 默认与更新 |
| `pseudofs/proc.rs` | meminfo、statm、status 和 overcommit sysctl 展示 |
| `file/memfd.rs` / page cache | memfd lifecycle、file page source、clean-page eviction adapter |

Kernel adapter 可以把 `FileBackend` 包装为 `VmFile`，但 `starry-mm` 不得反向引用 `ax_fs_ng::vfs::FileBackend`。

## 2. AddrSpace 与 VMA backend
