...
| LoongArch64 | PGDL 用户页表与四级 walk | load/store/fetch 相关页异常 | 同一 backend transaction 和 signal 转换 |
...
| fork/transaction hold | 在一次 `FRAME_TABLE` 临界区内 checked increment |
| unmap/rollback | 在一次 `FRAME_TABLE` 临界区内 release |
| last reference | 从 frame table 删除并归还物理 frame |

`FRAME_TABLE` 只有一个短临界区，不存在每个物理页的第二级锁。任何事务临时引用都必须在 abort、rollback 或 finalize 中有且仅有一次对应释放或所有权转移；物理页在表锁释放后归还分配器。
...
Starry kernel 的 `AddressSpacePageTable` 共同持有实际 `PageTable` 和 `MemoryAccounting`。`MappingBackend::prepare/commit/rollback/finalize` 因而从同一个显式事务对象访问页表和汇总计数，不使用 scope-local 原始指针或隐藏 registry。
