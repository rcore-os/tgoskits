...
Starry 将虚拟地址上限和全局 committed-memory 分开。`RLIMIT_AS` 始终检查；当前只实现 Linux Always overcommit，不按全局 CommitLimit 拒绝请求。
...
Starry 固定报告 `/proc/sys/vm/overcommit_memory=1`。全局原子计数仍记录 `Committed_AS` 并检查算术溢出，但不会因为超过 CommitLimit 拒绝映射。`/proc/meminfo` 的 CommitLimit 使用当前总可管理内存容量。当前不实现 Linux heuristic mode 0、Strict mode 2 或 swap-backed commit expansion；没有产品配置和完整交换空间语义前不保留空 feature。
...
StarryOS 的系统调用、跨虚拟内存区域失败注入、写时复制回滚和 committed-memory accounting 用例集中在[内存管理测试与验收](./testing.md)。
