...
普通查询使用 `select_route(dst)`，会过滤未 `UP` 的接口。TX packet 生成时，`TxToken::consume()` 使用 `select_route_for_source(dst, src)`，同时匹配 smoltcp 已经选出的源地址，避免多宿主主机从错误接口发出带另一接口源地址的包。
...
| `RouterQueues::tx` | smoltcp `TxToken` 生成的 `RoutedTxPacket` 队列 |
| `RouterQueues::rx` | 所有 Ethernet RX worker 共享的有界 RX 队列 |
| `DeviceHandle.tx_queue` | 每个真实设备独立的有界 TX 队列 |
| `RouteTable` | `TxToken::consume()` 计算出接口和 next-hop 的依据 |
| loopback fast path | 回环包直接写入 `rx_buffer`，不经过设备 worker |

`Router::poll()` 负责把设备 RX 队列推进到 smoltcp RX buffer；`TxToken::consume()` 负责把 smoltcp TX packet 转成 `RoutedTxPacket`，`Router::dispatch()` 再按预先计算好的 route 分发到 loopback 或真实设备。更细的 worker、队列和 ARP 行为见[多设备实现](devices.md)。
...
flowchart LR
    ...
    Smol --> TxToken["TxToken::consume()<br/>RouteTable select_route_for_source()"]
    TxToken --> TxQ0["Router routed TX queue"]
    TxQ0 --> Dispatch["Router::dispatch()"]
    Dispatch --> Loop["loopback: direct rx_buffer injection"]
    Dispatch --> TxQ["Ethernet: per-device TX queue"]
    TxQ --> Eth["EthernetDevice::send()"]
    Eth --> Arp["ARP / next-hop MAC"]
    Arp --> Hw["rd-net TX / NIC"]
```

普通 Ethernet 发送会进入设备 TX worker，由 `EthernetDevice` 完成 ARP 和 Ethernet frame 封装。loopback 不走外部设备队列：`Router::dispatch()` 消费 `TxRoute::Direct` 且目标设备为 `InterfaceId::LOOPBACK` 时直接把 IP packet 注入 `rx_buffer`，因此本机回环连接可在同一个 poll 周期内继续推进。
