...
| Socket 缓冲区 | 动态可调 sk_buff 链 | 固定大小 `PacketBuffer` + 内联 `PacketData` + 有界 packet queue |
...
- **多设备路由**：`Router` 实现了 smoltcp 的 `phy::Device` trait，内部管理多个 `DeviceHandle` 和路由表；`TxToken::consume()` 在 TX packet 生成后解析一次 IP 包并选择出接口。
...
RX worker 从 driver buffer 复制到有界队列中的内联 `PacketData`，Router 再复制到 smoltcp `PacketBuffer`；TX 方向由 smoltcp `TxToken` 填充 `PacketData` 并进入 routed TX queue，普通 Ethernet 直接把这份 `PacketData` 移交给 per-device queue 和 driver 发送。当前队列不做每包堆分配，loopback dispatch 也直接写 `rx_buffer`，但端到端 zero-copy 仍需要 `rd-net` buffer ownership、packet 生命周期和 smoltcp token 适配改造。
