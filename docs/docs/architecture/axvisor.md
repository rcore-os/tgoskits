...
该字段即报错。解析后归一化为
`PhysicalInterruptPolicy::{Mediated, HardwareForwarded}`。该策略只作用于已经分配的
物理 IRQ source，不改变虚拟设备的软件 IRQ 语义。
...
每个 endpoint 独立选择 backing：虚拟设备和 mediated host adapter 使用 software
backing；`HardwareForwarded` 策略下，已取得 host ownership 的物理 source 使用
HW-backed LR。两类 endpoint 可以连接同一个 VM-local GIC 和 CPU Interface。
...
AArch64 binding load/save 保存完整 GIC CPU interface；软件与 HW-backed LR 可在同一个
vCPU 上共存，host 私有 IRQ 始终由 host 保留。
