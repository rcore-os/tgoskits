...
| 内核服务模块层 | `os/arceos/modules/*` | 内存分配、页表、调度 runtime 接入、驱动、文件系统、网络、图形等 OS 能力 |
...
| `ax-task` | 由 `ax-runtime/multitask` 接入 | OS 无关的 EEVDF/FIFO/RR/Deadline 调度、SMP wake、sleep、wait queue |
...
| `ax-task` | `components/ax-task` | OS 无关调度器、任务状态机、等待队列与定时器 | `ax-runtime`、`ax-sync`、`ax-net` |
