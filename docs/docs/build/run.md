...
当 managed rootfs 不存在时，`ensure_rootfs_image()` 触发完整的下载管线：
...
`inject` 模块通过宿主端的 `debugfs` 工具操作 ext2/3/4 文件系统镜像内容：
...
### Rootfs 运行时依赖同步（`runtime`）

`sync_runtime_dependencies()` 自动补齐 overlay 目录中的 ELF 动态库依赖：
...
将 rootfs 镜像路径注入 QEMU 配置，支持两种补丁模式：
...
**DNS 注入**：写入 `/etc/resolv.conf`，内容为 `nameserver 10.0.2.3`（QEMU slirp 模式的内置 DNS 服务器地址）。
...
Guest 镜像信息存储在 TOML 格式的注册表文件中。注册表支持 `includes` 机制实现层级引用：
