...
| Test starry riscv64 qemu | `self-hosted linux` | 否 | 无 | `cargo xtask starry test qemu --arch riscv64`；仅 `rcore-os` 仓库触发 |
| Test starry aarch64 qemu | `self-hosted linux` | 否 | 无 | `cargo xtask starry test qemu --arch aarch64`；仅 `rcore-os` 仓库触发 |
| Test starry loongarch64 qemu | `self-hosted linux` | 否 | 无 | `cargo xtask starry test qemu --arch loongarch64`；仅 `rcore-os` 仓库触发 |
| Test starry x86_64 qemu | `self-hosted linux` | 否 | 无 | `cargo xtask starry test qemu --arch x86_64`；仅 `rcore-os` 仓库触发 |
| Test arceos x86_64 qemu | `self-hosted linux` | 否 | 无 | `cargo xtask arceos test qemu --arch x86_64`；仅 `rcore-os` 仓库触发 |
| Test arceos riscv64 qemu | `self-hosted linux` | 否 | 无 | `cargo xtask arceos test qemu --arch riscv64`；仅 `rcore-os` 仓库触发 |
| Test arceos aarch64 qemu | `self-hosted linux` | 否 | 无 | `cargo xtask arceos test qemu --arch aarch64`；仅 `rcore-os` 仓库触发 |
| Test arceos loongarch64 qemu | `self-hosted linux` | 否 | 无 | `cargo xtask arceos test qemu --arch loongarch64`；仅 `rcore-os` 仓库触发 |
...
self-hosted runner 任务只在 `rcore-os` 仓库内运行，避免 fork 仓库因没有对应 runner 而长时间排队。迁移到 self-hosted 的 StarryOS 和 ArceOS QEMU 测试直接在原生 runner 环境中运行，不再套 Docker container。

现有 label 约定：

| Label | 用途 |
|-------|------|
| `self-hosted`, `linux` | StarryOS / ArceOS QEMU 测试 |
...
| `test-axvisor-aarch64/riscv64/loongarch64` | Test axvisor QEMU | `push` 事件 | 各架构 Axvisor 编译产物 |
...
| 镜像 | Dockerfile | 用途 |
|------|------------|------|
| `base` | `container/Dockerfile` | 常规 clippy、sync-lint、std 测试、Axvisor aarch64/riscv64 QEMU |
...
|----------|--------|----------|----------|
| Publish base container | `ubuntu-latest` | push 或手动触发到 `main` / `dev`，且基础镜像相关路径需要发布 | 构建并推送 `ghcr.io/<repo>-container:latest` |
| Publish axvisor-lvz container | `ubuntu-latest` | push 或手动触发到 `main` / `dev`，且 LVZ 镜像相关路径需要发布 | 构建并推送 `ghcr.io/<repo>-container-axvisor-lvz:latest` |
