...
`cargo xtask arceos qemu` 在构建基础上增加运行环节：将编译好的 ArceOS app 部署到 QEMU 中执行并收集输出。本节描述 ArceOS 运行的完整流程及其特有行为；通用的 QEMU 配置获取、ostool 执行机制详见 [参数与配置](../configuration)。

> **ArceOS 当前只支持 QEMU 运行**，没有 `uboot` 和 `board` 顶层子命令（这两者主要服务于 [StarryOS](../starry/runtime) 和 [Axvisor](../axvisor/runtime) 的物理板卡场景）。如需在板卡上运行 ArceOS，请通过 Axvisor 间接引导或参考具体的板卡 bring-up 流程。
