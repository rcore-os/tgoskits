...
```mermaid
flowchart TD
    A["plat_dyn = false"] --> B["从 Cargo.toml 找到平台依赖包<br/>(如 ax-plat-riscv64-qemu-virt)"]
    B --> C["定位平台包配置文件"]
    C --> D["调用配置引擎库"]
...
| `platforms/` | `axplat-loongarch64-qemu-virt/` | `ax-plat-loongarch64-qemu-virt` | Workspace/deps metadata；必要时可按目录约定回退 |
...
**平台包命名规则**：
- 新命名格式 `ax-plat-{arch}-{board}`（如 `ax-plat-riscv64-qemu-virt`），是当前推荐格式
...
```mermaid
flowchart TD
    A["resolve_platform_config_path<br/>(包名: ax-plat-riscv64-qemu-virt)"] --> B["1. 在 workspace metadata 中查找<br/>→ 找到 Cargo.toml 所在目录<br/>→ 检查同目录下的 axconfig.toml"]
    B --> C{找到?}
    C -->|是| D["返回路径"]
...
- `ax-plat-riscv64-qemu-virt` → 去掉前缀 `ax-plat-` → `riscv64-qemu-virt` → 重新拼为 `axplat-riscv64-qemu-virt`
- 最终路径：`platforms/ax-plat-riscv64-qemu-virt/axconfig.toml`
