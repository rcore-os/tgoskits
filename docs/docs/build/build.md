...
平台配置生成的关键前提是**确定使用哪个平台包**。仓库中存在两套平台目录，同一平台可能出现在其中任一处：

| 目录 | 命名示例 | 包名格式 | 定位方式 |
|------|---------|---------|---------|
| `platform/` | `platform/riscv64-qemu-virt/` | `axplat-riscv64-qemu-virt-hv` | Workspace member，通过 cargo metadata 直接定位 |
| `components/axplat_crates/platforms/` | `axplat-riscv64-qemu-virt/` | `ax-plat-riscv64-qemu-virt` | 回退路径，通过包名 → 目录名映射定位 |
...
flowchart TD
    ...
    F -->|riscv64| H["→ axplat-riscv64-qemu-virt-hv"]
...
- 旧命名格式 `axplat-{arch}-{board}`（如 `axplat-riscv64-qemu-virt-hv`），向后兼容
...
flowchart TD
    ...
    H -->|否| I["错误：无法解析平台配置"]
```
**两级 metadata 查找**：第1步 `workspace metadata` 查找的是 workspace `Cargo.toml` 的 `[workspace.members]` 中声明的包。对 `platform/` 下的旧平台包（如 `axplat-riscv64-qemu-virt-hv`），其 `Cargo.toml`（如 `platform/riscv64-qemu-virt/Cargo.toml`）旁即为 `axconfig.toml`。第2步 `deps metadata` 查找的是传递依赖中的包，覆盖平台包位于 workspace 外部或被间接依赖的场景。只有在两步都找不到时，才进入第3步的目录约定回退。
