...
启动早期需要特别处理：主核在 `ax-runtime::task::initialize_primary()` 创建
`TaskSystem`、bootstrap/idle 线程时可能已经创建动态任务栈，但此时其他 CPU 还没有
初始化 IPI 队列，也不一定已经打开本地 IRQ。
...
- `ax-runtime` 根据 current thread header 与 runtime-owned stack resource 判断 guard page
  是否命中；`ax-task` 只保存 opaque stack/context handle，不解释 OS 内存布局。
