...
在 **Unix** 主机上，`cargo xtask arceos test qemu` 运行 **rust** 用例时，默认仅对 **名称或 package 含 `backtrace`** 的用例在 QEMU 结束后：
...
其他 rust 用例仍正常跑 QEMU，不会 tee 日志或自动 symbolize。因此 backtrace 相关 E2E 通常 **一条命令** 即可看到符号化栈，无需再手动 `tee` 与第二条 `symbolize`。
