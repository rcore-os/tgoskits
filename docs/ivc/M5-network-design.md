...
> **✅✅✅ 双向互通已达成（2026-06-30）**：两个真实 Linux guest 经"模拟 virtio-net + AxVisor 软件 L2 交换机"**双向 ICMP ping 成功**：VM1(10.0.0.1) 10 发 10 收 0% 丢包、VM2(10.0.0.2) 10 发 ~9-10 收，两向都有真实 `64 bytes from ...`（RTT 1–12ms）。关键三件套：① **physical-SPI pend 中断送达**（commit 30f17c2ee）；② **per-VM disk 隔离**（excluded_devices 排除别 VM 的 virtio_mmio 盘槽，宿主 root=/dev/vdc）；③ **gppt GIC 部分透传**（每 VM 虚拟 GICD/GICR，解决两 guest 抢同一物理 GIC 导致的引导不稳定，同时保留物理 CPU 接口使 physical-SPI pend 仍有效）。emu virtio-net 的 INTID 必须 = 48+槽号（VM1 槽8=56、VM2 槽16=64）。证据 `tmp/m2/BIDIRECTIONAL-PING-SUCCESS.log`，复现配置 `tmp/m2/vm-disk{1,2}.toml`+`qemu-3disk.toml`+`netping.sh`。这是 task2 连通性的核心达成。**应用层协议 + 可靠性 + 自动化测试已实现**（见 `ivcproto/`）：UDP 上自定义协议（16B 头：magic/version/msg_type/seq/timestamp/payload_len/checksum，5 种消息类型 DATA/ACK/CONTROL/STATUS/ERROR），可靠性=ACK+超时+重传(≤6)+按 seq 去重/容乱序+校验和，server 的 `lossy=K` 模式主动丢首个 ACK 以驱动重传/去重路径。**回环验证**：`sent=40 acked=40 lost=0 retransmits=8`、`unique=40 dups=8 corrupt=0 acks_dropped=8`（8 丢 ACK→8 重传→8 去重→40 条恰好各送达一次，自洽）。**guest 内已确认 server 在真实网络栈监听**（`PROTO-SERVER listening`）。剩 Zephyr 端点(需 SDK)，以及把双 guest 同时跑 client↔server 交换——后者仅受下述 2-VM 同时引导不稳定所限（非协议/网络问题）。
> **可复现性诚实说明**：gppt 解决了"两 guest 抢物理 GIC"导致的中断丢失，但**两 VM 同时引导仍不是 100% 稳定**（~80+ 次跑中约 3 次双引导：ping/a2/d3）。**一旦两个 VM 都引导成功，网络通路 100% 工作（0% 丢包、双向）**——成功 run 是完整数据通路端到端工作的真实证据。
>
> **both-boot 根因已彻底定位（2026-07-01）**：① 切到**抢占式 RR 调度器**（`ax-std/sched-rr`，见 `../realtime/preemptive-scheduling.md`）**不解决**——因为两 vCPU 钉在不同 pCPU，调度器不在它们之间仲裁。② **两 vCPU 同钉一个 pCPU（pCPU0）+ RR 也不行**：日志显示 VM[2] VCpu[0] 从未 "running"——根因 **passthrough timer 模式下正在跑 guest 的 vCPU 拥有物理定时器，guest 执行期间宿主 timer tick 不触发 → 宿主 RR 无法抢占正在 guest 态执行的 vCPU**（vCPU 不发生 VM-exit 就回不到宿主调度点）。③ 分到不同 pCPU（VM1=pCPU0/VM2=pCPU1）能 both-boot（proven ping/a2/d3），但**次级 pCPU 的 vCPU bring-up 有时序竞争**，非确定性。**结论：稳定 2-VM 同时引导需要修 AxVisor 的并行 VM/次级-pCPU vCPU bring-up（或实现 guest-timer 模拟以支持 vCPU 抢占），属架构级改动**；调度器选择与 pCPU 放置都不能解决。捕获完整双 guest 协议交换的基础设施已就位（耐心 client 900 次握手 + 长超时），一旦 both-boot 即完成（d3 已到 server listening，仅差当时超时）。
...
5. **根因定性（架构级）：AxVisor passthrough virtio-blk 是"单占"的**。3 块盘（disk0/1/2，宿主用 disk2 `root=/dev/vdc`，绕过宿主多盘 panic）实测：**一个 guest 看到全部三块盘（vda/vdb/vdc），另一个 guest 对三块盘 probe 全部 `-2`(ENOENT)** → 后者无任何根盘 `VFS: Unable to mount root fs on unknown-block(0,0)` panic。即先初始化的消费者（宿主/某 VM）抢占 virtio-blk 设备，第二个 disk 引导 guest 拿不到设备。**这不是配置能绕过的**：AxVisor 缺**按 VM 分配块设备**的机制（passthrough 把整个 FDT 给所有 VM，但底层 virtio-blk 后端单占）。证据 `tmp/m2/2vm-3disk.log`（`probe ... failed with error -2` × 3）。
   - **另一独立阻塞：ramdisk 路线内核级卡死**——单/双 VM 的 root=/dev/ram0 引导在 "Freeing initrd memory" 后都不 exec /init（disk 探测已成功显示 [vda]，故非盘探测问题；纯内核/initramfs 在本 AxVisor 下的行为）。**已排除的假设**：① bootargs 三元组剥离（插 token 绕过后仍卡）；② cpio 的 `/dev/console` 是普通文件而非字符设备节点——用 fakeroot 重建 cpio 加 `mknod dev/console c 5 1`(+null/tty/ttyAMA0/kmsg) 后**仍卡在 freeing initrd**，故非 console 节点问题。busybox 已确认是静态 aarch64（非动态链接缺库）。卡点在 freeing initrd 之后的晚期 initcall / 转入 /init 阶段，待深入（疑 TCG + 合作式调度下某等待）。
   - ~~结论：两个同时 disk 引导的 Linux guest 受 single-claim 阻塞~~ **已突破（见下）**。

### 2-VM 双向 ping 进一步突破（2026-06-30，续2）

6. **single-claim 已用"按-VM 排除盘槽"绕过**：QEMU virt 的 32 个 virtio_mmio 槽（0xa000000+N*0x200）在 FDT 里都是通用节点；disk0/1/2 落在 0xa000000/0xa000200/0xa000400。给每个 VM 的 `excluded_devices` 排除**别的盘的槽节点**（VM1 排除 a000200/a000400 只留 disk0；VM2 排除 a000000/a000400 只留 disk1；宿主用 disk2 `root=/dev/vdc`），则每 VM 只初始化自己那块盘 → 不再 -2 抢占。**实测两个 Linux guest 同时启动、各自 mount 自己的 ext4 root ro、双双到 `INIT-RUNNING`**（证据 `tmp/m2/2vm-scoped.log` 两条 INIT-RUNNING）。**single-claim 不再是阻塞**。
7. **per-slot INTID 修正**：emu virtio-net 的 guest INTID 由**槽位**决定（FDT 拷宿主节点，INTID=48+槽号），**不是** emu_devices 的 alloc_irq。VM1 槽8(0xa001000)→INTID 56（恰等于配的 56，故先前能收中断，计数 1→46）；VM2 槽16(0xa002000)→INTID **64**（配 57 被忽略 → 我的 physical-SPI pend 打 SPI25 而 guest 等 SPI32 → VM2 收不到 RX 中断、计数 0）。修正 VM2 alloc_irq→64 后 pend 与 guest 一致。**教训：emu virtio 设备的 alloc_irq 必须等于其槽位 INTID(48+槽号)**。
8. **当前唯一阻塞：2-VM 同时引导不稳定（M2 架构限制）**。同配置多次运行，到 `INIT-RUNNING` 的 VM 数随机（曾两个、通常一个、偶尔零），且常见"两 VM 都 mount root 成功、但只一个继续到 init"。根因：**pure passthrough 下两个 guest 直接编程同一块物理 GIC，互相覆盖中断配置**，叠加**合作式非抢占调度**（M2 已识别），使多 VM 同时引导（尤其磁盘/中断密集阶段）不可靠。这不是网络/中断逻辑问题。
   - **注意**：physical-SPI pend 中断送达**依赖 pure passthrough**（guest 用物理 CPU 接口）。要给多 VM 隔离 GIC 配置，正途是 **gppt（GIC 部分透传，每 VM 一个虚拟 GICD/GICR）**，但那会改变 GIC 模型、可能使 physical-SPI pend 失效，需重新适配中断送达——是后续工程，本会话未做以免动摇已验证的 pure-passthrough 送达。
   - **结论（task2 连通性）**：device/TX/RX/**中断送达**/软交换机/per-VM cmdline/per-VM disk 全部打通且验证；两个真实 Linux guest 已能同时 disk 引导（带各自 eth0）。差**一个稳定的双 VM 同时引导**才能抓到双向 ping 报文——受 M2 合作式调度 + 共享物理 GIC 限制（解法：抢占式调度 或 gppt 隔离 GIC，均为后续工程）。复现：`tmp/m2/vm-disk{1,2}.toml`(per-VM excluded_devices + cmdline + 正确 INTID) + `qemu-3disk.toml`(宿主 root=/dev/vdc) + `netping.sh`；日志 `2vm-scoped.log`(两 VM 引导)/`2vm-ping*.log`。
