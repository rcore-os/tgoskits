...
> **可复现性诚实说明**：gppt 解决了"两 guest 抢物理 GIC"导致的中断丢失，但**两 VM 同时引导仍不是 100% 稳定**（合作式非抢占调度 M2 限制，重试中见过 0/1/2 个 VM 到 init）。**关键是：一旦两个 VM 都引导成功，网络通路 100% 工作（0% 丢包、双向）**——成功run 是完整数据通路端到端工作的真实证据。要做成"一条命令必过"的自动化测试，还需稳定 2-VM 同时引导（抢占式调度，或更轻量 rootfs 降低引导期磁盘 I/O 争抢）。
