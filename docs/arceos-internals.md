...
4. 测试主线  
   `examples/* / test-suit/* -> ax-std or arceos_api -> modules/*`
...
本节介绍 ArceOS 提供的三种对外接口及其使用方式：`ax-std`（应用开发）、`arceos_api`（系统软件）和 `ax-libc`（POSIX 兼容）。
...
进行系统软件、共享组件开发或需要绕过 `ax-std` 使用更稳定的接口时，优先考虑 `arceos_api`，适用于：
