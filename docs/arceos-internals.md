...
| 目标 | 含义 | 典型实现 |
| --- | --- | --- |
| 编译期可裁剪 | 只链接被 feature 选中的能力，避免把不需要的子系统塞进镜像 | `ax-feat`、`axruntime/Cargo.toml` |
...
ArceOS 的装配逻辑分布在应用依赖、`ax-std/ax-feat` feature 以及 `axruntime` 的 feature 依赖图中。
...
| 应用编译通过但运行时缺能力 | Cargo feature 没有透传到 `ax-std` / `ax-feat` / `axruntime` | 从应用 `Cargo.toml` 逆推 feature 链 |
