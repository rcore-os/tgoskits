...
2026 年 5 月的开发工作仍在进行中。本文统计范围为 **2026 年 5 月 1 日至 2026 年 5 月 12 日**，截至目前 TGOSKits 工作区共产生 **96 次非合并提交**、**80 个已进入 dev 的 PR**，涉及 **20 位贡献者**。本月上旬的重点集中在 StarryOS Linux 兼容性继续补齐、Axvisor x86_64 虚拟化能力推进、RK3588/OrangePi 5 Plus 板级和 USB 支持增强、axbuild/rootfs/CI 流程重构，以及部分仍在评审中的图形、memfd、netlink、timerfd、SG2002 等方向。
...
平台配置系统也继续演进。我们添加了 RISC-V VisionFive2 平台支持，改进了平台包 alias 解析、manifest 目录解析和配置加载报告结构，并移除了对 `cargo-axplat` 的耦合。由于这部分仍有后续 PR 在推进，本文将其作为阶段性进展记录。

- [PR #541](https://github.com/rcore-os/tgoskits/pull/541) — RISC-V VisionFive2 平台支持与构建系统增强（ZCShou）
- [PR #552](https://github.com/rcore-os/tgoskits/pull/552) — 平台配置处理重构与 `cargo-axplat` 移除（ZCShou，未完成）
...
截至 2026 年 5 月 12 日，本月新建的独立 issue 较少，项目讨论主要集中在 PR review 中推进。由于当前还未到月底，以下 5 月创建的 PR 仍处于 open 状态，后续可能在本月剩余时间继续合入或调整：

- [PR #555](https://github.com/rcore-os/tgoskits/pull/555) — 修复动态平台串口控制台输入（周睿，未完成）
- [PR #554](https://github.com/rcore-os/tgoskits/pull/554) — SG2002 平台支持（周睿，未完成）
- [PR #552](https://github.com/rcore-os/tgoskits/pull/552) — 平台配置处理重构（ZCShou，未完成）
- [PR #545](https://github.com/rcore-os/tgoskits/pull/545) — futex wait 与 robust-list 语义加固（LetsWalkInLine，未完成）
- [PR #543](https://github.com/rcore-os/tgoskits/pull/543) — TCP/UDP bind 检查拆分（sunhaosheng，未完成）
...
- [PR #503](https://github.com/rcore-os/tgoskits/pull/503) ~ [PR #518](https://github.com/rcore-os/tgoskits/pull/518) — timerfd、DRM、memfd、netlink、evdev、Weston、视觉回归等 StarryOS 桌面/图形相关能力（Joseph Joshua Anggita 等，未完成）
...
- [PR #555](https://github.com/rcore-os/tgoskits/pull/555) — 动态平台串口控制台输入修复（周睿，未完成）
...
procfs 和 busybox 兼容性是 5 月上旬最密集的方向之一。我们实现或修复了 `/proc/stat`、`/proc/cpuinfo`、`/proc/uptime`、`/proc/meminfo` 和 `sysinfo()`，并补齐 busybox `pidof`、`arp`、`ip addr`、`ip link`、`hwclock`、`ttysize` 等命令依赖的内核接口。网络方面，最小 netlink socket 已进入 dev，DHCP、AF_NETLINK、UDP loopback 和 TCP/UDP bind 检查仍在评审中。
...
- [PR #478](https://github.com/rcore-os/tgoskits/pull/478)、[PR #477](https://github.com/rcore-os/tgoskits/pull/477)、[PR #484](https://github.com/rcore-os/tgoskits/pull/484) — busybox `iostat`、`nice`、`arping` 支持（未完成）
- [PR #512](https://github.com/rcore-os/tgoskits/pull/512)、[PR #520](https://github.com/rcore-os/tgoskits/pull/520)、[PR #529](https://github.com/rcore-os/tgoskits/pull/529)、[PR #543](https://github.com/rcore-os/tgoskits/pull/543) — AF_NETLINK、DHCP、UDP loopback 和 TCP/UDP bind 语义（未完成）

### Debian 文件系统与系统调用兼容性

Debian 和通用 Linux 兼容性方面，我们实现了 `vfork` 父进程阻塞语义并修复 `CLONE_VM` 下 exec 的问题，补齐 `getpgrp`、部分时间系统调用、POSIX timer 和 `mremap`。同时，`brk`、`fallocate`、`mmap` fd 处理、`fadvise64`、`clock_getres` 等系统调用继续向 Linux errno 行为靠拢，`sigwaitinfo`、timerfd 和 message queue 阻塞语义仍在 open PR 中推进。
...
- [PR #503](https://github.com/rcore-os/tgoskits/pull/503)、[PR #535](https://github.com/rcore-os/tgoskits/pull/535)、[PR #488](https://github.com/rcore-os/tgoskits/pull/488) — timerfd、`sigwaitinfo` 和 message queue 阻塞语义（未完成）
...
- [PR #501](https://github.com/rcore-os/tgoskits/pull/501)、[PR #472](https://github.com/rcore-os/tgoskits/pull/472) — loop/mount 缓存语义与 advisory file locks（未完成）
...
运行时覆盖率方面，Linux 兼容 KCOV 接口仍在 PR 中尝试。SG2002 方向则新增 riscv64 sg2002 platform、T-Head xuantie-c9xx CPU 扩展支持、MBR 分区解析和 SD card capacity 获取修复，完整平台支持仍在 PR #554 中推进。

- [PR #425](https://github.com/rcore-os/tgoskits/pull/425) — StarryOS Linux 兼容 KCOV 尝试（flying-mice987，未完成）
- [PR #554](https://github.com/rcore-os/tgoskits/pull/554) — SG2002 平台支持（周睿，未完成）
- SG2002 SD card capacity 获取、MBR 分区范围构造与 `driver-cvsd` feature（elliott10）
- xuantie-c9xx CPU 扩展、PTE 扩展属性与 icache 刷新支持（elliott10）
...
x86_64 是 Axvisor 本月上旬最重要的推进方向。AMD SVM 支持已经合入，包含 x86 virtualization backend 自动检测、默认 BIOS 镜像内嵌、x86_64 ArceOS QEMU 启动支持等基础工作；同时修复了 VM entry/exit 前后 host interrupt flag 保存恢复问题。Intel VMX QEMU guest 启动支持仍在评审中。

- [PR #445](https://github.com/rcore-os/tgoskits/pull/445) — Axvisor x86_64 AMD SVM 支持（Ivans）
- x86 virtualization backend 自动检测（Josen-B）
- 默认 x86 BIOS 镜像内嵌与 `bios_path` 省略时自动加载（Josen-B）
- x86_64 ArceOS on QEMU 启动支持（Josen-B）
- x86 vCPU host interrupt flag 保存恢复修复（Josen-B）
...
启动稳定性方面，我们实现了 x86 Linux boot protocol 支持，补齐 boot_params 与 GDT 配置，并将部分 QEMU 支持从 NimbOS 迁移到 Linux。Axvisor 与 Starry 的 rootfs 处理也在同一轮重构中统一，为后续多客户机启动和测试矩阵扩展打基础；OrangePi-5-Plus Linux smoke 中的 eMMC busy timeout 仍需继续跟踪。

- x86 Linux boot protocol、boot_params 和 GDT 配置（ZCShou）
- QEMU 支持从 NimbOS 替换为 Linux（ZCShou）
...
SG2002 方向新增 riscv64 sg2002 platform、T-Head xuantie-c9xx CPU 扩展支持、MBR 分区解析和 SD card capacity 获取修复，完整平台支持仍在 PR #554 中推进。
...
3. **Axvisor x86_64 支持**：AMD SVM 已进入 dev，VMX guest boot、x86 Linux boot protocol 和 BIOS 自动加载继续完善。
...
由于本文发布时仍是 **2026 年 5 月 12 日**，本月仍有多项 open PR 会在后续继续变化。月底正式回顾时，可基于这些未完成 PR 的最终状态继续补全统计数据和章节内容。
