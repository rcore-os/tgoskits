...
- 驱动与设备：`axdriver` → `ax-driver`、`axnet` → `ax-net`、`axnet` → `ax-net`、`axinput` → `ax-input`、`axdisplay` → `ax-display`
...
- 示例应用：`arceos-helloworld` → `arceos-helloworld`、`arceos-httpserver` → `arceos-httpserver` 等
