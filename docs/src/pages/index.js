...
function HeroTerminal() {
  const sessions = useMemo(() => [
    {
      os: 'ArceOS',
      command: 'cargo xtask arceos qemu --package arceos-std-helloworld --arch aarch64',
      output: [
        'Building ArceOS package arceos-std-helloworld',
        'Launching qemu-system-aarch64 on the virt platform',
        'Booting app: arceos-std-helloworld',
        'Hello, world!',
      ],
    },
    {
...
function WorkflowSection() {
  const steps = [
    ...
    { index: '02', title: '跑通 QEMU 构建', desc: '选择目标系统，用 xtask 一条命令完成编译、镜像生成和虚拟平台运行。', to: '/docs/quickstart/overview', linkLabel: '快速开始', command: 'cargo xtask arceos qemu --package arceos-std-helloworld --target riscv64gc-unknown-none-elf' },
