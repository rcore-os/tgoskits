...
TGOSKits 当前管理着 **60+ 组件**，按照功能和应用场景分为以下几大类：
...
所有组件的配置信息统一管理在 [scripts/repo/repos.csv](scripts/repo/repos.csv) 文件中，采用 CSV 格式：
...
https://github.com/arceos-hypervisor/arm_vcpu,v0.2.2,components/arm_vcpu,Hypervisor,
```
...
**配置特点：**
- **自动分支检测**：branch 留空时，自动检测 `main` 或 `master` 分支
- **版本锁定**：支持使用标签（如 `v0.2.2`）锁定特定版本
- **去重保护**：添加组件时自动检查 URL 和 target_dir 是否已存在

### 2.3 组件管理工具

`repo.py` 是一个功能完整的 Git Subtree 管理工具，提供以下核心命令：
...
当组件仓库有新的更新时，使用 `pull` 命令将更改同步到主仓库：
...
当在主仓库中修改了组件代码后，使用 `push` 命令将更改推送回组件仓库：
...
# 4. 推送到指定分支（可选）
python3 scripts/repo/repo.py push arm_vcpu -b feature-branch

# 5. 验证：访问组件仓库查看新提交
# https://github.com/arceos-hypervisor/arm_vcpu/commits/main
```
...
Pull 工作流由 `.github/workflows/pull.yml` 文件实现从子仓库拉取更新到主仓库，支持如下两种触发方式：

- `workflow_dispatch`：管理员手动同步特定组件或所有组件

    ![workflow_dispatch](./images/workflow_dispatch_pull.png)

- `repository_dispatch`：子仓库通过 webhook 发送 `subtree-update` 事件，子仓库有更新时自动通知主仓库，子仓库需要将如下 CI 文件放到自己仓库的 `.github/workflows/` 目录中
