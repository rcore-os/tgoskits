...
|------|--------|----------|
| T2 | 参数格式是否正确 | 工作流验证失败 |
| T3 | 是否检测到组件修改 | 无修改则跳过后续步骤 |
| T4 | `SUBTREE_PUSH_TOKEN` 是否配置 | 认证失败 |
| T5 | 目标仓库是否有写权限 | Push 失败，返回 403 |
| T5 | 是否存在冲突（远程有新提交） | 需先执行 Pull 同步 |
| T6 | 组件仓库 `mirror` 分支是否存在 | 自动创建 |
