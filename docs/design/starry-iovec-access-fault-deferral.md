...
- the raw `readv` regression fails before the change and passes on x86_64 and
  RISC-V StarryOS as well as the host Linux reference afterward.
...
It pins the calling process's current MM and obtains its captured `TASK_SIZE`
from `AddrSpace::end()`. The private range helper performs only that user-limit
check using subtraction, avoiding `start + len` overflow. `IoVectorBufIo`
continues to call `vm_read_slice` or `vm_write_slice` when a file or socket
actually transfers data, so inaccessible mappings still produce `EFAULT` at
the access point.
...
After rebasing onto the per-MM virtual-address layout, replacing the removed
fixed-size constant with the general `check_access()` helper regressed the same
case on RISC-V: it again returned `EFAULT(14)` instead of `EAGAIN(11)`. Reading
the numerical limit from the calling process's MM restored the intended split
without loosening the mapped-memory validator.
...
| `cargo xtask starry test qemu --arch riscv64 -c qemu/system/syscall-test-vectored-io` (2026-09-13 revalidation) | Passed: xtask summary 1/1; all Starry QEMU tests passed. |
