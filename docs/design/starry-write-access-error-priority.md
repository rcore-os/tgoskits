...
Status: implemented and verified; original regression set and reviewer
follow-up RISC-V QEMU case passed
...
- special descriptors report their object-level write error before importing
  user buffers, including descriptors whose ordinary `write` operation is not
  supported;
...
3. `os/StarryOS/kernel/src/file/mod.rs` exposes the `FileLike` capability
   boundary. Every implementation must explicitly declare its write-access
   result; file-backed wrappers delegate to their underlying `File`, while
   special descriptors return their object-specific error.
...
`FileLike::validate_write_access()` is an internal capability preflight with no
default implementation. Requiring every `FileLike` implementation to provide
the method makes newly added descriptor types fail to compile until their write
semantics are declared. Implementations fall into three groups:

- writable anonymous objects such as `EventFd`, `Socket`, and `NetlinkSocket`
  return success;
- `File` checks `FileFlags::WRITE`, `Pipe` checks its endpoint direction, and
  wrappers such as `Memfd`, `MountTableFile`, `UsbDeviceFile`, `Card1File`, and
  `RgaFile` delegate to the wrapped `File`;
- non-writable special descriptors return the same object-level error as their
  ordinary write path, before a syscall imports user memory.

The reviewer follow-up identified special descriptors that previously inherited
an unconditional success result. Their explicit policies preserve Linux's mode
checks and each object's unsupported-operation behavior:

| Descriptor | `validate_write_access()` result | Reason |
| --- | --- | --- |
| `Timerfd` | `EINVAL` | The descriptor is writable by mode, but has no ordinary write operation. |
| `Inotify` | `EBADF` | `inotify_init1` creates a read-only descriptor. |
| `PidFd` | `EINVAL` | The descriptor is writable by mode, but has no ordinary write operation. |
| `IonBufferFile` | `EINVAL` | Direct stream writes are unsupported by this project-specific object. |
| `MqDescriptor` opened read-only | `EBADF` | The open mode rejects writing. |
| `MqDescriptor` opened write-only or read-write | `EINVAL` | POSIX message queues use `mq_send`, not ordinary `write`. |

Memfd seals remain outside the access preflight. They are operation-specific
state checks and therefore stay after user-region validation, preserving the
existing `EFAULT` before `EPERM` priority.
...
The design was selected by comparing where the error can be decided, how many
syscall paths share that decision, and whether future `FileLike`
implementations remain safe by default.
...
| Add a required internal `FileLike` capability preflight | Chosen. The object that owns the write invariant reports it once, all shared write entry points use the same boundary, and the compiler prevents a new implementation from silently accepting writes. |

## Non-goals and rollback

The change is limited to ordering checks for ordinary write entry points. It
does not broaden the supported operations or alter unrelated ABI state.
...
- This change does not add support for new `RWF_*` flags or alter Memfd seal
  semantics.
- It does not add a corresponding read-access capability or otherwise change
  the `FileLike` read path; only write-access ordering is declared exhaustively.
...
The follow-up regression adds `test-special-fd-write-precedence` to
`qemu/system/syscall-test-vectored-io`. It calls `write`, `writev`, and
`pwritev2(offset=-1)` with an invalid data address on Timerfd, Inotify, PidFd,
and read-only and read-write POSIX message-queue descriptors. The expected
result is the descriptor-level `EINVAL` or `EBADF`, never `EFAULT`.

Before the follow-up implementation on 2026-09-13, the user-run RISC-V grouped
case emitted `STARRY_GROUPED_TEST_FAILED` and exited nonzero. Inspection of the
generated rootfs confirmed that the new binary was built and installed, so the
failure is valid pre-fix regression evidence.

After the implementation, the user reran the same command and the grouped case
passed in 7.08 seconds. The xtask summary reported 1/1 cases passed and
`all starry qemu tests passed`, confirming that all special-descriptor checks
now observe the object-level error before the invalid user address.

The local follow-up gates also passed on 2026-09-13: `cargo fmt`,
`cargo xtask clippy --package starry-kernel` across all 92 configurations,
`cargo xtask test --since HEAD^` for the selected `starry-kernel` unit,
integration, and documentation tests, and `git diff --check`.
