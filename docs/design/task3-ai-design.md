...
> Current submission path (2026-08-24): real PPM images are processed by the
> pinned ncnn/YOLO11n runtime inside the StarryOS AArch64 Guest. An accepted
> detection is converted to a bounded target, sent as a T2N1 `CONTROL` to an
> RTOS Guest, applied to the virtual plant, and returned as a `STATUS` carrying
> the same request ID. Low-confidence, small-area, malformed, non-finite, or
> over-deadline results enter the safe rejection path and emit no `CONTROL`.
>
> The canonical physical-board evidence is
> `results/atk-dlrk3588-task123-integrated-ab-20260824/`. It uses five frozen
> images with pinned SHA256 values: three accepted positions and two safe
> rejections. The accepted YOLO samples complete `CONTROL`/`STATUS` 3/3, the
> rejected samples complete safe rejection 2/2, and the raw perception-center
> MAE is 0.00. The runtime is Guest-local CPU ncnn, not fixture replay, a
> temporal CNN, an NPU, or a host-side inference proxy.
>
> Current implementation entry points:
>
> - `apps/starry/starryos-task2/rust/src/main.rs`: inference/control orchestration;
> - `apps/starry/starryos-task2/rust/src/ncnn/adapter.cc`: PPM preprocessing and
>   ncnn extraction;
> - `components/task3-model/src/perception.rs`: detection validation and bounded
>   target mapping;
> - `components/task2-net-protocol/`: reliable T2N1 protocol state machine;
> - `scripts/board/run-atk-task123-integrated-ab.py`: board evidence validator;
> - `scripts/test/net-dual-guest/task3_ab_metrics.py`: A/B metric derivation.
>
> Validation boundary: the current integration worktree passes host tests,
> Python evidence tests, formatting, Clippy, and AArch64 musl+ncnn static
> linking. Archived RK3588 raw logs and derived metrics have been revalidated.
> This integration worktree has not yet produced a fresh QEMU or board run;
> archived board measurements must therefore retain their recorded source
> snapshot and must not be presented as a run from the final merge commit.

## Historical design and QEMU evidence

The remaining sections preserve the earlier temporal-CNN and fixture-replay
work as historical comparison, protocol, and fault-injection evidence. They
are not the current Task-3 submission path and must not be used to claim that
fixture replay or the temporal CNN performs the final visual inference.
