...
六十四位 ARM 对等待策略有特殊要求，但“检查之后、真正休眠之前又发生通知”这一竞态并非 ARM 独有。通用运行期保存 VM 广播代数，并消费目标 vCPU 的 unblock 请求；单目标通知不推进 VM 广播代数。ARM 只覆盖中断与 timer 的可运行判定。这样竞态保护留在所有架构可复用的机制层，架构实现只描述策略差异。
