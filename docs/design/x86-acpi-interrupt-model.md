...
Static COM1 的 `ProbePriority` 是 `DEFAULT + 1`，晚于 `DEFAULT` 优先级的 ACPI NS16550 探测。rdrive 按优先级串行完成每组 Static、FDT 和 ACPI 回调，因此 legacy 探测读取资源索引时，所有可匹配的 `PNP0500/PNP0501` 设备已经完成注册；`SystemIO 0x3f8` 已被占用时返回 `NotMatch`，不会覆盖固件设备。

没有匹配设备时，Static 回调先确认 `SystemIO 0x3f8` 尚未被占用，再以新建描述符注册运行时串口，并把该资源关联到同一个 `DeviceId`。这两步只在串行的启动期探测路径执行；描述符尚未包含任何同类型能力，资源也经过同一回调的冲突预检，因此关联不会留下可恢复的半提交状态。SPCR 只有寄存器地址而没有可匹配的 namespace 设备时，`spcr_resource_device_id()` 仍能选择该串口；显式 `console=ttyS0` 与未指定控制台时的 `ttyS0` 回退因此指向同一设备。
...
默认自动启动用例证明 ACPI 初始化和客户机启动链路。运行时控制台改动还需在实体板卡上向 Linux 客户机输入命令，验证输出结果，并验证 `Ctrl+X,h` 返回 Axvisor Shell 以及重新连接客户机。运行时串口输入只使用 IRQ4 经 ISA override 解析后的 IOAPIC 路由，不增加 Raw HAL 轮询回退。
