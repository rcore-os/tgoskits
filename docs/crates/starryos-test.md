...
`starryos-test` 的核心定位是：StarryOS 系统级测试与回归入口
...
### 4.1 运行入口
```toml
# `starryos-test` 主要作为测试/验证入口使用，通常不作为普通库依赖。
...
2. 优先通过 `cargo xtask test starry` 跑完整测试入口，单包调试再退回 `run --package`。
...
## 6. 跨项目定位分析
### 6.1 ArceOS
当前未检测到 ArceOS 工程本体对 `starryos-test` 的显式本地依赖，若参与该系统，通常经外部工具链、配置或更底层生态间接体现。

### 6.2 StarryOS
`starryos-test` 虽不一定位于 `os/StarryOS/` 内部，但从命名与职责上属于 StarryOS 兼容栈的重要组件。

### 6.3 Axvisor
当前未检测到 Axvisor 工程本体对 `starryos-test` 的显式本地依赖，若参与该系统，通常经外部工具链、配置或更底层生态间接体现。
