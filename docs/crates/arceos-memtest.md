...
`arceos-memtest` 的核心定位是：ArceOS 系统级测试与回归入口
...
- 关键调用链示例：按当前源码布局，常见入口/初始化链可概括为 `main()` -> `test_btree_map()`。

## 3. 依赖关系图谱
```mermaid
graph LR
    current["arceos-memtest"]
    current --> axstd["axstd"]
```
...
### 4.1 运行入口
```toml
# `arceos-memtest` 主要作为测试/验证入口使用，通常不作为普通库依赖。
...
## 6. 跨项目定位分析
### 6.1 ArceOS
当前未检测到 ArceOS 工程本体对 `arceos-memtest` 的显式本地依赖，若参与该系统，通常经外部工具链、配置或更底层生态间接体现。

### 6.2 StarryOS
当前未检测到 StarryOS 工程本体对 `arceos-memtest` 的显式本地依赖，若参与该系统，通常经外部工具链、配置或更底层生态间接体现。

### 6.3 Axvisor
当前未检测到 Axvisor 工程本体对 `arceos-memtest` 的显式本地依赖，若参与该系统，通常经外部工具链、配置或更底层生态间接体现。
