...
Axvisor 在 TGOSKits 里是一条和 ArceOS / StarryOS 并列的系统路径，但它的开发体验和前两者最大的不同是：除了代码，还必须把板级配置、VM 配置和 Guest 镜像一起看。本文档介绍 Axvisor 在仓库中的位置、命令入口、第一条成功路径、组件关系以及常见开发动作，帮助你快速上手 Axvisor 的开发和调试。

## 1. Axvisor 在仓库里的位置

Axvisor 的代码分散在多个目录中：`os/axvisor/` 是 Hypervisor 的主体，`components/` 下有虚拟化核心组件，`platform/` 下有板级平台实现。理解这些目录的边界，有助于在遇到问题时快速定位需要修改的位置。
...
Axvisor 的 build/qemu 不是根 `cargo xtask` 的子命令，而是 `os/axvisor` 自带 xtask 提供的。根目录通过 `cargo axvisor` 别名调用它，你也可以直接进入 `os/axvisor/` 使用本地 xtask，两者完全等价。
...
第一次上手建议从 `qemu-aarch64` 开始，因为当前仓库里的现成配置与 CI 入口都优先覆盖这条路径。需要注意的是，Axvisor 不能只靠 `defconfig/build/qemu` 三条命令直接跑起来，因为默认 QEMU 配置会引用 `tmp/rootfs.img`，而这个文件不会自动生成。

### 3.1 推荐方式：使用官方 `setup_qemu.sh`

不要直接从 `defconfig/build/qemu` 开始。官方推荐流程是：
...
Axvisor 的启动依赖四类要素的协同工作：虚拟化核心组件提供 VM/vCPU/设备抽象，ArceOS 基础能力提供平台和运行时支撑，板级配置决定编译目标和 feature，VM 配置和 Guest 镜像则决定每个 Guest 的具体参数。下面这张图展示了它们之间的依赖关系。
...
本节列出 Axvisor 开发中最常见的几类改动。无论你改的是核心组件、运行时代码、板级配置还是 VM 参数，都应该按照推荐的顺序逐步验证，避免在配置未对齐的情况下直接运行 QEMU。
...
`os/axvisor/src/*` 更偏系统整合层。这类改动常常会同时依赖：
...
Axvisor 提供了从编译验证到 QEMU 运行再到自动化测试的多层验证入口。日常开发先用 build-only 确认编译通过，再在环境就绪时运行 QEMU，最后通过根工作区测试入口做自动化回归。
...
Axvisor 的启动失败往往不是代码问题，而是配置未对齐。本节总结了最常见的排查思路和可用的调试命令，帮助你快速定位问题。
