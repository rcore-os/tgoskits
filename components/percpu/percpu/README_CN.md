...
底层调度器和离线 CPU 启动代码可在尚不能构造 `CpuPin` 时，通过隐藏的
`with_scheduler_cpu`、`with_scheduler_cpu_mut` 回调访问当前 CPU 对象。这条路径直接
选择架构拥有的 CPU 区域，不经当前执行上下文发布状态。调用方必须在整个回调期间
禁止迁移和上下文切换；可变访问还必须排除 IRQ/重入及冲突远端访问。
当前还没有运行时路径调用这些回调；它们为后续调度保护和离线 CPU 启动集成预留。
...
| pin 前的调度器对象 | 禁止迁移和上下文切换；可变访问还须排除 IRQ/重入及远端冲突 |
