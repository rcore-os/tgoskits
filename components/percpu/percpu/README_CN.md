...
`ax-percpu` 只负责模板布局、symbol offset、不可变 runtime area 注册，以及 current/remote 地址计算。所有架构寄存器指令集中在仅依赖 value-only `trait-ffi` 的叶子 crate `cpu-local`。
...
内核平台使用 `custom-base` 时还必须启用 `linked-template`。链接脚本必须在所有普通 per-CPU 输入 section 之后保留 `.percpu_end`，并单独 `KEEP(*(.ax_percpu.align))`；随后由 `cpu-local` 提供 Rust 对象拥有的精确模板边界，由 alignment descriptor 表提供动态分配对齐，不依赖 target triple 猜测或架构专用 load/store。
