...
extern "C" int task3_ncnn_infer(
    ...
    const float norm[] = {1.f / 255.f, 1.f / 255.f, 1.f / 255.f};
    ...
    *infer_us = static_cast<uint64_t>(
    ...
    // pnnx emits YOLO11n as [84, 8400].  The first four channels are cx, cy,
    // w, h in 640-pixel input coordinates; the remaining channels are scores.
    ...
    for (int row = 0; row < rows; row++) {
        for (int class_id = 4; class_id < channels; class_id++) {
            const float score = data[class_id * rows + row];
            if (score > best_score) {
                best_score = score;
                best_row = row;
    ...
    // A valid forward with no candidate is not a runtime failure. The caller
    // maps status 1 to hold-last-target under the existing safety contract.
    if (best_row < 0 || best_score <= 0.f) return 1;
    const float center_x = data[best_row] / 640.f;
    const float center_y = data[rows + best_row] / 640.f;
    const float box_w = data[2 * rows + best_row] / 640.f;
    const float box_h = data[3 * rows + best_row] / 640.f;
    ...
    detection->area_milli = milli(box_w * box_h);
    return 0;
}
