...
use crate::{
    ...
    SchedulingKey, TaskError, TaskSystemConfig, ThreadId,
...
pub struct CpuLocal {
    ...
    scheduler_ipi_pending: AtomicBool,
    ...
    pub(crate) remote_wake_inbox: SchedulerInbox,
    pub(crate) migration_inbox: SchedulerInbox,
    pub(crate) reclaim_inbox: SchedulerInbox,
...
impl CpuLocal {
    pub(crate) fn create(owner: CpuId, config: TaskSystemConfig) -> Pin<Box<Self>> {
        Box::pin(Self {
            owner,
            ...
            scheduler_ipi_pending: AtomicBool::new(false),
    ...
    pub(crate) fn mark_online(self: Pin<&mut Self>) {
    ...
    pub(crate) fn set_idle(self: Pin<&mut Self>, idle: ThreadId) {
        // SAFETY: changing fields does not move this pinned object.
        unsafe { self.get_unchecked_mut() }.idle = Some(idle);
    }
    ...
    pub(crate) fn scheduler_enter(self: Pin<&mut Self>) -> bool {
        ...
        // wake, timer, IPI, or preemption-disable paths.
    ...
    pub(crate) fn publish_remote_wake(
        ...
        let result = self.remote_wake_inbox.publish(node, message);
        let send_ipi = if result == PublishResult::Published {
            self.request_scheduler_work();
            self.scheduler_ipi_pending
    ...
    pub(crate) fn publish_policy_update(
        ...
        let result = self.migration_inbox.publish(node, message);
        let send_ipi = if result == PublishResult::Published {
            self.request_scheduler_work();
            self.scheduler_ipi_pending
                .compare_exchange(false, true, Ordering::AcqRel, Ordering::Acquire)
    ...
    pub(crate) fn balance_request_node(&self) -> Pin<&'static InboxNode> {
        ...
        unsafe {
            // CpuLocal is pinned before online publication and remains at this
            // address until shutdown; one embedded node coalesces pull requests.
    ...
    pub(crate) fn arm_deferred_scheduler_deadline(&self, deadline_ns: u64) {
        ...
        loop {
            ...
            match self.deferred_scheduler_deadline_ns.compare_exchange_weak(
                ...
                Ok(_) => {
                    self.arm_scheduler_deadline(deadline_ns);
                    return;
                }
    ...
    pub(crate) fn take_due_scheduler_deadline(&self, now_ns: u64) -> bool {
        ...
        loop {
            ...
            match self.scheduler_deadline_ns.compare_exchange_weak(
                ...
                Ok(_) => {
                    self.clear_due_deferred_deadline(now_ns);
                    return true;
                }
                Err(observed) => current = observed,
            }
    ...
    pub(crate) fn scheduler_deadline_ns(&self) -> Option<u64> {
        let deadline_ns = self.scheduler_deadline_ns.load(Ordering::Acquire);
    ...
    fn recompute_scheduler_deadline(&mut self, now_ns: u64) {
        let mut next_deadline_ns =
            nonzero_deadline(self.deferred_scheduler_deadline_ns.load(Ordering::Acquire));
                ...
                > 1
        {
            next_deadline_ns = earliest(
                next_deadline_ns,
                self.fair_balance_deadline_ns.load(Ordering::Acquire),
            );
        }
    ...
    pub const fn migration_inbox(&self) -> &SchedulerInbox {
        &self.migration_inbox
    }

    /// Returns the deferred-reclaim publication endpoint for remote CPUs.
    pub const fn reclaim_inbox(&self) -> &SchedulerInbox {
        &self.reclaim_inbox
    }
    ...
    pub fn acknowledge_scheduler_ipi(&self) {
        self.scheduler_ipi_pending.store(false, Ordering::Release);
