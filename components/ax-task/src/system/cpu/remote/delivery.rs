...
/// A remote wake publication reserved before `Blocked` becomes `Waking`.
///
/// Linux hands an `on_cpu` wake to the descheduling CPU's `wake_list` so the
/// waker can release `p->pi_lock` without spinning through switch tail. This
/// carrier preserves the same no-fail boundary: it owns both the target CPU
/// publication lease and the thread's intrusive-inbox lifetime reservation.
#[derive(Debug)]
pub(crate) struct PreparedRemoteWakeDelivery {
    publication: Option<OwnedCpuRemotePublication>,
    core: Option<Arc<ThreadCore>>,
    owner: CpuId,
    park_generation: u64,
...
impl PreparedRemoteWakeDelivery {
    pub(crate) fn prepare(
        owner_remote: &Arc<CpuRemote>,
        core: &Arc<ThreadCore>,
        owner: CpuId,
        park_generation: u64,
    ) -> Result<Self, TaskError> {
        let publication = owner_remote
            .begin_owned_publication()
    ...
    pub(crate) fn commit(mut self) {
        let publication = self
            .publication
            .take()
            .expect("prepared remote wake must retain its owner CPU lease");
        let core = self
            .core
            .take()
            .expect("prepared remote wake must retain its thread delivery lease");
        let thread = core.id();
        let pointer = Arc::into_raw(core);
        let node = unsafe {
            // SAFETY: the transferred Arc count keeps the embedded node pinned
            // until one owner drain reconstructs and releases it.
            Pin::new_unchecked((*pointer).remote_wake_node())
        };
        ...
        match publication.publish_owner_control(node, message) {
            ...
            PublishResult::AlreadyPending => unsafe {
                // SAFETY: a coalesced publication did not consume this Arc.
                // The existing carrier owns the same still-Waking lifecycle.
                let retained = Arc::from_raw(pointer);
                retained.cancel_scheduler_inbox_delivery();
                drop(retained);
            },
            PublishResult::WrongKind => unsafe {
                // SAFETY: the prepared owner lease and typed node make this an
                // internal invariant rather than a recoverable wake fallback.
                let retained = Arc::from_raw(pointer);
                retained.cancel_scheduler_inbox_delivery();
                drop(retained);
                task_runtime::fatal_invariant(0x574b_0201, thread.as_u64() as usize);
            },
        }
    }
}

impl Drop for PreparedRemoteWakeDelivery {
    fn drop(&mut self) {
        if let Some(core) = self.core.take() {
            core.cancel_scheduler_inbox_delivery();
        }
