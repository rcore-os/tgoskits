...
struct TaskSystemState {
    ...
    free_slots: Vec<u32>,
    pending_resource_releases: Vec<PendingResourceRelease>,
    task_work_class_cursor: DeferredTaskWorkClass,
    thread_release_first: bool,
...
impl FairBalanceResult {
    const fn migrated(self) -> Option<ThreadId> {
        match self {
...
const FAIR_BALANCE_BALANCED_BACKOFF_FACTOR: u64 = 2;
const FAIR_BALANCE_CONSTRAINED_BACKOFF_FACTOR: u64 = 64;
...
impl TaskSystemState {
    ...
    fn allocate_thread_slot(&mut self) -> Result<(u32, u32), TaskError> {
    ...
    fn remove_unpublished_thread_with_handle(
        &mut self,
        handle: &ThreadHandle,
...
impl PendingResourceRelease {
    fn resources_mut(&mut self) -> &mut ThreadResources {
        match self {
...
mod tests {
    ...
    fn affinity_constrained_fair_balance_keeps_backing_off() {
        ...
        let system =
            TaskSystem::new(TaskSystemConfig::new(2).with_balance_interval_ns(INTERVAL_NS))
                .unwrap();
        ...
        let system = Box::pin(
            TaskSystem::new(
                TaskSystemConfig::new(1)
                    .with_balance_interval_ns(COMPLETION_NOW_NS.saturating_mul(2)),
            )
            .unwrap(),
        );
        let mut cpu = system.create_cpu_local(CpuId::new(0)).unwrap();
