...
struct UnitTestPiMutexTaskOps;

#[ax_crate_interface::impl_interface]
impl ax_sync::PiMutexTaskOps for UnitTestPiMutexTaskOps {
    fn current_task_id() -> u64 {
        pi_test_result(crate::current_thread_id(), "capture current PI mutex task").as_u64()
    }
    ...
    fn lock_slow(lock: &ax_sync::PiMutexCore, sequence: u64) -> ax_sync::PiMutexLockResult {
        let current = pi_test_result(crate::current_thread_token(), "capture PI mutex waiter");
    ...
    fn park_current_once(token: &ax_sync::PiWaitToken) {
        pi_test_result(crate::pi_park_current_once(token), "park PI mutex waiter");
    }

    fn try_cancel(token: &ax_sync::PiWaitToken) -> ax_sync::PiWaitCancelOutcome {
        pi_test_result(crate::pi_wait_try_cancel(token), "cancel PI mutex waiter")
    }

    fn claim(token: &ax_sync::PiWaitToken) -> ax_sync::PiMutexClaimOutcome {
        let current = pi_test_result(crate::current_thread_token(), "capture PI mutex claimant");
        pi_test_result(
    ...
    fn release_owned(lock: &ax_sync::PiMutexCore, old_owner: ax_sync::PiTaskId) {
        ...
        pi_test_result(
            unsafe {
                ...
                crate::pi_mutex_release_owned(lock, old_owner.into())
            },
    ...
    fn drop_wait_handle(wait_handle: *mut ()) {
        unsafe {
            ...
            crate::pi_drop_wait_handle(wait_handle)
        };
    }
}
