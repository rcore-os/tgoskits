...
impl<T: ?Sized> SpinLock<T> {
    ...
    #[inline(always)]
    fn with_state_mut<G: GuardState>(&mut self) -> &mut BaseSpinLock<G, T> {
...
impl<T: ?Sized> SpinRwLock<T> {
    ...
    #[inline(always)]
    fn with_state_mut<G: GuardState>(&mut self) -> &mut BaseSpinRwLock<G, T> {
        // SAFETY: see `with_state`; the exclusive borrow prevents aliases.
...
mod tests {
    ...
    fn host_context_snapshot() -> (usize, bool) {
        (
            test_runtime::active_preempt_guards(),
            test_runtime::local_irqs_enabled(),
        )
    }

    fn reset_host_context() {
