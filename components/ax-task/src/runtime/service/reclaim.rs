...
use crate::{
    ...
    runtime::{
        TaskSystem,
        context::{runtime_task_system, validate_task_context},
        delivery::inbox::PublishResult,
        switch::dispatch::yield_current_cpu,
        task_runtime,
    },
    sync::irq::IrqWaitToken,
    thread::{TaskError, ThreadBuilder, current::current_thread_handle},
};
...
fn task_work_service_loop() -> Result<(), TaskError> {
    ...
    let waiter = crate::sync::irq::worker::IrqWorkerWaiter::new(wake_owner);
...
pub(crate) enum TaskWorkServiceAction {
    Yield,
    Wait,
}

pub(crate) fn task_work_service_action(
    batch: Option<crate::runtime::service::DeferredTaskWorkBatch>,
    ...
    match action {
        TaskWorkServiceAction::Yield => {
            crate::diagnostics::counters::record_task_work_worker_yield()
        }
        TaskWorkServiceAction::Wait => crate::diagnostics::counters::record_task_work_worker_wait(),
    }
    action
}

pub(crate) fn service_task_work_pass(
    system: &TaskSystem,
    doorbell: &crate::runtime::delivery::work::TaskWorkDoorbell,
    limit: usize,
) -> Result<Option<crate::runtime::service::DeferredTaskWorkBatch>, TaskError> {
    match system.service_deferred_task_work(limit) {
        Ok(batch) => {
            #[cfg(feature = "qperf-metrics")]
            crate::diagnostics::counters::record_task_work_worker_pass(batch.processed());
            Ok(Some(batch))
        }
