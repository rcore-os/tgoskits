...
pub(crate) unsafe fn complete_current_context_switch_tail(
    ...
    let system = runtime_task_system()?;
    // SAFETY: the caller retains the transferred scheduler baton.
...
unsafe fn complete_current_context_switch_tail_in_scheduler_frame(
    ...
    let system = scheduler_frame.task_system();
    // SAFETY: the frame retains the same IRQ-off baton through completion.
    unsafe { finish_switch_tail(system, scheduler_frame) }
}

/// Completes the committed owner handoff under its caller's live scheduler baton.
///
/// # Safety
/// `pin` must retain the IRQ-off baton for the CPU belonging to `system`.
