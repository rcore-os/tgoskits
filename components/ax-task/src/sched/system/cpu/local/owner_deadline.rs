...
use super::*;
use crate::sched::system::cpu::remote::SchedulerNonTimerDeadlines;
...
impl CpuLocal {
    ...
    fn record_scheduler_deadline_derivation(&self, source: SchedulerDeadlineDerivationSource) {
        ...
        crate::diagnostics::counters::record_scheduler_deadline_derivation(source);
        #[cfg(not(feature = "qperf-metrics"))]
        let _ = source;
    }

    fn update_scheduler_deadline_publication_in_base(
        task_deadlines: &mut crate::sched::system::cpu::remote::CpuDeadlineState,
    ...
    fn promote_due_task_deadlines_in_base(
        ...
        let crate::sched::system::cpu::remote::CpuDeadlineState {
            queue,
            expired_buffer,
            ..
        } = &mut *task_deadlines;
