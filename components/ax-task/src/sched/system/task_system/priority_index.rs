...
impl DeadlineCpuHeap {
    ...
    fn find_later(
        ...
        (crate::sched::algorithm::scheduler_time_cmp(
            entry.absolute_deadline_ns,
    ...
    fn insert_or_update(&mut self, cpu: CpuId, absolute_deadline_ns: u64) {
        if let Some(index) = self.indices[cpu.as_usize()] {
            ...
            if crate::sched::algorithm::scheduler_time_cmp(absolute_deadline_ns, previous)
                == core::cmp::Ordering::Greater
            {
                self.sift_up(index);
            } else if crate::sched::algorithm::scheduler_time_cmp(absolute_deadline_ns, previous)
    ...
    fn sift_up(&mut self, mut index: usize) {
        while index != 0 {
            ...
            if crate::sched::algorithm::scheduler_time_cmp(
