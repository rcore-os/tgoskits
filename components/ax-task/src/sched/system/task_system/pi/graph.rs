...
impl TaskSystem {
    ...
    pub(in crate::sched::system::task_system) fn replace_owner_lock_top_locked(
        &self,
    ...
    pub(in crate::sched::system::task_system) fn claim_ownerless_lock_waiter(
        &self,
    ...
    pub(in crate::sched::system::task_system) fn insert_lock_waiter(
        &self,
        lock_state: &mut PiMutexWaiters,
        owner_core: Option<&Arc<ThreadCore>>,
    ...
    pub(in crate::sched::system::task_system) fn remove_lock_waiter(
        &self,
        lock_state: &mut PiMutexWaiters,
        owner_core: Option<&Arc<ThreadCore>>,
        waiter_core: &Arc<ThreadCore>,
        ...
        let old_top = lock_state.waiters.first_entry();
        let ((registration, removed, _donation), _owner_next_lock) =
            publish_owner_after_waiter_detach(
                ...
                |lock_state| {
                    ...
                    let removed = lock_state
                        ...
                        .ok_or(TaskError::InvalidPiState)?;
                    waiter_sched.pi.blocked_on = None;
                    Ok((registration, removed, donation))
                },
                |lock_state, _detached| {
                    self.publish_lock_top_change(
                        owner_core,
                        old_top.clone(),
                        lock_state.waiters.first_entry(),
                    )
                },
                |lock_state, (registration, removed, donation)| {
                    let mut waiter_sched = waiter_core.sched().lock();
                    if waiter_sched.pi.blocked_on.is_some() {
        ...
        // cleared the only registration which could name it.
        unsafe { waiter_core.pi_wait_nodes().return_lock_waiter(removed) };
        waiter_core
            .pi_wait_state()
            .clear_top(registration.generation);
    ...
    pub(in crate::sched::system::task_system) fn recompute_pi_chain(
        &self,
        start: ThreadId,
    ...
    pub(in crate::sched::system::task_system) fn recompute_pi_cleanup_chain(
        &self,
        start: ThreadId,
        top_task: ThreadId,
    ) -> Result<(), TaskError> {
    ...
    pub(in crate::sched::system::task_system) fn propagate_pi_waiter_key_after_policy_change(
        &self,
        thread: ThreadId,
    ) -> Result<(), TaskError> {
