...
impl TaskSystem {
    ...
    pub(crate) fn current_membarrier_targets<'system>(
        ...
        targets: MembarrierCpuTargets<'system>,
    ) -> Result<MembarrierCpuTargets<'system>, crate::sync::membarrier::MembarrierError> {
        self.ensure_owner_cpu_online(&cpu)?;
    ...
    pub(crate) fn current_private_membarrier_target(
        &self,
        cpu: Pin<&mut CpuLocal>,
    ) -> Result<MembarrierTarget, crate::sync::membarrier::MembarrierError> {
        self.ensure_owner_cpu_online(&cpu)?;
        ...
        if !state.ready(MembarrierRegistration::PrivateExpedited) {
            return Err(crate::sync::membarrier::MembarrierError::NotRegistered);
        }
