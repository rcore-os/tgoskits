...
use super::*;
use crate::runtime::service::{SchedulerTickMode, SchedulerTickWorkDisposition};
...
impl TaskSystem {
    ...
    pub fn service_deferred_task_work(
        ...
        let outcome = (|| {
            ...
            crate::diagnostics::counters::record_task_work_classes(
    ...
    fn dispatch_scheduler_tick_work_inner(
        ...
        let mut messages = [InboxMessage::EMPTY; crate::runtime::config::DEFAULT_BATCH_LIMIT];
        let batch = self.deferred_scheduler_ticks.drain(
            limit.min(crate::runtime::config::DEFAULT_BATCH_LIMIT),
            &mut messages,
        );
        let mut callbacks = 0;
