...
use crate::{
    sched::{
        CpuSet, SchedulePolicy,
        algorithm::{ActiveSchedulingState, SchedulingEntity},
        system::{CurrentRemotePublication, SwitchEndpoint},
    },
...
pub(crate) struct RqTaskMetadata {
    ...
    pub(crate) runtime_binding: crate::runtime::switch::ThreadRuntimeBinding,
}
...
pub(crate) struct RunQueueNodeStorage {
    ...
    fair: UnsafeCell<Option<Box<crate::sched::algorithm::fair_queue::FairNode>>>,
...
impl RunQueueNodeStorage {
    ...
    pub(crate) unsafe fn take_fair(&self) -> Box<crate::sched::algorithm::fair_queue::FairNode> {
        unsafe { &mut *self.fair.get() }
            .take()
            .expect("one thread cannot own two fair runqueue links")
    }

    pub(crate) unsafe fn return_fair(
        &self,
        node: Box<crate::sched::algorithm::fair_queue::FairNode>,
    ) {
        assert!(
