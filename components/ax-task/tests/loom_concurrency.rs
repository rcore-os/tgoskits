...
#[test]
fn stale_wake_route_falls_back_to_a_reachable_carrier() {
    loom::model(|| {
        const OFFLINE: usize = 1usize << (usize::BITS - 1);
        const INACTIVE: usize = 1usize << (usize::BITS - 2);
        const DRAINING: usize = OFFLINE | INACTIVE;

        let cpu0 = Arc::new(AtomicUsize::new(0));
        let cpu1 = Arc::new(AtomicUsize::new(0));
        let delivered = Arc::new(AtomicBool::new(false));

        let wake = {
            let cpu0 = Arc::clone(&cpu0);
            let cpu1 = Arc::clone(&cpu1);
            let delivered = Arc::clone(&delivered);
            thread::spawn(move || {
                let acquire = |lifecycle: &AtomicUsize| {
                    let mut state = lifecycle.load(Ordering::Acquire);
                    loop {
                        if state & OFFLINE != 0 {
                            return false;
                ...
                // The wake sampled CPU 1 before CPU-down began. If that lease
                // is already closed, any online carrier may transport the
                // value-owned wake to an owner that revalidates placement.
                let carrier = if acquire(&cpu1) {
                    &cpu1
                } else {
                    assert!(acquire(&cpu0));
                    &cpu0
                ...
                carrier.fetch_sub(1, Ordering::Release);
            })
        };
        let offliner = {
            let cpu1 = Arc::clone(&cpu1);
            thread::spawn(move || {
                ...
                if cpu1
                    ...
                    .is_ok()
                {
                    cpu1.store(OFFLINE, Ordering::Release);
                    return;
                }
                let mut state = cpu1.load(Ordering::Acquire);
                loop {
                    assert_eq!(state & DRAINING, INACTIVE);
                    match cpu1.compare_exchange_weak(
                        state,
                        state & !INACTIVE,
                        Ordering::Release,
                        Ordering::Acquire,
        ...
        assert_eq!(cpu0.load(Ordering::Acquire), 0);
        assert!(matches!(cpu1.load(Ordering::Acquire), 0 | OFFLINE));
    });
}
