...
fn workspace_dependency_does_not_impose_an_image_register_mode() {
    ...
    assert!(
        ...
        "workspace ax-std must disable defaults so each final image selects its register mode"
    );
}
    ...
    for relative_path in ARCEOS_UNIKERNEL_DEFAULT_CONSUMERS {
        ...
        for dependency in dependencies {
            assert!(
                ...
                "{relative_path} must explicitly select ax-std/default and its UnikernelTls mode"
            );
        }
    }
}
    ...
    for relative_path in LINUX_CURRENT_IMAGES {
        ...
        for dependency in dependencies {
            assert!(
                ...
                "{relative_path} must leave ax-std/default and tls disabled for LinuxCurrent"
            );
        }
    }
}
