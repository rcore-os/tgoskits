...
fn pcid_invpcid_supported() -> bool {
    __cpuid(1).ecx & (1 << 17) != 0 && invpcid_supported()
}

fn invpcid_supported() -> bool {
    __cpuid(0).eax >= 7 && __cpuid_count(7, 0).ebx & (1 << 10) != 0
}
...
pub unsafe fn install_user_address_space(address_space: HardwareAddressSpace) {
    {
        ...
        let tagged = address_space.hardware_tag() != 0
...
pub fn flush_tlb(vaddr: Option<VirtAddr>) {
    if let Some(vaddr) = vaddr {
        // SAFETY: this CPU backend executes at CPL0.
        unsafe { tlb::flush(vaddr.into()) }
    } else if invpcid_supported() {
        // SAFETY: CPUID confirmed INVPCID. The all-contexts operation includes
        // global translations and is valid with CR4.PCIDE both clear and set.
        unsafe { flush_pcid(InvPcidCommand::All) }
    } else {
        flush_tlb_without_invpcid();
    }
}

fn flush_tlb_without_invpcid() {
    let restore_irqs = irqs_enabled();
    disable_irqs();
    // SAFETY: IRQ exclusion pins this CPL0 operation and prevents a local
    // interrupt from modifying CR4 between the two writes. Toggling PGE
    // invalidates global entries too; the exact original CR4 is restored.
    // Without PGE capability this backend never enables PCID, so CR3 reload
    // invalidates every possible translation. This follows Linux's global
    // versus local full-flush distinction in arch/x86/mm/tlb.c.
    unsafe {
        if __cpuid(1).edx & (1 << 13) != 0 {
