...
pub unsafe fn install_user_address_space(address_space: HardwareAddressSpace) {
    let tagged = address_space.hardware_tag() != 0
        && u32::from(address_space.hardware_tag()) < address_space_tag_capacity();
    if tagged {
...
pub fn read_thread_pointer() -> KernelTlsBase {
    KernelTlsBase::new(super::registers::read_tp())
}
...
pub unsafe fn write_thread_pointer(kernel_tls: KernelTlsBase) {
    // SAFETY: this TLS boundary owns the raw register transition.
    unsafe { super::registers::write_tp(kernel_tls.as_usize()) }
}
