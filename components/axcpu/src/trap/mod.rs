...
pub use fault::PageFaultFlags;
pub use interrupted::{InterruptedContext, InterruptedPrivilege};
...
/// IRQ trap hook type.
pub type IrqHandler = fn(usize, TrapOrigin, Option<InterruptedContext>) -> bool;
...
fn default_irq_handler(
    irq: usize,
    _origin: TrapOrigin,
    _context: Option<InterruptedContext>,
) -> bool {
...
pub fn irq_handler(irq: usize) -> bool {
    dispatch_irq(irq, TrapOrigin::Kernel, None)
}
...
pub use crate::arch::current::unaligned::{
