...
use crate::paging::{
    GenericPTE, MappingFlags, PageFrameProvider, PageSize, PagingError, PagingMetaData,
    PagingResult, TlbFlusher, TlbInvalidator,
};
...
pub struct PageTable32<M: PagingMetaData, PTE: GenericPTE, H: PageFrameProvider + Default> {
    root_paddr: PhysAddr,
...
impl<M: PagingMetaData, PTE: GenericPTE, H: PageFrameProvider + Default> PageTable32<M, PTE, H> {
    ...
    pub fn try_new() -> PagingResult<Self> {
        let (root_paddr, size_pages) = {
            #[cfg(target_arch = "arm")]
            {
                ...
                let root_paddr = H::default()
                    .alloc_frames(L1_SIZE_PAGES, L1_ALIGN)
            ...
            #[cfg(not(target_arch = "arm"))]
            {
                ...
                let root_paddr = H::default().alloc_frame().ok_or(PagingError::NoMemory)?;
                (root_paddr, SIZE_PAGES)
            }
        };

        // Zero out the root page table
        let virt = H::default().phys_to_virt(root_paddr);
        unsafe {
    ...
    fn get_entry_mut_or_create(
        ...
        if entry.is_unused() {
            ...
            let paddr = H::default().alloc_frame().ok_or(PagingError::NoMemory)?;

            // Zero out the L2 page table
            let virt = H::default().phys_to_virt(paddr);
            unsafe {
    ...
    fn get_table<'a>(&self, paddr: PhysAddr) -> &'a [PTE] {
        let ptr = H::default().phys_to_virt(paddr).as_ptr() as *const PTE;
        unsafe { core::slice::from_raw_parts(ptr, ENTRY_COUNT) }
    }

    fn get_table_mut<'a>(&self, paddr: PhysAddr) -> &'a mut [PTE] {
        let ptr = H::default().phys_to_virt(paddr).as_mut_ptr() as *mut PTE;
...
impl<M: PagingMetaData, PTE: GenericPTE, H: PageFrameProvider + Default> Drop
    for PageTable32<M, PTE, H>
{
    fn drop(&mut self) {
        ...
        for (i, entry) in table.iter().enumerate() {
            ...
            if !entry.is_unused() && !entry.is_huge() {
                ...
                H::default().dealloc_frame(entry.paddr());
            }
        }
        // Deallocate L1 page table (16KB = 4 pages)
        H::default().dealloc_frames(self.root_paddr, 4);
    }
}

/// A cursor created by [`PageTable32::cursor`] to modify the page table.
pub struct PageTable32Cursor<'a, M: PagingMetaData, PTE: GenericPTE, H: PageFrameProvider + Default>
{
    inner: &'a mut PageTable32<M, PTE, H>,
    flusher: TlbFlusher<M>,
}

impl<M: PagingMetaData, PTE: GenericPTE, H: PageFrameProvider + Default> Deref
...
impl<'a, M: PagingMetaData, PTE: GenericPTE, H: PageFrameProvider + Default>
    PageTable32Cursor<'a, M, PTE, H>
{
    ...
    pub fn map_region(
        ...
        while size > 0 {
            ...
            if let Err(error) = self.map(vaddr, paddr, page_size, flags) {
                ...
                let mapped_size = total_size - size;
                if mapped_size != 0 {
                    let _ = self.unmap_region(start, mapped_size);
                }
                return Err(error);
            }
    ...
    pub fn flush(&mut self) {
        ...
        match &self.flusher {
            ...
            TlbFlusher::Array(addrs) => {
                M::Tlb::invalidate_list(addrs);
            }
            TlbFlusher::Full => {
                M::Tlb::invalidate(None);
            }
        }
