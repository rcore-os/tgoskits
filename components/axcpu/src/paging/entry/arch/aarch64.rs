...
use crate::paging::entry::{GenericPTE, MappingFlags};
...
/// Single source of truth for AArch64 MAIR slots and PTE `AttrIndx` values.
pub struct MemAttrLayout;

impl MemAttrLayout {
    /// Device-nGnRE MAIR slot.
    pub const DEVICE_INDEX: u64 = MemAttr::Device as u64;
    /// Normal write-back cacheable MAIR slot.
    pub const NORMAL_INDEX: u64 = MemAttr::Normal as u64;
    /// Normal non-cacheable MAIR slot.
    pub const NORMAL_NON_CACHEABLE_INDEX: u64 = MemAttr::NormalNonCacheable as u64;

    /// MAIR_EL1 value matching the indices emitted by [`A64PTE`].
    ///
    /// Slot 0 is Device-nGnRE (`0x04`), slot 1 is Normal write-back with
    /// read/write allocation (`0xff`), and slot 2 is Normal non-cacheable
    /// (`0x44`). Keeping the architectural encodings here makes this layout
    /// usable by boot code, page-table code, and host-side tooling without a
    /// register-access dependency.
