...
AArch64 宿主替换中，把不可变固件计划中的每个 GICR 区域和步长，与传给运行时的 `ArmVgicConfig` 比较。不得通过向下转换已注册 GIC 前端推断配置。宿主 GIC 内存映射区域保持陷入，客户机写入不能改变宿主 GICD 或 GICR。`resolve_machine_resources_from_host()` 只为直通地址空间客户机采用宿主固件的 GIC 地址；`virtualized` 客户机从最终客户机设备树选择虚拟 GIC 地址。生成的 QEMU 虚拟平台设备树使用 `0x0800_0000`，板卡原生 Zephyr 提供的设备树使用 `0xfe60_0000`。遇到客户机 GIC 寄存器访问错误时，应同时核对镜像编译目标、客户机设备树和虚拟 GIC 注册地址。
