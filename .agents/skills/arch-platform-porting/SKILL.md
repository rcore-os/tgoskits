...
- **RISC-V AIA 开发板调通**：设备树同时使用 `riscv,aplic` 和 `riscv,imsics` 时，必须把 APLIC 发布为独立外部中断域，不得伪装成平台级中断控制器来源。IMSIC 文件是监管者外部中断的认领路径，APLIC 保持为扁平设备树 `interrupt-parent` provider，负责翻译 `<source, trigger>` 描述符，并在完成电平触发 MSI 时重新采样。现有平台级中断控制器延迟完成 API 保持独立，供已有 passthrough 用户使用。
