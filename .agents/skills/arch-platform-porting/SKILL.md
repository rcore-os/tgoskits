...
- Axvisor 文件后端虚拟盘启动慢而宿主文件 I/O 耗时很短：检查 MMIO 队列通知与 WFI 的顺序。一个 vCPU 运行片段内可以先提交队列通知再退出到 WFI；主 vCPU 必须在等待事件前处理该片段留下的设备请求，否则存储 worker 尚未收到请求，客户机只能等无关定时器唤醒后才推进。先分段测量排队、存储、完成回收及 WFI 等待，再判断是否需要改变缓存或 I/O 粒度。
