...
- x86 固定 COM1 回退由与 ACPI 或 FDT 并列启用的 rdrive `Static` 来源探测，只能在 someboot 报告当前早期控制台确实是 `Ns16550Port` 且端口为 `0x3f8` 时发布。运行时驱动读取该只读资源描述中的端口与输入时钟，不通过 IIR 等带设备语义的寄存器重新猜测硬件是否存在；ISA IRQ4 仍需经过 MADT Interrupt Source Override 与已发布的 IOAPIC 域解析后才能接管。
...
- **x86 ACPI 中断模型交接**：BSP 在启动早期屏蔽一次 8259，次处理器不得重复写全局 PIC。动态 x86 平台从 MADT 的 `PCAT_COMPAT`、IOAPIC 与 Interrupt Source Override 选择 `X86InterruptModel`，IOAPIC 优先；没有 IOAPIC 时进入明确的 `todo!()`，表示 legacy 8259 PIC 回退尚未实现。AML 表加载完成后、执行设备 `_STA`、`_INI` 和解析 PCI `_PRT` 前，rdrive 对 IOAPIC 选择执行可选根方法 `\_PIC(1)`；方法不存在可以继续，其他执行错误必须阻止发布 ACPI `System`。显式禁用 AML 的诊断模式仍按 MADT 注册 IOAPIC，但必须记录未执行 `_PIC` 交接。System 只保存待提交选择，somehal 成功注册匹配的 IOAPIC domain 后才能一次性发布最终模型。COM1 IRQ4 保持普通 `AcpiGsi(4)` 路由，不增加板卡 feature、环境变量或 8259 控制台域。AML interpreter 的 `TermArg` 对 `LocalN/ArgN` 取当前值，`SuperName/Target` 才保留可写引用；`DerefOf` 必须物化 BufferField，IndexField 必须按字节选择器串行读写。方法调用栈属于单次求值，失败不得把父帧泄漏到后续方法。SystemMemory operation region 先由平台确认是否已在 direct map，只对未覆盖的动态 MMIO 页执行并缓存 `ioremap`。平台尚未实现 SCI Global Lock 唤醒时，固件锁竞争返回结构化超时并隔离当前设备方法，禁止永久自旋或假装取得锁。与 `docs/design/x86-acpi-interrupt-model.md` 保持一致。
