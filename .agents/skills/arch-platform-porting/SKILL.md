...
- **Axvisor 可配置 virtio-blk 契约**：通用 virtio-blk 模型由 AxVM 显式注册并拥有，backing file 的创建、格式化、容量和回写属于独立 image-policy 边界。配置实例只申请自动分配的内存映射输入输出和有线中断资源；体系结构资源池解析它们，同一份 `ResolvedDeviceGraph` 同时驱动运行时和类型化扁平设备树或高级配置与电源接口贡献。不得恢复体系结构局部 virtio-blk 地址、中断、固件节点或文件镜像策略。
