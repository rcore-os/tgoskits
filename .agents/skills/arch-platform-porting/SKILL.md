...
- **裸 RTOS 客户机与 IVC 映像**：沿所选 `image_location` 和加载器追踪 `kernel_path`，在转换或启动裸映像前验证 ELF 入口及每个非空 `PT_LOAD` 都位于虚拟机 GPA 内存范围。`MapAlloc(0)` 保持声明的客户机 GPA，`MapIdentical(1)` 可以重定位客户机可见基址，两者不能互换。向 `cargo xtask axvisor qemu` 显式传入控制端根文件系统，因为 axbuild 可能用托管默认值替换 QEMU 文件中的磁盘路径。把设备资源编译进映像的裸客户机必须与已解析设备图中的 MMIO 基址和大小、虚拟控制器 INTID、MAC、头格式及段一致；启用 MMU 时通过客户机的 ioremap 或设备接口映射设备 GPA，不直接解引用物理值，DMA 队列地址另需明确的客户机物理地址转换契约。原生 QEMU 启动只能作为基线证据，不能代替 Axvisor 客户机或 IVC 证据。多路复用控制台只移除精确的行首 `[VM n] ` 标记后再解析身份与终止标记。用 `debugfs` 注入 ext4 映像时，在写入前后运行 `e2fsck`，并确认 `stat` 输出包含 inode；每次修复后保留原始失败并使用新的不可变证据目录。
...
- **AArch64 客户机扁平设备树发布**：选择客户机树后，在共享 AxVM FDT 层把 `kernel.cmdline` 作为每虚拟机 `/chosen/bootargs` 覆盖，同时保留宿主启动参数作为后备，Axvisor 应用不得复制该策略。实体 RK3588 上的客户机树只描述客户机可见的模拟 GICv3、体系结构定时器、PL011 和 virtio-mmio GPA，不得把整棵宿主设备树伪装成直通设备。机器所有的控制器、定时器和串口拓扑安装后，才从 `DeviceModel::firmware()` 与 `ResolvedDeviceResources` 发布常规节点；机器补丁不得按模型名预建图所有节点。Linux 和 Starry 模板不得包含同兼容字符串的节点，重复节点应失败。Zephyr 等把资源编译进映像的客户机可以保留与确定性图槽一致的构建叠加层，但运行时树仍由图生成并由契约测试核对。不得恢复旧 `emu_devices`、裸地址或 IRQ 字段、`interrupt_mode` 或物理 IRQ 直通捷径。每个 GICR 帧为 128 KiB，互不重叠，只有最后一个客户机重分发器设置 `GICR_TYPER.Last`。PSCI `CPU_SUSPEND` 尚未支持时移除每处理器 `cpu-idle-states` 引用和 `/cpus/idle-states`。Linux 使用的模拟 PL011 必须接受字节、半字和字访问；实体多处理器控制台记录与 Axvisor 日志共享宿主输出锁。宿主 FDT 处理器解析仍更新客户机物理处理器标识，仅在 `phys_cpu_sets` 缺失时合成单元素调度掩码，已验证的显式亲和掩码始终优先。默认中断控制器缺少固件 phandle 时，先分配 phandle 再发布 `interrupt-parent`。
...
- **AArch64 直通客户机中断所有权**：为异常级 2 宿主初始化的 GIC 处理器接口使用分离 EOI 与停用，而直通客户机可能只写 `EOIR` 完成物理中断；首次运行前把固定处理器接口交给客户机的合并 EOI 模式。客户机进入和退出钩子保持宿主 IRQ 屏蔽并精确保留 DAIF；模拟 SPI 经每虚拟处理器预分配所有权门控，先取得全局 GIC 锁再取得门控，只路由非活动线路，并在恢复宿主 IRQ 前回收待处理状态。再次进入时保留活动线路路由。设备生产者不得直接置位面向客户机的物理 SPI，也不得在客户机拥有该接口时发送宿主 IPI。
...
9. 协作式调度器下的客户机对称多处理应在有效亲和掩码允许时，把新建虚拟处理器任务分配到不同的初始宿主运行队列。客户机硬件处理器标识与稠密宿主调度器标识保持分离，保留每个任务的完整亲和性供后续迁移；激活远端任务前发布虚拟机运行时记录。激活时重新验证运行时处理器边界和当前亲和性，不匹配时返回明确错误，并回滚已发布的虚拟处理器及运行时生命周期状态。
10. 客户机 `CPU_ON` 退出发生时，调用虚拟处理器仍登记为当前宿主处理器上的活动上下文。只能在目标的 `Starting` 保留状态下配置其已保存且未活动的后端状态；目标发布的宿主任务首次在选定处理器运行前，不得把目标安装为当前或绑定。PSCI 要求异步操作：激活前发布目标任务及其虚拟机生命周期参与，激活被接受即返回成功，并让 `Starting` 作为 `ON_PENDING` 保持可观察，直到目标任务绑定。不得在调用虚拟处理器不可抢占的运行边界等待目标发布；目标侧取消或绑定失败必须同时回滚虚拟处理器状态和生命周期参与。
