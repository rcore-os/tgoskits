...
- **AArch64 短复制内联**：裸机目标通过 `BareBuildTarget.rustflags` 统一提供 `--max-store-memcpy=16` 的 LLVM 代价参数，构建与 Clippy 共用，仍保留严格对齐、soft-float 与禁用 NEON 的目标契约。涉及复制性能或调整该参数时，读取[短复制内联设计](../../../docs/design/aarch64-inline-copy.md)，核对实际机器码和原生结果；不得以关闭严格对齐、开启内核 SIMD 或 QEMU 指令计数代替原生性能证明。
