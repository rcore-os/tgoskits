...
- **Axvisor 客户机 CPU 编号与 vCPU 超额分配**：显式 `phys_cpu_sets` 时每个条目是对应 vCPU 的最终宿主物理核亲和掩码（bit N = 逻辑宿主 pCPU N），`phys_cpu_ids` 只承载客户机可见的 vCPU 标识（`VMPIDR_EL2 = 1<<31 | id`），因此客户机 vCPU 数可以超过宿主物理核数，也不再要求该标识出现在宿主扁平设备树中；缺省 `phys_cpu_sets` 时 `phys_cpu_ids` 同时是宿主 CPU 节点索引与客户机标识，必须存在于宿主 FDT，并由 `resolve_phys_cpu_sets` 反查运行时逻辑序号生成单核掩码。客户机 FDT 由 `create_guest_fdt` 经 `need_cpu_node` 过滤 `/cpus/cpu@X`，只保留 `phys_cpu_ids` 中的节点，缺号由 `FdtTree::ensure_guest_cpu_nodes` 从宿主首个 CPU 节点克隆 `cpu@<id>` 并写入 `reg`，模板带 `mpidr-affinity` 时同步覆写该属性；克隆节点不得继承模板的 `phandle` 或 `linux,phandle`，必须分配同时高于宿主全部 phandle 的新值，否则保留的 `/cpus/cpu-map` 引用会因句柄重复而指向错误节点，或占用客户机已丢弃却仍被引用的宿主句柄。`FdtTree::prune_stale_cpu_map_entries` 随后删除 `cpu` 引用已不在客户机树中的条目，以及只为承载这些条目而存在的 `core`、`cluster`；客户机 SMP 引导按 `/cpus/cpu@<id>` 的 `reg` 与 `enable-method` 经 PSCI 启动次核，不读取 `cpu-map`。虚拟中断控制器亲和性与 PSCI `CPU_ON`、`AFFINITY_INFO` 匹配消费同一个客户机 MPIDR，两者都必须与客户机 FDT 的 CPU 节点一致。掩码宽度按宿主 CPU 位宽校验：宿主 CPU 数等于位宽时 `usize::MAX` 是合法全宽掩码，只有超过位宽才拒绝。掩码为多比特时 vCPU 可以在所选物理核之间迁移，该路径没有现成用例覆盖，启用前必须有目标 QEMU 或板卡证据。超额分配、客户机 CPU 节点数量与掩码宽度边界都必须在目标 QEMU 或板卡上用真实客户机引导（PSCI 次核上线）验证，不能只以宿主侧配置解析通过为证据。
