...
- 没有匹配的运行时串口时，底层硬件抽象层控制台继续独占，并通过同一任务控制台能力输出。只有底层控制台具备中断时才能提供任务输入：公共层在硬中断中把接收数据排入有界队列并唤醒 `WaitQueue`。x86 运行客户机时可能丢失一次边沿触发的物理 UART 唤醒；Raw HAL 输入在保持 IRQ 主路径的同时使用有界定时等待，并在超时后轮询一次硬件。中断与轮询必须通过同一个保存中断状态的生产端锁串行化，不能并发读取 UART。没有控制台中断时，`take_input` 仍返回不支持；禁止通过 `yield_now` 忙轮询伪造可休眠能力，也禁止用亲和性保留一个处理器。底层任务写入先取得可休眠任务锁，再取得中断安全硬件锁；早期次处理器和硬中断日志没有有效当前任务，只能非阻塞尝试硬件锁。一旦选中运行时进入 `Preparing`，任何失败都进入封闭状态；成功提交后，对称多处理代码不得退回早期串口。
- 固件选中的硬件 `DeviceId` 必须精确匹配。只有 `NotSpecified` 可以退回通用 Linux `ttyS0` 编号。`Preparing` 前找不到匹配项时保留底层所有者，不得猜测其他运行时串口；移交已经开始后才采用失败封闭。
- 控制台专用中断传输覆盖必须同时匹配固件选中的 `DeviceId` 与完整固件 route。不得只凭 GSI、陷阱向量或控制器类型把通用 ACPI/PCI route 改写为控制台中断域；未匹配的 route 保持其固件声明的控制器、触发方式和极性。
...
- 对称多处理后出现交错失败时，先确认 `runtime console active` 早于次处理器启动消息，并确认后续日志进入所有者工作任务或 Axvisor 多路复用器；不得放宽冒烟测试失败匹配规则，也不得保留专用轮询处理器绕过问题。Raw HAL 的有界定时恢复属于同一控制台任务，不得扩展成无等待忙轮询。
...
- **x86 ACPI 中断模型交接**：BSP 在启动早期屏蔽一次 8259，次处理器不得重复写全局 PIC。动态 x86 平台从 MADT 的 `PCAT_COMPAT`、IOAPIC 与 Interrupt Source Override 选择 `X86InterruptModel`，IOAPIC 优先；没有 IOAPIC 时进入明确的 `todo!()`，表示 legacy 8259 PIC 回退尚未实现。AML 表加载完成后、执行设备 `_STA`、`_INI` 和解析 PCI `_PRT` 前，rdrive 对 IOAPIC 选择执行可选根方法 `\_PIC(1)`；方法不存在可以继续，其他执行错误必须阻止发布 ACPI `System`。显式禁用 AML 的诊断模式仍按 MADT 注册 IOAPIC，但必须记录未执行 `_PIC` 交接。System 只保存待提交选择，somehal 成功注册匹配的 IOAPIC domain 后才能一次性发布最终模型。COM1 IRQ4 保持普通 `AcpiGsi(4)` 路由，不增加板卡 feature、环境变量或 8259 控制台域。AML interpreter 的调用栈属于单次求值，失败不得把父帧泄漏到后续方法。平台尚未实现 SCI Global Lock 唤醒时，固件锁竞争返回结构化超时并隔离当前设备方法，禁止永久自旋或假装取得锁。与 `docs/design/x86-acpi-interrupt-model.md` 保持一致。
