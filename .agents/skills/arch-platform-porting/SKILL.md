...
- **目标与工具链**：检查 `scripts/targets` 目标规格、目标三元组、内核恐慌策略、重定位与代码模型、二进制接口、软浮点、musl 或标准库支持、链接器、目标文件复制工具和 `rust-src`。LoongArch 裸机构建必须通过共享 `BareBuildTarget` 解析 `scripts/targets/bare/loongarch64-unknown-none-softfloat.json`；该目标保持 `lp64s` soft-float，并在 target specification 的 `features` 中声明 `-ual`。不得恢复会产生 unstable target feature 警报的命令行 `-Ctarget-feature=-ual`。逻辑目标名和 `AX_TARGET` 仍使用 `loongarch64-unknown-none-softfloat`，改动后同时运行 ArceOS `unaligned-fixup` 与 Starry `c-regression-test-loongarch-unaligned-cross-page` 回归，确认未对齐异常修复、跨页访问和 `SIGBUS` 语义。
