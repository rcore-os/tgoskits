...
测试按行为规则组织，不按配置、平台、软件包或测例清单逐项组织。新增一个使用既有规则的实例，通常不需要新增工具层测试；只有引入新的分支、边界、协议或独立失败模式时，才补对应的行为验证。将逐项用例改成遍历真实清单的参数表，并不能消除对清单的重复维护。
...
判断实例是否只是样例，可以尝试替换其名称、路径及无关配置：保持同一规则后，测试仍应成立。若必须绑定某个实例，先说明绑定保护的独立契约。真实部署配置的兼容性或装配验证应放在相应集成边界；特定应用、脚本或测例自身的功能回归由其所属测试套件负责，共享工具测试负责通用执行机制与失败传播。
...
| 构造对象后回读字段、默认值，或只验证类型可用 | 删除独立用例，由实际使用行为覆盖 | 构造包含校验、资源取得或对外兼容责任，并能观察其失败后果 |
| 逐项验证中间表示、准备步骤或阶段状态 | 合并到有最终判定的功能用例；没有独立风险时删除 | 中间状态本身可被消费者观察，或构成提交、回滚、发布等契约 |
| 枚举参数组合、常量或展示文本，但没有项目规则 | 删除机械枚举，保留实际规则的代表性边界 | 参数包含项目负责的验证或转换；文本、布局是外部消费者依赖的协议 |
| 每增加一个配置、平台、软件包或测例，就增加固定该实例内容的测试 | 删除实例清单复刻，按第 3.4 节验证通用规则 | 实例具有独立兼容契约或历史失败模式，且验证位于实际责任边界 |
