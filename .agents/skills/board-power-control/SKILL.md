...
```bash
python -m pip install -r .agents/skills/board-power-control/requirements.txt
```
...
```bash
python .agents/skills/board-power-control/scripts/board_power.py status
```

Power on is safe to retry and does not require confirmation:

```bash
python .agents/skills/board-power-control/scripts/board_power.py on
```

Power off and cold-cycle are disruptive and require `--yes`:

```bash
python .agents/skills/board-power-control/scripts/board_power.py off --yes
python .agents/skills/board-power-control/scripts/board_power.py cycle --yes
```
...
```bash
python .agents/skills/board-power-control/scripts/board_power.py status --json
```
