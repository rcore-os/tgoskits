...
use axvmconfig::{EmulatedDeviceType, GuestConfig, GuestType, PassThroughDeviceConfig};
...
pub(crate) fn build_axvm_config(cfg: &GuestConfig) -> AxVMConfig {
    ...
    let mut emulated_devices = machine.emulated_devices;
    emulated_devices.extend(
        cfg.devices
            .legacy_emulated
            .iter()
            .filter(|device| is_guest_configurable_emulated_device(device.emu_type))
            .cloned(),
    );
    ...
    AxVMConfig::new(AxVMConfigParams {
        ...
        emu_devices: emulated_devices,
        pass_through_devices: passthrough_devices,
        excluded_devices: cfg.devices.disabled_device_paths(),
        pass_through_addresses: cfg.devices.legacy_passthrough_addresses.clone(),
        ...
        interrupt_mode: cfg
            .devices
            .legacy_interrupt_mode
            .unwrap_or_else(|| cfg.base.guest_type.interrupt_mode()),
...
fn is_guest_configurable_emulated_device(device_type: EmulatedDeviceType) -> bool {
    matches!(
        device_type,
