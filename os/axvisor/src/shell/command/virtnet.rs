...
use axvm::virtio_net_control as virtio_net;
...
fn virtnet_drop(cmd: &ParsedCommand) {
    ...
    match state.as_str() {
        "on" => {
            virtio_net::set_blackout(true);
            println!("virtnet: blackout ON (all switch traffic dropped)");
        }
        "off" => {
            virtio_net::set_blackout(false);
...
fn virtnet_capture_on(cmd: &ParsedCommand) {
    ...
    match state.as_str() {
        "on" => {
            virtio_net::capture_set_enabled(true);
            println!("virtnet: capture ON");
        }
        "off" => {
            virtio_net::capture_set_enabled(false);
            println!(
                "virtnet: capture OFF ({} frames buffered)",
                virtio_net::capture_frame_count()
            );
        }
        #[cfg(feature = "fs")]
        "dump" => {
            if let Some(path) = args.get(1) {
                match virtio_net::dump_capture(path) {
                    Ok((vm1, vm2)) => {
                        println!(
                ...
                let (vm1, vm2) = virtio_net::visit_capture_lines(|line| println!("{line}"));
                println!("CAPDUMP_END");
...
fn virtnet_show(_cmd: &ParsedCommand) {
    println!(
        ...
        if virtio_net::blackout_is_active() {
            "ON"
        } else {
            "off"
        },
        if virtio_net::capture_is_enabled() {
            "ON"
        } else {
            "off"
        },
    ...
    for (vm_id, mac, active) in virtio_net::switch_ports() {
        println!(
