...
function detail(status: string, entry = 0, park = 0): VmDetail {
  return {
    id: 1,
    name: 'guest',
    status: status as VmDetail['status'],
    cpu_num: 1,
    memory_mb: 16,
    guest_entry_count: entry,
    guest_park_count: park,
  }
}
...
describe('countersOf', () => {
  it('missing fields are treated as 0', () => {
    expect(countersOf(detail('ready'))).toEqual(BEFORE)
  })
})
...
describe('display degradation', () => {
  ...
  it('an empty error body yields readable text from the status code', () => {
    ...
    expect(describeError(new ApiError(404, ''))).toContain('未注册')
    expect(describeError(new ApiError(503, ''))).toBe('HTTP 503 · 宿主资源不足（内存或浏览器终端通道已用尽）')
  })
})
