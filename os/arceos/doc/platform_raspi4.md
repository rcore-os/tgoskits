...
> Note: the in-tree static AArch64 Raspberry Pi platform crate has been
> removed. The commands below describe the board bring-up flow, but a current
> checkout needs an external custom platform package or an explicit platform
> config for Raspberry Pi 4 support.
...
Then run with `ARCH=aarch64` and an external Raspberry Pi 4 platform selection,
and use the command `make chainboot` to transmit the generated image to your
raspi4.
