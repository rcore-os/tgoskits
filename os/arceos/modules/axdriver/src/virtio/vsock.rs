...
fn probe_mmio(info: StaticInfo, plat_dev: PlatformDevice) -> Result<(), OnProbeError> {
    ...
    for (base, size) in ax_config::devices::VIRTIO_MMIO_RANGES {
        ...
        if ty == DeviceType::Socket {
            return register_vsock(plat_dev, transport);
        }
    }
...
fn probe_pci(
    ...
    register_vsock(plat_dev, transport)
}

fn register_vsock<T: Transport + 'static>(
    plat_dev: PlatformDevice,
    transport: T,
) -> Result<(), OnProbeError> {
    let driver = ax_driver_virtio::VirtIoSocketDev::<VirtIoHalImpl, T>::try_new(transport)
        .map_err(|err| {
            OnProbeError::other(format!("failed to initialize virtio-socket: {err:?}"))
        })?;
    crate::vsock::register_legacy_vsock(plat_dev, driver);
    log::info!("registered static virtio socket device");
    Ok(())
}
