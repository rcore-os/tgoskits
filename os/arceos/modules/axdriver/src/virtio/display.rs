...
fn register_display<T: Transport + 'static>(
    ...
    let driver = ax_driver_virtio::VirtIoGpuDev::<VirtIoHalImpl, T>::try_new(transport)
        .map_err(|err| OnProbeError::other(format!("failed to initialize virtio-gpu: {err:?}")))?;
    crate::display::register_legacy_display(plat_dev, driver);
    log::info!("registered static virtio GPU device");
    Ok(())
}
