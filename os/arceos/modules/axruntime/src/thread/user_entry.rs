...
/// require_send::<ax_runtime::thread::UserExecutionContext>();
/// ```
...
impl UserExecutionContext {
    ...
    pub fn bind(registers: UserContext) -> Result<Self, TaskError> {
        if crate::guard::validate_schedule_context(
            ax_task::runtime::switch::RuntimeScheduleOrigin::Preempt,
        ) != RuntimeStatus::Success
        {
            return Err(TaskError::UnsafeContext);
        }

        let irq = crate::task::sync::IrqSaveGuard::new();
        let selected_address_space = ax_task::runtime::resource::current_address_space_handle()?;
        let binding = unsafe {
