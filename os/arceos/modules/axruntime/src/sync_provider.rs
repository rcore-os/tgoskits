...
static CONTEXT_OPERATIONS: ax_task::runtime::sync::ContextOperations =
    ax_task::runtime::sync::ContextOperations {
...
fn into_sync_context_state(
    state: ax_task::runtime::sync::ContextState,
...
) -> ax_task::runtime::sync::ContextState {
    ax_task::runtime::sync::ContextState::new(state.preempt(), state.irq())
}
...
impl ax_sync::interface::ContextOps for RuntimeContextOps {
    fn enter(context: u8) -> ax_sync::interface::ContextState {
        into_sync_context_state(ax_task::runtime::sync::context_enter(
            context,
            &CONTEXT_OPERATIONS,
        ))
    }
    ...
    fn exit(context: u8, state: ax_sync::interface::ContextState) {
        ax_task::runtime::sync::context_exit(
            context,
    ...
    fn irq_return_preempt_enter() -> usize {
        ax_task::runtime::sync::irq_return_preempt_enter(&CONTEXT_OPERATIONS)
    }

    fn irq_return_preempt_exit(state: usize) {
        // SAFETY: ax-sync returns only the token created by the paired enter.
        unsafe {
            ax_task::runtime::sync::irq_return_preempt_exit(state, &CONTEXT_OPERATIONS);
        }
    }

    fn hardirq_enter() {
        ax_task::runtime::sync::hardirq_enter(&CONTEXT_OPERATIONS);
    }

    fn hardirq_exit() {
        ax_task::runtime::sync::hardirq_exit(&CONTEXT_OPERATIONS);
    }
}
...
impl ax_sync::interface::SpinOps for RuntimeSpinOps {
    fn acquire(
        ...
        let context_state = ax_task::runtime::sync::spin_acquire(
            ax_task::runtime::sync::SpinAcquireRequest {
                locked,
                class: ax_task::runtime::sync::LockClass {
    ...
    fn try_acquire(
        ...
        let (acquired, context_state) = ax_task::runtime::sync::spin_try_acquire(
            ax_task::runtime::sync::SpinAcquireRequest {
                locked,
                class: ax_task::runtime::sync::LockClass {
                    class_id: metadata.class_id(),
    ...
    fn release(
        ...
        ax_task::runtime::sync::spin_release(
            locked,
            lock_addr,
            context,
    ...
    fn force_release(locked: &AtomicBool, lock_addr: usize, context: u8) {
        ax_task::runtime::sync::spin_force_release(locked, lock_addr, context);
    }

    fn is_locked(locked: &AtomicBool) -> bool {
        ax_task::runtime::sync::spin_is_locked(locked)
    }
}
...
impl ax_sync::interface::RwLockOps for RuntimeRwLockOps {
    fn acquire(
        ...
        let context_state = ax_task::runtime::sync::rwlock_acquire(
            ax_task::runtime::sync::RwLockAcquireRequest {
                state,
                class: ax_task::runtime::sync::LockClass {
                    class_id: metadata.class_id(),
                    class_key: metadata.class_key(),
    ...
    fn try_acquire(
        ...
        let (acquired, context_state) = ax_task::runtime::sync::rwlock_try_acquire(
            ax_task::runtime::sync::RwLockAcquireRequest {
                state,
                class: ax_task::runtime::sync::LockClass {
                    class_id: metadata.class_id(),
    ...
    fn release(
        ...
        ax_task::runtime::sync::rwlock_release(
            state,
            lock_addr,
            context,
            into_task_context_state(context_state),
...
impl ax_sync::interface::MutexOps for RuntimeMutexOps {
    fn acquire(
        ...
        ax_task::runtime::sync::mutex_acquire(ax_task::runtime::sync::MutexAcquireRequest {
    ...
    fn try_acquire(
        ...
        ax_task::runtime::sync::mutex_try_acquire(ax_task::runtime::sync::MutexAcquireRequest {
            storage: into_task_pi_storage(storage),
            next_waiter_sequence,
            class: ax_task::runtime::sync::LockClass {
                class_id: metadata.class_id(),
...
impl ax_sync::interface::LockdepOps for RuntimeLockdepOps {
    fn set_trace_enabled(enabled: bool) {
        ax_task::runtime::sync::set_lockdep_trace_enabled(enabled);
    }

    fn dump_trace() {
        ax_task::runtime::sync::dump_lockdep_trace();
    }
}
