...
/// Compatibility wrapper for callers being migrated to [`legacy`].
#[doc(hidden)]
pub fn run_on_cpu<T: Into<legacy::Callback>>(cpu: usize, callback: T) {
    legacy::run_on_cpu(cpu, callback)
        .unwrap_or_else(|error| panic!("failed to deliver legacy IPI to CPU {cpu}: {error:?}"));
}

/// Compatibility wrapper for callers being migrated to [`legacy`].
#[doc(hidden)]
pub fn run_on_each_cpu<T: Into<legacy::MulticastCallback>>(callback: T) {
    legacy::run_on_each_cpu(callback)
        .unwrap_or_else(|error| panic!("failed to deliver legacy broadcast IPI: {error:?}"));
}

/// Compatibility bridge for the generic IRQ framework.
///
/// # Safety
///
/// The caller must uphold the contract of [`call_on_cpu`].
#[doc(hidden)]
pub unsafe fn run_on_cpu_sync_raw(
    ...
    unsafe { call_on_cpu(CpuId(cpu), operation, arg) }
}

/// Compatibility combined handler used until ax-runtime owns source ordering.
#[doc(hidden)]
pub fn ipi_handler() {
    claim_current_delivery();
    drain_hard_calls()
        .unwrap_or_else(|error| panic!("failed to continue hard-call draining: {error:?}"));
    legacy::drain_current_callbacks();
}
