//! Lock-local state and task capability boundary for PI mutexes.
...
/// Inline storage whose concrete waiter representation belongs to the OS.
///
/// `ax-sync` owns only initialization and lifetime publication. Providers must
/// validate their concrete type's size and alignment before installing it.
...
impl PiMutexWaitStorage {
    ...
    /// through [`PiMutexTaskOps::drop_wait_handle`].
    #[doc(hidden)]
...
/// object behind that handle and must keep its per-lock waiter tree alive until
/// [`PiMutexTaskOps::drop_wait_handle`] receives it during lock destruction.
...
impl Drop for PiMutexCore {
    fn drop(&mut self) {
        if let Some(wait_handle) = self.wait_storage.take_initialized() {
            ax_crate_interface::call_interface!(PiMutexTaskOps::drop_wait_handle, wait_handle);
        }
    }
}
...
impl PiWaitToken {
    ...
    pub fn is_granted(&self) -> bool {
        ax_crate_interface::call_interface!(PiMutexTaskOps::waiter_is_granted, self)
    }
    ...
    pub fn is_top_waiter(&self) -> bool {
        ax_crate_interface::call_interface!(PiMutexTaskOps::waiter_is_top, self)
    }

    /// Returns whether the owner observed at registration still occupies a CPU.
    pub fn initial_owner_is_on_cpu(&self) -> bool {
        ax_crate_interface::call_interface!(PiMutexTaskOps::initial_owner_is_on_cpu, self)
    }
}
...
/// Task and scheduler operations required by the PI mutex slow path.
///
/// `ax-sync` owns the physical state machine. The task crate implements this
/// capability with its `blocked_on`, donor-tree, park, wake, and PI-chain state.
#[ax_crate_interface::def_interface]
pub trait PiMutexTaskOps {
    /// Returns the current non-zero generation-bearing task identity.
    fn current_task_id() -> u64;

    /// Rejects hard-IRQ or other contexts that cannot publish a waiter.
    fn validate_blocking_context();

    /// Registers the current task in one physical mutex waiter tree.
    fn lock_slow(lock: &PiMutexCore, sequence: u64) -> PiMutexLockResult;

    /// Returns whether the token's task-local handoff generation is granted.
    fn waiter_is_granted(token: &PiWaitToken) -> bool;

    /// Returns whether the token names the live first waiter.
    ...
    /// Returns the pinned current CPU's sticky reschedule state.
    fn current_needs_reschedule_pinned() -> bool;

    /// Performs one scheduler park transaction for this waiter.
    fn park_current_once(token: &PiWaitToken);

    /// Cancels the waiter unless ownerless handoff already selected it.
    fn try_cancel(token: &PiWaitToken) -> PiWaitCancelOutcome;

    /// Claims an ownerless handoff if the waiter remains first.
    fn claim(token: &PiWaitToken) -> PiMutexClaimOutcome;

    /// Deboosts the old owner, publishes handoff, and wakes the selected task.
    fn release_owned(lock: &PiMutexCore, old_owner: PiTaskId);

    /// Verifies and releases one scheduler-owned per-lock waiter handle.
    ///
    /// # Safety
    ///
    /// `wait_handle` must be the unique handle installed in a destroyed
    /// [`PiMutexCore`] and must no longer be observable by any waiter.
