fn main() {
    ...
    fn gen_pthread_mutex(out_file: &std::path::Path) -> std::io::Result<()> {
        ...
        writeln!(output, "// Generated by ax-libc/build.rs - DO NOT edit!")?;
        writeln!(
            output,
            r#"
typedef struct {{
...
"#
    ...
    let axlibc_include = std::path::Path::new("include");
    let out_dir = std::path::PathBuf::from(std::env::var_os("OUT_DIR").unwrap());
    let out_file = out_dir.join("libctypes_gen.rs");
    if axlibc_include.exists() {
        ...
        panic!("ax-libc headers are required to generate C ABI bindings");
    }
}
