...
**根因**: 早期 RISC-V QEMU 启动路径依赖静态平台 axconfig 中的固定内存大小，`phys_ram_ranges()` 会按该常量计算可用内存，忽略 FDT 中的实际物理 RAM。

**修复**: RISC-V QEMU 默认构建改走 dynamic platform，`axplat-dyn/src/mem.rs` 的 `phys_ram_ranges()` 从 `somehal::mem::memory_map()` 动态读取 Free 区域。
...
| 平台 | 检测方式 | 状态 |
|------|---------|------|
| x86_64 dynamic platform | UEFI / dynamic platform discovery | 已支持 |
| RISC-V QEMU dynamic platform | `somehal::mem::memory_map()` | 已支持 |
| AArch64 dynamic platform | `somehal::mem::memory_map()` | 已支持 |
| axplat-dyn (all arches) | `somehal::mem::memory_map()` | 已支持 |

dynamic platform 的 `phys_ram_ranges()` 在启动时从 somehal memory map 动态发现物理 RAM。这使得在不同 RAM 大小的 QEMU 上运行自编译时不需重新编译内核。
