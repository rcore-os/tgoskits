...
## PR797 深入分析：为什么仅设置中断标志不足以唤醒任务？

> 本节的提交表记录历史修复过程；当前实现已经由
> `components/ax-task`、`ax-runtime`/`ax-std` facade 和 Starry 的线程适配层取代旧调度器 API。

调度器只管理通用 `ThreadHandle`。Starry 用户任务身份由
`StarryUserTaskExtension` 和检查后的 `UserTaskRef` 提供；bootstrap、idle 与
`spawn_kernel_thread*` 创建的内核服务线程不安装该扩展。任务身份与 trap 来源正交：
用户任务进入 syscall 后仍保持用户身份，但当前异常是否来自用户态必须由 typed trap frame
或 `UserContext` 判断，不能从任务类型推断。
...
cargo (父进程)
  ...
  ├─ waitpid(子进程) ───→ block_on_user(&task, interruptible_for(&task, wait_future))
  │                         └─ WaitQueue park → 状态=Parking/Blocked，脱离运行队列
  └─ 信号到达(SIGCHLD等) → UserTaskRef::interrupt
                            ├─ 发布 interrupted 标志
                            └─ ThreadWakeHandle::wake → 目标 CPU inbox/Ready
```

### 阻塞点：Future Pending 后任务会进入 WaitQueue

当前 `block_on_user` 位于 `os/StarryOS/kernel/src/task/future.rs`。通用内核 worker
使用不查询 Starry 身份的 `block_on`；只有已证明的用户路径显式传入 `UserTaskRef`：

1. 在调用它的 Starry 线程中轮询 future；future 不会被迁移到通用线程池。
2. 自定义线程 waker 持有该线程的 `ThreadWakeHandle`；future 返回 `Poll::Pending` 后，线程在 `WaitQueue` 上 park。
3. `ax-task` 使用通知位与 `Parking/Parked` 状态握手，在真正阻塞前重查通知，覆盖 wake-before-park 和 wake-during-park。

任务进入 `Blocked` 后不会仅因某个普通原子标志变为 true 而自动回到运行队列，因此信号路径仍必须执行一次调度器可见的 wake。

### 为何只有 flag 不够：标志与直接 wake header 的闭环

`UserTaskRef::interrupt()` 位于 `os/StarryOS/kernel/src/task/scheduler_task.rs`，做两件事：
```rust
...
`ThreadWakeHandle` 直接指向独立于完整线程对象生命周期的 wake header，携带 generation-bearing `ThreadId`，不需要从 hard IRQ 或远端 CPU 查询全局 ID 表。wake 会合并重复通知，并把目标线程发布到所属 CPU 的 inbox；晚到 wake 遇到已退出状态或 generation 不匹配时为 no-op。

**历史问题的关键就是只有第一步。** 被 park 的任务不再执行，自然也没有机会消费 `interrupted` 标志。当前实现把信号入队、Release 发布标志与直接 wake 组成一个完整协议。
...
|-------------|---------|-----------|---------|
| `waitpid` | 子进程未退出 | `block_on_user(&task, interruptible_for(&task, wait_future))` | SIGCHLD |
| `futex` | 互斥锁/条件变量 | `block_on_user(&task, interruptible_for(&task, futex_future))` | 任意信号 |
| `read` (pipe) | 管道空，writer 关闭 | `block_on_user(&task, poll_io_for(&task, ...))` | SIGPIPE |
| `sigtimedwait` | 无 pending signal | `block_on_user(&task, interruptible_for(&task, signal_future))` | 目标信号 |

历史实现中，当子进程也因同一问题而挂起时，父进程在 `waitpid` 中永远等不到子进程退出，而子进程又可能等待另一个信号，最终形成死锁循环。

### I/O 与信号使用同一线程唤醒协议

I/O future（如 block read）在任务上下文注册同一个线程 waker。`poll_io` 严格执行“非阻塞操作 → 注册 waker → 再执行一次操作”，关闭状态检查与注册之间的丢 wake 窗口。设备 hard IRQ 不直接扫描 wait queue 或调用任意 waker，而是用固定 `IrqNotify` 唤醒预先存在的 service thread，再由普通任务上下文 fan-out。

`waitpid`、futex 和 `sigtimedwait` 没有设备完成事件可依赖；它们由另一个线程或信号路径调用 `UserTaskRef::interrupt()`。无论 wake 来自 I/O service thread 还是信号发送者，最终都通过 generation-checked `ThreadWakeHandle` 唤醒同一 owner 线程。

### 修复演进历史

| 提交 | 变更 |
|------|------|
| `04686fbd5` | 旧实现新增 `ax_task::wake_task()`，在信号传递后显式 unblock |
...
| 后续合并 | 将 wake 集成进任务 interrupt，移除独立 `wake_task` 函数 |
| 当前重构 | `UserTaskRef::interrupt()` 使用 `ThreadWakeHandle`；`WaitQueue` 与线程状态机负责无丢失 park/wake |

### 与 `interrupted` 标志的配合

`interruptible` future 包装器轮询两个条件：
1. `curr.poll_interrupt(cx)` — 以 AcqRel 原子操作检查并消费标志
2. `f.as_mut().poll(cx)` — 原始 future

当信号触发 `UserTaskRef::interrupt()`：
- wake 将目标线程发布为 Ready → 调度器选中 → `block_on` 循环重新轮询
...
| 文件 | 角色 |
|------|------|
| `os/StarryOS/kernel/src/task/future.rs` | 线程本地 `block_on`、`interruptible`、I/O 注册协议与 `IrqNotify` |
| `os/StarryOS/kernel/src/task/scheduler_task.rs` | Starry 用户扩展校验、`UserTaskRef::interrupt()` 与直接 wake handle |
| `components/ax-task/src/wait_queue.rs` | `WaitQueue` 的 park/notify generation 握手 |
| `components/ax-task/src/thread/` | generation-bearing `ThreadId`、状态机与 wake header |
| `os/arceos/modules/axruntime/src/task.rs` | runtime-owned `TaskSystem`/`CpuLocal` 和 ArceOS 线程 facade |
