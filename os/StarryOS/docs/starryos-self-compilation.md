...
cargo (父进程)
  ...
  └─ 信号到达(SIGCHLD等) → StarryTaskRef::interrupt
                            ├─ 发布 interrupted 标志
...
当前 `block_on` 位于 `os/StarryOS/kernel/src/task/future.rs`：
...
`StarryTaskRef::interrupt()` 位于 `os/StarryOS/kernel/src/task/scheduler_task.rs`，做两件事：
```rust
...
`waitpid`、futex 和 `sigtimedwait` 没有设备完成事件可依赖；它们由另一个线程或信号路径调用 `StarryTaskRef::interrupt()`。无论 wake 来自 I/O service thread 还是信号发送者，最终都通过 generation-checked `ThreadWakeHandle` 唤醒同一 owner 线程。
...
| 当前重构 | `StarryTaskRef::interrupt()` 使用 `ThreadWakeHandle`；`WaitQueue` 与线程状态机负责无丢失 park/wake |
...
| `os/StarryOS/kernel/src/task/scheduler_task.rs` | Starry extension 校验、`StarryTaskRef::interrupt()` 与直接 wake handle |
