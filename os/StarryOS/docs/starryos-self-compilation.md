...
| riscv64-qemu-virt (static) | FDT `/memory` 节点解析 | 已实现 (this session) |
| aarch64-qemu-virt (static) | FDT `/memory` 节点解析 | 已实现 (this session) |
| axplat-dyn (all arches) | `somehal::mem::memory_map()` | 已支持 |

静态平台的 `phys_ram_ranges()` 在启动时从 FDT 或 Multiboot 动态发现物理 RAM。如果 FDT 解析失败，回退到 `axconfig.toml` 中硬编码的值。这使得在不同 RAM 大小的 QEMU 上运行自编译时不需重新编译内核。
...
- **现象**: SMP=4 + KVM 时，内部脚本在 `cat > ext_linker.ld`（ext4 写入）后冻结；SMP=1 时正常完成
...
- **证据**: 
  - SMP=1 简单 `cat >` 测试 → ✅ 通过
  - SMP=4 + KVM 完整脚本 → ❌ 冻结于 `cat > ext_linker.ld`
