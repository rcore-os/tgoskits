...
fn disabled_trace_and_worker_transitions_cannot_wake_the_deferred_worker() {
    ...
    const RECLAIM_WORKER: u64 = 9;
    const WORKERS: [u64; 3] = [SCHED_WORKER, PIPE_WORKER, RECLAIM_WORKER];
    ...
    assert!(!should_defer_sched_switch(true, WORKERS, RECLAIM_WORKER, 2));
    assert!(!should_defer_sched_switch(true, WORKERS, 1, RECLAIM_WORKER));
    assert!(should_defer_sched_switch(true, WORKERS, 1, 2));
    assert!(should_defer_sched_switch(true, [0, 0, 0], 1, 2));
}
