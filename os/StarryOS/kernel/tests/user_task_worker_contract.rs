...
fn published_user_thread_cannot_be_reported_as_a_recoverable_spawn_failure() {
    let stage = function_body(SCHEDULER_TASK, "pub fn stage(");
    let activate = function_body(SCHEDULER_TASK, "pub fn activate(");
    let finish = function_body(SCHEDULER_TASK, "fn finish_published_user_thread(");

    assert!(stage.contains("self.scheduler.stage()?"));
    assert!(activate.contains("finish_published_user_thread(self.scheduler.activate())"));
    assert!(!activate.contains("try_from_scheduler"));
    assert!(finish.contains("Ok(None) => panic!"));
    assert!(finish.contains("Err(error) => panic!"));
}
...
#[test]
fn console_tty_binding_precedes_runtime_start_and_user_activation() {
    ...
    let tty = bind
        ...
        .expect("the controlling TTY must bind to the init process");
    let start = bind
        .find("entry.backend.ensure_started()?")
        .expect("the selected UART runtime must start after TTY binding");
    assert!(tty < start);
    ...
    let stage = init
        .find("prepared_task.stage()")
        .expect("the init task must complete fallible scheduler staging");
    let publish = init
        .find("let published_identity = reservation")
        .expect("the init PID identity must publish before activation");
    let arm = init
        .find("tty::arm_console_irq()")
        .expect("console IRQ must arm after task publication");
    let activate = init
        .find("staged_task.activate()")
        .expect("the init task must activate last");
    assert!(bind < stdio && stdio < stage && stage < publish && publish < arm && arm < activate);
}
...
fn irq_identity_tracks_live_tid_and_uses_a_bounded_comm_snapshot() {
    ...
    let identity = SCHEDULER_TASK
        ...
        .0;
    assert!(tid.contains("thread_identity()"));
    assert!(tid.contains("root_number()"));
    assert!(!tid.contains("thread.tid()"));
    assert!(identity.contains("thread_identity: PidSnapshot"));
