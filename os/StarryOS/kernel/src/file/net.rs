...
/// Device-level socket ioctls (`SIOCGIF*`/`SIOCSIF*`/route control), shared
/// across every socket family.
///
...
pub(super) fn device_ioctl(cmd: u32, arg: usize) -> Option<AxResult<usize>> {
    ...
    // CAP_NET_ADMIN, matching Linux (`dev_ioctl`/`devinet_ioctl` gate SIOCSIF*
    // address/netmask/flags/mtu and `ip_rt_ioctl` gates SIOCADDRT/SIOCDELRT on
    // CAP_NET_ADMIN). Query ioctls (SIOCGIF*, ...) stay available to
    ...
    {
        return Some(Err(AxError::OperationNotPermitted));
    }
...
impl FileLike for Socket {
    ...
    fn ioctl(&self, cmd: u32, arg: usize) -> AxResult<usize> {
        ...
        // (SIOCGIF*/SIOCSIF*/route control), mirroring Linux sock_ioctl
        // dispatching to dev_ioctl.
        if cmd == FIONREAD {
