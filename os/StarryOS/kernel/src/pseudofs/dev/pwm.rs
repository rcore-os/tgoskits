...
use crate::pseudofs::{DirMaker, NodeOpsMux, RwFile, SimpleDir, SimpleDirOps, SimpleFile, SimpleFileOperation, SimpleFs};
...
struct PwmClassDir { fs: Arc<SimpleFs> }
...
impl SimpleDirOps for PwmClassDir {
    ...
    fn lookup_child(&self, name: &str) -> VfsResult<NodeOpsMux> {
        ...
        Ok(NodeOpsMux::Dir(SimpleDir::new_maker(
            self.fs.clone(),
            Arc::new(PwmChipDir { fs: self.fs.clone(), chip_index }),
        )))
    }
...
struct PwmChipDir { fs: Arc<SimpleFs>, chip_index: u8 }
...
impl SimpleDirOps for PwmChipDir {
    ...
    fn lookup_child(&self, name: &str) -> VfsResult<NodeOpsMux> {
        match name {
            "export" => Ok(SimpleFile::new_regular(self.fs.clone(), RwFile::new({
                let chip_index = self.chip_index;
                move |req| match req {
                    SimpleFileOperation::Read => Ok(Some(Vec::new())),
                    SimpleFileOperation::Write(data) => {
                        if data.is_empty() || data.iter().all(|b| b.is_ascii_whitespace()) { return Ok(None); }
            ...
            "unexport" => Ok(SimpleFile::new_regular(self.fs.clone(), RwFile::new({
                let chip_index = self.chip_index;
                move |req| match req {
                    SimpleFileOperation::Read => Ok(Some(Vec::new())),
                    SimpleFileOperation::Write(data) => {
            ...
            _ => {
                ...
                let state = PWM_SYSFS_STATE.lock();
                let exported = state.chips.get(self.chip_index as usize)
                    .and_then(|chip| chip.channels.get(local_index as usize))
                    .map(|ch| ch.exported)
                    .unwrap_or(false);
                if !exported { return Err(VfsError::NotFound); }
                Ok(NodeOpsMux::Dir(SimpleDir::new_maker(
                    self.fs.clone(),
                    Arc::new(PwmChannelDir { fs: self.fs.clone(), chip_index: self.chip_index, channel_index: local_index }),
                )))
            }
        }
    }
...
struct PwmChannelDir { fs: Arc<SimpleFs>, chip_index: u8, channel_index: u8 }

impl SimpleDirOps for PwmChannelDir {
    fn child_names<'a>(&'a self) -> Box<dyn Iterator<Item = Cow<'a, str>> + 'a> {
        Box::new(["period", "duty_cycle", "enable"].iter().map(|s| Cow::Borrowed(*s)))
    }
    ...
    fn lookup_child(&self, name: &str) -> VfsResult<NodeOpsMux> {
        ...
        let file = match name {
            "period" => SimpleFile::new_regular(self.fs.clone(), RwFile::new(move |req| match req {
                SimpleFileOperation::Read => Ok(Some(format!("{}\n", pwm_read_period(chip_index, channel_index)?).into_bytes())),
                SimpleFileOperation::Write(data) => {
                    if data.is_empty() || data.iter().all(|b| b.is_ascii_whitespace()) { return Ok(None); }
                    pwm_write_period(chip_index, channel_index, data)?; Ok(None)
                }
            })),
            "duty_cycle" => SimpleFile::new_regular(self.fs.clone(), RwFile::new(move |req| match req {
                SimpleFileOperation::Read => Ok(Some(format!("{}\n", pwm_read_duty(chip_index, channel_index)?).into_bytes())),
                SimpleFileOperation::Write(data) => {
                    if data.is_empty() || data.iter().all(|b| b.is_ascii_whitespace()) { return Ok(None); }
                    pwm_write_duty(chip_index, channel_index, data)?; Ok(None)
                }
            })),
            "enable" => SimpleFile::new_regular(self.fs.clone(), RwFile::new(move |req| match req {
                SimpleFileOperation::Read => Ok(Some(format!("{}\n", pwm_read_enable(chip_index, channel_index)?).into_bytes())),
                SimpleFileOperation::Write(data) => {
                    if data.is_empty() || data.iter().all(|b| b.is_ascii_whitespace()) { return Ok(None); }
                    pwm_write_enable(chip_index, channel_index, data)?; Ok(None)
                }
            })),
...
fn parse_pwmchip_index(name: &str) -> Option<u8> {
    ...
    if value % PWM_SYSFS_CHANNELS_PER_CHIP != 0 { return None; }
    let index = value / PWM_SYSFS_CHANNELS_PER_CHIP;
    if index < PWM_SYSFS_CHIPS { Some(index) } else { None }
}

fn parse_pwm_local_index(name: &str) -> Option<u8> {
    let value = name.strip_prefix("pwm")?.parse::<u8>().ok()?;
    if value < PWM_SYSFS_CHANNELS_PER_CHIP { Some(value) } else { None }
}

fn parse_u64(data: &[u8]) -> VfsResult<u64> {
    core::str::from_utf8(data).ok().and_then(|t| t.trim().parse::<u64>().ok()).ok_or(VfsError::InvalidInput)
}

fn parse_u8(data: &[u8]) -> VfsResult<u8> {
    core::str::from_utf8(data).ok().and_then(|t| t.trim().parse::<u8>().ok()).ok_or(VfsError::InvalidInput)
}

fn export_pwm_channel(chip_index: u8, channel: u8) -> VfsResult<()> {
    if channel >= PWM_SYSFS_CHANNELS_PER_CHIP { return Err(VfsError::InvalidInput); }
    let mut state = PWM_SYSFS_STATE.lock();
    let chip = state.chips.get_mut(chip_index as usize).ok_or(VfsError::InvalidInput)?;
    let entry = &mut chip.channels[channel as usize];
    if !entry.exported { *entry = PwmChannelState { exported: true, ..Default::default() }; }
    Ok(())
}
...
fn pwm_read_period(chip_index: u8, channel_index: u8) -> VfsResult<u64> {
    let state = PWM_SYSFS_STATE.lock();
    Ok(state.chips.get(chip_index as usize).and_then(|c| c.channels.get(channel_index as usize)).ok_or(VfsError::InvalidInput)?.period_ns)
}

fn pwm_read_duty(chip_index: u8, channel_index: u8) -> VfsResult<u64> {
    let state = PWM_SYSFS_STATE.lock();
    Ok(state.chips.get(chip_index as usize).and_then(|c| c.channels.get(channel_index as usize)).ok_or(VfsError::InvalidInput)?.duty_ns)
}

fn pwm_read_enable(chip_index: u8, channel_index: u8) -> VfsResult<u8> {
    let state = PWM_SYSFS_STATE.lock();
    Ok(state.chips.get(chip_index as usize).and_then(|c| c.channels.get(channel_index as usize)).ok_or(VfsError::InvalidInput)?.enabled as u8)
}

fn pwm_write_period(chip_index: u8, channel_index: u8, data: &[u8]) -> VfsResult<()> {
    let value = parse_u64(data)?;
    if value == 0 { return Err(VfsError::InvalidInput); }
    let mut state = PWM_SYSFS_STATE.lock();
    let chip = state.chips.get_mut(chip_index as usize).ok_or(VfsError::InvalidInput)?;
    let enabled = { let e = chip.channels.get_mut(channel_index as usize).ok_or(VfsError::InvalidInput)?; e.period_ns = value; e.enabled };
...
fn pwm_write_duty(chip_index: u8, channel_index: u8, data: &[u8]) -> VfsResult<()> {
    ...
    let mut state = PWM_SYSFS_STATE.lock();
    let chip = state.chips.get_mut(chip_index as usize).ok_or(VfsError::InvalidInput)?;
    let enabled = { let e = chip.channels.get_mut(channel_index as usize).ok_or(VfsError::InvalidInput)?; e.duty_ns = value; e.enabled };
    pwm_apply_channel(chip, channel_index, enabled)
}

fn pwm_write_enable(chip_index: u8, channel_index: u8, data: &[u8]) -> VfsResult<()> {
    let value = parse_u8(data)?;
    if value > 1 { return Err(VfsError::InvalidInput); }
    let mut state = PWM_SYSFS_STATE.lock();
    let chip = state.chips.get_mut(chip_index as usize).ok_or(VfsError::InvalidInput)?;
    let channel = PwmChannel::from_u8(channel_index).ok_or(VfsError::InvalidInput)?;
    if value == 1 {
        let period_ns = chip.channels.get(channel_index as usize).ok_or(VfsError::InvalidInput)?.period_ns;
        if period_ns == 0 { return Err(VfsError::InvalidInput); }
...
fn pwm_apply_channel(chip: &mut PwmChipState, channel_index: u8, running: bool) -> VfsResult<()> {
    let entry = chip.channels.get(channel_index as usize).ok_or(VfsError::InvalidInput)?;
    if entry.period_ns == 0 { return Ok(()); }
    if entry.duty_ns > entry.period_ns { return Err(VfsError::InvalidInput); }
    let frequency_hz = (PWM_PERIOD_NS / entry.period_ns) as u32;
    if frequency_hz == 0 { return Err(VfsError::InvalidInput); }
    let high_percent = (entry.duty_ns * 100 / entry.period_ns) as u8;
    let low_percent = 100u8.saturating_sub(high_percent);
    let channel = PwmChannel::from_u8(channel_index).ok_or(VfsError::InvalidInput)?;
    let result = if running {
        chip.pwm.update_frequency_duty(channel, frequency_hz, low_percent)
    } else {
        chip.pwm.configure_channel(channel, frequency_hz, low_percent, PwmPolarity::ActiveHigh)
    };
