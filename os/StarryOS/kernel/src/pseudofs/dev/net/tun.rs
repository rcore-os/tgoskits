...
use crate::{mm::UserPtr, pseudofs::DeviceOps, sync::IrqMutex as Mutex, task::AsThread};
...
pub struct TunFile {
    ...
    /// must release `state`'s `IrqMutex` before calling into the network
...
impl TunFile {
    ...
    fn set_iff(&self, arg: usize) -> VfsResult<usize> {
        ...
        // outside `state`'s lock to avoid holding an `IrqMutex` across the
    ...
    ///
    /// Runs with no `IrqMutex` held: `mark_dying`/`detach` and `destroy_tun`
    /// take sleeping service/registry locks (and `destroy_tun` joins the device
    /// workers); sleeping under an `IrqMutex` aborts with an atomic-context-sleep
    ...
    fn get_iff(&self, arg: usize) -> VfsResult<usize> {
        ...
        // release before touching user memory (faulting under `IrqMutex` wedges
        // the CPU).
        let (name, flags) = {
...
impl Pollable for TunFile {
    ...
    fn register(&self, context: &mut core::task::Context<'_>, events: IoEvents) {
        ...
        // under this `IrqMutex` (preemption and IRQs disabled), and the waker
        ...
        match shared {
            Some(shared) => {
                ...
                // `IrqMutex` lock, because its waker path may re-enter poll
