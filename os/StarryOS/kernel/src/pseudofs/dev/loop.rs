...
use linux_raw_sys::{
    ioctl::{
        BLKDISCARD, BLKFLSBUF, BLKGETSIZE, BLKGETSIZE64, BLKIOMIN, BLKIOOPT, BLKPG, BLKRAGET,
        BLKRASET, BLKROGET, BLKROSET, BLKRRPART, BLKSSZGET,
    },
    loop_device::{
...
impl LoopDevice {
    ...
    pub fn as_dyn_block_device(&self) -> VfsResult<Box<dyn ax_driver::prelude::BlockDriverOps>> {
        ...
        // Allowing a second mount would give two independent LoopBlockDevice
        // adapters backed by different CacheData instances, but only the latest
        // cache is tracked in block_cache — the old mount's writes could be
        // lost on umount (flush_cache_to_file only sees the newest cache), and
        // LOOP_CLR_FD could clear the mounted flag while the old adapter is
        // still receiving ext4 writes.
        {
            ...
            if let Some(ref cd) = cache.data {
                ...
                // (without losetup -d), where the in-memory writes must be
                // persisted to the backing file before we reload from it.
                //
                // If writeback fails we must NOT proceed to overwrite the cache
                // with fresh data read from the backing file — that would
                // silently discard the dirty data.  Return I/O error and leave
                // the dirty cache intact so that a subsequent BLKFLSBUF or
                // LOOP_CLR_FD can retry.
    ...
    pub fn flush_cache_to_file(&self) -> AxResult<()> {
        ...
        if let Some(ref cd) = cache.data {
            ...
            // Always clear mounted after VFS unmount — the filesystem
            // is gone regardless of writeback outcome.
