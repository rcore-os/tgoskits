...
pub fn sys_faccessat2(dirfd: c_int, path: *const c_char, mode: u32, flags: u32) -> AxResult<isize> {
    // man 2 faccessat2: valid flags are AT_EACCESS, AT_SYMLINK_NOFOLLOW,
    // and AT_EMPTY_PATH. Any other bit is EINVAL.
    const FACCESSAT2_VALID: u32 = AT_EACCESS | AT_SYMLINK_NOFOLLOW | AT_EMPTY_PATH;
    if flags & !FACCESSAT2_VALID != 0 {
        return Err(AxError::InvalidInput);
    }
