...
fn wait_ptrace_resume(thr: &Thread, tid: u32, uctx: &mut UserContext) {
    loop {
        // Acknowledge interrupts already pending before this wait (dev's
        // snapshot/acknowledge model, in place of the old `clear_interrupt()`),
        // so a stale wake does not spuriously abort the re-park below.
