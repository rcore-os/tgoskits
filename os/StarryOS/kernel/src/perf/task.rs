...
fn arm_slice(ptc: &PerTaskCounter, n: usize, now: u64) {
    ...
    // Non-sampling counters expose an rdpmc page for userspace direct reads; a
    // freshly-armed slice makes it valid (points at the live slot).
    if !ptc.is_sampling {
...
pub fn perf_sched_in(thr: &Thread) {
    ...
    for ptc in counters.iter() {
        ...
        // Arm if a programmable counter is free on this core; otherwise leave
        // it degraded (running == false, no slot). The rotation tick
...
pub fn free_hw(ptc: &PerTaskCounter) {
    ...
    if ptc.is_sampling {
        ...
        *ptc.redirect_anchor.lock() = None;
        ...
        // The live slice (if any) was already torn down above via
        // `teardown_slice_local` / the owner-core IPI; for a non-sampling
        // counter just release its rdpmc page mapping.
        ptc.publish_rdpmc_page(false);
