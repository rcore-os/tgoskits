...
pub trait VmPtr: Copy {
    ...
    /// initialized.
    fn vm_read_uninit<I: VmIo>(self, vm: &mut I) -> VmResult<MaybeUninit<Self::Target>> {
        let mut uninit = MaybeUninit::<Self::Target>::uninit();
        vm_read_slice(vm, self.as_ptr(), slice::from_mut(&mut uninit))?;
        Ok(uninit)
    }

    /// Reads the value from this virtual memory pointer.
    fn vm_read<I: VmIo>(self, vm: &mut I) -> VmResult<Self::Target>
    where
        Self::Target: AnyBitPattern,
    {
        let uninit = self.vm_read_uninit(vm)?;
    ...
    unsafe fn vm_read_any<I: VmIo>(self, vm: &mut I) -> VmResult<Self::Target> {
        let uninit = self.vm_read_uninit(vm)?;
...
pub trait VmMutPtr: VmPtr {
    ...
    fn vm_write<I: VmIo>(self, vm: &mut I, value: Self::Target) -> VmResult
    where
        Self::Target: NoUninit,
    {
        vm_write_slice(vm, self.as_ptr().cast_mut(), slice::from_ref(&value))
    }
}
